...
export const BroadcastCard = ({ url, title, date }) => {
  ...
  // Extrait l’id de diffusion de l’URL pour l’utiliser ensuite
  const broadcastId = url.match(/\/i\/broadcasts\/([^\/\?#]+)/)?.[1];
  
  // Si la date figure entre parenthèses dans le titre, en extraire le titre et la date
  ...
  return (
    <a 
      ...
      style={{ margin: 0 }}
    >
      {/* Conteneur d’aperçu au ratio 16:9 */}
      <div 

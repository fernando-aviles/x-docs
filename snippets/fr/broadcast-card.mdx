...
export const BroadcastCard = ({ url, title, date }) => {
  ...
  // Extraire l’ID de diffusion depuis l’URL pour une éventuelle utilisation ultérieure
  const broadcastId = url.match(/\/i\/broadcasts\/([^\/\?#]+)/)?.[1];
  
  // Analyser le titre et la date si la date est entre parenthèses dans le titre
  ...
  return (
    <a 
      ...
      style={{ margin: 0 }}
    >
      {/* Conteneur d’aperçu au format 16:9 */}
      <div 

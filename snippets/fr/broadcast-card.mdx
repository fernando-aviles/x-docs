...
export const BroadcastCard = ({ url, title, date }) => {
  ...
  // Extraire l’id de diffusion depuis l’URL pour un usage potentiel ultérieur
  const broadcastId = url.match(/\/i\/broadcasts\/([^\/\?#]+)/)?.[1];
  
  // Analyser le titre et la date si la date figure entre parenthèses dans le titre
  ...
  return (
    <a 
      ...
      style={{ margin: 0 }}
    >
      {/* Conteneur d’aperçu avec un format 16:9 */}
      <div 
        className="relative w-full overflow-hidden"
        style={{ 
          paddingTop: '56.25%', // format 16:9
          background: 'linear-gradient(135deg, #0f0f0f 0%, #050505 100%)',
          backgroundSize: 'cover',
          backgroundPosition: 'center'
        }}
      >
        {/* Conteneur du logo (position décalée) */}
        <div 
        ...
        {/* Superposition avec bouton de lecture */}
        <div 

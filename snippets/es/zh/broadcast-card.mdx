...
export const BroadcastCard = ({ url, title, date }) => {
  ...
  // Extrae el ID de la transmisión de la URL para usarlo después
  const broadcastId = url.match(/\/i\/broadcasts\/([^\/\?#]+)/)?.[1];
  
  // Si la fecha aparece entre paréntesis en el título, extrae de ahí el título y la fecha

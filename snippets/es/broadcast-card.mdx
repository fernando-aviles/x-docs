...
export const BroadcastCard = ({ url, title, date }) => {
  ...
  // Extrae el id de la transmisión de la URL para un posible uso posterior
  const broadcastId = url.match(/\/i\/broadcasts\/([^\/\?#]+)/)?.[1];
  
  // Procesa el título y la fecha si la fecha está entre paréntesis en el título
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Superposición con botón de reproducción */}
        <div 

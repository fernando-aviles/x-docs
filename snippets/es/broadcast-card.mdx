...
export const BroadcastCard = ({ url, title, date }) => {
  ...
  // Extraer el id de la transmisión de la URL para un posible uso futuro
  const broadcastId = url.match(/\/i\/broadcasts\/([^\/\?#]+)/)?.[1];
  
  // Analizar título y fecha si la fecha aparece entre paréntesis en el título
  ...
  return (
    ...
    >
      ...
      >
        {/* Contenedor del logo (posición desplazada) */}
        <div 

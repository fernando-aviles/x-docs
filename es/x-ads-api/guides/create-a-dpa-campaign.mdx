...
keywords: ["campaña DPA", "dynamic product ads", "DPA", "crear campaña DPA", "product ads", "dynamic ads"]
---

En el siguiente ejemplo se asume que ya instalaste, configuraste y autorizaste tu aplicación y usuario usando twurl. twurl es una herramienta de línea de comandos similar a cURL que gestiona de forma eficaz la autenticación OAuth de X. twurl es una excelente herramienta para probar y depurar rápidamente la funcionalidad de Ads API (y REST API). Para ver todos los encabezados de la solicitud y la Respuesta, usa -t para trazar la llamada, aproximadamente equivalente a la opción -v de cURL.

En este ejemplo, crearemos una campaña Dynamic Product Ads (DPA).
...
Realiza una solicitud GET a la API accounts/:account&#95;id/funding&#95;instruments usando el id de la cuenta obtenido en el comando anterior.
...
Especifica una hora de inicio y un presupuesto para la campaña.

* Campos obligatorios: instrumento de financiación, nombre de la campaña, presupuesto (total/diario), hora de inicio e ID de la cuenta de anuncios
...
<div id="step-2-create-line-item-with-sales-objective">
  ## Paso 2: Crear un line item con objetivo de ventas
</div>
...
* Después de este paso, podrás ver la campaña en el Administrador de anuncios de X.


<div id="step-3-create-dpa-card">
  ## Paso 3: Crear una tarjeta DPA
</div>

Crea una tarjeta DPA.

* Parámetros obligatorios: id de la cuenta de anuncios y URL:
* La URL debe comenzar con https://twitter.com; cualquier UTM adicional se puede agregar después, como en el ejemplo siguiente:
  * `https://twitter.com?utm_source=%7B%7Bsite_source%7D%7D`
* Opcionalmente, puedes elegir el formato de tu tarjeta especificando creative&#95;type con uno de los siguientes valores: &quot;CAROUSEL&quot;, &quot;SINGLE&quot;, &quot;COLLECTION&quot;. Si no se especifica, tu tarjeta se creará como una tarjeta carrusel.
...
* Parámetros obligatorios: id de cuenta publicitaria, id de usuario, URI de la tarjeta (del paso 3), texto, nombre y anuncio de producto dinámico
* dynamic&#95;product&#95;ad siempre se debe establecer en true
...
* Parámetros obligatorios: id de la cuenta de anuncios, id del elemento de línea (del Paso 2) e id del tuit (id&#95;str del Paso 4)

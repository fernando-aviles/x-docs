---
title: Seguridad
sidebarTitle: Seguridad
icon: shield-halved
description: Prácticas recomendadas de seguridad para desarrolladores de X API
keywords: ["seguridad", "seguridad de la API", "seguridad de OAuth", "seguridad de claves de la API", "seguridad de tokens", "programación segura", "informe de vulnerabilidades"]
---

Crear aplicaciones seguras protege tanto a tus usuarios como a la plataforma X. Esta guía abarca las prácticas de seguridad esenciales para desarrolladores de X API.

***

<div id="core-requirements">
  ## Requisitos principales
</div>

<CardGroup cols={2}>
  <Card title="TLS obligatorio" icon="lock">
    Todas las solicitudes a la API deben usar HTTPS. Se rechaza el HTTP sin cifrar.
  </Card>

  <Card title="Seguridad de credenciales" icon="key">
    Nunca expongas claves o tokens de la API en código del lado del cliente, registros o repositorios.
  </Card>
</CardGroup>

***

<div id="protecting-credentials">
  ## Protección de credenciales
</div>

Tus claves y tokens de API son las llaves de acceso a tu aplicación. Mantenlas seguras:

<Steps>
  <Step title="Usa variables de entorno">
    Almacena las credenciales en variables de entorno, no en el código.

    ```bash
    export X_API_KEY="your-api-key"
    export X_API_SECRET="your-api-secret"
    ```
  </Step>

  <Step title="Nunca hagas commit de secretos">
    Agrega los archivos de credenciales a `.gitignore`. Usa herramientas como `git-secrets` para evitar commits accidentales.
  </Step>

  <Step title="Rota con regularidad">
    Regenera las claves periódicamente e inmediatamente si sospechas que se han visto comprometidas.
  </Step>

  <Step title="Usa permisos mínimos">
    Solicita solo los permisos (scopes) de OAuth que tu aplicación realmente necesita.
  </Step>
</Steps>

<div id="if-credentials-are-compromised">
  ### Si tus credenciales se ven comprometidas
</div>

1. **Regénralas de inmediato** en la [Consola de Desarrollador](https://console.x.com)
2. **Revoca los tokens antiguos** — al regenerarlas, se invalidan automáticamente las credenciales anteriores
3. **Audita el uso** — comprueba si hay actividad no autorizada en la API
4. **Actualiza tu aplicación** — despliega las nuevas credenciales en todos los entornos

***

<div id="application-security">
  ## Seguridad de aplicaciones
</div>

<div id="input-validation">
  ### Validación de datos de entrada
</div>

Nunca confíes en los datos proporcionados por el usuario. Valida y sanea todos los datos antes de usarlos:

```python
# Bad - vulnerable to injection
query = f"from:{user_input}"

# Bueno - validar la entrada primero
import re
if re.match(r'^[a-zA-Z0-9_]{1,15}$', user_input):
    query = f"from:{user_input}"
```

<div id="output-encoding">
  ### Codificación de salida
</div>

Aplica escape a los datos de X API antes de mostrarlos en HTML para evitar XSS:

```javascript
// Malo: vulnerable a XSS
element.innerHTML = tweet.text;

// Good - escape HTML
element.textContent = tweet.text;
```

<div id="common-vulnerabilities-to-prevent">
  ### Vulnerabilidades comunes que se deben prevenir
</div>

| Vulnerabilidad | Prevención |
|:--------------|:-----------|
| **XSS** | Escapar todo el contenido generado por el usuario antes de renderizarlo |
| **CSRF** | Usar tokens anti-CSRF en los formularios; verificar el parámetro `state` de OAuth |
| **SQL Injection** | Usar consultas parametrizadas; nunca concatenar la entrada del usuario |
| **Open redirects** | Validar las URLs de callback contra una lista de permitidos (allowlist) |

***

<div id="oauth-security">
  ## Seguridad en OAuth
</div>

<div id="state-parameter">
  ### Parámetro state
</div>

Usa siempre el parámetro `state` en los flujos de OAuth para prevenir ataques CSRF:

```python
import secrets

# Generar estado antes de la autorización
state = secrets.token_urlsafe(32)
session['oauth_state'] = state

# Verificar estado después del callback
if request.args.get('state') != session.get('oauth_state'):
    abort(403)  # El estado no coincide - posible CSRF
```

<div id="token-storage">
  ### Almacenamiento de tokens
</div>

| Tipo de token | Recomendación de almacenamiento |
|:-----------|:-----------------------|
| **Access tokens** | Base de datos cifrada o bóveda segura |
| **Refresh tokens** | Base de datos cifrada con controles de acceso adicionales |
| **Bearer tokens** | Variables de entorno o configuración segura |

***

<div id="secure-development-practices">
  ## Prácticas de desarrollo seguro
</div>

<CardGroup cols={2}>
  <Card title="Auditorías de seguridad" icon="magnifying-glass">
    Realiza revisiones de seguridad periódicas y pruebas de penetración.
  </Card>

  <Card title="Escaneo de dependencias" icon="box">
    Mantén las dependencias actualizadas. Usa herramientas para detectar paquetes vulnerables.
  </Card>

  <Card title="Registro de actividad" icon="file-lines">
    Registra eventos de seguridad, pero nunca registres credenciales ni datos sensibles.
  </Card>

  <Card title="Monitoreo" icon="chart-line">
    Configura alertas para patrones inusuales de uso de la API.
  </Card>
</CardGroup>

***

<div id="reporting-security-issues">
  ## Informar problemas de seguridad
</div>

Si descubres una vulnerabilidad de seguridad que afecte a X:

<Warning>
  **Comunícalo en un plazo de 48 horas.** Los usuarios de X Developer Platform deben notificarlo a X en un máximo de 48 horas desde que sospechen un incidente de seguridad.
</Warning>

<CardGroup cols={2}>
  <Card title="Programa de recompensas por errores de X" icon="bug" href="https://hackerone.com/twitter">
    Informa de vulnerabilidades en los sistemas de X a través de HackerOne.
  </Card>

  <Card title="Incidente en tu aplicación" icon="triangle-exclamation" href="https://hackerone.com/twitter">
    Si se produce una brecha en tu aplicación que use datos de X, infórmala a través del mismo canal.
  </Card>
</CardGroup>

***

<div id="compliance-checklist">
  ## Lista de verificación de cumplimiento
</div>

<Accordion title="Requisitos de seguridad para desarrolladores de X API">
  * [ ] Todas las solicitudes a la API usan TLS/HTTPS
  * [ ] Credenciales almacenadas de forma segura (no en el código ni en los registros)
  * [ ] Tokens de usuario cifrados en reposo
  * [ ] Validación de entrada en todos los datos proporcionados por el usuario
  * [ ] Codificación de salida para evitar XSS
  * [ ] Protección CSRF en los flujos de OAuth
  * [ ] Registros de seguridad habilitados (sin datos confidenciales)
  * [ ] Plan de respuesta ante incidentes documentado
  * [ ] Dependencias actualizadas periódicamente
  * [ ] Solicitar solo los alcances mínimos necesarios de OAuth
</Accordion>

***

<div id="resources">
  ## Recursos
</div>

<CardGroup cols={2}>
  <Card title="Guía de autenticación" icon="key" href="/es/resources/fundamentals/authentication/overview">
    Implementa OAuth correctamente.
  </Card>

  <Card title="Permisos de la aplicación" icon="shield-check" href="/es/resources/fundamentals/developer-apps#app-permissions">
    Configura los permisos mínimos obligatorios.
  </Card>
</CardGroup>
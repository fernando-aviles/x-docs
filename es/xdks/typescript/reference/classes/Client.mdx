...
Clase principal de cliente para la X API

Este es el punto de entrada principal para interactuar con la X API. Proporciona
...
Crea una nueva instancia de Client para X API
...
<ResponseField name="media" type="MediaClient" required>
  cliente de contenido multimedia
</ResponseField>
...
<ResponseField name="request" type="Promise<T>" required>
  ...
  Este método gestiona la autenticación, el formateo de la solicitud y el manejo de errores
  para todas las solicitudes de API. Añade automáticamente los encabezados de autenticación
  apropiados según la configuración del cliente.
</ResponseField>
...
<ResponseField name="mapSecuritySchemeToAuthTypes" type="string[]" required>
  Asigna nombres de esquemas de seguridad de OpenAPI a tipos de autenticación internos
</ResponseField>

<ResponseField name="validateAuthentication" type="void" required>
  Valida que el método de autenticación obligatorio esté disponible
</ResponseField>

...
Clase principal del cliente para la X API

Este es el punto de entrada principal para interactuar con la X API. Proporciona
acceso a todos los endpoints de la API a través de módulos de cliente especializados y gestiona
...
Crea una nueva instancia del Client de X API
...
| :------ | :------ | :------ |
| `config` | `any` | Opciones de configuración para el Client |
...
<ResponseField name="request" type="Promise<T>" required>
  Realiza una solicitud autenticada a la X API

  Este método gestiona la autenticación, el formateo de la solicitud y el manejo
  de errores para todas las solicitudes a la API. Agrega automáticamente los

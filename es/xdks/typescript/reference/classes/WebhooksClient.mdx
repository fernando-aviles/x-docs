...
de la X API. Gestiona la autenticación, el formato de las solicitudes y el procesamiento de las respuestas
...
| Nombre | Tipo | Descripción |
| :------ | :------ | :------ |
| `webhookId` | `string` | El id de webhook que se va a vincular a tu conjunto de reglas de FilteredStream. |
| `options` | `Object` | - |
...
Obtiene una lista de enlaces de webhook asociados a un conjunto de reglas de flujo filtrado.
...
Crea un trabajo de reproducción para recuperar los eventos de hasta las últimas 24 horas para todos los eventos entregados o cuya entrega se haya intentado al webhook.
...
Promise que se resuelve en la respuesta de la API, o la Response sin procesar si `requestOptions.raw` es `true`
...
Valida el webhook
Inicia una verificación CRC para el webhook indicado.
...
| Nombre | Tipo | Descripción |
| :------ | :------ | :------ |
| `webhookId` | `string` | El id del webhook que se va a validar. |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

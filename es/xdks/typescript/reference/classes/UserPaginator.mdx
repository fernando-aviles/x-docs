...
Obtiene los metadatos actuales de paginación
...
instancia actual del paginador. Actualiza el estado de paginación y los metadatos.
...
```typescript
const followers = await client.users.getFollowers('783214');
await followers.fetchNext(); // Recuperar la primera página

if (!followers.done) {
  const nextPage = await followers.next(); // Obtener la siguiente página como una nueva instancia
  console.log(followers.items.length); // Todavía es la primera página

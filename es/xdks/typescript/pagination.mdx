...
El SDK proporciona utilidades de paginación genéricas que puedes usar con cualquier endpoint que devuelva respuestas paginadas. Los métodos devuelven respuestas simples, que puedes envolver con un paginador.
...
// Envuelve cualquier endpoint de lista con tipado adecuado
...
while (!followers.done) {
  await followers.fetchNext();        // siguientes páginas
}

const userCount: number = followers.users.length;  // todos los usuarios obtenidos
...
<div id="next-page-as-a-new-instance">
  ### Página siguiente como nueva instancia
</div>
...
await followers.fetchNext();
if (!followers.done) {
  const page2: UserPaginator = await followers.next(); // paginador independiente que comienza en la página siguiente
...
<div id="error-handling-and-rate-limits">
  ### Gestión de errores y límites de frecuencia
</div>
...
try {
  ...
  if (followers.rateLimited) {
    console.error('Límite de frecuencia alcanzado, es necesario aplicar backoff');
...
<Info>
Para obtener ejemplos de código detallados que usen el XDK para JavaScript/TypeScript, consulta nuestro [repositorio de ejemplos en GitHub](https://github.com/xdevplatform/samples/tree/main/javascript).
</Info>
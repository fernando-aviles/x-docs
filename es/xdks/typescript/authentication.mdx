...
(async () => {
  ...
  // Después de la autorización, el usuario es redirigido nuevamente con un parámetro code
  // Extrae el code de la URL de callback (por ejemplo, de los parámetros de consulta)
  const authCode = 'code-from-callback-url'; // Reemplaza con el code real del callback de OAuth
...
<Info>
  Para obtener ejemplos de código detallados con el XDK para JavaScript/TypeScript, consulta nuestro [repositorio de ejemplos en GitHub](https://github.com/xdevplatform/samples/tree/main/javascript).
</Info>

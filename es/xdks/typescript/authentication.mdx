...
El SDK de TypeScript admite varios métodos de autenticación para diferentes casos de uso.

<div id="bearer-token-app-only-auth">
  ## Bearer Token (autenticación solo para aplicaciones)
</div>
...
(async (): Promise<void> => {
  ...
  const authCode: string = 'code-from-callback-url'; // Reemplázalo por el código real del callback de OAuth
...
(async () => {
  ...
  // Después de la autorización, el usuario es redirigido con un parámetro code
  // Extrae el código de la URL de callback (por ejemplo, de los parámetros de consulta)
  const authCode = 'code-from-callback-url'; // Reemplázalo por el código real del callback de OAuth

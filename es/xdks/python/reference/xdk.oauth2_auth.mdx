---
title: "OAuth2PKCEAuth"
sidebarTitle: "OAuth2PKCEAuth"
---

Este módulo proporciona funcionalidades de autenticación OAuth2 PKCE (Proof Key for Code Exchange)
para flujos de autorización seguros. Incluye la generación de `code verifier`,
la gestión de tokens y la actualización automática de tokens.

<div id="class-xdkoauth2_authoauth2pkceauth">
  ### `class xdk.oauth2_auth.OAuth2PKCEAuth`
</div>

Autenticación OAuth 2.0 PKCE para X API.

<div id="parameters">
  #### Parámetros
</div>

<ParamField path="path.base_url" type="str" default="'https://api.x.com'"></ParamField>

<ParamField path="path.authorization_base_url" type="str" default="'https://x.com/i'"></ParamField>

<ParamField path="path.client_id" type="str or None" default="None"></ParamField>

<ParamField path="path.client_secret" type="str or None" default="None"></ParamField>

<ParamField path="path.redirect_uri" type="str or None" default="None"></ParamField>

<ParamField path="path.token" type="Dict[str, Any] or None" default="None"></ParamField>

<ParamField path="path.scope" type="str or List[str] or None" default="None"></ParamField>

<div id="__init__">
  ### `__init__`
</div>

Inicializa la autenticación OAuth2 con PKCE.

<div id="parameters">
  #### Parámetros
</div>

<ParamField path="path.base_url" type="str" default="'https://api.x.com'">
La URL base para el endpoint de tokens de X API (de forma predeterminada, [https://api.x.com](https://api.x.com)).
</ParamField>

<ParamField path="path.authorization_base_url" type="str" default="'https://x.com/i'">
La URL base para la autorización OAuth2 (de forma predeterminada, [https://x.com/i](https://x.com/i)).
</ParamField>

<ParamField path="path.client_id" type="str or None" default="None">
El ID de cliente para X API.
</ParamField>

<ParamField path="path.client_secret" type="str or None" default="None">
El secreto de cliente para X API.
</ParamField>

<ParamField path="path.redirect_uri" type="str or None" default="None">
El URI de redirección para la autorización OAuth2.
</ParamField>

<ParamField path="path.token" type="Dict[str, Any] or None" default="None">
Un diccionario de tokens OAuth2 existente (si está disponible).
</ParamField>

<ParamField path="path.scope" type="str or List[str] or None" default="None">
Cadena de texto separada por espacios o lista de cadenas de texto para los ámbitos de autorización OAuth2.
</ParamField>

<div id="exchange_code">
  ### `exchange_code`
</div>

Intercambia el código de autorización por tokens (se corresponde con la API de TypeScript).

<div id="parameters">
  #### Parámetros
</div>

<ParamField path="path.code" type="str">
El código de autorización procedente del callback.
</ParamField>

<ParamField path="path.code_verifier" type="str or None" default="None">
Verificador de código opcional (usa el verificador almacenado si no se indica).
</ParamField>

<div id="returns">
  #### Devuelve
</div>

`Dictstr, Any`

<div id="fetch_token">
  ### `fetch_token`
</div>

Obtener un token mediante la URL de la respuesta de autorización (método heredado, usa internamente exchange_code).

<div id="parameters">
  #### Parámetros
</div>

<ParamField path="path.authorization_response" type="str">
La URL de devolución de llamada completa recibida después de la autorización
</ParamField>

<div id="returns">
  #### Devuelve
</div>

`Dictstr, Any`

<div id="get_authorization_url">
  ### `get_authorization_url`
</div>

Obtén la URL de autorización para el flujo OAuth2 PKCE.

<div id="parameters">
  #### Parámetros
</div>

<ParamField path="path.state" type="str or None" default="None">
Parámetro opcional `state` por motivos de seguridad.
</ParamField>

<div id="returns">
  #### Devuelve
</div>

`str`

<div id="get_code_challenge">
  ### `get_code_challenge`
</div>

Obtiene el code challenge actual (para PKCE).
:returns: Devuelve el code challenge actual o None si no se ha establecido.
:rtype: Optional[str]

<div id="returns">
  #### Devuelve
</div>

`str | None`

<div id="get_code_verifier">
  ### `get_code_verifier`
</div>

Obtiene el verificador de código actual (para PKCE).
:returns: El verificador de código actual, o None si no se ha establecido.
:rtype: Optional[str]

<div id="returns">
  #### Devuelve
</div>

`str | None`

<div id="is_token_expired">
  ### `is_token_expired`
</div>

Comprueba si el token está caducado.
:returns: True si el token ha caducado, False en caso contrario.
:rtype: bool

<div id="returns">
  #### Devuelve
</div>

`bool`

<div id="refresh_token">
  ### `refresh_token`
</div>

Renueva el token de acceso.
:returns: El diccionario del token renovado
:rtype: Dict[str, Any]

<div id="returns">
  #### Devuelve
</div>

`Dictstr, Any`

<div id="set_pkce_parameters">
  ### `set_pkce_parameters`
</div>

Establece manualmente los parámetros de PKCE.

<div id="parameters">
  #### Parámetros
</div>

<ParamField path="path.code_verifier" type="str">
El verificador de código que se utilizará.
</ParamField>

<ParamField path="path.code_challenge" type="str or None" default="None">
Desafío de código opcional (se generará si no se proporciona).
</ParamField>
...
La X API admite datos en tiempo real a través de endpoints como el [Filtered Stream Endpoint](https://docs.x.com/x-api/posts/filtered-stream/introduction), que entregan Publicaciones coincidentes a medida que se producen. Esto requiere mantener una conexión HTTP persistente.
...
Las reglas definen filtros sobre los datos específicos que buscas (por ejemplo, palabras clave, usuarios, etc.). Puedes obtener más información sobre cómo crear reglas con [esta guía](https://docs.x.com/x-api/posts/filtered-stream/integrate/build-a-rule)
**Agregar reglas**:
...
- **420 Enhance Your Calm**: Se alcanzó el límite de solicitudes; espera y vuelve a intentarlo.
- **No Data**: Revisa las reglas con `get_rules()`; asegúrate de que existan publicaciones coincidentes.
Para ver ejemplos de código detallados con el XDK para Python, consulta nuestro [repositorio de ejemplos de código en GitHub](https://github.com/xdevplatform/samples/tree/main/python).
Para más ejemplos y la Referencia de la API, consulta las docstrings en línea (por ejemplo, `help(client.tweets.search_recent)`) o los stubs generados en el código fuente. Envía tus comentarios a través del [repositorio de GitHub](https://github.com/xdevplatform/xdk/tree/main/xdk/python).
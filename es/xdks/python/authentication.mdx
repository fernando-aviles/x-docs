...
Obtén las credenciales en la [Consola de desarrollador de X](https://developer.x.com/en/portal/dashboard). Necesitarás una cuenta de desarrollador aprobada y una App con los permisos adecuados (por ejemplo, Lectura + Escritura).
...
Este ejemplo muestra cómo usar OAuth 2.0 con Proof Key for Code Exchange (PKCE). Utiliza este flujo para acceso específico de usuario (por ejemplo, publicar en nombre de un usuario o subir contenido multimedia para un usuario).
**Pasos**:
...
# Paso 3: Manejar callback (en una aplicación real, usa un framework web como Flask)
# Supón que callback_url = "http://localhost:8080/callback?code=AUTH_CODE_HERE"
callback_url = input("Pega la URL completa del callback aquí: ")
...
# Opción 2: Pasar el diccionario completo de tokens para soporte de renovación automática
...
# Paso 2: Crear cliente con OAuth1

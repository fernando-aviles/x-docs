...
Powerstream es nuestra API de streaming en tiempo real más rápida para acceder a los datos públicos de X. Al igual que la API heredada GNIP Powetrack, utiliza reglas para filtrar Publicaciones en función de palabras clave, operadores y metadatos. Una vez que se establece una conexión HTTP persistente con el endpoint de Powerstream, puedes empezar a recibir Publicaciones que coincidan casi en tiempo real.

Actualmente, Powerstream admite hasta 1.000 reglas y cada regla puede tener 2048 caracteres.
...
* **Entrega**: Respuesta JSON mediante HTTP/1.1 con codificación de transferencia fragmentada.
* **Compatibilidad con centros de datos locales**: Obtén publicaciones solo del centro de datos local para reducir la latencia evitando el retraso de replicación.

<Note>
  La API Powerstream es una oferta premium disponible con determinados planes Enterprise.

  Si estás interesado en acceder a Powerstream o en obtener más información sobre nuestras ofertas Enterprise, ponte en contacto con nuestro equipo de ventas enviando el [Formulario de solicitud Enterprise](/es/forms/enterprise-api-interest).
...
Agrega reglas para filtrar el flujo.
...
**Consejo**: Para eliminar todas las reglas, primero realiza una solicitud GET para obtenerlas, extrae sus id y luego elimínalas en bloque.
...
Conéctate al stream para recibir Publicaciones en tiempo real. Usa `stream=True` para leer línea por línea. Implementa una lógica de reconexión para mejorar la robustez.
...
Para optimizar la latencia, Powerstream ofrece una opción para obtener solo publicaciones que se originaron o se crearon en el centro de datos local donde se establece la conexión. Esto evita la latencia de replicación, lo que resulta en una entrega más rápida en comparación con las publicaciones de otros centros de datos. Para habilitarlo, agrega el parámetro de consulta `?localDcOnly=true` al endpoint de stream (por ejemplo, `/2/powerstream?localDcOnly=true`). El centro de datos al que estás conectado se indicará tanto en la carga útil inicial de data del stream como en un encabezado HTTP de la respuesta.
...
| `retweets_of:` | Coincide con republicaciones de un usuario específico | `retweets_of:xdevelopers` |
...
| `is:` | Busca publicaciones que son de tipos específicos o tienen propiedades específicas (opciones: retweet, reply) | `is:retweet`, `is:reply` |
...
| Operador | Resumen | Ejemplo |
|----------|---------|---------|
| `bio:` | Coincide con publicaciones de usuarios cuya biografía contiene contenido específico (utiliza coincidencia de frases) | N/D |
| `bio_name:` | Coincide con publicaciones de usuarios con un nombre específico en su biografía (utiliza coincidencia de frases) | N/D |
...
El payload de la API de Powestream tiene el mismo formato que el de la API heredada GNIP Powertrack. Un ejemplo de respuesta JSON es el siguiente:
...
* Límites de tasa: 50 solicitudes/24 h para la gestión de reglas; sin límite en los flujos (pero se aplican límites de conexión).
* Reconexión: reintentos con retroceso exponencial cuando se producen desconexiones.
* Supervisión: usa encabezados `Connection: keep-alive`.
...
description: Envía Mensajes Directos y crea conversaciones grupales
keywords: ["inicio rápido para gestionar Mensajes Directos", "inicio rápido para enviar MD", "inicio rápido de gestión de MD", "crear MD grupales"]
---

import { Button } from '/snippets/es/button.mdx';

Esta guía te explica cómo enviar Mensajes Directos y crear conversaciones de grupo.
...
<Note>
  ...
  * Token de acceso de usuario (OAuth 2.0 PKCE con permisos `dm.write` y `dm.read`)
</Note>
...
<Steps>
  <Step title="Obtener el id de usuario del destinatario">
    Necesitas el id de usuario de la persona a la que quieres enviar un mensaje. Puedes obtenerlo a través del [endpoint de búsqueda de usuarios](/es/x-api/users/lookup/introduction).
  </Step>
  ...
  <Step title="Enviar el mensaje">
    <CodeGroup dropdown>
      ...
      curl -X POST "https://api.x.com/2/dm_conversations/with/9876543210/messages" \
        ...
        -d '{"text": "Hola. Este es un mensaje enviado desde la API."}'
      ```
      ...
      response = client.dm.send_message(
          ...
          text="Hola. Este es un mensaje enviado desde la API."
      )
...
<Steps>
  <Step title="Definir participantes">
    Reúne los ID de usuario de todas las personas que quieres en el grupo (excepto tú).
  </Step>
...
Envía un mensaje en una conversación en la que ya participas:
...
<CodeGroup dropdown>
  ...
  curl -X POST "https://api.x.com/2/dm_conversations/1582103724607971328/messages" \
    -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
    -H "Content-Type: application/json" \
    -d '{"text": "Añadiendo otro mensaje a la conversación."}'
  ```
  ...
  response = client.dm.send_message_to_conversation(
      dm_conversation_id="1582103724607971328",
      text="Añadiendo otro mensaje a la conversación."
  )
...
<Steps>
  <Step title="Cargar el contenido multimedia">
    Primero, carga el contenido multimedia mediante el [endpoint Media Upload](/es/x-api/media/quickstart/media-upload-chunked).
  </Step>
...
Cuando uses OAuth 2.0 PKCE, tu token de acceso debe incluir estos permisos:

| Scope | Descripción |
|:------|:------------|
| `dm.write` | Enviar y eliminar mensajes |
| `dm.read` | Leer conversaciones (obligatorio con dm.write) |
| `tweet.read` | Obligatorio para algunas expansions |
| `users.read` | Obligatorio para expansions de usuario |

...
description: Conceptos clave y prácticas recomendadas para integrar endpoints de consulta de mensajes directos (DM)
...
Los endpoints de Mensajes Directos (DM) requieren autenticación de usuario para acceder a conversaciones privadas:
...
<div id="required-scopes-oauth-20">
  ### Permisos (scopes) obligatorios (OAuth 2.0)
</div>

| Alcance | Obligatorio para |
|:------|:-------------|
...
<CardGroup cols={2}>
  <Card title="Uno a uno" icon="message">
    Siempre tiene exactamente dos participantes. Formato del id de la conversación: `{smaller_user_id}-{larger_user_id}`
  </Card>
...
|:------|:------------|:-------|
| `dm_conversation_id` | ID de la conversación | All |
| `created_at` | Marca de tiempo del evento | All |
...
| `attachments.media_keys` | Objetos multimedia |
| `referenced_tweets.id` | Objetos de publicaciones |
...
<CodeGroup dropdown>
  ...
  # Obtener eventos de mensajes directos (DM) con expansions
...
<Note>
  Los eventos de hasta **30 días de antigüedad** están disponibles.
</Note>
...
Los IDs de conversación y de eventos se comparten entre los endpoints v1.1 y v2. Esto significa que puedes:

* Usar v2 para obtener eventos y luego usar v1.1 para eliminar mensajes específicos
* Hacer referencia a IDs de conversación de URL de x.com en solicitudes a la API
...
<CardGroup cols={2}>
  <Card title="Guía de inicio rápido" icon="rocket" href="/es/x-api/direct-messages/lookup/quickstart">
    Realiza tu primera solicitud para consultar Mensajes Directos
  </Card>

  <Card title="Enviar Mensajes Directos" icon="paper-plane" href="/es/x-api/direct-messages/manage/introduction">

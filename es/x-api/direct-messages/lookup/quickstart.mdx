...
description: Obtén eventos y conversaciones de Mensajes Directos
...
<CodeGroup dropdown>
  ...
  # Obtener todos los eventos de DM con paginación
...
<CodeGroup dropdown>
  ...
  # Obtener eventos de mensajes directos de una conversación por id
...
<CodeGroup dropdown>
  ...
  # Obtén solo eventos MessageCreate
  for page in client.dm_events.list(
      event_types=["MessageCreate"],
      dm_event_fields=["created_at", "sender_id", "text"]
  ):
...
<CodeGroup dropdown>
  ...
  for await (const page of paginator) {
    ...
    // Los objetos de usuario del remitente están en page.includes.users
  }
  ```
</CodeGroup>

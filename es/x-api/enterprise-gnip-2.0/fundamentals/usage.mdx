---
title: Usage API
description: "La Usage API es una API REST gratuita que proporciona acceso programático y visibilidad del consumo de actividad en todos los productos de tu cuenta empresarial."
sidebarTitle: Usage API
keywords: ["Usage API", "enterprise usage API", "seguimiento de uso", "seguimiento de consumo", "uso empresarial", "uso de la API para empresas"]
---
...
Es la herramienta más importante y *la mejor* para ayudarte a supervisar y gestionar el uso de las diferentes API de tu cuenta.

**Aviso importante:**

Los recuentos de uso devueltos por la Usage API pueden no coincidir con los de una factura debido a periodos de prueba y otros ajustes de facturación. Todas las cifras se basan en actividades deduplicadas consumidas dentro de un día determinado (en UTC).
...
* Datos granulares y descriptivos: las “requests” de búsqueda se desglosan por productos Full-Archive y 30-Day Search 
...
A continuación se muestra una lista de las API que actualmente son compatibles con la Usage API:
...
* Usage API te permite acceder a datos de uso desde el 1 de mayo de 2018. A partir del 1 de julio de 2019, Usage API te permite acceder a datos de uso de los **últimos 13 meses naturales**

* Puedes acceder a datos de uso en **intervalos de tres meses** definidos con fromDate y toDate
...
* :account&#95;name es el nombre (distingue entre mayúsculas y minúsculas) asociado con tu cuenta, tal como se muestra en console.gnip.com

<div id="authentication-and-rate-limit">
  #### **Autenticación y límite de solicitudes**
</div>
...
La API Usage impone un límite de dos solicitudes por minuto.
...
Los datos de uso se basan en actividades sin duplicados consumidas hasta el último período de tiempo completo (UTC) en el que se procesaron los datos. En general, los datos se procesan y actualizan al minuto, excepto en los casos en que Gnip esté realizando despliegues de sistemas.

* Usage API te permite acceder a los datos de uso desde el 1 de mayo de 2018. Después del 1 de julio de 2019, Usage API te permite acceder a los datos de uso de los **últimos 13 meses naturales**.
...
La Usage API funciona realizando una solicitud HTTP GET con credenciales HTTP BASIC-AUTH al endpoint de la API correspondiente a tu cuenta.
...
|     |     |
| :--- | :--- |
| bucket | Opcional. La unidad de tiempo para la cual se proporcionarán los datos de uso. Los datos de uso se pueden devolver con granularidad diaria o mensual.  <br />  <br />Las solicitudes realizadas sin un bucket especificado devolverán datos con granularidad mensual.  <br />  <br />Opciones: &#39;month&#39; o &#39;day&#39; |
| fromDate (YYYYMMDDHHMM) | Opcional. Los datos de uso solo están disponibles a partir del 1 de mayo de 2018. La marca de tiempo UTC más antigua desde la cual se proporcionarán los datos de uso. La marca de tiempo tiene granularidad de día y es inclusiva (es decir, 201805010000 incluye el día 0501). Las solicitudes que contengan valores distintos de &#39;0000&#39; para la granularidad de hora y minuto se ajustarán de forma predeterminada a &#39;0000&#39;.  <br />  <br />Las solicitudes realizadas sin un fromDate ni un toDate devolverán datos de uso por mes para el mes actual e incluirán una referencia histórica de los dos meses anteriores.  <br />  <br />**Ten en cuenta:** A partir del 1 de junio de 2019, puedes acceder a los datos de uso de los últimos 13 meses calendario. Por ejemplo, si fuera el 10 de octubre, puedes acceder a datos de uso hasta el 1 de septiembre del año anterior.  <br />**Ejemplo:** 201810010000 devolverá datos a partir del 1 de octubre de 2018 en adelante, incluyendo el 1 de octubre. |
| toDate (YYYYMMDDHHMM) | Opcional. La marca de tiempo UTC más reciente hasta la cual se proporcionarán los datos de uso. La marca de tiempo tiene granularidad de día y no es inclusiva (es decir, 201703020000 no incluye datos para el día 0302). Cuando se especifica un toDate para el día actual o un día futuro, los datos de uso se devolverán hasta el último día completo (UTC). Las solicitudes que contengan valores distintos de &#39;0000&#39; para la granularidad de hora y minuto se ajustarán de forma predeterminada a &#39;0000&#39;.  <br />  <br />Una solicitud sin toDate tomará por defecto el siguiente bucket (mañana para bucket=day y el próximo mes para bucket=month). Una solicitud realizada sin fromDate ni toDate tomará por defecto bucket=month y mostrará datos para el mes actual más los dos meses inmediatamente anteriores.  <br />  <br />**Ejemplo:** 201703050000 devolverá datos hasta el 5 de marzo de 2017, sin incluir ningún dato del 5 de marzo. |
...
Las siguientes tablas describen las estructuras de datos de nivel raíz de los datos de uso devueltos por la Usage API. Para los campos con varios niveles de subcampos, haz clic en los enlaces proporcionados para ver los detalles de los subcampos.
...
| **fromDate** | La marca de tiempo UTC más temprana para la que deseas obtener datos de uso (incluida). |
| **toDate** | La marca de tiempo UTC más reciente para la que deseas obtener datos de uso (excluida). |

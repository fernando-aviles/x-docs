...
description: Conceptos clave y mejores prácticas para integrar la búsqueda de usuarios
keywords: ["integración de búsqueda de usuarios", "guía de integración de usuarios", "implementación de la búsqueda de usuarios"]
---

import { Button } from '/snippets/es/button.mdx';

Esta guía abarca los conceptos clave que debes conocer para integrar los endpoints de búsqueda de usuarios en tu aplicación.
...
Todos los endpoints de X API v2 requieren autenticación. Elige el método que se adapte a tu caso de uso:

| Método | Ideal para | ¿Puede acceder a métricas privadas? |
|:-------|:-----------|:------------------------------------|
| [OAuth 2.0 App-Only](/es/resources/fundamentals/authentication#oauth-2-0) | Comunicación entre servidores, datos públicos | No |
...
Para los datos públicos de usuario, utiliza un Bearer Token:
...
Obligatorio para el endpoint de usuario autenticado (`/2/users/me`):
...
<CodeGroup dropdown>
  ...
  # Uso de un token de acceso de usuario de OAuth 2.0
...
<Warning>
  El endpoint `/2/users/me` solo funciona con autenticación en contexto de usuario. Los tokens de solo aplicación devolverán un error.
</Warning>
...
X API v2 devuelve datos mínimos de forma predeterminada. Usa `fields` y `expansions` para solicitar exactamente lo que necesitas.
...
<Accordion title="user.fields">
  ...
  | `entities` | URL extraídas de la biografía |
...
<Accordion title="tweet.fields (requiere expansión pinned_tweet_id)">
  | Campo | Descripción |
  |:------|:------------|
  ...
  | `entities` | Hashtags, menciones y URL |
</Accordion>
...
<CodeGroup dropdown>
  ...
  # Obtener usuario con campos adicionales y expansiones
...
<Card title="Guía sobre campos y expansions" icon="sliders" href="/es/x-api/fundamentals/fields">
...
Busca varios usuarios en una sola solicitud:
...
<CodeGroup dropdown>
  ...
  # Obtener varios usuarios por nombre de usuario
...
| 403 | Acceso prohibido | Verifica los permisos de la aplicación |
| 404 | No encontrado | El usuario no existe o ha sido suspendido |
...
* La consulta de múltiples usuarios omite a ese usuario de los resultados e incluye un array `errors`
...
Para las cuentas protegidas que no sigues:
...
<CardGroup cols={2}>
  <Card title="Solicitudes por lotes" icon="layer-group">
    Utiliza endpoints multiusuario para obtener hasta 100 usuarios a la vez y reducir las llamadas a la API.
  </Card>
  ...
  <Card title="Almacena en caché los datos de usuario" icon="database">
    Almacena en caché los perfiles de usuario localmente para reducir las solicitudes repetidas.
  </Card>

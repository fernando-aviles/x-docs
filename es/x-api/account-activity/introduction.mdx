---
title: API de actividad de la cuenta
sidebarTitle: Introducción
description: Recibe en tiempo real la actividad de la cuenta mediante webhooks
keywords: ["actividad de la cuenta", "webhooks", "eventos en tiempo real", "API de actividad de la cuenta", "eventos de webhook"]
---

import { Button } from '/snippets/es/button.mdx';

La Account Activity API entrega eventos de actividad de cuenta en tiempo real a tu webhook. Suscríbete a cuentas de usuario y recibe notificaciones cuando publiquen, reciban mensajes directos (DM), las sigan y más.

<div id="overview">
  ## Descripción general
</div>

<CardGroup cols={2}>
  <Card title="Entrega mediante webhooks" icon="webhook">
    Eventos enviados a tu servidor
  </Card>

  <Card title="En tiempo real" icon="bolt">
    Notificaciones de eventos en tiempo real
  </Card>

  <Card title="Cobertura completa" icon="list">
    Publicaciones, mensajes directos (DMs), follows, likes y más
  </Card>

  <Card title="Basado en suscripción" icon="bell">
    Suscríbete a cuentas de usuario
  </Card>
</CardGroup>

***

<div id="how-it-works">
  ## Cómo funciona
</div>

1. **Registrar webhook** — Registra la URL de tu webhook
2. **Suscribir usuarios** — Agrega suscripciones de usuarios
3. **Recibir eventos** — Recibe eventos de actividad mediante solicitudes POST
4. **Procesar eventos** — Procesa los eventos en tu aplicación

***

<div id="event-types">
  ## Tipos de eventos
</div>

| Categoría | Eventos |
|:---------|:-------|
| **Publicaciones** | Crear, eliminar, marcar como favorito, quitar de favoritos |
| **Mensajes directos** | Recibidos, enviados, indicador de escritura |
| **Seguimientos** | Seguir, dejar de seguir |
| **Bloqueos** | Bloquear, desbloquear |
| **Silencios** | Silenciar, dejar de silenciar |

***

<div id="webhook-security">
  ## Seguridad de los webhooks
</div>

Todas las solicitudes de webhook incluyen:

* **Validación CRC** — Verificar la autenticidad del webhook
* **Encabezado de firma** — `x-twitter-webhooks-signature`
* **HTTPS obligatorio** — Todas las URL de webhooks deben usar HTTPS

***

<div id="example-register-a-webhook">
  ## Ejemplo: Registrar un webhook
</div>

```bash
curl -X POST "https://api.x.com/1.1/account_activity/all/env-name/webhooks.json?\
url=https%3A%2F%2Fyour-server.com%2Fwebhook" \
  -H "Authorization: OAuth oauth_consumer_key=..."
```

***

<div id="getting-started">
  ## Primeros pasos
</div>

<Note>
  **Requisitos previos**

  * Una [cuenta de desarrollador](https://developer.x.com/en/portal/petition/essential/basic-info) aprobada
  * Un [Project and App](/es/resources/fundamentals/developer-apps) en la Consola de Desarrollador
  * Un endpoint de webhook HTTPS accesible públicamente
</Note>

<CardGroup cols={2}>
  <Card title="Flujo de actividad" icon="stream" href="/es/x-api/activity/introduction">
    Alternativa basada en streaming a los webhooks
  </Card>

  <Card title="Guía de migración" icon="right-left" href="/es/x-api/account-activity/migrate/overview">
    Migrar desde v1.1
  </Card>

  <Card title="Referencia de la API" icon="code" href="/es/x-api/account-activity/register-webhook">
    Documentación completa del endpoint
  </Card>
</CardGroup>
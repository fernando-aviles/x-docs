---
title: Inicio rápido
sidebarTitle: Inicio rápido
description: Consultar listas por id o propietario
keywords: ["inicio rápido consulta de listas", "tutorial para obtener una lista", "guía para consultar listas"]
---

import { Button } from '/snippets/es/button.mdx';

Esta guía te explica cómo consultar información de List utilizando los endpoints de consulta de List.

<Note>
  **Requisitos previos**

  Antes de comenzar, necesitarás:

  * Una [cuenta de desarrollador](https://developer.x.com/en/portal/petition/essential/basic-info) con una App aprobada
  * El Bearer Token de tu App
</Note>

***

<div id="get-a-list-by-id">
  ## Obtener una Lista por ID
</div>

Obtén los detalles de una Lista específica:

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/lists/1234567890?\
  list.fields=description,owner_id,member_count,follower_count,private,created_at" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # Obtener una Lista por ID
  response = client.lists.get(
      "1234567890",
      list_fields=["description", "owner_id", "member_count", "follower_count", "private", "created_at"]
  )

  print(f"Lista: {response.data.name}")
  print(f"Miembros: {response.data.member_count}")
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // Obtener una Lista por ID
  const response = await client.lists.get("1234567890", {
    listFields: ["description", "owner_id", "member_count", "follower_count", "private", "created_at"],
  });

  console.log(`Lista: ${response.data?.name}`);
  console.log(`Miembros: ${response.data?.member_count}`);
  ```
</CodeGroup>

<div id="response">
  ### Respuesta
</div>

```json
{
  "data": {
    "id": "1234567890",
    "name": "Tech News",
    "description": "Top tech journalists and publications",
    "owner_id": "2244994945",
    "private": false,
    "member_count": 50,
    "follower_count": 1250,
    "created_at": "2023-01-15T10:00:00.000Z"
  }
}
```

***

<div id="get-lists-owned-by-a-user">
  ## Obtener Listas que pertenecen a un usuario
</div>

Recupera todas las Listas que pertenecen a un usuario específico:

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/users/2244994945/owned_lists?\
  list.fields=description,member_count,follower_count&\
  max_results=100" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # Obtener Listas que pertenecen a un usuario con paginación
  for page in client.lists.get_owned_lists(
      "2244994945",
      list_fields=["description", "member_count", "follower_count"],
      max_results=100
  ):
      for lst in page.data:
          print(f"{lst.name} - {lst.member_count} miembros")
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // Obtener Listas que pertenecen a un usuario con paginación
  const paginator = client.lists.getOwnedLists("2244994945", {
    listFields: ["description", "member_count", "follower_count"],
    maxResults: 100,
  });

  for await (const page of paginator) {
    page.data?.forEach((lst) => {
      console.log(`${lst.name} - ${lst.member_count} miembros`);
    });
  }
  ```
</CodeGroup>

<div id="response">
  ### Respuesta
</div>

```json
{
  "data": [
    {
      "id": "1234567890",
      "name": "Tech News",
      "description": "Top tech journalists",
      "member_count": 50,
      "follower_count": 1250
    },
    {
      "id": "9876543210",
      "name": "Developer Tools",
      "description": "Useful tools for developers",
      "member_count": 25,
      "follower_count": 500
    }
  ],
  "meta": {
    "result_count": 2
  }
}
```

***

<div id="include-owner-information">
  ## Incluir información del propietario
</div>

Amplía los datos del usuario propietario:

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/lists/1234567890?\
  list.fields=description,owner_id&\
  expansions=owner_id&\
  user.fields=username,verified" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # Obtener la lista con información del propietario
  response = client.lists.get(
      "1234567890",
      list_fields=["description", "owner_id"],
      expansions=["owner_id"],
      user_fields=["username", "verified"]
  )

  print(f"List: {response.data.name}")
  # La información del propietario se encuentra en response.includes.users
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // Obtener la lista con información del propietario
  const response = await client.lists.get("1234567890", {
    listFields: ["description", "owner_id"],
    expansions: ["owner_id"],
    userFields: ["username", "verified"],
  });

  console.log(`List: ${response.data?.name}`);
  // La información del propietario se encuentra en response.includes?.users
  ```
</CodeGroup>

<div id="response-with-expansion">
  ### Respuesta con expansiones
</div>

```json
{
  "data": {
    "id": "1234567890",
    "name": "Tech News",
    "description": "Top tech journalists",
    "owner_id": "2244994945"
  },
  "includes": {
    "users": [
      {
        "id": "2244994945",
        "username": "XDevelopers",
        "verified": true
      }
    ]
  }
}
```

***

<div id="available-fields">
  ## Campos disponibles
</div>

| Campo | Descripción |
|:------|:------------|
| `description` | Descripción de la Lista |
| `owner_id` | ID de usuario del propietario |
| `private` | Indica si la Lista es privada |
| `member_count` | Número de miembros |
| `follower_count` | Número de seguidores |
| `created_at` | Fecha de creación de la Lista |

***

<div id="next-steps">
  ## Próximos pasos
</div>

<CardGroup cols={2}>
  <Card title="Publicaciones de listas" icon="list" href="/es/x-api/lists/list-tweets/quickstart">
    Obtén las publicaciones de una lista
  </Card>

  <Card title="Miembros de listas" icon="users" href="/es/x-api/lists/list-members/quickstart/list-members-lookup">
    Obtén los miembros de una lista
  </Card>

  <Card title="Administrar listas" icon="pen" href="/es/x-api/lists/manage-lists/quickstart">
    Crea y actualiza listas
  </Card>

  <Card title="Referencia de la API" icon="code" href="/es/x-api/lists/list-lookup-by-list-id">
    Documentación completa del endpoint
  </Card>
</CardGroup>
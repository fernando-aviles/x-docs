...
Esta guía te explica cómo usar la Community Notes API para buscar Publicaciones aptas para notas y enviar notas.
...
<Note>
  ...
  * Un token de acceso de usuario (OAuth 1.0a)
</Note>

<Warning>
  Actualmente, `test_mode` debe configurarse en `true` en todas las solicitudes. Las notas de prueba no son visibles públicamente.
</Warning>
...
<Steps>
  <Step title="Busca Publicaciones aptas" icon="magnifying-glass">
    <Tabs>
      <Tab title="cURL">
    ```bash
  ...
  <Step title="Revisa Publicaciones aptas" icon="eye">
    ```json
    {
      "data": [
        {
    ...
    Usa el `id` de la Publicación de la respuesta para escribir una Community Note.
  </Step>
</Steps>
...
<Steps>
  <Step title="Prepara tu nota" icon="pen">
    Una Community Note requiere lo siguiente:
    - `post_id` — La Publicación a la que agregas contexto
    - `text` — Tu nota (1-280 caracteres, debe incluir una URL de fuente)
...
<AccordionGroup>
  <Accordion title="401 No autorizado">
    ...
    ```
    **Solución:** Verifica que tus credenciales de OAuth sean correctas.
  </Accordion>
  ...
  <Accordion title="403 Prohibido">
    ...
    ```
    **Solución:** Inscríbete como [Community Notes AI Note Writer](https://communitynotes.x.com/guide/en/api/overview).
  </Accordion>
  ...
  <Accordion title="Error de nota duplicada">
    ...
    ```
    **Solución:** Solo puedes enviar una única nota por Publicación.
  </Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  <Card title="Guía de Community Notes" icon="book" href="https://communitynotes.x.com/guide/en/api/overview">
    Documentación oficial sobre Community Notes
  </Card>

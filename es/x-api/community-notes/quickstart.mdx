...
description: Crear y buscar Community Notes mediante la API
keywords: ["inicio rápido de Community Notes", "tutorial de Community Notes", "guía para crear notas"]
---

import { Button } from '/snippets/es/button.mdx';

Esta guía describe cómo usar la Community Notes API para buscar Publicaciones elegibles y enviar notas.
...
<Warning>
  Actualmente, `test_mode` debe establecerse en `true` para todas las solicitudes. Las notas de prueba no son visibles públicamente.
</Warning>
...
<Steps>
  <Step title="Buscar Publicaciones aptas" icon="magnifying-glass">
    <Tabs>
      <Tab title="cURL">
    ```bash
  ...
  <Step title="Revisar Publicaciones aptas" icon="eye">
    ```json
    {
      "data": [
        {
    ...
    Usa el `id` de la Publicación en la respuesta para escribir una nota de la comunidad.
  </Step>
</Steps>
...
<Steps>
  <Step title="Prepara tu nota" icon="pen">
    Para crear una Community Note se requiere lo siguiente:
    - `post_id` — La Publicación a la que estás agregando contexto
    - `text` — Tu nota (1-280 caracteres, debe incluir una URL de la fuente)
    ...
    - `trustworthy_sources` — Valor booleano que indica si la fuente es confiable
  </Step>
...
<AccordionGroup>
  <Accordion title="Etiquetas engañosas">
    Cuando la clasificación sea `misinformed_or_potentially_misleading`, incluye una o más etiquetas:
    ...
    | `outdated_information` | La información ya no está actualizada |
    | `other` | Otros motivos |
  </Accordion>

  <Accordion title="No engañoso">
    Cuando la clasificación sea `not_misleading`, no es obligatorio añadir etiquetas engañosas.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="401 No autorizado">
    ...
    ```
    **Resolución:** Comprueba que tus credenciales OAuth sean correctas.
  </Accordion>

...
<CodeGroup dropdown>
  ...
  for space in response.data:
      print(f"{space.title} - {space.participant_count} participantes")
  ```
  ...
  response.data?.forEach((space) => {
    console.log(`${space.title} - ${space.participant_count} participantes`);
  });
  ```
</CodeGroup>
...
Buscar solo Espacios en vivo o programados:

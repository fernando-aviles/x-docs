...
keywords: ["inicio rápido: búsqueda de Spaces", "tutorial de búsqueda de Spaces", "guía para encontrar Spaces"]
---

import { Button } from '/snippets/es/button.mdx';

Esta guía te explica cómo buscar Spaces por palabra clave.
...
Busca Spaces que coincidan con una palabra clave específica:
...
Busca únicamente Spaces en vivo o programados:

<div id="live-spaces-only">
  ### Solo Spaces en directo
</div>
...
<CodeGroup dropdown>
  ...
  # Buscar solo Spaces programados
...
Expandir los datos del usuario anfitrión:
...
<CodeGroup dropdown>
  ...
  # Buscar con información del anfitrión
  response = client.spaces.search(
      query="AI",
...
| `max_results` | Número de resultados que se devolverán (1-100) |
| `space.fields` | Campos de Space que se incluirán |
| `expansions` | Objetos relacionados que se incluirán |
| `user.fields` | Campos de usuario que se incluirán |

---
title: Inicio rápido
sidebarTitle: Inicio rápido
description: Obtén los detalles de un Space por id o por creador
keywords: ["inicio rápido de Spaces", "tutorial de búsqueda de Spaces", "guía de Spaces"]
---

import { Button } from '/snippets/es/button.mdx';

Esta guía te explica cómo obtener información de Spaces usando los endpoints de consulta de Spaces.

<Note>
  **Requisitos previos**

  Antes de comenzar, necesitarás:

  * Una [cuenta de desarrollador](https://developer.x.com/en/portal/petition/essential/basic-info) con una App aprobada
  * El Bearer Token de tu App
</Note>

***

<div id="get-a-space-by-id">
  ## Obtener un Space por ID
</div>

Recupera los detalles de un Space específico:

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/spaces/1DXxyRYNejbKM?\
  space.fields=title,host_ids,participant_count,scheduled_start,state,created_at" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # Obtener un Space por ID
  response = client.spaces.get(
      "1DXxyRYNejbKM",
      space_fields=["title", "host_ids", "participant_count", "scheduled_start", "state", "created_at"]
  )

  print(f"Space: {response.data.title}")
  print(f"Estado: {response.data.state}")
  print(f"Participantes: {response.data.participant_count}")
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // Obtener un Space por ID
  const response = await client.spaces.get("1DXxyRYNejbKM", {
    spaceFields: ["title", "host_ids", "participant_count", "scheduled_start", "state", "created_at"],
  });

  console.log(`Space: ${response.data?.title}`);
  console.log(`Estado: ${response.data?.state}`);
  console.log(`Participantes: ${response.data?.participant_count}`);
  ```
</CodeGroup>

<div id="response">
  ### Respuesta
</div>

```json
{
  "data": {
    "id": "1DXxyRYNejbKM",
    "state": "live",
    "title": "Discussing AI and the Future",
    "host_ids": ["2244994945"],
    "participant_count": 245,
    "created_at": "2024-01-15T09:00:00.000Z"
  }
}
```

***

<div id="get-multiple-spaces">
  ## Obtener varios Spaces
</div>

Consulta varios Spaces a la vez:

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/spaces?\
  ids=1DXxyRYNejbKM,1YqJDqWYNQDGW&\
  space.fields=title,state,participant_count" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # Obtener varios Spaces
  response = client.spaces.get_spaces(
      ids=["1DXxyRYNejbKM", "1YqJDqWYNQDGW"],
      space_fields=["title", "state", "participant_count"]
  )

  for space in response.data:
      print(f"{space.title} - {space.state}")
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // Obtener varios Spaces
  const response = await client.spaces.getSpaces({
    ids: ["1DXxyRYNejbKM", "1YqJDqWYNQDGW"],
    spaceFields: ["title", "state", "participant_count"],
  });

  response.data?.forEach((space) => {
    console.log(`${space.title} - ${space.state}`);
  });
  ```
</CodeGroup>

***

<div id="get-spaces-by-creator">
  ## Obtener Spaces por creador
</div>

Encuentra Spaces organizados por usuarios específicos:

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/spaces/by/creator_ids?\
  user_ids=2244994945,783214&\
  space.fields=title,state,scheduled_start" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # Obtener Spaces por creador
  response = client.spaces.get_by_creator_ids(
      user_ids=["2244994945", "783214"],
      space_fields=["title", "state", "scheduled_start"]
  )

  for space in response.data:
      print(f"{space.title} - {space.state}")
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // Obtener Spaces por creador
  const response = await client.spaces.getByCreatorIds({
    userIds: ["2244994945", "783214"],
    spaceFields: ["title", "state", "scheduled_start"],
  });

  response.data?.forEach((space) => {
    console.log(`${space.title} - ${space.state}`);
  });
  ```
</CodeGroup>

***

<div id="include-host-information">
  ## Incluir información del anfitrión
</div>

Amplía los datos del usuario anfitrión:

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/spaces/1DXxyRYNejbKM?\
  space.fields=title,host_ids,state&\
  expansions=host_ids&\
  user.fields=username,verified" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # Obtener un Space con información del anfitrión
  response = client.spaces.get(
      "1DXxyRYNejbKM",
      space_fields=["title", "host_ids", "state"],
      expansions=["host_ids"],
      user_fields=["username", "verified"]
  )

  print(f"Space: {response.data.title}")
  # La información del anfitrión se encuentra en response.includes.users
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // Obtener un Space con información del anfitrión
  const response = await client.spaces.get("1DXxyRYNejbKM", {
    spaceFields: ["title", "host_ids", "state"],
    expansions: ["host_ids"],
    userFields: ["username", "verified"],
  });

  console.log(`Space: ${response.data?.title}`);
  // La información del anfitrión se encuentra en response.includes?.users
  ```
</CodeGroup>

<div id="response-with-expansion">
  ### Respuesta con expansiones
</div>

```json
{
  "data": {
    "id": "1DXxyRYNejbKM",
    "state": "live",
    "title": "Discussing AI and the Future",
    "host_ids": ["2244994945"]
  },
  "includes": {
    "users": [
      {
        "id": "2244994945",
        "username": "XDevelopers",
        "verified": true
      }
    ]
  }
}
```

***

<div id="space-states">
  ## Estados de un Space
</div>

| Estado | Descripción |
|:------|:------------|
| `live` | En curso |
| `scheduled` | Programado |
| `ended` | Finalizado |

***

<div id="available-fields">
  ## Campos disponibles
</div>

| Campo | Descripción |
|:------|:------------|
| `title` | Título del Space |
| `host_ids` | ID de usuarios anfitriones |
| `speaker_ids` | ID de usuarios oradores |
| `participant_count` | Número actual de participantes |
| `scheduled_start` | Hora de inicio programada |
| `started_at` | Hora de inicio real |
| `ended_at` | Hora de finalización |
| `is_ticketed` | Indica si el Space tiene entradas de pago |
| `state` | Estado actual |

***

<div id="next-steps">
  ## Próximos pasos
</div>

<CardGroup cols={2}>
  <Card title="Buscar Spaces" icon="magnifying-glass" href="/es/x-api/spaces/search/quickstart">
    Busca Spaces por palabra clave
  </Card>

  <Card title="Referencia de la API" icon="code" href="/es/x-api/spaces/space-lookup-by-space-id">
    Documentación completa del endpoint
  </Card>
</CardGroup>
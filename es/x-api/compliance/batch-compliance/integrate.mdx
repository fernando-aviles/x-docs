...
keywords: ["integración con cumplimiento por lotes", "integración de cumplimiento por lotes", "integración de trabajos de cumplimiento", "configuración de cumplimiento por lotes"]
---

import { Button } from '/snippets/es/button.mdx';

<div id="working-with-resumable-uploads">
  ## Uso de cargas reanudables
</div>

Al usar los endpoints de cumplimiento por lotes, los desarrolladores pueden cargar por lotes grandes cantidades de datos de X y entender qué acción se necesita para garantizar que sus conjuntos de datos reflejen la intención del usuario y el estado actual del contenido en X. Cargar grandes cantidades de datos a un servidor remoto es una operación relativamente sencilla cuando los sistemas y la conectividad son estables y confiables. Sin embargo, esto no siempre es así. Algunos entornos pueden imponer un tiempo de espera de la conexión, lo que efectivamente corta la conexión entre tu aplicación y el servidor de carga después de un período de tiempo determinado; también puedes encontrarte con problemas de conexión, por ejemplo, al intentar cargar un archivo grande desde tu portátil a través de una conexión Wi‑Fi. En estas circunstancias, es preferible cargar porciones más pequeñas de ese archivo a la vez, en lugar de mantener una única conexión continua.

Los endpoints de cumplimiento por lotes de X se basan en Google Cloud Storage para procesar archivos grandes. Este tipo de almacenamiento está optimizado para diversas aplicaciones; Cloud Storage admite una técnica para gestionar archivos grandes denominada cargas reanudables.
...
<div id="creating-a-resumable-job">
  ### Creación de una tarea reanudable
</div>
...
Primero, deberás crear un trabajo de cumplimiento y especificar si subirás ID de Publicación o ID de usuario (usando el parámetro type). Además, agrega resumable al cuerpo y establécelo en true. Asegúrate de reemplazar $APP&#95;ACCESS&#95;TOKEN que aparece a continuación por tu App-only Access Token.
...
A continuación, deberás iniciar la carga reanudable. Para ello, realiza una solicitud **POST** a upload&#95;url del paso anterior y asegúrate de incluir los siguientes encabezados:
...
Si esta llamada se realiza correctamente, recibirás un código de respuesta 201. Luego, en el encabezado de respuesta, copia el valor del encabezado Location, que tendrá un aspecto similar a este:

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

A continuación, puedes subir las id de tus Publicaciones o Usuarios a esta ubicación siguiendo a partir del paso dos de [la guía de inicio rápido](/es/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints).

Debido a su complejidad técnica, las subidas reanudables se usan mejor junto con código. Esta guía utilizará Node.js con la [biblioteca de peticiones needle](https://www.npmjs.com/package/needle).

<div id="install-the-dependencies">
  ### Instalar las dependencias
</div>

Antes de continuar, debes tener un entorno de Node.js instalado; puedes obtener Node.js desde su sitio web. Una vez instalado, Node.js incluirá una utilidad llamada npm; asegúrate de que tanto Node como npm estén instalados ejecutando el siguiente comando y verifica que no produzca un error.

`$ npm -v
6.4.1`

Un número de versión similar a este indica que tu entorno está listo (ten en cuenta que tu número de versión puede ser diferente). Usaremos npm para instalar la biblioteca de subida. Ejecuta este comando:
...
Al crear un nuevo trabajo, establece el parámetro `resumable` en `true` para obtener un destino que admita una carga reanudable. En la carga útil de la respuesta, recibirás un valor `upload&#95;url`.
...
De forma predeterminada, la biblioteca creará un nuevo destino de carga a partir de una ubicación de carga (llamada bucket) y el nombre del archivo que deseas subir. Debido a que los endpoints de batch compliance crean su propio destino, necesitaremos indicarle a la biblioteca que ya tenemos una ubicación lista para aceptar nuestra carga.

Necesitaremos pasar este valor a la biblioteca de carga, junto con el nombre del archivo que contiene los datos que se van a subir. Crea un archivo y asígnale el nombre ***twitter-upload.js***. Agrega el siguiente código:
...
async function resumeUpload(response, file, url) {
  console.log('Carga no completada, reanudando');
...
Guarda el archivo donde tenga más sentido. A continuación, en tu línea de comandos, ejecuta el script y pásale dos parámetros:

1. El primero será la ubicación del archivo (con los id de Publicación o de usuario) que deseas subir.
2. El segundo será la URL de carga que recibimos en la respuesta del endpoint de cumplimiento.
...
Puedes pausar la carga en cualquier momento presionando Ctrl + C o cerrando la línea de comandos. Podrás reanudarla desde donde la dejaste cuando vuelvas a ejecutar el mismo comando más adelante. Una vez que el archivo se haya cargado por completo, verás el siguiente mensaje:

`Upload complete`

En este punto, podrás usar el [endpoint de estado de conformidad](/es/x-api/compliance/get-compliance-job) para comprobar el estado de tu trabajo de conformidad y descargar el resultado de conformidad cuando se haya completado.

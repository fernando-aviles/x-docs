---
title: Tendencias por WOEID
sidebarTitle: Introducción
description: Obtén temas del momento para una ubicación geográfica específica
keywords: ["tendencias por WOEID", "tendencias por ubicación", "tendencias geográficas", "tendencias regionales", "API de WOEID"]
---

import { Button } from '/snippets/es/button.mdx';

El endpoint Trends by WOEID devuelve los temas del momento para una ubicación geográfica específica, identificada mediante un Yahoo! Where On Earth ID (WOEID).

<div id="overview">
  ## Resumen
</div>

<CardGroup cols={2}>
  <Card title="Específico por ubicación" icon="location-dot">
    Tendencias para cualquier ubicación compatible
  </Card>

  <Card title="Cobertura global" icon="globe">
    Países, ciudades y regiones
  </Card>

  <Card title="En tiempo real" icon="bolt">
    Temas en tendencia ahora
  </Card>
</CardGroup>

***

<div id="endpoint">
  ## Endpoint
</div>

| Método | Endpoint | Descripción |
|:-------|:---------|:------------|
| GET | [`/2/trends/by/woeid/:id`](/es/x-api/trends/get-trends-by-woeid) | Obtener las tendencias para un WOEID |

***

<div id="common-woeids">
  ## WOEID comunes
</div>

| Ubicación | WOEID |
|:---------|:------|
| Mundial | 1 |
| Estados Unidos | 23424977 |
| Reino Unido | 23424975 |
| Japón | 23424856 |
| Nueva York | 2459115 |
| Los Ángeles | 2442047 |
| Londres | 44418 |
| Tokio | 1118370 |

***

<div id="example-request">
  ## Ejemplo de solicitud
</div>

```bash
curl "https://api.x.com/2/trends/by/woeid/1" \
  -H "Authorization: Bearer $BEARER_TOKEN"
```

<div id="example-response">
  ## Ejemplo de respuesta
</div>

```json
{
  "data": [
    {
      "trend_name": "#AI",
      "tweet_count": 250000
    },
    {
      "trend_name": "Breaking News",
      "tweet_count": 180000
    }
  ]
}
```

***

<div id="getting-started">
  ## Primeros pasos
</div>

<Note>
  **Requisitos previos**

  * Una [cuenta de desarrollador](https://developer.x.com/en/portal/petition/essential/basic-info) aprobada
  * Un [Project y una App](/es/resources/fundamentals/developer-apps) en la Developer Console
  * El [Bearer Token](/es/resources/fundamentals/authentication) de tu App
</Note>

<CardGroup cols={2}>
  <Card title="Tendencias personalizadas" icon="user" href="/es/x-api/trends/personalized-trends/introduction">
    Obtén tendencias para el usuario autenticado
  </Card>

  <Card title="Referencia de la API" icon="code" href="/es/x-api/trends/trends-by-woeid">
    Documentación completa del endpoint
  </Card>
</CardGroup>
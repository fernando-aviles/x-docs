...
keywords: ["campos", "campos de Tuit", "campos de usuario", "parámetros de campos", "campos de API", "campos de datos", "personalizar la respuesta"]
---

La X API v2 devuelve datos mínimos de forma predeterminada. Usa los **parámetros fields** para solicitar datos adicionales para cada tipo de objeto.
...
De forma predeterminada, una consulta de una Publicación devuelve únicamente `id`, `text` y `edit_history_tweet_ids`. Para obtener más datos, agrega parámetros de campos a tu solicitud:
...
Solicita campos de usuario específicos mediante `user.fields`:
...
* Los campos que faltan en las respuestas indican que el valor es `null` o está vacío
* Algunos campos requieren autenticación específica (por ejemplo, las métricas privadas necesitan contexto de usuario)
* Consulta la Referencia de la API de cada endpoint para conocer los campos disponibles

...
keywords: ["noticias", "API de noticias", "titulares", "última hora", "búsqueda de noticias", "artículos de noticias", "búsqueda de noticias"]
---

El endpoint de búsqueda de noticias permite a los desarrolladores obtener noticias y titulares de última hora en X.

Este endpoint admite autenticación de aplicación (app-auth) y de usuario (user-auth) para OAuth1 y OAuth2 PKCE.

<div id="getting-started">
  ## Primeros pasos
</div>

Para usar el endpoint, necesitas un [Bearer Token](/es/fundamentals/authentication/oauth-2-0/application-only) desde la [Developer Console](https://developer.x.com/en/portal/dashboard).

Una vez que tengas el Bearer Token, puedes realizar llamadas a la API de noticias como se muestra a continuación:

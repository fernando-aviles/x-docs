...
title: 移行ガイド
sidebarTitle: 移行ガイド
keywords: ["返信非表示 移行", "返信非表示 移行する", "移行ガイド", "返信非表示 を移行", "v2 移行"]
---
...
v2 の返信非表示エンドポイントは、Labs の返信非表示エンドポイントの後継です。すでにこの Labs 版エンドポイントを使用しているコードやアプリ、ツールがあり、新しい X API v2 エンドポイントへの移行を検討している場合は、このガイドを参照してください。

新しい X API v2（返信非表示エンドポイントを含む）を利用するには、[新しい開発者コンソールにオプトイン](https://developer.x.com/en/portal/opt-in)し、[Project](/ja/resources/fundamentals/developer-apps) を作成し、その Project に App を追加する必要があります。そのうえで、その App に紐づく認証情報を使用して返信非表示エンドポイントにリクエストを送信できます。Labs v2 の返信非表示エンドポイント用に登録済みの同じ App を追加すれば、ユーザーの認証状態を維持できます。

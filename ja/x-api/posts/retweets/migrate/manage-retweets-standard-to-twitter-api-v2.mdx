...
また、Standard v1.1 エンドポイントを使用する場合は、投稿をリツイートするユーザーの id は必須ではありません。[OAuth 1.0a User Context](/ja/resources/fundamentals/authentication) とともに渡される [Access Tokens](/ja/resources/fundamentals/authentication#obtaining-access-tokens-using-3-legged-oauth-flow) によって、どのユーザーがリツイートを実行または取り消しているかが判断されるためです。

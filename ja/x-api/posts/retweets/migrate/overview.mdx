...
| デフォルトのリクエスト [レート制限](/ja/resources/fundamentals/rate-limits) | なし<br /><br />3 時間あたり 300 件のリクエスト（ユーザーごと、App ごと）。これは POST Tweet エンドポイントと共有されます | 15 分あたり 50 件のリクエスト（ユーザーごと）<br /><br />3 時間あたり 300 件のリクエスト（ユーザーごと、App ごと）。これは投稿管理用 POST Tweet エンドポイントと共有されます。 |

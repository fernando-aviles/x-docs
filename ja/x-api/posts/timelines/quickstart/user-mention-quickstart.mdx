---
title: ユーザー宛てメンションのタイムライン
sidebarTitle: ユーザー宛てメンション
description: 特定のユーザーがメンションされている投稿を取得する
keywords: ["ユーザー宛てメンション", "メンションタイムライン", "メンションクイックスタート", "メンションチュートリアル", "メンションタイムライン", "メンションを取得"]
---

import { Button } from '/snippets/ja/button.mdx';

このガイドでは、特定のユーザーへのメンションを含む投稿を取得する方法を説明します。
...
<Note>
  ...
  * App の Bearer Token（公開データ用）または User Access Token（非公開メトリクス用）
</Note>
...
<Steps>
  <Step title="ユーザーIDを取得する">
    [ユーザーlookupエンドポイント](/ja/x-api/users/lookup/introduction)を使ってユーザーIDを特定します。たとえば、@XDevelopers のユーザーIDは `2244994945` です。
  </Step>
...
ユーザーをメンションしている投稿のうち、返信ではない元の投稿のみを取得します：

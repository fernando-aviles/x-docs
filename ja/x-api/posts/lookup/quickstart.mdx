...
description: 数分で最初の投稿ルックアップリクエストを実行できるようになります
keywords: ["投稿ルックアップ クイックスタート", "ツイートルックアップ クイックスタート", "ルックアップ チュートリアル", "ルックアップ ガイド", "ツイート取得 クイックスタート", "ルックアップ例"]
---

import { Button } from "/snippets/ja/button.mdx"

このガイドでは、X API v2 を使って最初の投稿ルックアップリクエストを送信する手順を説明します。
...
<Steps>
  <Step title="投稿IDを取得する">
    すべての投稿には固有のIDがあります。投稿のURLに含まれています:
  ...
  <Step title="追加フィールドをリクエストする">
    ...
    <CodeGroup dropdown>
      ...
      print(response.data)
      print(response.includes)  # 投稿者のユーザーオブジェクトを含む
      ```
      ...
      console.log(response.data);
      console.log(response.includes); // 投稿者のユーザーオブジェクトを含む
      ```
    </CodeGroup>

...
filtered stream を使用する場合、演算子は引用ツイートのコンテンツ**と**引用された元の投稿のコンテンツの両方にマッチします。

<Note>
  [Search Posts](/ja/x-api/posts/search/introduction) は挙動が異なり、引用ツイートのコンテンツのみにマッチし、元の投稿のコンテンツにはマッチしません。
</Note>

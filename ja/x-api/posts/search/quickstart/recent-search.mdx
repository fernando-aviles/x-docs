...
title: 最近検索クイックスタート
sidebarTitle: 最近検索
description: 数分で最初の最近検索リクエストを送信できるようになります
keywords: ["最近検索", "検索クイックスタート", "最近検索チュートリアル", "ツイートを検索", "最近検索ガイド", "検索例"]
---

import { Button } from '/snippets/ja/button.mdx';

このガイドでは、過去7日間の投稿を検索するための、最初の recent search リクエストの送信手順を説明します。
...
<Note>
  ...
  * 開発者コンソールの「Keys and tokens」で確認できる、アプリの Bearer Token
</Note>
...
<Steps>
  <Step title="クエリを作成する" icon="magnifying-glass">
    検索クエリではオペレーターを使って、条件に合う投稿を検索します。まずは単純なキーワードから始めてみましょう。
    ...
    これは、英語の投稿のうち「python」を含み、リポストを除外したものに一致します。

    <Tip>
      利用可能なすべてのオプションについては、[完全な演算子リファレンス](/ja/x-api/posts/search/integrate/operators)を参照してください。
    </Tip>
  </Step>
  ...
  <Step title="レスポンスを確認する" icon="eye">
    デフォルトのレスポンスには、`id`、`text`、`edit_history_tweet_ids` がインクルードされています。
  ...
  <Step title="フィールドと展開の追加" icon="sliders">
    クエリパラメーターで追加データをリクエストする：
...
<CardGroup cols={2}>
  <Card title="クエリを構築" icon="magnifying-glass" href="/ja/x-api/posts/search/integrate/build-a-query">
    クエリ構文とオペレーターをマスター
  </Card>

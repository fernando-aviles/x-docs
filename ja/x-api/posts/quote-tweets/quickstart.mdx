...
keywords: ["引用ツイートのクイックスタート", "引用ツイートのチュートリアル", "引用ツイート取得ガイド"]
---
...
<Steps>
  <Step title="投稿 ID を取得する" icon="message">
    引用を取得したい投稿の ID を取得します。投稿の URL から確認できます。
  ...
  <Step title="結果をページングする" icon="arrow-right">
    SDK はページネーションを自動的に処理します。cURL を使用する場合は、`next_token` を使ってさらに引用投稿を取得します。
...
<CardGroup cols={2}>
  ...
  <Card title="投稿の取得" icon="magnifying-glass" href="/ja/x-api/posts/lookup/introduction">
    IDで投稿を取得する
  </Card>

...
title: 最近の投稿数
sidebarTitle: 最近の投稿数
description: 過去7日間の投稿ボリュームを取得する
keywords: ["最近のツイート数クイックスタート", "最近の件数クイックスタート", "ツイートボリュームクイックスタート", "件数クイックスタート", "最近の検索件数"]
---
...
<Steps>
  <Step title="クエリを作成する">
    recent search エンドポイントと同じクエリ構文を使用します。たとえば、@XDevelopers からの投稿数を集計するには次のようにします:
...
<CodeGroup dropdown>
  ...
  print(f"合計投稿数: {response.meta.total_tweet_count}")
  ```
  ...
  console.log(`合計投稿数: ${response.meta?.total_tweet_count}`);
  ```
</CodeGroup>
...
| `start_time` | 最も古いタイムスタンプ（ISO 8601） | 7日前 |
...
<CardGroup cols={2}>
  <Card title="フルアーカイブ件数" icon="vault" href="/ja/x-api/posts/counts/quickstart/full-archive-tweet-counts">
    過去の投稿件数を取得
  </Card>

  <Card title="クエリを作成" icon="magnifying-glass" href="/ja/x-api/posts/counts/integrate/build-a-query">
    クエリ構文をマスター
  </Card>

...
title: いいねの管理
sidebarTitle: いいねの管理
keywords: ["「いいね」管理の移行", "v1.1 から v2 への「いいね」管理", "「いいね」管理を移行", "Standard から v2 への「いいね」", "「いいね」の移行"]
---
...
| :--- | :--- |
| id  | id  |
| includes&#95;entities | 対応するパラメータはありません |
...
また、Standard v1.1 エンドポイントを使用する場合、投稿に「いいね」するユーザーの id を指定する必要はありません。これは、[OAuth 1.0a User Context](/ja/resources/fundamentals/authentication) とともに渡される [Access Tokens](/ja/resources/fundamentals/authentication#obtaining-access-tokens-using-3-legged-oauth-flow) によって、どのユーザーが「いいね」や「いいね」の取り消しを実行しているかが推論されるためです。

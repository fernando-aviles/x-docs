...
誰かが投稿を投稿し、他の人が返信すると、すべての返信は同じ `conversation_id` を共有します。これは、その会話を開始した元の投稿のidです。
...
| `in_reply_to_user_id` | 返信対象の投稿のユーザーID |
| `referenced_tweets` | `type: "replied_to"` と親投稿のIDを持つ配列 |

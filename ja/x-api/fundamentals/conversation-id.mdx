...
title: 会話ID
sidebarTitle: 会話ID
description: 会話スレッドを追跡して再構築する
keywords: ["会話ID", "会話スレッド", "返信スレッド", "スレッドの追跡", "会話の追跡"]
---

X 上のすべての返信は、会話スレッドに属しています。`conversation_id` フィールドを使うと、会話ツリー全体を特定・追跡・再構築できます。
...
誰かが投稿し、他のユーザーが返信すると、すべての返信は同じ `conversation_id` を共有します。これは、その会話を開始した元の投稿のIDです。
...
スレッドがどれだけ深くなっても、すべての投稿には同じ `conversation_id` が付きます。
...
スレッド内のすべての投稿を取得するには、`conversation_id` を検索演算子として使用します。
...
これは、元の投稿へのすべての返信を、新しいものから順に返します。
...
| フィールド | 説明 |
|:------|:------------|
| `conversation_id` | スレッドを開始した元となる投稿のID |
| `in_reply_to_user_id` | 返信先の投稿のユーザーID |
| `referenced_tweets` | `type: "replied_to"` と親となる投稿のIDを含む配列 |
...
* 会話をリアルタイムに監視するために、[filtered stream](/ja/x-api/posts/filtered-stream/introduction) と併用します
* 大規模なスレッドを扱う場合は、[pagination](/ja/x-api/fundamentals/pagination) と組み合わせて使用します

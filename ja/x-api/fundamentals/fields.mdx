...
デフォルトでは、投稿の取得では `id`、`text`、`edit_history_tweet_ids` のみが返されます。より多くのデータを取得するには、リクエストにフィールドパラメータを指定してください。
...
| オブジェクト | パラメータ | ドキュメント |
|:-------|:----------|:--------------|
| 投稿（ツイート） | `tweet.fields` | [投稿のフィールド](/ja/x-api/fundamentals/data-dictionary#tweet) |
...
`tweet.fields` を使用して特定の投稿のフィールドをリクエストします:

---
title: 投稿を編集する
sidebarTitle: 投稿を編集する
description: X API で編集された投稿を扱う
keywords: ["投稿の編集", "ツイートの編集", "編集履歴", "投稿の編集内容", "編集メタデータ", "編集可能時間"]
---

X の投稿は、投稿から30分以内であれば最大 5 回まで編集できます。X API では、編集履歴およびメタデータに完全にアクセスできます。
...
すべての投稿のレスポンスには、`edit_history_tweet_ids` がデフォルトで含まれます。
...
編集ステータスを取得するには、`edit_controls` フィールドをリクエストします。
...
| `editable_until`   | 編集可能期間が終了するタイムスタンプ |
...
すべてのバージョンの完全な投稿オブジェクトを取得するには、`edit_history_tweet_ids` 拡張を使用します。
...
デフォルトでは、API は編集済みの投稿の**最新バージョン**を返します。

特定のバージョンを取得するには、その投稿の id を指定して直接リクエストしてください。
...
編集された投稿の各バージョンには、それぞれ個別のエンゲージメントメトリクスがあります。メトリクスは、エンゲージメントが発生した時点で表示されていたバージョンに対して計上されます。
...
* 検索、タイムライン、ストリーム、ルックアップなどを含む

この日付以前に作成された投稿には編集メタデータはありません。

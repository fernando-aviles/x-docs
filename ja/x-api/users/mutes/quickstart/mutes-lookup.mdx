...
sidebarTitle: ミュートしているユーザーの取得
description: ミュートしているユーザーの一覧を取得する
keywords: ["ミュートユーザー取得クイックスタート", "ミュートユーザー取得クイックスタート", "ミュートユーザー取得チュートリアル", "ミュートユーザー取得ガイド", "ミュートクイックスタート"]
---

import { Button } from '/snippets/ja/button.mdx';

このガイドでは、X API を使用してミュート済みユーザーのリストを取得する方法を説明します。
...
<Steps>
  <Step title="自分のユーザーIDを取得する">
    認証済みユーザーのIDが必要です。[ユーザー参照エンドポイント](/ja/x-api/users/lookup/introduction) を使うか、Access Token から確認できます（数値の部分があなたのユーザーIDです）。
  </Step>

  <Step title="ミュートしているユーザーを取得するリクエストを送る">
    <CodeGroup dropdown>
      ```bash cURL
...
SDK はページネーションを自動的に処理します。cURL を使用する場合は、レスポンスに含まれる `next_token` を使用します。

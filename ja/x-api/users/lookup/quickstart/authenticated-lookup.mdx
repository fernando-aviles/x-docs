...
keywords: ["認証済みユーザー", "me エンドポイント", "現在のユーザー", "認証済みユーザーのルックアップ"]
---
...
<Note>
  ...
  * ユーザーアクセストークン（User Access Token、OAuth 1.0a または OAuth 2.0 PKCE）
</Note>
...
`/me` エンドポイントは、次のような場合に不可欠です。

* **認証の検証** — ユーザーが正しく認証されているかを確認する
* **ユーザー ID の取得** — 他の API 呼び出しのために、認証済みユーザーの ID を取得する
* **体験のパーソナライズ** — アプリ内でユーザーのプロフィールを表示する
* **代理でのリクエスト** — 誰の代わりにリクエストを行っているかを把握する
...
<CodeGroup dropdown>
  ...
  print(f"Username: {response.data.username}")
  # 固定された投稿は response.includes.tweets に含まれます
  ```
...
<Warning>
  `/me` エンドポイントの呼び出しにはユーザーコンテキストでの認証が必要です。App-Only（Bearer Token）認証はサポートされていません。
</Warning>

次のいずれかの認証方式を使用してください：

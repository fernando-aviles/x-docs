...
title: はじめに
sidebarTitle: はじめに
keywords: ["ニュース", "ニュース API", "ヘッドライン", "速報ニュース", "ニュース検索", "ニュース記事", "ニュース検索"]
---

ニュース検索エンドポイントを使用すると、開発者は X 上で速報として配信されているニュースやヘッドラインを取得できます。

このエンドポイントは、OAuth1 および OAuth2 PKCE に対して app-auth と user-auth の両方の認証をサポートしています。

<div id="getting-started">
  ## はじめに
</div>

このエンドポイントを使用するには、[開発者コンソール](https://developer.x.com/en/portal/dashboard)から取得した[ベアラートークン](/ja/fundamentals/authentication/oauth-2-0/application-only)が必要です。

ベアラートークンを取得したら、次のようにニュース API を呼び出すことができます。

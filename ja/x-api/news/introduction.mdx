...
title: はじめに
sidebarTitle: はじめに
keywords: ["ニュース", "ニュース API", "ヘッドライン", "速報", "ニュース参照", "ニュース記事", "ニュース検索"]
---

news lookup エンドポイントを使用すると、開発者は X 上で速報として配信されるニュースやヘッドラインを取得できます。

このエンドポイントは、OAuth1 および OAuth2 PKCE における app-auth と user-auth の両方の認証方式をサポートします。

<div id="getting-started">
  ## はじめに
</div>

このエンドポイントを使用するには、[開発者コンソール](https://developer.x.com/en/portal/dashboard) から取得した [Bearer Token](/ja/fundamentals/authentication/oauth-2-0/application-only) が必要です。

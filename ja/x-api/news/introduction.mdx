...
title: 概要
sidebarTitle: 概要
keywords: ["ニュース", "ニュース API", "見出し", "速報ニュース", "ニュース参照", "ニュース記事", "ニュース検索"]
---

news lookup エンドポイントを使用すると、開発者は X 上で速報として配信されているニュースや見出しを取得できます。

このエンドポイントは、OAuth1 および OAuth2 PKCE における app-auth と user-auth の両方の認証方式をサポートしています。

<div id="getting-started">
  ## はじめに
</div>

このエンドポイントを使用するには、[Developer Console](https://developer.x.com/en/portal/dashboard) から取得した [Bearer Token](/ja/fundamentals/authentication/oauth-2-0/application-only) が必要です。

Bearer Token を取得したら、次のように news API を呼び出せます。

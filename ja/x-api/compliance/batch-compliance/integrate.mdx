...
Batch Compliance エンドポイントを使用すると、開発者は大量の X のデータを一括アップロードし、自分たちのデータセットがユーザーの意図と X 上のコンテンツの現在の状態を正しく反映するために、どのような対応が必要かを把握できます。システムやネットワーク接続が安定かつ信頼できる場合、リモートサーバーに大量のデータをアップロードすることは比較的単純な操作です。とはいえ、常にそうとは限りません。環境によっては、一定時間が経過すると接続タイムアウトが発生し、アプリとアップロードサーバー間の接続が切断されることがあります。また、ノートパソコンから Wi-Fi 接続を使って大きなファイルをアップロードしようとしたときなどに、接続の問題に遭遇する場合もあります。このような状況では、1 本の長時間にわたる単一の接続でアップロードするのではなく、そのファイルを小さな部分に分割して順次アップロードできることが望ましくなります。

X の Batch Compliance エンドポイントは、大きなファイルを処理するために Google Cloud Storage を利用しています。この種のストレージはさまざまな用途向けに最適化されており、Cloud Storage では「再開可能アップロード」と呼ばれる、大きなファイルを扱うための手法がサポートされています。

アップロード中のどこかで問題が発生した場合でも、Google Cloud Storage は中断された地点から処理を再開することができます。
...
まず、コンプライアンスジョブを作成し、`type` パラメータを使って、投稿 ID とユーザー ID のどちらをアップロードするかを指定する必要があります。さらに、リクエストボディに `resumable` を追加し、その値を `true` に設定します。以下の `$APP&#95;ACCESS&#95;TOKEN` は、必ずご自身の App-only アクセストークンに置き換えてください。
...
`upload_url` の値を控えておいてください。この後の手順で使用します。

<div id="step-two">
  #### 手順 2：
</div>

次に、再開可能なアップロードを開始します。そのためには、前の手順で取得した upload&#95;url に **POST** リクエストを送信し、次のヘッダーを必ず含めてください:
...
この呼び出しに成功すると、ステータスコード 201 が返されます。次に、レスポンスヘッダー内の `location` ヘッダーの値をコピーします。この値は次のような形式になります。

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

その後、[クイックスタートガイド](/ja/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints)のステップ 2 以降に従って、この場所に投稿 ID またはユーザー ID をアップロードできます。

技術的に複雑であるため、レジューム可能アップロードはコードと組み合わせて利用するのが最適です。本ガイドでは、Node.js と [needle リクエストライブラリ](https://www.npmjs.com/package/needle) を使用します。

<div id="install-the-dependencies">
  ### 依存関係をインストールする
</div>

先に進む前に、Node.js の実行環境をインストールしておく必要があります。Node.js は公式サイトから入手できます。インストールが完了すると、Node.js には npm というユーティリティが含まれます。次のコマンドを実行して Node と npm の両方がインストールされていることを確認し、エラーにならないことを確かめてください。

`$ npm -v
6.4.1`

これと同様のバージョン番号が表示されれば、環境の準備は完了です（表示されるバージョン番号は異なる場合があります）。npm を使ってアップロード用ライブラリをインストールします。次のコマンドを実行してください。

`$ npm install -g needle`

以上で準備完了です。追加の設定は不要です。

<div id="request-a-resumable-destination">
  #### 再開可能なアップロード先をリクエストする
</div>

新しいジョブを作成する際に、`resumable` パラメータを `true` に設定して、再開可能アップロードをサポートするアップロード先を取得できるようにします。レスポンスペイロードでは、`upload_url` の値が返されます。
...
デフォルトでは、このライブラリはアップロード先のロケーション（「バケット」と呼びます）と、アップロードしたいファイルの名前を受け取って、新しいアップロード先を作成します。バッチコンプライアンスエンドポイントは独自のアップロード先を作成するため、すでにアップロードを受け付けるロケーションが用意されていることをライブラリに伝える必要があります。 

アップロードライブラリには、この値に加えて、アップロードするデータを含むファイルの名前を渡す必要があります。ファイルを作成し、***twitter-upload.js*** という名前を付けます。次のコードを追加します。
...
ファイルは、最も適切な場所に保存してください。次に、コマンドラインからスクリプトを実行し、2つのパラメータを渡します。

1. 1つ目は、アップロードしたいファイル（投稿またはユーザーのIDを含むファイル）の場所です。
2. 2つ目は、コンプライアンスエンドポイントのレスポンスから受け取ったアップロード用URLです。

URL はダブルクォーテーション（&quot;）で囲み、ファイル名にスペースやその他の文字が含まれる場合は、ファイル名も同様にダブルクォーテーションで囲んでください。
...
この時点で、[compliance status endpoint](/ja/x-api/compliance/get-compliance-job) を使用してコンプライアンスジョブのステータスを確認でき、完了後にコンプライアンス結果をダウンロードできるようになります。

...
keywords: ["バッチコンプライアンスインテグレーション", "コンプライアンスバッチインテグレーション", "コンプライアンスジョブインテグレーション", "バッチコンプライアンスセットアップ"]
---
...
Batch compliance エンドポイントを使用すると、開発者は大量の X data を一括アップロードし、自身のデータセットがユーザーの意図と X 上のコンテンツの現在の状態を正しく反映するために、どのような対応が必要かを把握できます。システムや接続が安定して信頼できる状態であれば、大量の data をリモートサーバーにアップロードすること自体は比較的単純な操作です。しかし、常にそうとは限りません。環境によっては接続のタイムアウトが設定されており、一定時間が経過するとアプリとアップロードサーバー間の接続が切断される場合があります。また、ノートパソコンから Wi-Fi 接続経由で大きなファイルをアップロードしようとした際などに、接続の問題に遭遇することもあります。このような状況では、1 回の連続した接続でアップロードするのではなく、ファイルをより小さな単位に分割して順次アップロードできることが望まれます。

X の Batch compliance エンドポイントは、大きなファイルを処理するために Google Cloud Storage を利用しています。この種のストレージはさまざまな用途向けに最適化されており、Cloud Storage は「再開可能アップロード」と呼ばれる大容量ファイルを扱うための手法をサポートしています。

アップロードの途中で問題が発生した場合でも、Google Cloud Storage は中断された箇所から処理を再開できます。
...
まず、コンプライアンスジョブを作成し、`type` パラメータを使って、投稿 ID とユーザー ID のどちらをアップロードするかを指定します。さらに、リクエストボディに `resumable` を追加し、その値を `true` に設定します。以下の `$APP&#95;ACCESS&#95;TOKEN` は、必ずあなたの App-only アクセストークンに置き換えてください。
...
`upload_url` の値を控えておいてください。次の手順で必要になります。

<div id="step-two">
  #### ステップ 2:
</div>

次に、再開可能アップロードを開始する必要があります。そのためには、前のステップで取得した `upload_url` に **POST** リクエストを送信し、以下のヘッダーを必ず含めてください:
...
この呼び出しが成功すると、HTTP ステータスコード 201 が返されます。次に、レスポンスヘッダーに含まれる `location` ヘッダーの値をコピーします。この値は次のようになります。

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

その後、[クイックスタートガイド](/ja/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints)のステップ 2 以降に従って、投稿またはユーザーの ID をこの場所にアップロードできます。

技術的に複雑であるため、レジューム可能なアップロードはコードと組み合わせて使用するのが最適です。本ガイドでは、Node.js と [needle リクエストライブラリ](https://www.npmjs.com/package/needle) を使用します。

<div id="install-the-dependencies">
  ### 依存関係をインストールする
</div>

先に進む前に、Node.js の実行環境を用意しておく必要があります。Node.js は公式サイトから入手できます。インストールが完了すると、Node.js には npm というユーティリティが含まれます。次のコマンドを実行して、Node と npm の両方がインストールされていることを確認し、エラーにならないことを確かめてください。

`$ npm -v
6.4.1`

これと同様のバージョン番号が表示されれば、環境の準備は完了しています（表示されるバージョン番号は異なる場合があります）。ここでは、アップロード用ライブラリを npm でインストールします。次のコマンドを実行してください。

`$ npm install -g needle`

これで準備は完了です。追加の設定は必要ありません。

<div id="request-a-resumable-destination">
  #### 再開可能なアップロード先をリクエストする
</div>

新しいジョブを作成する際に、再開可能なアップロードをサポートするアップロード先を取得するには、`resumable` パラメーターを `true` に設定します。レスポンスペイロード内で、`upload_url` の値が返されます。
...
デフォルトでは、このライブラリはアップロード先のロケーション（bucket と呼びます）と、アップロードしたいファイルの名前を受け取り、新しいアップロード先を作成します。バッチコンプライアンスエンドポイントは独自のアップロード先を作成するため、すでにアップロードを受け付ける準備ができているロケーションがあることをライブラリに指定する必要があります。

この値を、アップロード対象の data を含むファイルの名前とあわせてアップロードライブラリに渡す必要があります。ファイルを 1 つ作成し、***twitter-upload.js*** という名前を付けます。次のコードを追加します:
...
ファイルは、最も適切な場所に保存してください。次に、コマンドラインからスクリプトを実行し、2つのパラメーターを渡します。

1. 1つ目は、アップロードしたいファイル（投稿またはユーザーのIDを含む）のパスです。
2. 2つ目は、コンプライアンスエンドポイントのレスポンスで受け取ったアップロードURLです。

URLは必ず二重引用符で囲み、ファイル名にスペースやその他の文字が含まれる場合も、同様に二重引用符で囲んでください。
...
Ctrl + C を押すか、コマンドラインを閉じることで、いつでもアップロードを一時停止できます。同じコマンドを後で再度実行すると、中断したところからアップロードを再開できます。ファイルのアップロードが完了すると、次のメッセージが表示されます:

`Upload complete`

この時点で、[compliance status endpoint](/ja/x-api/compliance/get-compliance-job) を使用してコンプライアンス・ジョブのステータスを確認でき、完了後にコンプライアンス結果をダウンロードできるようになります。

...
sidebarTitle: 概要
mode: wide
description: X上の投稿、ユーザー、スペースなどへのプログラムによるアクセス
keywords: ["X API", "X API v2", "Twitter API", "API v2", "API pricing", "pay-per-usage", "API endpoints", "REST API"]
---

import { Button } from "/snippets/ja/button.mdx"

X API を使用すると、X 上の公開された会話にプログラムからアクセスできます。投稿を読み取り、コンテンツを投稿し、ユーザーを管理し、トレンドを分析できます。これらはすべて、柔軟な従量課金制の料金体系を備えた最新の REST エンドポイント経由で行えます。

<CardGroup cols={3}>
  <Card title="はじめに" icon="rocket" href="/ja/x-api/getting-started/make-your-first-request">
    アプリを作成して、数分で最初のリクエストを送信しましょう。
  </Card>
...
<CardGroup cols={3}>
  <Card title="投稿" icon="message" href="/ja/x-api/posts/lookup/introduction">
    投稿を検索、取得、公開できます。タイムライン、スレッド、引用投稿にアクセスできます。
  </Card>
...
X API は **従量課金制** の料金モデルを採用しています。サブスクリプションや月額上限はなく、使った分だけ支払います。

<CardGroup cols={2}>
  <Card title="柔軟なスケーリング" icon="chart-line">
    小さく始めて、必要に応じて拡張できます。コストは実際の利用量に応じて変動します。
  </Card>

  <Card title="コミットメント不要" icon="unlock">
    契約や最低利用金額はありません。いつでも停止できます。
  </Card>

  <Card title="リアルタイムトラッキング" icon="gauge-high">
    Developer Console 上で利用状況とコストをリアルタイムに確認できます。
  </Card>
...
<Tabs>
  <Tab title="データアクセス">
    ...
    投稿、ユーザー、メディアなどに関する詳細で構造化されたデータにアクセスできます。

    * **投稿**: 本文テキスト全体、メトリクス、エンティティ、注釈、会話スレッド
    * **ユーザー**: プロフィール、フォロワー数、認証ステータス
    * **メディア**: メタデータ付きの画像、動画、GIF
    * **投票**: 選択肢と投票数

    必要なデータだけを取得できるよう、[fields](/ja/x-api/fundamentals/fields) と [expansions](/ja/x-api/fundamentals/expansions) を使ってレスポンスをカスタマイズできます。
  </Tab>

  <Tab title="リアルタイムストリーミング">
    ### フィルタ済みストリーム

    投稿が公開されると同時にリアルタイムで受信できます。最大 1,000 件までフィルタリングルールを定義して、一致する投稿だけを受信できます。
    ...
    [Filtered stream についてさらに詳しく →](/ja/x-api/posts/filtered-stream/introduction)
  </Tab>

  <Tab title="検索 & 分析">
    ### 全アーカイブ検索

    2006 年までさかのぼる公開投稿の履歴全体を検索できます。ユーザー、キーワード、日付などのオペレーターを組み合わせてクエリを構築できます。
    ...
    [検索についてさらに詳しく →](/ja/x-api/posts/search/introduction)
  </Tab>
</Tabs>
...
| **Enterprise** | 利用可能 | 大量アクセス向け、専任サポート付き |

<Tip>
  新しいプロジェクトでは、必ず **X API v2** を使用してください。新機能や改善はすべてここでリリースされます。
</Tip>
...
<Steps>
  <Step title="開発者アカウントを作成する">
    [console.x.com](https://console.x.com) にサインアップして、アプリを作成します。
  </Step>

  <Step title="認証情報を取得する">
    アプリのみのリクエスト用の Bearer Token を生成します。
  </Step>
...
<Button href="/ja/x-api/getting-started/make-your-first-request">クイックスタートガイド（完全版）</Button>

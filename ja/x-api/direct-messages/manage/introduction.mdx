...
Manage Direct Messages エンドポイントを使用すると、認証済みユーザーに代わってダイレクトメッセージを送信および削除できます。
...
<CardGroup cols={2}>
  ...
  <Card title="新しい会話を開始" icon="comments">
    新しい会話を開始する
  </Card>
...
| POST | `/2/dm_conversations/with/:participant_id/messages` | 1対1の会話に送信 |
...
<Note>
  ...
  * [OAuth 2.0 PKCE](/ja/resources/fundamentals/authentication#oauth-2-0-authorization-code-flow-with-pkce-2) を利用したユーザーアクセス・トークン
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="DMルックアップ" icon="messages" href="/ja/x-api/direct-messages/lookup/introduction">
    DM イベントを取得する
  </Card>

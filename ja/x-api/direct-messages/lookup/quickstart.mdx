...
description: ダイレクトメッセージのイベントと会話を取得する
keywords: ["DM 検索クイックスタート", "ダイレクトメッセージ クイックスタート", "DM 取得チュートリアル", "DM 検索ガイド"]
---
...
`9876543210` を、相手側ユーザーのユーザーIDに置き換えてください。
...
<CodeGroup dropdown>
  ...
  for await (const page of paginator) {
    ...
    // 送信者のユーザーオブジェクトは page.includes.users に含まれています
  }
  ```
</CodeGroup>
...
| パラメーター | 説明 |
|:----------|:------------|
| `max_results` | 1ページあたりのイベント数（1〜100、デフォルト 100） |
| `pagination_token` | 次ページを取得するためのトークン |
| `dm_event.fields` | 返されるイベントフィールド |

...
description: チャンク（分割）アップロードを使用して動画や大容量メディアファイルをアップロードする
keywords: ["chunked upload", "media upload", "chunked media", "video upload", "large file upload", "chunked upload guide"]
---

このガイドでは、チャンク（分割）アップロードのワークフローを使用して、動画や大容量メディアファイルをアップロードする方法を説明します。

動画や大容量メディアをアップロードする場合は、次の手順を実行する必要があります。
...
<Note>
  完全な Python サンプルについては、[このサンプルコード](https://github.com/xdevplatform/large-video-upload-python)を参照してください。
</Note>
...
アップロードセッションを開始し、`media_id` を取得します:
...
<CodeGroup dropdown>
  ...
  # チャンクをアップロード
  chunk_size = 1024 * 1024  # 1 MB チャンク
  ...
  // チャンクをアップロード
  const chunkSize = 1024 * 1024; // 1 MB チャンク
...
<div id="step-3-finalize-upload-finalize">
  ## ステップ 3: アップロードを完了 (FINALIZE)
</div>

すべてのチャンクの送信が完了したら、アップロードを完了します:
...
<Note>
  `processing_info` が返された場合は、処理完了を待つためにステップ4に進んでください。返されない場合は、そのメディアはすぐに利用できます。
</Note>
...
<CardGroup cols={2}>
  ...
  <Card title="投稿の作成" icon="message" href="/ja/x-api/posts/manage-tweets/quickstart">
    メディア付き投稿
  </Card>

---
title: チャンク分割メディアアップロード
sidebarTitle: クイックスタート
description: チャンクアップロードを使用して動画や大容量のメディアファイルをアップロードする
keywords: ["チャンクアップロード", "メディアアップロード", "チャンクメディア", "動画アップロード", "大容量ファイルアップロード", "チャンクアップロードガイド"]
---

このガイドでは、チャンクアップロードワークフローを使用して動画や大容量のメディアファイルをアップロードする手順を説明します。

動画や大容量のメディアをアップロードするには、次の手順を実行する必要があります。

1. **INIT** — アップロードを初期化し、`media_id` を取得する
2. **APPEND** — ファイルの各チャンクをアップロードする
3. **FINALIZE** — アップロードを完了する
4. **STATUS** — （必要に応じて）処理の完了を待つ

<Note>
  完全な Python の例については、[このサンプルコード](https://github.com/xdevplatform/large-video-upload-python)を参照してください。
</Note>

***

<div id="step-1-initialize-upload-init">
  ## ステップ 1: アップロードの初期化 (INIT)
</div>
...
<div id="step-2-upload-chunks-append">
  ## ステップ 2: チャンクのアップロード (APPEND)
</div>

ファイルをチャンクごとにアップロードします。たとえば、3 MB のファイルを 3 個のチャンクに分割します:
...
<Note>
  `processing_info` が返された場合は、処理が完了するまで待つためにステップ 4 に進んでください。返されない場合は、メディアをすぐに利用できます。
</Note>
...
レスポンスに `processing_info` が含まれている場合は、処理が完了するまでポーリングします:
...
<div id="step-5-create-post-with-media">
  ## ステップ 5: メディア付き投稿を作成する
</div>

処理が完了したら、メディア付きの投稿を作成します。
...
<CodeGroup dropdown>
  ...
  print(f"投稿しました: {response.data.id}")
  ```
  ...
  console.log(`投稿しました: ${response.data?.id}`);
  ```
</CodeGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="投稿を作成する" icon="message" href="/ja/x-api/posts/manage-tweets/quickstart">
    メディア付き投稿
  </Card>

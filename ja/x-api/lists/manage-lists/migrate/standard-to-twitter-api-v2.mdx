...
どちらのバージョンのエンドポイントも [OAuth 1.0a ユーザーコンテキスト](https://developer.x.com/content/developer-twitter/resources/fundamentals/authentication) をサポートしています。したがって、以前に List を管理する standard v1.1 のいずれかのエンドポイントを使用していた場合、X API v2 のエンドポイントに移行しても、同じ認証方法を引き続き使用できます。
...
X API v2 のエンドポイントでは、リクエストを認証する際に、[開発者用 App](/ja/resources/fundamentals/developer-apps) として作成され、[Project](/ja/resources/fundamentals/developer-apps) に関連付けられている資格情報を使用する必要があります。X API v1.1 のすべてのエンドポイントでは、Project に関連付けられているかどうかにかかわらず、App の資格情報を使用できます。

**リクエストパラメータ**

次の Standard v1.1 のリクエストパラメータには、X API v2 での対応パラメータがあります。
...
<Note>
  **ご注意:** Standard v1.1 のパラメータはクエリパラメータとして渡されますが、X API v2 のパラメータは POST エンドポイントではボディパラメータとして、DELETE と PUT エンドポイントではパスパラメータとして渡されます。
</Note>

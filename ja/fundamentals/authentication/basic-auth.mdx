...
keywords: ["ベーシック認証", "HTTP ベーシック認証", "エンタープライズ認証", "ベーシック認証", "HTTP 認証", "エンタープライズ API 認証"]
---
...
X のエンタープライズ API の多くは、HTTP Basic 認証の使用を必要とします。Basic 認証が必須の API に正常にリクエストを送信するには、有効なメールアドレスとパスワードの組み合わせを、各リクエストの認証ヘッダーとして渡す必要があります。メールアドレスとパスワードの組み合わせは、[enterprise API console](https://console.gnip.com/) にアクセスする際に使用するものと同じであり、このコンソール内で編集できます。 

Basic 認証を使用してリクエストを構築する際は、HTTPS 上でエンコードされた認証情報を含む `Authentication: Basic` HTTP ヘッダーを必ず追加してください。

...
- **Bearer Token**: app-auth（アプリ認証）をサポートするエンドポイントへの読み取り専用アクセスに使用します（例: 投稿検索、ストリーミングエンドポイント）。
- **OAuth 2.0 with PKCE**: スコープベースでユーザーが許可したアクセスを安全に行うために使用します（例: 認証済みユーザーの投稿の非公開メトリクスを取得）。
- **OAuth 1.0a**: ユーザー固有の操作（例: ユーザーに代わって投稿する、リストを管理する）向けのレガシーな認証方式です。
...
この例では、Proof Key for Code Exchange（PKCE）を用いた OAuth 2.0 の使い方を示します。ユーザー単位のアクセス（例: ユーザーに代わって投稿する）、ユーザー用のメディアアップロードなどに使用します。
**手順**:
...
print(f"この URL にアクセスして認可してください: {auth_url}")
webbrowser.open(auth_url)
# ステップ 3: コールバックを処理(実際のアプリではFlaskなどのWebフレームワークを使用)
# callback_url = "http://localhost:8080/callback?code=AUTH_CODE_HERE" と仮定
callback_url = input("コールバックURLの完全なURLをここに貼り付けてください: ")
# ステップ 4: コードをトークンに交換
tokens = auth.fetch_token(authorization_response=callback_url)
access_token = tokens["access_token"]
refresh_token = tokens["refresh_token"]  # 更新用に保存
# ステップ 5: クライアントを作成
# オプション 1: bearer_token を使用（OAuth2 アクセストークンはベアラートークンとして機能）
client = Client(bearer_token=access_token)
# オプション 2: 自動更新サポートのために完全なトークン辞書を渡す
...
oauth_verifier = input("コールバックから受け取った OAuth 検証コードを入力してください: ")
# ステップ 5: アクセストークンに交換

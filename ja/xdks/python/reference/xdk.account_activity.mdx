...
このモジュールは X API のアカウントアクティビティ関連エンドポイントへのアクセス手段を提供し、アカウントアクティビティに関するすべての機能への主なエントリーポイントとして機能します。
...
リプレイジョブの作成
指定された webhook に関連付けられたすべてのサブスクリプションについて、過去 5 日間までさかのぼってアクティビティを取得するリプレイジョブを作成します。
...
<ParamField path="path.from_date" type="str">
  イベントが提供される開始点となる、最も古い UTC タイムスタンプ（含む）。形式は yyyymmddhhmm。
</ParamField>

<ParamField path="path.to_date" type="str">
  イベントが提供される終了点となる、最新の UTC タイムスタンプ（含まない）。形式は yyyymmddhhmm。
</ParamField>
...
<ParamField path="path.webhook_id" type="Any">
  サブスクリプションの有無を確認する対象の webhook の ID。
</ParamField>
...
サブスクリプション数を取得します
現在有効な Account Activity のサブスクリプション数を取得します。
:returns: レスポンス data
...
サブスクリプションを取得する
指定された webhook に対する、すべての有効なサブスクリプションの一覧を取得します。
...
サブスクリプションの検証
指定した webhook について、ユーザーの Account Activity サブスクリプションを検証します。
...
<ParamField path="path.webhook_id" type="Any">
  サブスクリプションの検証対象となる webhook の ID。
</ParamField>

...
投稿の作成または編集
認証済みユーザーの新しい投稿を新規作成するか、`edit_options` が指定されている場合は既存の投稿を編集します。
body: リクエストボディ
:returns: レスポンスデータ
...
カンマ区切りの投稿IDのリストです。1回のリクエストで最大100個まで指定できます。
</ParamField>
...
いいねしたユーザーを取得
IDで指定した特定の投稿をいいねしたユーザーのリストを取得します。
...
引用された投稿を取得
指定した投稿のidを指定して、その投稿を引用している投稿のリストを取得します。
...
Reposted by の取得
特定の投稿IDを指定して、その投稿をリポストしたユーザーの一覧を取得します。
...
リポストを取得
特定の投稿をそのIDで指定し、その投稿をリポストしている投稿のリストを取得します。
...
<ParamField path="path.start_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ。投稿が返される最も古い UTC タイムスタンプです。タイムスタンプは秒精度で、かつ両端を含みます（例: 12:00:01 はその分の最初の 1 秒を含みます）。
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ。投稿が返される最も新しい UTC タイムスタンプです。タイムスタンプは秒精度で、かつ片側排他的です（例: 12:00:01 はその分の最初の 1 秒を含みません）。
</ParamField>

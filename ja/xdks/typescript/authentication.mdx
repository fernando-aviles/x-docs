...
<div id="bearer-token-app-only-auth">
  ## Bearer Token（アプリケーションのみ認証）
</div>

読み取り専用の操作や公開データへのアクセス向け:
...
(async (): Promise<void> => {
  ...
  // 承認後、ユーザーは code パラメータ付きでアプリにリダイレクトされる
...
<Info>
  JavaScript/TypeScript 向け XDK を使用したより詳細なコード例については、[コードサンプル用の GitHub リポジトリ](https://github.com/xdevplatform/samples/tree/main/javascript) を参照してください。
</Info>

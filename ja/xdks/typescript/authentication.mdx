...
TypeScript SDK は、さまざまなユースケースに対応する複数の認証方式をサポートしています。

<div id="bearer-token-app-only-auth">
  ## Bearer Token（アプリ専用認証）
</div>

読み取り専用操作および公開データへのアクセスに使用します。
...
(async (): Promise<void> => {
  ...
  // コールバック URL から code を取得する（例: クエリパラメータから）
  const authCode: string = 'code-from-callback-url'; // 実際の OAuth コールバックから取得した code に置き換える
...
(async () => {
  ...
  // 承認後、ユーザーは code パラメータ付きでリダイレクトされる
  // コールバック URL から code を取得する（例: クエリパラメータから）
  const authCode = 'code-from-callback-url'; // 実際の OAuth コールバックから取得した code に置き換える

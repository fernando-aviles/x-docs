...
この SDK には、ページネーション対応のレスポンスを返す任意のエンドポイントで利用できる、汎用的なページネーター用ユーティリティが用意されています。メソッドは通常のレスポンスを返すので、それらをページネーターでラップして使います。
...
const userCount: number = followers.users.length;  // これまでに取得したすべてのユーザー
...
for await (const user of followers) {
  ...
  console.log(typedUser.username);  // 型情報に基づいた完全なアクセス
}
```
...
await followers.fetchNext();
if (!followers.done) {
  const page2: UserPaginator = await followers.next(); // 次のページから開始する独立したページネーター
  await page2.fetchNext();
  console.log(page2.users.length);  // 2ページ目の要素
}
```
...
<Info>
JavaScript/TypeScript 向け XDK を使用した詳細なコード例については、[code samples の GitHub リポジトリ](https://github.com/xdevplatform/samples/tree/main/javascript)を参照してください。
</Info>
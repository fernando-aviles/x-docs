...
| 名前 | 型 | 説明 |
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/ja/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`any`&gt;&gt; | ページネーション用トークンを指定して、1ページ分のデータを取得する関数 |
...
このメソッドは、次のページのデータを取得し、その項目を現在のpaginatorインスタンスに追加します。これにより、ページネーションの状態およびメタデータが更新されます。

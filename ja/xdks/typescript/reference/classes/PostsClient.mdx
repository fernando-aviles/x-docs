...
| :------ | :------ | :------ |
| `tweetIds` | `any`[] | 28時間メトリクス取得用の投稿IDのリスト。 |
...
Reposts を取得します\
特定の投稿をリポストしている投稿の一覧を、その投稿の id を指定して取得します。
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `query` | `string` | 投稿に一致する 1つのクエリ／ルール／フィルター。最大クエリ長については https://t.co/rulelength を参照してください。 |
...
引用された投稿を取得
特定の投稿をIDで指定し、その投稿を引用している投稿の一覧を取得します。
...
過去の投稿インサイトを取得\
指定した投稿について、定義された期間内の過去のエンゲージメント指標を取得します。
...
投稿を削除します
認証済みユーザーが所有している場合に、指定された ID の投稿を削除します。

#### パラメーター

| Name | Type | 説明 |
| :------ | :------ | :------ |
| `id` | `string` | 削除対象の投稿の ID。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |
...
いいねしたユーザーを取得
特定の投稿のIDを指定して、その投稿にいいねしたユーザーの一覧を取得します。

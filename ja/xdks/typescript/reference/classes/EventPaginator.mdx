...
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/ja/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`any`&gt;&gt; | ページネーション用トークンを指定してデータの1ページ分を取得する関数 |
...
このメソッドは、現在の paginator の状態に影響を与えることなく、次のページから開始する新しい paginator インスタンスを生成します。

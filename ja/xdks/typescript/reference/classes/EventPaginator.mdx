...
| 名前 | タイプ | 説明 |
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/ja/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`any`&gt;&gt; | ページネーショントークンを指定して、1ページ分のデータを取得する関数 |
...
次のページを取得し、現在のインスタンスに要素を追加します。

このメソッドは次のページのdataを取得し、その要素を現在のpaginatorインスタンスに追加します。ページネーションの状態とメタデータを更新します。

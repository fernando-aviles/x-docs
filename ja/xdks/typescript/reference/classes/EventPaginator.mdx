...
• **new EventPaginator**(`fetchPage`): [`EventPaginator`](/ja/xdks/typescript/reference/classes/EventPaginator)

新しい `EventPaginator` インスタンスを作成します
...
| 名前 | 型 | 説明 |
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/ja/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`any`&gt;&gt; | ページネーション用トークンを受け取り、data の 1 ページ分を取得する関数 |
...
このメソッドは次のページのデータを取得し、その項目を現在のpaginatorインスタンスに追加します。これにより、ページネーションの状態とメタデータが更新されます。
...
次のページから開始される新しい paginator インスタンスを作成します。

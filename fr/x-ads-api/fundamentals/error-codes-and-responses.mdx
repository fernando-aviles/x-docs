...
Les réponses réussies sont indiquées par un code HTTP de la série 200 et une charge utile au format JSON contenant le ou les objets demandés, créés, modifiés ou supprimés, ainsi qu’une indication de la façon dont le serveur a interprété votre requête.

Si vous avez envoyé une requête réussie, vous recevrez dans votre réponse un nœud `request` qui reprend votre requête.

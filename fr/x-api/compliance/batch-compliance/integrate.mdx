...
keywords: ["intégration de la conformité par lots", "intégration par lots pour la conformité", "intégration des jobs de conformité", "configuration de la conformité par lots"]
---

import { Button } from '/snippets/fr/button.mdx';

<div id="working-with-resumable-uploads">
  ## Utilisation des téléversements reprenables
</div>

Lors de l&#39;utilisation des endpoints Batch compliance, les développeurs peuvent téléverser par lots de grandes quantités de données X et déterminer les actions nécessaires pour s&#39;assurer que leurs jeux de données reflètent l’intention des utilisateurs ainsi que l’état actuel du contenu sur X. Le téléversement de grandes quantités de données vers un serveur distant est une opération relativement simple lorsque les systèmes et la connectivité sont stables et fiables. Cependant, ce n’est pas toujours le cas. Certains environnements peuvent imposer un délai d’expiration de la connexion, ce qui coupe effectivement la connexion entre votre App et le serveur de téléversement après un certain temps ; vous pouvez également rencontrer des problèmes de connexion, par exemple lorsque vous essayez de téléverser un gros fichier depuis votre ordinateur portable via une connexion Wi‑Fi. Dans ces circonstances, il est préférable de téléverser de plus petites portions de ce fichier à la fois, plutôt que de s’appuyer sur une seule connexion continue.

Les endpoints Batch compliance de X s’appuient sur Google Cloud Storage pour traiter les gros fichiers. Ce type de stockage est optimisé pour différentes applications ; Cloud Storage prend en charge une technique de gestion des gros fichiers appelée téléversements reprenables.

Si le téléversement échoue à un moment quelconque, Google Cloud Storage est capable de reprendre l’opération là où elle s’est arrêtée.

<div id="creating-a-resumable-job">
  ### Création d’une tâche de téléversement reprenable
</div>
...
Tout d&#39;abord, vous devez créer un job de conformité et préciser si vous allez téléverser des ID de Publication ou des ID d&#39;utilisateur (en utilisant le paramètre `type`). Ajoutez également `resumable` au corps de la requête et définissez-le sur `true`. Assurez-vous de remplacer ci-dessous `$APP&#95;ACCESS&#95;TOKEN` par le App only Access Token de votre App.
...
Si votre appel à l’API aboutit, vous recevrez une réponse semblable à la suivante :
...
Ensuite, vous devrez initialiser le téléversement reprenable. Pour ce faire, effectuez une requête **POST** vers upload&#95;url de l’étape précédente et assurez-vous d’inclure les en-têtes suivants :
...
Si cet appel réussit, vous recevrez un code de réponse 201. Ensuite, dans l’en-tête de la réponse, copiez la valeur de l’en-tête Location, qui aura une forme similaire à ceci :

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

Vous pouvez ensuite téléverser vos ID de Publication ou d’utilisateur à cet emplacement à partir de l’étape deux, [dans le guide de démarrage rapide](/fr/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints).

En raison de leur complexité technique, les téléversements reprenables sont plus adaptés à une utilisation avec du code. Ce guide utilisera Node.js avec la [bibliothèque de requêtes HTTP needle](https://www.npmjs.com/package/needle).

<div id="install-the-dependencies">
  ### Installer les dépendances
</div>

Avant de continuer, vous devez avoir installé un environnement Node.js. Vous pouvez télécharger Node.js depuis son site officiel. Une fois installé, Node.js inclut un utilitaire appelé npm. Vérifiez que Node et npm sont bien installés en exécutant la commande suivante et en vous assurant qu’elle ne renvoie pas d’erreur.

`$ npm -v
6.4.1`

Un numéro de version similaire indique que votre environnement est prêt (notez que votre numéro de version peut être différent). Nous utiliserons npm pour installer la bibliothèque d’upload. Exécutez cette commande :
...
<div id="request-a-resumable-destination">
  #### Demander une destination pour reprise de téléversement
</div>

Lors de la création d’une nouvelle tâche, définissez le paramètre `resumable` sur `true` afin d’obtenir une destination qui prend en charge un téléversement pouvant être repris. Dans le corps de la réponse, vous recevrez une valeur `upload&#95;url`.
...
<div id="prepare-the-code-to-upload-a-file">
  #### Préparer le code pour téléverser un fichier
</div>

Par défaut, la bibliothèque créera une nouvelle destination de téléversement en fournissant un emplacement de téléversement (appelé bucket) et le nom du fichier que vous souhaitez téléverser. Puisque les endpoints de conformité par lot créent leur propre destination, nous devons indiquer à la bibliothèque que nous avons déjà un emplacement prêt à accepter notre téléversement. 
...
async function uploadFile(file, url) {
  // rangeEnd est l'index du dernier octet du fichier, c'est-à-dire le nombre d'octets dans le fichier
...
Enregistrez le fichier à l’emplacement le plus approprié. Ensuite, dans votre ligne de commande, exécutez le script et passez-lui deux paramètres :

1. Le premier sera l’emplacement du fichier (avec les IDs de Publication ou d’utilisateur) que vous souhaitez téléverser.
2. Le second sera l’URL de téléversement que nous avons reçue dans la réponse de l’endpoint de conformité.
...
Vous pouvez mettre le téléversement en pause à tout moment en appuyant sur Ctrl + C ou en fermant votre terminal. Vous pourrez reprendre le téléversement là où vous l&#39;avez interrompu en relançant la même commande ultérieurement. Une fois que le fichier a été entièrement téléversé, vous verrez le message suivant :

`Upload complete`

À ce stade, vous pourrez utiliser le [point de terminaison de statut de conformité](/fr/x-api/compliance/get-compliance-job) pour vérifier l&#39;état de votre tâche de conformité, et vous pourrez télécharger le résultat de conformité une fois qu&#39;il sera prêt.

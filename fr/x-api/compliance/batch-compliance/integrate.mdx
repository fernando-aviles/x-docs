---
title: Guide d’intégration
sidebarTitle: Guide d’intégration
keywords: ["intégration de conformité par lots", "intégration par lots de conformité", "intégration de tâches de conformité", "configuration de la conformité par lots"]
---

import { Button } from '/snippets/fr/button.mdx';

<div id="working-with-resumable-uploads">
  ## Utilisation des téléversements reprenables
</div>

Lors de l’utilisation des endpoints Batch compliance, les développeurs peuvent téléverser en lot de grandes quantités de données X et déterminer les actions à entreprendre pour s’assurer que leurs jeux de données reflètent l’intention des utilisateurs et l’état actuel du contenu sur X. Le téléversement de grandes quantités de données vers un serveur distant est une opération relativement simple lorsque les systèmes et la connectivité sont stables et fiables. Cependant, ce n’est pas toujours le cas. Certains environnements peuvent imposer un délai d’expiration de la connexion, coupant ainsi la connexion entre votre application et le serveur de téléversement après un certain temps ; vous pouvez également rencontrer des problèmes de connexion, par exemple lorsque vous essayez de téléverser un fichier volumineux depuis votre ordinateur portable via une connexion Wi-Fi. Dans ces circonstances, il est préférable de téléverser de plus petites portions de ce fichier à la fois, plutôt que d’avoir une seule connexion continue.

Les endpoints Batch compliance de X s’appuient sur Google Cloud Storage pour traiter des fichiers volumineux. Ce type de stockage est optimisé pour diverses applications ; Cloud Storage prend en charge une technique de gestion des fichiers volumineux appelée téléversements reprenables.

Si le téléversement échoue à un moment donné, Google Cloud Storage est capable de reprendre l’opération là où elle s’est arrêtée.

<div id="creating-a-resumable-job">
  ### Création d&#39;une tâche de téléversement pouvant être reprise
</div>

<div id="step-one">
  #### Première étape :
</div>

Tout d’abord, vous devez créer un job de conformité et spécifier si vous allez importer des ID de Publication ou des ID d’utilisateur (en utilisant le paramètre type). De plus, ajoutez `resumable` au corps de la requête et définissez sa valeur sur `true`. Assurez-vous de remplacer le $APP&#95;ACCESS&#95;TOKEN ci-dessous par votre App only Access Token.

```
curl --request POST \
 'https://api.x.com/2/compliance/jobs' --header 'Authorization: Bearer $APP_ACCESS_TOKEN --header 'Content-Type: application/json' --data-raw '{
   "type": "tweets",
   "resumable": true
}'
```

Si votre appel à l’API réussit, vous obtiendrez une réponse similaire à la suivante :

```
{
   "data": {
       "download_expires_at": "2021-08-18T19:42:55.000Z",
       "status": "created",
       "upload_url": "https://storage.googleapis.com/twttr-tweet-compliance/1425543269983784962/submission/1202726487847104512_1425543269983784962?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210811%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210811T194255Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-type%3Bhost&X-Goog-Signature=",
       "created_at": "2021-08-11T19:42:55.000Z",
       "resumable": false,
       "id": "1425543269983784962",
       "type": "tweets",
       "download_url": "https://storage.googleapis.com/twttr-tweet-compliance/1425543269983784962/delivery/1202726487847104512_1425543269983784962?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210811%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210811T194255Z&X-Goog-Expires=604800&X-Goog-SignedHeaders=host&X-Goog-Signature=",
       "upload_expires_at": "2021-08-11T19:57:55.000Z"
   }
}
```

Notez la valeur d’`upload&#95;url`, vous en aurez besoin pour les étapes suivantes.

<div id="step-two">
  #### Étape 2 :
</div>

Ensuite, vous devez initialiser le téléversement reprenable. Pour ce faire, effectuez un appel **POST** vers l’upload&#95;url de l’étape précédente et assurez-vous d’inclure les en-têtes suivants :

Content-Type: text/plain

Content-Length: 0

x-goog-resumable: start

```
curl --request POST \
'https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=' \
--header 'Content-Type: text/plain' --header \
 'Content-Length: 0' --header \
 'x-goog-resumable: start
```

Si cet appel aboutit, vous recevrez un code de réponse 201. Ensuite, dans l’en-tête de réponse, copiez la valeur de l’en-tête Location, qui ressemblera à ceci :

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

Vous pouvez ensuite envoyer les identifiants (id) de vos Publications ou Utilisateurs à cet emplacement en suivant l’étape deux et les suivantes, [à partir du guide de démarrage rapide](/fr/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints).

En raison de leur complexité technique, les téléversements reprenables sont plus simples à utiliser lorsqu’ils sont effectués via du code. Ce guide utilise Node.js avec la [bibliothèque de requêtes needle](https://www.npmjs.com/package/needle).

<div id="install-the-dependencies">
  ### Installer les dépendances
</div>

Avant de continuer, vous devez disposer d’un environnement Node.js ; vous pouvez télécharger Node.js depuis son site web. Une fois installé, Node.js inclut un utilitaire appelé npm ; vérifiez que Node et npm sont tous deux installés en exécutant la commande suivante, et assurez-vous qu’elle ne renvoie pas d’erreur.

`$ npm -v
6.4.1`

Un numéro de version similaire signifie que votre environnement est prêt (notez que votre numéro de version peut être différent). Nous allons utiliser npm pour installer la bibliothèque de téléversement. Exécutez cette commande :

`$ npm install -g needle`

Tout est en place ; aucune configuration supplémentaire n’est nécessaire.

<div id="request-a-resumable-destination">
  #### Demander une destination de téléversement reprenable
</div>

Lors de la création d’un nouveau job, définissez le paramètre resumable sur true afin d’obtenir une destination qui prend en charge un téléversement reprenable. Dans le corps de la réponse, vous recevrez une valeur upload&#95;url.

```
"upload_url":\
"https://storage.googleapis.com/compliance_tweet_ids/customer_test_object_12950882_GlYjiE?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=193969463581-compute%40developer.gserviceaccount.com%2F20200618%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20200618T184154Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-type%3Bhost&X-Goog-Signature="
```

<div id="prepare-the-code-to-upload-a-file">
  #### Préparer le code pour téléverser un fichier
</div>

Par défaut, la bibliothèque crée une nouvelle destination de téléversement à partir d’un emplacement de téléversement (appelé bucket) et du nom du fichier que vous souhaitez téléverser. Comme les endpoints de conformité par lots créent leur propre destination, nous devons indiquer à la bibliothèque que nous avons déjà un emplacement prêt à accepter notre téléversement.

Nous devrons transmettre cette valeur à la bibliothèque de téléversement, ainsi que le nom du fichier contenant les données à téléverser. Créez un fichier et nommez‑le ***twitter-upload.js***. Ajoutez le code suivant :

```
const needle = require('needle');
const fs = require('fs');
const path = require('path');

const [, scriptName, filename, uploadURL] = process.argv;
if (!filename || !uploadURL) {
  console.error(`Usage: node ${path.basename(scriptName)} filename upload_url`);
  process.exit(-1);
}

async function uploadFile(file, url) {
  // rangeEnd est l'index du dernier octet du fichier, c'est-à-dire le nombre d'octets dans le fichier
  const rangeEnd = (await fs.promises.stat(file)).size;

  let options = {
    headers: {
      'Content-Range': `bytes */${rangeEnd}`,
    },
  };

  const response = await needle('put', url, null, options);

  switch (response.statusCode) {
    case 200:
    case 201:
      console.log('Téléversement terminé');
      return;
    case 308:
      return resumeUpload(response, file, url);
    default:
      console.log('Code de réponse inattendu reçu : ', response.statusCode);
      return;
  }
}

async function resumeUpload(response, file, url) {
  console.log('Téléversement non terminé, reprise en cours');
  if (response.headers.range) {
    let resumeOffset = Number(response.headers.range.split('-')[1]) + 1;

    let options = {
      headers: {
        'Content-Range': `bytes ${resumeOffset}-${rangeEnd-1}/${rangeEnd}`,
        'Content-Length': `${rangeEnd-resumeOffset}`,
      },
    };

    let readStream = fs.createReadStream(file, {start: resumeOffset});
    return needle('put', url, readStream, options);
  } else {
    console.log('Initialisation du téléversement');
    let options = {
      headers: {
        'Content-Type': 'text/plain'
      }
    };

    let readStream = fs.createReadStream(file);
    return needle('put', url, readStream, options);
  }
}

// Demander l'URL de session reprise
async function requestResumableSession(url) {
  const options = {
    headers: {
      'Content-Type': 'text/plain',
      'Content-Length': '0',
      'x-goog-resumable': 'start',
    },
  };

  const res = await needle('post', url, null, options);
  if (res.statusCode === 201) {
    const resumableSessionURL = res.headers['location'];
    console.log('Démarrage du téléversement vers : ', resumableSessionURL);

    await uploadFile(filename, resumableSessionURL);
  } else {
    console.log('Échec de la création de l\'URI de session reprise');
  }

}

requestResumableSession(uploadURL).then(result => console.log('Téléversement terminé'));
```

Enregistrez le fichier à l’emplacement le plus approprié. Ensuite, dans votre terminal, exécutez le script et passez deux paramètres :

1. Le premier sera l’emplacement du fichier (contenant les ID de Publication ou d’utilisateur) que vous souhaitez téléverser.
2. Le second sera l’URL de téléversement que nous avons reçue dans la réponse de l’endpoint de conformité.

Assurez-vous que l’URL est entourée de guillemets doubles, et faites de même pour le nom de votre fichier s’il contient des espaces ou d’autres caractères :

```
node twitter-upload.js compliance_upload.txt\
"https://storage.googleapis.com/compliance_tweet_ids/customer_test_object_12950882_GlYjiE?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=193969463581-compute%40developer.gserviceaccount.com%2F20200618%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20200618T184154Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-type%3Bhost&X-Goog-Signature="
```

Vous verrez une sortie similaire à celle-ci :

`Starting upload to:  https://storage.googleapis.com/twttr-tweet-compliance/<redacted>
Upload not completed, resuming
Initiating upload`

Vous pouvez mettre en pause le téléversement à tout moment en appuyant sur Ctrl + C ou en fermant votre terminal. Vous pourrez ensuite reprendre le téléversement là où il s’est arrêté en exécutant la même commande ultérieurement. Une fois que le fichier a été entièrement téléversé, le message suivant s’affichera :

`Upload complete`

À ce stade, vous pourrez utiliser le [compliance status endpoint](/fr/x-api/compliance/get-compliance-job) pour vérifier l’état de votre tâche de conformité, et vous pourrez télécharger le résultat de conformité une fois celle-ci terminée.

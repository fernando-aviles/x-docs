...
keywords: ["intégration de conformité par lots", "intégration par lots de conformité", "intégration des jobs de conformité", "configuration de la conformité par lots"]
---
...
Les endpoints Batch compliance permettent aux développeurs de téléverser en lot de grandes quantités de données X et de déterminer quelles actions sont nécessaires pour garantir que leurs ensembles de données reflètent l’intention des utilisateurs et l’état actuel du contenu sur X. Le téléversement de grandes quantités de data vers un serveur distant est une opération relativement simple lorsque les systèmes et la connectivité sont stables et fiables. Cependant, ce n’est pas toujours le cas. Certains environnements peuvent imposer un délai d’expiration de la connexion, ce qui coupe la connexion entre votre application et le serveur de téléversement après une durée déterminée ; vous pouvez également rencontrer des problèmes de connexion, par exemple lorsque vous essayez de téléverser un fichier volumineux depuis votre ordinateur portable via une connexion Wi‑Fi. Dans ces circonstances, il est préférable de téléverser de plus petites portions de ce fichier à la fois, plutôt que d’établir une seule connexion continue.

Les endpoints Batch compliance de X s’appuient sur Google Cloud Storage pour traiter des fichiers volumineux. Ce type de stockage est optimisé pour diverses applications ; Cloud Storage prend en charge une technique de gestion des fichiers volumineux appelée téléversement reprenable.
...
Tout d’abord, vous devez créer un job de conformité et indiquer si vous allez téléverser des ID de Publication ou des ID d’utilisateur (en utilisant le paramètre type). Ajoutez également resumable au corps de la requête et définissez-le sur true. Assurez-vous de remplacer $APP&#95;ACCESS&#95;TOKEN ci-dessous par le jeton d’accès App only de votre application.
...
Notez la valeur de `upload&#95;url`, dont vous aurez besoin pour les étapes suivantes.

<div id="step-two">
  #### Étape deux :
</div>

Ensuite, vous devrez initialiser le téléversement reprenable. Pour ce faire, effectuez une requête **POST** vers l’`upload_url` de l’étape précédente et veillez à inclure les en-têtes suivants :
...
Vous pouvez ensuite téléverser vos identifiants de Publications ou d’utilisateurs à cet emplacement en suivant, à partir de l’étape 2, les instructions du [guide de démarrage rapide](/fr/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints).

En raison de leur complexité technique, les téléversements reprenables sont plus efficaces lorsqu’ils sont pilotés par du code. Ce guide utilise Node.js avec la [bibliothèque de requêtes needle](https://www.npmjs.com/package/needle).

<div id="install-the-dependencies">
  ### Installer les dépendances
</div>

Avant de continuer, vous devez avoir un environnement Node.js installé ; vous pouvez obtenir Node.js depuis son site web. Une fois installé, Node.js contient un utilitaire appelé npm ; assurez-vous que Node et npm sont tous deux installés en exécutant la commande suivante et vérifiez qu’elle ne renvoie pas d’erreur.

`$ npm -v
6.4.1`

Un numéro de version similaire à celui-ci indique que votre environnement est prêt (notez que votre numéro de version peut différer). Nous utiliserons npm pour installer la bibliothèque de téléversement. Exécutez cette commande :
...
<div id="request-a-resumable-destination">
  #### Demander une destination pour téléversement reprenable
</div>

Lors de la création d’une nouvelle tâche, définissez le paramètre resumable sur true afin d’obtenir une destination qui prend en charge un téléversement reprenable. Dans la charge utile de la Réponse, vous recevrez une valeur upload&#95;url.
...
Enregistrez le fichier à l’emplacement le plus approprié. Ensuite, dans votre terminal, exécutez le script et passez-lui deux paramètres :
...
Vous pouvez mettre le téléversement en pause à tout moment en appuyant sur Ctrl + C ou en fermant votre terminal. Vous pourrez reprendre le téléversement là où vous vous êtes arrêté en relançant la même commande ultérieurement. Une fois que le fichier a été entièrement téléversé, vous verrez le message suivant :

`Upload complete`

À ce stade, vous pourrez utiliser le [compliance status endpoint](/fr/x-api/compliance/get-compliance-job) pour vérifier l’état de votre tâche de conformité, et vous pourrez télécharger le résultat de cette tâche une fois qu’il sera prêt.

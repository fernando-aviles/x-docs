...
description: Créer et supprimer des publications à l'aide de l'API X
keywords: ["démarrage rapide pour gérer des Tweets", "démarrage rapide pour publier un Tweet", "tutoriel de création de Tweet", "guide de création de Tweet", "exemple de publication de Tweet"]
---

import { Button } from '/snippets/fr/button.mdx';

Ce guide décrit comment créer et supprimer des Publications à l&#39;aide de la X API.
...
<Steps>
  ...
  <Step title="Examiner la réponse">
    Une réponse réussie inclut l’`id` et le champ `text` de la nouvelle publication :
...
<AccordionGroup>
  ...
  <Accordion title="Publication avec un média">
    First, upload media using the [Media Upload endpoint](/fr/x-api/media/quickstart/media-upload-chunked), then reference the `media_id` :
...
<Steps>
  <Step title="Obtenir l'ID de la Publication">
    Vous avez besoin de l&#39;ID de la Publication que vous souhaitez supprimer. Cet ID vous est fourni lorsque vous créez une Publication.
  </Step>
...
<Warning>
  Vous ne pouvez supprimer que les Publications que vous avez créées.
</Warning>

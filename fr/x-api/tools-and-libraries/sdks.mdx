...
title: SDK officiels
sidebarTitle: SDKs
description: SDK officiels de X API pour TypeScript et Python
keywords: ["SDK", "kit de développement logiciel", "SDK client", "SDK d'API", "SDK officiel"]
---

import { Button } from '/snippets/fr/button.mdx';

X propose des SDK officiels pour TypeScript et Python. Ces bibliothèques gèrent l’authentification, la pagination et assurent un typage entièrement sécurisé.
...
|:--------|:------------|
| **Toujours à jour** | Gérés par X, mis à jour avec les nouveaux endpoints |
| **Type safety** | Définitions de types complètes pour tous les objets et méthodes |
...
<Tabs>
  ...
  <Tab title="OAuth 2.0 (contexte utilisateur)">
    Pour effectuer des actions au nom des utilisateurs (publication, suivi, etc.).
  ...
  <Tab title="OAuth 1.0a (contexte utilisateur)">
    Pour les anciennes applications ou certains cas d’usage spécifiques.
...
Les SDK fournissent des méthodes pour l’ensemble des endpoints de X API v2 :
...
| **Espaces** | `client.spaces.get()` | `client.spaces.findSpaceById()` |
| **Listes** | `client.lists.get()` | `client.lists.getList()` |
| **Messages privés** | `client.direct_messages.get()` | `client.directMessages.lookup()` |

Consultez la documentation complète des SDK pour une référence complète des méthodes.

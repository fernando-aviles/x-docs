...
<Steps>
  <Step title="Obtenir l'identifiant utilisateur du destinataire">
    Vous avez besoin de l&#39;identifiant utilisateur de la personne à qui vous souhaitez envoyer un message. Vous pouvez le récupérer à partir de l&#39;[endpoint de recherche d’utilisateurs](/fr/x-api/users/lookup/introduction).
  </Step>
...
<Steps>
  ...
  <Step title="Récupérer l’id de la conversation">
    ```json
    {
      "data": {
...
<CodeGroup dropdown>
  ...
  response = client.dm.send_message_to_conversation(
      dm_conversation_id="1582103724607971328",
      text="Ajout d’un autre message dans la conversation."
  )
...
<Steps>
  <Step title="Téléverser le média">
    Tout d&#39;abord, téléversez votre média à l&#39;aide de l&#39;endpoint [Media Upload](/fr/x-api/media/quickstart/media-upload-chunked).
  </Step>

  <Step title="Envoyer avec une pièce jointe multimédia">
    <CodeGroup dropdown>
      ```bash cURL
      curl -X POST "https://api.x.com/2/dm_conversations/with/9876543210/messages" \
        -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
...
| `dm.read` | Lire les conversations (requis avec `dm.write`) |
| `tweet.read` | Requis pour certaines Expansions |
| `users.read` | Requis pour les Expansions d&#39;utilisateurs |

...
keywords: ["démarrage rapide gestion des messages privés", "démarrage rapide envoi de messages privés", "démarrage rapide gestion des messages privés", "création d'un groupe de messages privés"]
---
...
<Note>
  ...
  * Un jeton d’accès utilisateur (OAuth 2.0 PKCE avec les scopes `dm.write` et `dm.read`)
</Note>
...
<Steps>
  <Step title="Obtenir l’identifiant utilisateur du destinataire">
    Vous avez besoin de l’identifiant utilisateur de la personne à qui vous souhaitez envoyer un message. Vous pouvez l’obtenir depuis l’[endpoint de recherche d’utilisateur](/fr/x-api/users/lookup/introduction).
  </Step>
...
<Steps>
  ...
  <Step title="Créer le groupe avec le premier message">
    <CodeGroup dropdown>
      ```bash cURL
      curl -X POST "https://api.x.com/2/dm_conversations" \
        ...
        -d '{
          ...
          "message": {"text": "Bienvenue dans notre nouveau groupe !"}
        }'
      ```
      ...
      print(f"Groupe créé : {response.data.dm_conversation_id}")
      ```
      ...
      console.log(`Groupe créé : ${response.data?.dm_conversation_id}`);
      ```
    </CodeGroup>
  </Step>

  <Step title="Obtenir l’ID de la conversation">
    ```json
    {
      "data": {
...
<Steps>
  <Step title="Téléversez le média">
    Commencez par téléverser votre média à l’aide du [point de terminaison Media Upload](/fr/x-api/media/quickstart/media-upload-chunked).
  </Step>

  <Step title="Envoyer le message avec une pièce jointe média">
    <CodeGroup dropdown>
      ```bash cURL
      curl -X POST "https://api.x.com/2/dm_conversations/with/9876543210/messages" \
        -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
      ...
      print(f"Message avec média envoyé : {response.data.dm_event_id}")
      ```
      ...
      console.log(`Message avec média envoyé : ${response.data?.dm_event_id}`);
      ```
    </CodeGroup>
  </Step>
</Steps>
...
| `users.read` | Obligatoire pour les expansions utilisateur |
...
<CardGroup cols={2}>
  <Card title="Recherche de DM" icon="inbox" href="/fr/x-api/direct-messages/lookup/quickstart">
    Récupérez les conversations de DM
  </Card>

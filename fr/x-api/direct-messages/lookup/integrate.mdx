...
description: Concepts clés et bonnes pratiques pour intégrer les endpoints de recherche de messages privés (MP)
keywords: ["intégration des messages privés", "intégration des MP", "intégration des endpoints de recherche de MP"]
---

import { Button } from '/snippets/fr/button.mdx';

Ce guide présente les principaux concepts nécessaires pour intégrer les endpoints de consultation des Messages privés dans votre application.
...
Les endpoints de messages privés (DM) nécessitent une authentification utilisateur pour accéder aux conversations privées :
...
| [OAuth 1.0a User Context](/fr/resources/fundamentals/authentication) | Pour compatibilité ascendante |

<Warning>
  L’authentification App-Only n’est pas prise en charge. Tous les messages privés (Direct Messages) sont privés.
</Warning>
...
<CardGroup cols={2}>
  <Card title="Un à un" icon="message">
    A toujours exactement deux participants. Format d’identifiant de conversation : `{smaller_user_id}-{larger_user_id}`
  </Card>

  <Card title="Groupe" icon="comments">
    Deux participants ou plus. Les membres du groupe peuvent changer au fil du temps.
  </Card>
</CardGroup>
...
| `ParticipantsJoin` | Un utilisateur a rejoint un groupe | `participant_ids`, `sender_id` |
| `ParticipantsLeave` | Un utilisateur a quitté un groupe | `participant_ids` |
...
| `attachments` | Pièces jointes multimédias | MessageCreate |
...
<CodeGroup dropdown>
  ...
  # Récupérez les événements DM avec expansions
...
Les événements de MD sont renvoyés dans l&#39;ordre chronologique inversé (du plus récent au plus ancien) :
...
<CodeGroup dropdown>
  ...
  print(f"{len(all_events)} événements de MD trouvés")
  ```
  ...
  console.log(`${events.length} événements de MD trouvés`);
  ```
</CodeGroup>

<Note>
  Les événements des **30 derniers jours** sont disponibles.
</Note>
...
* Utiliser des identifiants de conversation provenant d’URL x.com dans des requêtes API
...
<CardGroup cols={2}>
  <Card title="Démarrage rapide" icon="rocket" href="/fr/x-api/direct-messages/lookup/quickstart">
    Effectuez votre première requête de recherche de messages privés
  </Card>

  <Card title="Envoyer des messages privés" icon="paper-plane" href="/fr/x-api/direct-messages/manage/introduction">

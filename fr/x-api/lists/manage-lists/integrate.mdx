...
keywords: ["intégration pour gérer les listes", "intégration de la gestion des listes", "guide d’intégration des listes", "configuration de la gestion des listes"]
---
...
Vous souhaitez utiliser cet endpoint avec du code dans votre langage de programmation préféré ? Nous mettons à votre disposition plusieurs exemples de code que vous pouvez utiliser comme point de départ sur notre [page GitHub](https://github.com/xdevplatform/Twitter-API-v2-sample-code).

<div id="third-party-libraries">
  #### Bibliothèques tierces
</div>

Profitez de l&#39;une des [bibliothèques tierces](/fr/x-api/tools-and-libraries/overview) de notre communauté pour vous aider à démarrer. Vous pouvez trouver une bibliothèque adaptée aux endpoints v2 en recherchant le tag de version approprié.
...
Veuillez noter qu&#39;OAuth 1.0a peut être délicat à utiliser. Si vous ne connaissez pas cette méthode d&#39;authentification, nous vous recommandons d&#39;utiliser une [bibliothèque](/fr/x-api/tools-and-libraries/overview) ou un outil comme Postman pour authentifier correctement vos requêtes.
...
Chaque jour, des milliers de développeurs envoient des requêtes vers l’X API. Pour aider à gérer le volume de ces requêtes, des [limites de débit](/fr/x-api/fundamentals/rate-limits) sont appliquées à chaque endpoint, ce qui limite le nombre de requêtes que vous pouvez effectuer pour le compte de votre application ou pour le compte d’un utilisateur authentifié.

Ces endpoints sont limités au niveau de l’utilisateur, ce qui signifie que l’utilisateur authentifié pour le compte duquel vous effectuez la requête ne peut appeler l’endpoint qu’un certain nombre de fois, quelle que soit l’App développeur utilisée.
...
<CodeGroup dropdown>
  ...
  curl -X POST "https://api.x.com/2/lists" \
    ...
    -d '{"name": "Ma Liste", "description": "Une liste de comptes intéressants"}'
  ```
  ...
  response = client.lists.create(
      name="Ma Liste",
      description="Une liste de comptes intéressants"
  )
  print(response.data)
  ```

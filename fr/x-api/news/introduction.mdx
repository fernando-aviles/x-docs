...
keywords: ["actualités", "API d’actualités", "titres d’actualité", "informations de dernière minute", "recherche d’actualités", "articles d’actualité", "recherche d’articles d’actualité"]
---

L’endpoint de recherche d’actualités permet aux développeurs de récupérer les actualités et les titres qui viennent de tomber sur X.

Cet endpoint prend en charge l’authentification app-auth et user-auth pour OAuth 1.0a et OAuth 2.0 avec PKCE.

<div id="getting-started">
  ## Prise en main
</div>

Pour utiliser cet endpoint, vous avez besoin d&#39;un [Bearer Token](/fr/fundamentals/authentication/oauth-2-0/application-only) obtenu via le [Developer Console](https://developer.x.com/en/portal/dashboard).

Une fois que vous disposez du Bearer Token, vous pouvez appeler l&#39;API d’actualités comme indiqué ci-dessous :

...
keywords: ["actualités", "API d’actualités", "titres d’actualités", "actualités de dernière minute", "recherche d’actualités", "articles d’actualité", "recherche d’actualités"]
---

L’endpoint de recherche d’actualités permet aux développeurs d’obtenir les actualités et les titres de dernière minute sur X.

Cet endpoint prend en charge les authentifications app-auth et user-auth avec OAuth1 et OAuth2 PKCE.

<div id="getting-started">
  ## Premiers pas
</div>

Pour utiliser l’endpoint, vous avez besoin d’un [jeton Bearer](/fr/fundamentals/authentication/oauth-2-0/application-only) à partir de la [Console de développement](https://developer.x.com/en/portal/dashboard).

Une fois que vous disposez du jeton Bearer, vous pouvez appeler l’API d’actualités comme indiqué ci-dessous :

...
description: Téléverser des vidéos et des fichiers médias volumineux à l'aide du téléversement par segments
keywords: ["téléversement par segments", "téléversement de médias", "média en segments", "téléversement vidéo", "téléversement de fichiers volumineux", "guide de téléversement par segments"]
---

Ce guide vous explique comment téléverser des vidéos et des fichiers médias volumineux à l&#39;aide du workflow de téléversement par segments.

Pour les téléversements de vidéos ou d&#39;autres médias volumineux, vous devez :
...
<CodeGroup dropdown>
  ...
  # Initialiser un téléversement segmenté
...
<Info>
  ...
  * Meilleure fiabilité sur les réseaux lents
  * Les téléversements peuvent être mis en pause et repris
  * Les segments en échec peuvent être renvoyés individuellement
</Info>
...
Terminez le téléversement après l’envoi de tous les segments :
...
<Note>
  Si `processing_info` est présent, passez à l’étape 4 pour attendre la fin du traitement. Sinon, le média est prêt à être utilisé.
</Note>
...
Si `processing_info` a été retourné, effectuez des requêtes régulières jusqu’à la fin du traitement :
...
Une fois le traitement terminé, créez une publication avec le média :
...
<CodeGroup dropdown>
  ...
  print(f"Publié : {response.data.id}")
  ```
  ...
  console.log(`Publié : ${response.data?.id}`);
  ```
</CodeGroup>

...
for page in client.posts.search_recent(query="python", max_results=10):
    if page.data and len(page.data) > 0:
        ...
        print(post_text)  # Accéder à la première Publication
        break
```
...
import webbrowser
# Étape 1 : Créer une instance PKCE
auth = OAuth2PKCEAuth(
...
# Étape 3 : Gérer le callback (dans une vraie application, utilisez un framework web comme Flask)
# Supposons callback_url = "http://localhost:8080/callback?code=AUTH_CODE_HERE"
callback_url = input("Collez l'URL complète du callback ici : ")
# Étape 4 : Échanger le code contre des jetons
tokens = auth.fetch_token(authorization_response=callback_url)
access_token = tokens["access_token"]
refresh_token = tokens["refresh_token"]  # Stocker pour le renouvellement
...
# Option 2 : Passer le dictionnaire complet des jetons pour le support de rafraîchissement automatique
...
```python
# Si le jeton d'accès expire, rafraîchissez-le avec le refresh_token stocké
# La méthode refresh_token utilise le jeton stocké de l'instance OAuth2PKCEAuth
tokens = auth.refresh_token()
# Utiliser le jeton rafraîchi
client = Client(bearer_token=tokens["access_token"])
# Ou passer le dictionnaire complet des jetons : client = Client(token=tokens)
```
...
from xdk.oauth1_auth import OAuth1
import webbrowser
# Étape 1 : Créer une instance OAuth1
oauth1 = OAuth1(
    api_key="YOUR_API_KEY",
    api_secret="YOUR_API_SECRET",
    callback="http://localhost:8080/callback"
)
# Étape 2 : Obtenir le jeton de requête
request_token = oauth1.get_request_token()
# Étape 3 : Obtenir l'URL d'autorisation
...
oauth_verifier = input("Entrez le vérificateur OAuth du callback : ")
# Étape 5 : Échanger contre un jeton d'accès
access_token = oauth1.get_access_token(oauth_verifier)
# Étape 6 : Créer le client
client = Client(auth=oauth1)
# Vous pouvez maintenant utiliser le client
response = client.users.get_me()
```

---
title: "Client"
sidebarTitle: "Client"
---

Ce module fournit la classe principale Client pour interagir avec l’API X.
Il coordonne tous les sous-clients et gère l’authentification, la gestion des sessions
et les flux OAuth2 PKCE. Toutes les fonctionnalités sont générées à partir de la spécification OpenAPI.

<div id="client">
  ## Client
</div>

<Badge color="blue">Classe</Badge>

<Badge color="gray">Bases : object</Badge>

| None = None)

Client pour interagir avec X API.

<div id="constructors">
  ## Constructeurs
</div>

<div id="__init__">
  ### `__init__`
</div>

Initialise le client X API.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.base_url" type="str" default="'https://api.x.com'">
L’URL de base de X API (par défaut : [https://api.x.com](https://api.x.com)).
</ParamField>

<ParamField path="path.bearer_token" type="str or None" default="None">
Le Bearer Token pour X API (authentification d’application uniquement).
</ParamField>

<ParamField path="path.access_token" type="str or None" default="None">
Le jeton d’accès OAuth2 pour le contexte utilisateur (peut être utilisé directement comme Bearer Token).
</ParamField>

<ParamField path="path.client_id" type="str or None" default="None">
L’id client pour X API (obligatoire pour le flux OAuth2 PKCE).
</ParamField>

<ParamField path="path.client_secret" type="str or None" default="None">
Le secret client pour X API.
</ParamField>

<ParamField path="path.redirect_uri" type="str or None" default="None">
L’URI de redirection pour l’autorisation OAuth2.
</ParamField>

<ParamField path="path.token" type="Dict[str, Any] or None" default="None">
Un dictionnaire de jetons OAuth2 existant (si disponible). S’il est fourni, `access_token` sera extrait.
</ParamField>

<ParamField path="path.scope" type="str or List[str] or None" default="None">
Une chaîne de caractères avec des éléments séparés par des espaces, ou une liste de chaînes de caractères, représentant les scopes d’autorisation OAuth2.
</ParamField>

<ParamField path="path.authorization_base_url" type="str" default="'https://x.com/i'">
L’URL de base pour l’autorisation OAuth2 (par défaut : [https://x.com/i](https://x.com/i)).
</ParamField>

<ParamField path="path.auth" type="OAuth1">
Instance OAuth1 pour l’authentification OAuth1.0a.
</ParamField>

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.base_url" type="str" default="'https://api.x.com'"></ParamField>

<ParamField path="path.bearer_token" type="str or None" default="None"></ParamField>

<ParamField path="path.access_token" type="str or None" default="None"></ParamField>

<ParamField path="path.client_id" type="str or None" default="None"></ParamField>

<ParamField path="path.client_secret" type="str or None" default="None"></ParamField>

<ParamField path="path.redirect_uri" type="str or None" default="None"></ParamField>

<ParamField path="path.token" type="Dict[str, Any] or None" default="None"></ParamField>

<ParamField path="path.scope" type="str or List[str] or None" default="None"></ParamField>

<ParamField path="path.authorization_base_url" type="str" default="'https://x.com/i'"></ParamField>

<ParamField path="path.auth" type="OAuth1"></ParamField>

<div id="exchange_code">
  ### `exchange_code`
</div>

Échange un code d’autorisation contre des jetons (équivalent à l’API TypeScript).

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.code" type="Any">
Le code d’autorisation provenant du callback.
</ParamField>

<ParamField path="path.code_verifier" type="Any" default="None">
Code de vérification facultatif (utilise le code de vérification stocké si aucun n’est fourni).
</ParamField>

<div id="fetch_token">
  ### `fetch_token`
</div>

Récupère un jeton à partir de l’URL de réponse d’autorisation (ancienne méthode).

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.authorization_response" type="Any">
L’URL complète de redirection reçue après l’autorisation.
</ParamField>

<div id="get_authorization_url">
  ### `get_authorization_url`
</div>

Renvoie l’URL d’autorisation pour le flux OAuth2 PKCE.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.state" type="Any" default="None">
Paramètre optionnel `state` utilisé à des fins de sécurité.
</ParamField>

<div id="is_token_expired">
  ### `is_token_expired`
</div>

Vérifie si le jeton OAuth2 a expiré.

<div id="refresh_token">
  ### `refresh_token`
</div>

Actualise le jeton OAuth2.
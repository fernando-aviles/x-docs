---
title: "PostsClient"
sidebarTitle: "PostsClient"
---

<div id="postsclient">
  ## PostsClient
</div>

<Badge color="blue">Classe</Badge>

<Badge color="gray">Bases : object</Badge>

Client pour les opérations liées aux publications

<div id="constructors">
  ## Constructeurs
</div>

<div id="__init__">
  ### `__init__`
</div>

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.client" type="Client"></ParamField>

<div id="create">
  ### `create`
</div>

Créer ou modifier une Publication
Crée une nouvelle Publication pour l’utilisateur authentifié, ou modifie une Publication existante lorsque des edit_options sont fournies.
body : Corps de la requête
:returns: Données de la réponse
:rtype: CreateResponse

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="body.body" type="CreateRequest"></ParamField>

<div id="delete">
  ### `delete`
</div>

Supprimer une publication
Supprime une publication spécifique par son id, si elle appartient à l’utilisateur authentifié.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.id" type="Any">
L’ID de la Publication à supprimer.
</ParamField>

#### Renvoie

`DeleteResponse` - données de la réponse

<div id="get_analytics">
  ### `get_analytics`
</div>

Obtenir les statistiques de Publication
Récupère les statistiques pour les Publications spécifiées sur un intervalle de temps défini.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.ids" type="List">
Une liste d’ID de publications séparés par des virgules. Jusqu’à 100 sont autorisés dans une seule requête.
</ParamField>

<ParamField path="path.end_time" type="str">
YYYY-MM-DDTHH:mm:ssZ. L’horodatage UTC représentant la fin de l’intervalle de temps.
</ParamField>

<ParamField path="path.start_time" type="str">
YYYY-MM-DDTHH:mm:ssZ. L’horodatage UTC représentant le début de l’intervalle de temps.
</ParamField>

<ParamField path="path.granularity" type="str">
La granularité des résultats du décompte de recherche.
</ParamField>

<ParamField path="path.analytics_fields" type="List or None" default="None">
Une liste de champs Analytics à afficher, séparés par des virgules.
</ParamField>

#### Renvoie

`GetAnalyticsResponse` - données de la réponse

<div id="get_by_id">
  ### `get_by_id`
</div>

Obtenir une Publication par identifiant
Récupère les détails d'une Publication spécifique à partir de son identifiant.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.id" type="Any">
L’id d’une seule Publication.
</ParamField>

<ParamField path="path.tweet_fields" type="List or None" default="None">
Une liste de champs de Tweet séparés par des virgules à afficher.
</ParamField>

<ParamField path="path.expansions" type="List or None" default="None">
Une liste de champs séparés par des virgules à étendre.
</ParamField>

<ParamField path="path.media_fields" type="List or None" default="None">
Une liste de champs de Media séparés par des virgules à afficher.
</ParamField>

<ParamField path="path.poll_fields" type="List or None" default="None">
Une liste de champs de Poll séparés par des virgules à afficher.
</ParamField>

<ParamField path="path.user_fields" type="List or None" default="None">
Une liste de champs de User séparés par des virgules à afficher.
</ParamField>

<ParamField path="path.place_fields" type="List or None" default="None">
Une liste de champs de Place séparés par des virgules à afficher.
</ParamField>

#### Renvoie

`GetByIdResponse` - Données de la réponse

<div id="get_by_ids">
  ### `get_by_ids`
</div>

Obtenir des Publications par ID
Récupère les détails de plusieurs Publications à partir de leurs ID.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.ids" type="List">
Une liste d’ID de Publications séparés par des virgules. Jusqu’à 100 peuvent être transmis dans une seule requête.
</ParamField>

<ParamField path="path.tweet_fields" type="List or None" default="None">
Une liste de champs de Tweet séparés par des virgules à afficher.
</ParamField>

<ParamField path="path.expansions" type="List or None" default="None">
Une liste de champs à étendre, séparés par des virgules.
</ParamField>

<ParamField path="path.media_fields" type="List or None" default="None">
Une liste de champs de Media séparés par des virgules à afficher.
</ParamField>

<ParamField path="path.poll_fields" type="List or None" default="None">
Une liste de champs de Poll séparés par des virgules à afficher.
</ParamField>

<ParamField path="path.user_fields" type="List or None" default="None">
Une liste de champs de User séparés par des virgules à afficher.
</ParamField>

<ParamField path="path.place_fields" type="List or None" default="None">
Une liste de champs de Place séparés par des virgules à afficher.
</ParamField>

#### Renvoie

`GetByIdsResponse` - Données de la réponse

<div id="get_counts_all">
  ### `get_counts_all`
</div>

Obtenir le nombre total de Publications
Renvoie le nombre de Publications correspondant à une requête de recherche dans l’archive complète.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.query" type="str">
Une requête/règle/filtre pour faire correspondre des Publications. Reportez-vous à [https://t.co/rulelength](https://t.co/rulelength) pour identifier la longueur maximale de la requête.
</ParamField>

<ParamField path="path.start_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. L’horodatage UTC le plus ancien (parmi les 7 derniers jours) à partir duquel les Publications seront fournies. L’horodatage a une précision à la seconde et est inclusif (c.-à-d. 12:00:01 inclut la première seconde de la minute).
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. L’horodatage UTC le plus récent jusqu’auquel les Publications seront fournies. L’horodatage a une précision à la seconde et est exclusif (c.-à-d. 12:00:01 exclut la première seconde de la minute).
</ParamField>

<ParamField path="path.since_id" type="Any or None" default="None">
Renvoie des résultats avec un id de Publication supérieur (c’est-à-dire plus récent) à l’id spécifié.
</ParamField>

<ParamField path="path.until_id" type="Any or None" default="None">
Renvoie des résultats avec un id de Publication inférieur (c’est-à-dire plus ancien) à l’id spécifié.
</ParamField>

<ParamField path="path.next_token" type="Any or None" default="None">
Ce paramètre est utilisé pour obtenir la prochaine « page » de résultats. La valeur utilisée avec le paramètre est extraite directement de la Réponse fournie par l’API et ne doit pas être modifiée.
</ParamField>

<ParamField path="path.pagination_token" type="Any or None" default="None">
Ce paramètre est utilisé pour obtenir la prochaine « page » de résultats. La valeur utilisée avec le paramètre est extraite directement de la Réponse fournie par l’API et ne doit pas être modifiée.
</ParamField>

<ParamField path="path.granularity" type="str or None" default="None">
La granularité pour le décompte des résultats de recherche.
</ParamField>

<ParamField path="path.search_count_fields" type="List or None" default="None">
Une liste, séparée par des virgules, de champs SearchCount à afficher.
</ParamField>

<div id="returns">
  #### Valeur de retour
</div>

`IteratorGetCountsAllResponse`

<div id="get_counts_recent">
  ### `get_counts_recent`
</div>

Obtenir le nombre de Publications récentes
Récupère le nombre de Publications au cours des 7 derniers jours correspondant à une requête de recherche.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.query" type="str">
Une requête/règle/filtre pour faire correspondre des Publications. Consultez [https://t.co/rulelength](https://t.co/rulelength) pour déterminer la longueur maximale d’une requête.
</ParamField>

<ParamField path="path.start_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. L’horodatage UTC le plus ancien (parmi les 7 derniers jours) à partir duquel les Publications seront fournies. L’horodatage est à la granularité de la seconde et est inclusif (c.-à-d. 12:00:01 inclut la première seconde de la minute).
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. L’horodatage UTC le plus récent jusqu’auquel les Publications seront fournies. L’horodatage est à la granularité de la seconde et est exclusif (c.-à-d. 12:00:01 exclut la première seconde de la minute).
</ParamField>

<ParamField path="path.since_id" type="Any or None" default="None">
Renvoie des résultats avec un id de Publication supérieur (c’est-à-dire plus récent) à l’id spécifié.
</ParamField>

<ParamField path="path.until_id" type="Any or None" default="None">
Renvoie des résultats avec un id de Publication inférieur (c’est-à-dire plus ancien) à l’id spécifié.
</ParamField>

<ParamField path="path.next_token" type="Any or None" default="None">
Ce paramètre est utilisé pour obtenir la « page » de résultats suivante. La valeur utilisée avec le paramètre est extraite directement de la réponse fournie par l’API et ne doit pas être modifiée.
</ParamField>

<ParamField path="path.pagination_token" type="Any or None" default="None">
Ce paramètre est utilisé pour obtenir la « page » de résultats suivante. La valeur utilisée avec le paramètre est extraite directement de la réponse fournie par l’API et ne doit pas être modifiée.
</ParamField>

<ParamField path="path.granularity" type="str or None" default="None">
La granularité appliquée aux résultats de comptage de recherche.
</ParamField>

<ParamField path="path.search_count_fields" type="List or None" default="None">
Une liste, séparée par des virgules, de champs SearchCount à afficher.
</ParamField>

<div id="returns">
  #### Valeur de retour
</div>

`IteratorGetCountsRecentResponse`

<div id="get_insights28hr">
  ### `get_insights28hr`
</div>

Obtenir les statistiques de Publication sur 28 heures
Récupère les métriques d’engagement pour les Publications spécifiées au cours des 28 dernières heures.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.tweet_ids" type="List">
Liste de PostIds pour les métriques sur 28 h.
</ParamField>

<ParamField path="path.granularity" type="str">
granularité de la réponse de métriques.
</ParamField>

<ParamField path="path.requested_metrics" type="List">
Métriques demandées pour la requête historique.
</ParamField>

<ParamField path="path.engagement_fields" type="List or None" default="None">
Une liste de champs d'engagement séparés par des virgules à afficher.
</ParamField>

#### Renvoie

`GetInsights28hrResponse` - données de la réponse

<div id="get_insights_historical">
  ### `get_insights_historical`
</div>

Obtenir des statistiques historiques sur les Publications
Récupère les indicateurs d’engagement historiques pour les Publications spécifiées sur une période définie.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.tweet_ids" type="List">
Liste de PostIds pour les mesures historiques.
</ParamField>

<ParamField path="path.end_time" type="str">
YYYY-MM-DDTHH:mm:ssZ. L'horodatage UTC représentant la fin de l'intervalle de temps.
</ParamField>

<ParamField path="path.start_time" type="str">
YYYY-MM-DDTHH:mm:ssZ. L'horodatage UTC représentant le début de l'intervalle de temps.
</ParamField>

<ParamField path="path.granularity" type="str">
Granularité des mesures dans la réponse.
</ParamField>

<ParamField path="path.requested_metrics" type="List">
Mesures demandées pour la requête historique.
</ParamField>

<ParamField path="path.engagement_fields" type="List or None" default="None">
Liste de champs d'engagement séparés par des virgules à afficher.
</ParamField>

#### Renvoie

`GetInsightsHistoricalResponse` - données de la réponse

<div id="get_liking_users">
  ### `get_liking_users`
</div>

Obtenir les utilisateurs ayant aimé
Récupère la liste des utilisateurs qui ont aimé une publication spécifique à partir de son id.

#### Parameters

<ParamField path="path.id" type="Any">
Un seul identifiant de Publication.
</ParamField>

<ParamField path="path.max_results" type="int or None" default="None">
Le nombre maximal de résultats.
</ParamField>

<ParamField path="path.pagination_token" type="Any or None" default="None">
Ce paramètre permet d’obtenir la « page » suivante de résultats.
</ParamField>

<ParamField path="path.user_fields" type="List or None" default="None">
Une liste de champs utilisateur, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.expansions" type="List or None" default="None">
Une liste de champs à étendre, séparés par des virgules.
</ParamField>

<ParamField path="path.tweet_fields" type="List or None" default="None">
Une liste de champs de Tweet, séparés par des virgules, à afficher.
</ParamField>

#### Renvoie

`IteratorGetLikingUsersResponse`

<div id="get_quoted">
  ### `get_quoted`
</div>

Obtenir les Publications citées
Renvoie une liste de Publications qui citent une Publication spécifique, identifiée par son ID.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.id" type="Any">
L’identifiant d’une seule Publication.
</ParamField>

<ParamField path="path.max_results" type="int or None" default="None">
Le nombre maximal de résultats à renvoyer.
</ParamField>

<ParamField path="path.pagination_token" type="Any or None" default="None">
Ce paramètre est utilisé pour obtenir une « page » spécifique de résultats.
</ParamField>

<ParamField path="path.exclude" type="List or None" default="None">
L’ensemble des entités à exclure (par exemple « replies » ou « retweets »).
</ParamField>

<ParamField path="path.tweet_fields" type="List or None" default="None">
Une liste de champs de Tweet, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.expansions" type="List or None" default="None">
Une liste de champs à étendre, séparés par des virgules.
</ParamField>

<ParamField path="path.media_fields" type="List or None" default="None">
Une liste de champs Media, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.poll_fields" type="List or None" default="None">
Une liste de champs Poll, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.user_fields" type="List or None" default="None">
Une liste de champs User, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.place_fields" type="List or None" default="None">
Une liste de champs Place, séparés par des virgules, à afficher.
</ParamField>

#### Renvoie

`IteratorGetQuotedResponse`

<div id="get_reposted_by">
  ### `get_reposted_by`
</div>

Obtenir les utilisateurs ayant repartagé
Récupère la liste des utilisateurs qui ont repartagé une Publication spécifique à l'aide de son id.

#### Parameters

<ParamField path="path.id" type="Any">
Un seul id de Publication.
</ParamField>

<ParamField path="path.max_results" type="int or None" default="None">
Le nombre maximal de résultats.
</ParamField>

<ParamField path="path.pagination_token" type="Any or None" default="None">
Ce paramètre est utilisé pour obtenir la « page » suivante de résultats.
</ParamField>

<ParamField path="path.user_fields" type="List or None" default="None">
Une liste de champs User, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.expansions" type="List or None" default="None">
Une liste de champs, séparés par des virgules, à étendre.
</ParamField>

<ParamField path="path.tweet_fields" type="List or None" default="None">
Une liste de champs Tweet, séparés par des virgules, à afficher.
</ParamField>

#### Renvoie

`IteratorGetRepostedByResponse`

<div id="get_reposts">
  ### `get_reposts`
</div>

Obtenir les Reposts
Récupère une liste de Publications qui ont reposté une Publication spécifique à partir de son id.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.id" type="Any">
Un seul identifiant de publication.
</ParamField>

<ParamField path="path.max_results" type="int or None" default="None">
Le nombre maximal de résultats.
</ParamField>

<ParamField path="path.pagination_token" type="Any or None" default="None">
Ce paramètre permet d’obtenir la « page » suivante de résultats.
</ParamField>

<ParamField path="path.tweet_fields" type="List or None" default="None">
Une liste de champs de Tweet, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.expansions" type="List or None" default="None">
Une liste de champs à étendre, séparés par des virgules.
</ParamField>

<ParamField path="path.media_fields" type="List or None" default="None">
Une liste de champs Media, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.poll_fields" type="List or None" default="None">
Une liste de champs Poll, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.user_fields" type="List or None" default="None">
Une liste de champs User, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.place_fields" type="List or None" default="None">
Une liste de champs Place, séparés par des virgules, à afficher.
</ParamField>

#### Renvoie

`IteratorGetRepostsResponse`

<div id="hide_reply">
  ### `hide_reply`
</div>

Masquer une réponse
Masque ou rétablit l’affichage d’une réponse dans une conversation appartenant à l’utilisateur authentifié.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.tweet_id" type="Any">
L’ID de la réponse que vous voulez masquer ou réafficher.
</ParamField>

<ParamField path="body.body" type="HideReplyRequest">
Corps de la requête
</ParamField>

<div id="search_all">
  ### `search_all`
</div>

Rechercher toutes les publications
Récupère les publications de l’archive complète qui correspondent à une requête de recherche.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.query" type="str">
Une requête/règle/un filtre pour faire correspondre des Publications. Consultez [https://t.co/rulelength](https://t.co/rulelength) pour connaître la longueur maximale d’une requête.
</ParamField>

<ParamField path="path.start_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. L’horodatage UTC le plus ancien à partir duquel les Publications seront fournies. L’horodatage est à une granularité de la seconde et est inclusif (c.-à-d. 12:00:01 inclut la première seconde de la minute).
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. L’horodatage UTC le plus récent jusqu’auquel les Publications seront fournies. L’horodatage est à une granularité de la seconde et est exclusif (c.-à-d. 12:00:01 exclut la première seconde de la minute).
</ParamField>

<ParamField path="path.since_id" type="Any or None" default="None">
Renvoie des résultats avec un id de Publication supérieur (c’est-à-dire plus récent) à l’id spécifié.
</ParamField>

<ParamField path="path.until_id" type="Any or None" default="None">
Renvoie des résultats avec un id de Publication inférieur (c’est-à-dire plus ancien) à l’id spécifié.
</ParamField>

<ParamField path="path.max_results" type="int or None" default="None">
Le nombre maximal de résultats de recherche renvoyés par une requête.
</ParamField>

<ParamField path="path.next_token" type="Any or None" default="None">
Ce paramètre est utilisé pour obtenir la « page » de résultats suivante. La valeur utilisée avec le paramètre est extraite directement de la réponse fournie par l’API et ne doit pas être modifiée.
</ParamField>

<ParamField path="path.pagination_token" type="Any or None" default="None">
Ce paramètre est utilisé pour obtenir la « page » de résultats suivante. La valeur utilisée avec le paramètre est extraite directement de la réponse fournie par l’API et ne doit pas être modifiée.
</ParamField>

<ParamField path="path.sort_order" type="str or None" default="None">
L’ordre dans lequel renvoyer les résultats.
</ParamField>

<ParamField path="path.tweet_fields" type="List or None" default="None">
Une liste, séparée par des virgules, des champs Tweet à afficher.
</ParamField>

<ParamField path="path.expansions" type="List or None" default="None">
Une liste, séparée par des virgules, des champs à développer.
</ParamField>

<ParamField path="path.media_fields" type="List or None" default="None">
Une liste, séparée par des virgules, des champs Media à afficher.
</ParamField>

<ParamField path="path.poll_fields" type="List or None" default="None">
Une liste, séparée par des virgules, des champs Poll à afficher.
</ParamField>

<ParamField path="path.user_fields" type="List or None" default="None">
Une liste, séparée par des virgules, des champs User à afficher.
</ParamField>

<ParamField path="path.place_fields" type="List or None" default="None">
Une liste, séparée par des virgules, des champs Place à afficher.
</ParamField>

#### Renvoie

`IteratorSearchAllResponse`

<div id="search_recent">
  ### `search_recent`
</div>

Rechercher des Publications récentes
Récupère les Publications au cours des 7 derniers jours correspondant à une requête de recherche.

<div id="parameters">
  #### Paramètres
</div>

<ParamField path="path.query" type="str">
Une requête, une règle ou un filtre permettant de faire correspondre des Publications. Consultez [https://t.co/rulelength](https://t.co/rulelength) pour identifier la longueur maximale de la requête.
</ParamField>

<ParamField path="path.start_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. L’horodatage UTC le plus ancien à partir duquel les Publications seront fournies. L’horodatage a une granularité à la seconde et est inclusif (c’est‑à‑dire que 12:00:01 inclut la première seconde de la minute).
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. L’horodatage UTC le plus récent jusqu’auquel les Publications seront fournies. L’horodatage a une granularité à la seconde et est exclusif (c’est‑à‑dire que 12:00:01 exclut la première seconde de la minute).
</ParamField>

<ParamField path="path.since_id" type="Any or None" default="None">
Renvoie des résultats avec un id de Publication supérieur (c’est‑à‑dire plus récent) à l’id spécifié.
</ParamField>

<ParamField path="path.until_id" type="Any or None" default="None">
Renvoie des résultats avec un id de Publication inférieur (c’est‑à‑dire plus ancien) à l’id spécifié.
</ParamField>

<ParamField path="path.max_results" type="int or None" default="None">
Le nombre maximal de résultats de recherche renvoyés par une requête.
</ParamField>

<ParamField path="path.next_token" type="Any or None" default="None">
Ce paramètre est utilisé pour obtenir la « page » de résultats suivante. La valeur utilisée avec le paramètre est extraite directement de la Réponse fournie par l’API et ne doit pas être modifiée.
</ParamField>

<ParamField path="path.pagination_token" type="Any or None" default="None">
Ce paramètre est utilisé pour obtenir la « page » de résultats suivante. La valeur utilisée avec le paramètre est extraite directement de la Réponse fournie par l’API et ne doit pas être modifiée.
</ParamField>

<ParamField path="path.sort_order" type="str or None" default="None">
L’ordre dans lequel renvoyer les résultats.
</ParamField>

<ParamField path="path.tweet_fields" type="List or None" default="None">
Une liste de champs Tweet, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.expansions" type="List or None" default="None">
Une liste de champs, séparés par des virgules, à développer.
</ParamField>

<ParamField path="path.media_fields" type="List or None" default="None">
Une liste de champs Media, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.poll_fields" type="List or None" default="None">
Une liste de champs Poll, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.user_fields" type="List or None" default="None">
Une liste de champs User, séparés par des virgules, à afficher.
</ParamField>

<ParamField path="path.place_fields" type="List or None" default="None">
Une liste de champs Place, séparés par des virgules, à afficher.
</ParamField>

#### Renvoie

`IteratorSearchRecentResponse`
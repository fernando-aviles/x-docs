...
<div id="bearer-token-app-only-auth">
  ## Bearer Token (authentification applicative uniquement)
</div>
...
(async (): Promise<void> => {
  ...
  // L'utilisateur visite authUrl et autorise l'application
  // Après l'autorisation, l'utilisateur est redirigé vers votre application avec un paramètre code
  // Extrayez le code de l'URL de rappel (par exemple à partir des paramètres de requête)
  const authCode: string = 'code-from-callback-url'; // Remplacez par le code réel renvoyé par le callback OAuth
...
(async () => {
  ...
  // Après l'autorisation, l'utilisateur est redirigé vers votre application avec un paramètre code
  // Extrayez le code de l'URL de rappel (par exemple à partir des paramètres de requête)
  const authCode = 'code-from-callback-url'; // Remplacez par le code réel renvoyé par le callback OAuth
...
Conservez les identifiants sensibles dans des variables d&#39;environnement :

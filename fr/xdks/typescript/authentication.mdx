...
Le SDK TypeScript prend en charge plusieurs méthodes d’authentification adaptées à différents cas d’utilisation.

<div id="bearer-token-app-only-auth">
  ## Jeton Bearer (authentification App-only)
</div>
...
Pour les anciennes applications ou des cas d'utilisation spécifiques :
...
Pour les opérations spécifiques à un utilisateur :
...
(async (): Promise<void> => {
  ...
  // Après l'autorisation, l'utilisateur est redirigé vers votre site avec un paramètre code
  // Récupérez ce code à partir de l'URL de rappel (par exemple, depuis les paramètres de requête)
  const authCode: string = 'code-from-callback-url'; // Remplacez par le code réel provenant du callback OAuth
...
(async () => {
  ...
  // Après l'autorisation, l'utilisateur est redirigé vers votre site avec un paramètre code
  // Récupérez ce code à partir de l'URL de rappel (par exemple, depuis les paramètres de requête)
  const authCode = 'code-from-callback-url'; // Remplacez par le code réel provenant du callback OAuth
...
<Info>
  Pour des exemples de code détaillés utilisant le XDK pour JavaScript/TypeScript, consultez notre [dépôt GitHub d&#39;exemples de code](https://github.com/xdevplatform/samples/tree/main/javascript).
</Info>

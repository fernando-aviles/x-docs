...
Paginator pour X API avec des fonctionnalités avancées
...
* Accès aux métadonnées pour les jetons de pagination et les décomptes
* Gestion des erreurs et détection des limitations de débit
...
| Nom | Description |
| :------ | :------ |
| `T` | Le type des éléments faisant l’objet de la pagination |
...
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/fr/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`T`&gt;&gt; | Fonction qui récupère une page de données à partir d’un jeton de pagination donné |
...
Récupère les inclusions actuelles
...
if (!followers.done) {
  ...
  console.log(followers.items.length); // Total d'abonnés sur toutes les pages
}
```

**`Throws`**

Si la requête adressée à l’API échoue
...
Retourne la page suivante sous forme d’une nouvelle instance
...
Récupère la page précédente (si cette opération est prise en charge)
...
Réinitialise le pagineur dans son état initial

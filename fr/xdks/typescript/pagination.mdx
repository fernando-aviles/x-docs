...
Le SDK fournit des utilitaires de pagination génériques que vous pouvez utiliser avec n'importe quel endpoint renvoyant des réponses paginées. Les méthodes renvoient des réponses simples ; vous les encapsulez dans un pagineur.
...
for await (const user of followers) {
  ...
  console.log(typedUser.username);  // accès avec typage complet
}
```
...
<Info>
Pour obtenir des exemples de code détaillés utilisant le XDK pour JavaScript et TypeScript, consultez notre [dépôt GitHub d'exemples de code](https://github.com/xdevplatform/samples/tree/main/javascript).
</Info>
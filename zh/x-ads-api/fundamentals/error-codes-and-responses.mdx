...
成功的响应会以 200 系列 HTTP 状态码表示，并带有 JSON 格式的响应体（payload），其中包含所请求、创建、修改或删除的对象，以及服务器对你的请求的理解方式。

如果你发出的请求成功，你会在响应中看到一个 `request` 节点，用于回显你的请求。
...
JSON 响应中的 `data` 字段将包含与所调用资源关联的特定对象。当响应中可能包含一个或多个结果时，`data` 节点的格式将是一个 JSON 数组。当响应中只可能存在一个结果时，它将作为 JSON 哈希返回。在某些少见的情况下，你可能会看到原本应返回集合的响应却只返回了一个 hashmap。在这种情况下，应假定这个单一的 hashmap 是一个与 `type` 字段中指定的相同 type 的对象。

<div id="error-response-structure">
  ## 错误响应结构
</div>

错误响应会返回非 200 系列的 HTTP 状态码。通常会附带 JSON 响应，但有些错误会返回不同类型的响应体。在这些无法解析响应体结构的情况下，应优先依据 HTTP 状态码本身的含义。例如，你有时可能会看到带有 HTML 响应体的 HTTP 404。在这种情况下，可以放心地认为请求的内容不存在（HTTP 404 表示 “Not Found”）。

典型的错误响应在结构上与成功响应类似。错误的性质会通过响应中的 `errors` 节点进行传达。`errors/code` 节点会提供一个 CAPS&#95;CASE 形式的常量错误代码，你可以在程序中使用它来做出处理和恢复方面的决策。`errors/message` 节点会提供一段（通常）是人类易于阅读理解的英文错误描述。还可能附带其他字段，以提供关于该错误的更细粒度信息。
...
在上面的示例中，向 analytics 端点发起的请求中包含一个无效的 `start_time` 参数。对于带有无效参数的请求，其 `errors/code` 值为 `INVALID_PARAMETER`。

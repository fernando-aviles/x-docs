...
成功的响应会以 2xx 范围内的 HTTP 状态码标识，并返回一个 JSON 格式的负载，其中包含被请求、创建、修改或删除的对象，以及服务器对你的请求的理解结果。

如果你发出了一个成功的请求，你会在响应中收到一个 `request` 节点，用于回显你发出的请求。
...
JSON 响应中的 `data` 字段包含与所访问资源关联的特定对象。当响应中可能包含一个或多个结果时，`data` 节点的格式为 JSON 数组。当响应中只可能有一个结果时，则会以 JSON 哈希（哈希表）的形式返回。在一些少见的情况下，原本通常会返回集合的响应，可能会改为返回一个哈希表。在这种情况下，可以认为该单个哈希表是一个与 `type` 字段中指定的类型相同的对象。

<div id="error-response-structure">
  ## 错误响应结构
</div>

错误响应会返回非 2xx 范围的 HTTP 状态码。通常会包含一个 JSON 响应，但有些错误会返回不同类型的响应体。在这些无法解析响应结构的情况下，应优先依据 HTTP 状态码本身的含义进行判断。例如，你可能偶尔会看到一个 HTTP 404 配合一个 HTML 响应体。在这种情况下，可以安全地认为请求的内容不存在（HTTP 404 表示 “Not Found”）。

典型的错误响应在结构上与成功响应类似。错误的具体情况会通过响应中的 `errors` 节点来传达。`errors/code` 节点会给出一个 CAPS&#95;CASE 常量错误代码，你可以在程序中使用它来决定如何处理。`errors/message` 节点会给出一个（通常是）人类可读的、用英文描述的错误说明。还可能附加其他字段，以提供关于该错误更细粒度的信息。
...
在上述示例中，向某个 analytics 端点发起的请求中使用了无效的 `start_time` 参数值。对于包含无效参数的请求，其 `errors/code` 值为 `INVALID_PARAMETER`。

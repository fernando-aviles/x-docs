...
大多数返回对象列表的 Ads API 集合端点都支持排序。具体可用的排序参数因端点而异。由这些参数返回的大多数字段（ID 和 ENUM 除外）都可以用于排序。
...
在你的 API 请求中，只需追加 `sort_by=[attribute name]-[asc/desc]`，其中 `[attribute name]` 是你的 GET 请求返回 JSON 中的一个有效属性名。
...
下面是一个示例 API 请求，用于按 `bid_amount_local_micro` 降序排列返回所有 line item：

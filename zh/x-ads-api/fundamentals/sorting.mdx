...
大多数返回对象列表的 Ads API 集合端点都支持排序。不同端点支持的排序参数有所不同。通过这些参数返回的大多数字段（除 id 和 ENUM 类型外）都可以用于排序。

| API Collection | 可排序参数 |
| :--- | :--- |
...
在你的 API 请求中，只需添加 `sort_by=[attribute name]-[asc/desc]`，其中 attribute 是你的 GET 请求返回的 JSON 中的一个有效属性。
...
下面是一个示例 API 请求，用于按照 `bid_amount_local_micro` 降序排序返回所有 line item：

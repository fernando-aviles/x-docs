---
title: 排序
keywords: ["排序", "排序结果", "Ads API 排序", "排序参数", "结果顺序", "排序数据"]
---

大多数 Ads API 集合型端点（返回对象列表的端点）都支持排序功能。根据具体端点的不同，会提供不同的参数用于排序。通过这些参数返回的大多数字段（ID 和枚举类型除外）都可以参与排序。
...
在你的 API 请求中，只需追加 `sort_by=[attribute name]-[asc/desc]`，其中 `[attribute name]` 是你的 GET 请求返回的 JSON 中包含的一个有效属性名称。

例如，你可以使用 `?name-asc` 按名称的字母顺序排序，或使用 `?name-desc` 按相反顺序排序。

<div id="going-deeper">
  ## 深入了解
</div>

下面是一个示例 API 请求，用于按 `bid_amount_local_micro` 字段降序返回所有 line item：

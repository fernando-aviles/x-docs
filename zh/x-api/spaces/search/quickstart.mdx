---
title: 快速入门
sidebarTitle: 快速入门
description: 按关键词搜索 Spaces
keywords: ["Spaces 搜索快速入门", "搜索 Spaces 教程", "查找 Spaces 指南"]
---

import { Button } from '/snippets/zh/button.mdx';

本指南将引导你按关键字搜索 Spaces。

<Note>
  **先决条件**

  在开始之前，你需要：

  * 一个包含已通过审核 App 的[开发者账号](https://developer.x.com/en/portal/petition/essential/basic-info)
  * 你的 App 的 Bearer Token
</Note>

***

<div id="search-for-spaces">
  ## 搜索 Spaces
</div>

搜索与关键词匹配的 Spaces：

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/spaces/search?\
  query=AI&\
  space.fields=title,host_ids,participant_count,state&\
  state=live" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # 搜索 Spaces
  response = client.spaces.search(
      query="AI",
      space_fields=["title", "host_ids", "participant_count", "state"],
      state="live"
  )

  for space in response.data:
      print(f"{space.title} - {space.participant_count} 位参与者")
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // 搜索 Spaces
  const response = await client.spaces.search({
    query: "AI",
    spaceFields: ["title", "host_ids", "participant_count", "state"],
    state: "live",
  });

  response.data?.forEach((space) => {
    console.log(`${space.title} - ${space.participant_count} 位参与者`);
  });
  ```
</CodeGroup>

<div id="response">
  ### 响应
</div>

```json
{
  "data": [
    {
      "id": "1DXxyRYNejbKM",
      "state": "live",
      "title": "Discussing AI and the Future",
      "host_ids": ["2244994945"],
      "participant_count": 245
    },
    {
      "id": "1YqJDqWYNQDGW",
      "state": "live",
      "title": "AI in Healthcare",
      "host_ids": ["783214"],
      "participant_count": 89
    }
  ],
  "meta": {
    "result_count": 2
  }
}
```

***

<div id="filter-by-state">
  ## 按状态筛选
</div>

仅搜索正在进行或已排程的 Space：

<div id="live-spaces-only">
  ### 仅限正在直播的 Spaces
</div>

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/spaces/search?query=tech&state=live" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # 仅搜索正在直播的 Spaces
  response = client.spaces.search(query="tech", state="live")

  for space in response.data:
      print(f"LIVE: {space.title}")
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // 仅搜索正在直播的 Spaces
  const response = await client.spaces.search({
    query: "tech",
    state: "live",
  });

  response.data?.forEach((space) => {
    console.log(`LIVE: ${space.title}`);
  });
  ```
</CodeGroup>

<div id="scheduled-spaces-only">
  ### 仅限已排期的 Spaces
</div>

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/spaces/search?query=tech&state=scheduled" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # 仅搜索已排期的 Spaces
  response = client.spaces.search(query="tech", state="scheduled")

  for space in response.data:
      print(f"SCHEDULED: {space.title}")
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // 仅搜索已排期的 Spaces
  const response = await client.spaces.search({
    query: "tech",
    state: "scheduled",
  });

  response.data?.forEach((space) => {
    console.log(`SCHEDULED: ${space.title}`);
  });
  ```
</CodeGroup>

***

<div id="include-host-information">
  ## 包含主持人信息
</div>

扩展主持人用户数据：

<CodeGroup dropdown>
  ```bash cURL
  curl "https://api.x.com/2/spaces/search?\
  query=AI&\
  space.fields=title,host_ids,state&\
  expansions=host_ids&\
  user.fields=username,verified" \
    -H "Authorization: Bearer $BEARER_TOKEN"
  ```

  ```python Python SDK
  from xdk import Client

  client = Client(bearer_token="YOUR_BEARER_TOKEN")

  # 搜索并包含主持人信息
  response = client.spaces.search(
      query="AI",
      space_fields=["title", "host_ids", "state"],
      expansions=["host_ids"],
      user_fields=["username", "verified"]
  )

  for space in response.data:
      print(f"{space.title}")
  # 主持人信息位于 response.includes.users 中
  ```

  ```javascript JavaScript SDK
  import { Client } from "@xdevplatform/xdk";

  const client = new Client({ bearerToken: "YOUR_BEARER_TOKEN" });

  // 搜索并包含主持人信息
  const response = await client.spaces.search({
    query: "AI",
    spaceFields: ["title", "host_ids", "state"],
    expansions: ["host_ids"],
    userFields: ["username", "verified"],
  });

  response.data?.forEach((space) => {
    console.log(space.title);
  });
  // 主持人信息位于 response.includes?.users 中
  ```
</CodeGroup>

***

<div id="common-parameters">
  ## 常用参数
</div>

| 参数 | 说明 |
|:----------|:------------|
| `query` | 搜索查询（必需） |
| `state` | 筛选条件：`live`、`scheduled` 或 `all` |
| `max_results` | 要返回的结果数量（1-100） |
| `space.fields` | 要包含的 Space 字段 |
| `expansions` | 要包含的相关对象 |
| `user.fields` | 要包含的用户字段 |

***

<div id="next-steps">
  ## 后续步骤
</div>

<CardGroup cols={2}>
  <Card title="Space 查找" icon="microphone" href="/zh/x-api/spaces/lookup/quickstart">
    通过 ID 查询 Space
  </Card>

  <Card title="API 参考" icon="code" href="/zh/x-api/spaces/search-spaces">
    完整的端点文档
  </Card>
</CardGroup>
...
keywords: ["已认证用户", "me 端点", "当前用户", "认证查询"]
---

import { Button } from '/snippets/zh/button.mdx';

本指南将引导你通过 `/me` 端点获取当前已认证用户的个人资料。
...
* **验证身份认证** — 确认用户已正确完成认证
* **获取用户 ID** — 为后续 API 调用获取已认证用户的 ID
* **个性化体验** — 在你的 App 中展示用户的个人资料
...
<Warning>
  `/me` 端点需要使用用户上下文的身份验证。不支持仅 App 的（Bearer 令牌）身份验证。
</Warning>

请使用以下任一方式：

* [OAuth 1.0a 用户上下文](/zh/resources/fundamentals/authentication)
* [OAuth 2.0 带 PKCE 的授权码流程](/zh/resources/fundamentals/authentication#oauth-2-0-authorization-code-flow-with-pkce-2)

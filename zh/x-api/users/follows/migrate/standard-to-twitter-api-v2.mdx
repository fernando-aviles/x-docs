...
keywords: ["v1.1 到 v2 的迁移", "关注关系迁移", "迁移关注关系", "从标准版到 v2 的关注关系", "从 v1 到 v2 的关注关系", "迁移指南"]
---
...
这两个端点版本都支持 [OAuth 1.0a 用户上下文](/zh/resources/fundamentals/authentication#oauth-1-0a-2)。因此，如果你之前使用的是某个标准的 v1.1 关注管理端点，在迁移到 X API v2 版本后，可以继续使用相同的身份验证方式。
...
在对请求进行认证时，X API v2 endpoint 要求你使用与某个 [Project](/zh/resources/fundamentals/developer-apps) 关联的 [developer App](/zh/resources/fundamentals/developer-apps) 的凭证。所有 X API v1.1 endpoint 都可以使用来自独立 App 的凭证，或来自隶属于某个 Project 的 App 的凭证。
...
| 标准 v1.1 | X API v2 |
| :--- | :--- |
| 无对应参数 | id（POST）、source&#95;user&#95;id（DELETE） |
| user&#95;id | target&#95;user&#95;id |
| screen&#95;name | 无对应参数 |

请注意，标准 v1.1 参数是通过查询参数传递的，而 X API v2 参数则是通过请求体参数（用于 POST endpoint）或路径参数（用于 DELETE endpoint）传递的。

此外，在使用标准 v1.1 endpoint 时，不需要提供 v2 的 id 和 source&#95;user&#95;id，因为通过 OAuth 1.0a 用户上下文传递的 Access Token 可以推断出是哪位用户发起了关注/取消关注操作。

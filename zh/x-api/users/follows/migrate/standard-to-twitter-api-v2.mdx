...
这两个版本的端点都支持 [OAuth 1.0a User Context](/zh/resources/fundamentals/authentication#oauth-1-0a-2)。因此，如果你之前使用的是标准的 v1.1 关注管理端点之一，那么在迁移到 X API v2 版本时，可以继续使用相同的认证方式。
...
在对请求进行认证时，X API v2 端点要求你使用来自与某个 [Project](/zh/resources/fundamentals/developer-apps) 关联的[开发者 App](/zh/resources/fundamentals/developer-apps) 的凭证。所有 X API v1.1 端点都可以使用来自 App，或来自与 Project 关联的 App 的凭证。
...
请注意，标准 v1.1 参数是作为查询参数传递的，而 X API v2 参数是作为请求体参数（对于 POST 端点）或路径参数（对于 DELETE 端点）传递的。

另外，当使用标准 v1.1 端点时，不需要 v2 的 id 和 source&#95;user&#95;id，因为与 OAuth 1.0a 用户上下文一起传递的访问令牌即可推断出是哪个用户在发起关注/取消关注操作。

...
title: 快速入门
sidebarTitle: 快速入门
description: 获取关注者与关注列表，并管理关注关系
keywords: ["关注快速入门", "关注者快速入门", "关注教程", "关注指南"]
---

import { Button } from '/snippets/zh/button.mdx';

本指南将引导你完成获取粉丝和关注列表以及管理关注关系的操作。
...
<Note>
  ...
  * 一个已通过审批的 App 所属的[开发者账号](https://developer.x.com/en/portal/petition/essential/basic-info)
...
<Note>
  如果目标账号是受保护账号，在关注请求被批准之前，`pending_follow` 将为 `true`。
</Note>
...
| 参数 | 描述 |
|:----------|:------------|
| `max_results` | 每页返回的结果数量（1-1000，默认为 100） |
| `pagination_token` | 用于获取下一页的令牌 |
| `user.fields` | 附加的用户字段 |
| `expansions` | 要一并返回的关联对象 |

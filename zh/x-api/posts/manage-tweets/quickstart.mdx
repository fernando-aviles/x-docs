...
<AccordionGroup>
  ...
  <Accordion title="包含媒体的帖子">
    首先使用 [Media Upload 端点](/zh/x-api/media/quickstart/media-upload-chunked) 上传媒体，然后在请求中引用 `media_id`：
...
<Steps>
  <Step title="获取帖子 ID">
    你需要想要删除的帖子的 ID。该 ID 会在你创建帖子时返回。
  </Step>

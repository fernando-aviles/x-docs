...
本指南将引导你获取自 2006 年 3 月以来的历史帖子计数。

<Warning>
  获取完整存档的帖子计数需要具备 [Self-serve](/zh/x-api/getting-started/about-x-api) 或 [Enterprise](/zh/x-api/getting-started/about-x-api) 访问权限。
</Warning>
...
<Note>
  ...
  * 拥有具备 Self-serve 或 Enterprise 访问权限的 [开发者账号](https://developer.x.com/en/portal/petition/essential/basic-info)
  * 你的应用的 Bearer Token
</Note>
...
<Steps>
  <Step title="构建查询">
    使用与完整存档搜索相同的查询语法。例如，要统计来自 @XDevelopers 的帖子数量：
...
| 访问权限要求 | 所有开发者均可使用 | 按次付费、企业版 |
| 默认时间范围 | 最近 7 天 | 最近 30 天 |
...
|:----------|:------------|:--------|
| `query` | 搜索查询（必需） | — |
| `granularity` | 时间分桶粒度 | `hour` |
| `start_time` | 最早时间戳（ISO 8601） | 30 天前 |
| `end_time` | 最新时间戳（ISO 8601） | 现在 |
| `next_token` | 分页令牌 | — |

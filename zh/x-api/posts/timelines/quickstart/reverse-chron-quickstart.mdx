...
title: 主页时间线快速入门
sidebarTitle: 主页时间线
description: 按时间逆序获取某个用户的主页时间线
keywords: ["主页时间线", "时间逆序", "主页时间线快速入门", "时间线快速入门", "主页时间线教程"]
---

import { Button } from '/snippets/zh/button.mdx';

本指南将引导你向按时间倒序排列的首页时间线（home timeline）端点发出你的第一个请求。
...
要获取某个账号的主页时间线，你需要先知道该账号的用户 ID。可以使用按用户名查询的端点来获取：
...
使用用户 ID 和 User Access Token 发送一个 GET 请求：
...
SDK 会自动处理分页。对于 cURL，请使用响应中的 `next_token` 来获取更多结果：
...
| `exclude` | 排除 `retweets`、`replies`，或两者同时排除 | — |

...
你需要先获取要拉取其首页时间线的账号的用户 ID。可以使用按用户名查找的端点来获取：
...
<CodeGroup dropdown>
  ...
  for page in client.posts.get_home_timeline(
      ...
      for post in page.data:
          print(f"{post.text[:50]}... - 点赞数: {post.public_metrics.like_count}")
  ```
  ...
  for await (const page of paginator) {
    page.data?.forEach((post) => {
      console.log(`${post.text?.slice(0, 50)}... - 点赞数: ${post.public_metrics?.like_count}`);
    });
  }
  ```
</CodeGroup>
...
SDK 会自动处理分页。对于 cURL 请求，请使用响应中的 `next_token` 来获取更多结果：

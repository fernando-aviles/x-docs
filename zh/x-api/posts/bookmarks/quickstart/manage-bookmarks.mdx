...
<Note>
  ...
  * 具有 `bookmark.write` 权限范围的用户访问令牌（OAuth 2.0 PKCE）
</Note>
...
<Steps>
  <Step title="获取你的用户 ID">
    你需要当前已认证用户的 ID。你可以使用 `/2/users/me` 端点或 [用户查找端点](/zh/x-api/users/lookup/introduction) 获取它。
  </Step>
...
当使用 OAuth 2.0 PKCE 时，你的访问令牌必须具备以下权限范围：

| Scope | 说明 |
|:------|:------------|
| `bookmark.write` | 添加和删除书签 |
| `tweet.read` | 读取帖子数据 |
| `users.read` | 读取用户数据 |

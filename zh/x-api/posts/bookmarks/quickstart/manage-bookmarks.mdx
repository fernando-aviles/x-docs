...
<Note>
  ...
  * 具有 `bookmark.write` 作用域的用户访问令牌（OAuth 2.0 PKCE）
</Note>
...
<Steps>
  <Step title="获取你的用户 ID">
    你需要经过身份验证的用户 ID。你可以通过 `/2/users/me` 端点或 [用户查询端点](/zh/x-api/users/lookup/introduction) 获取它。
  </Step>
...
使用 OAuth 2.0 PKCE 时，你的访问令牌必须具有以下 scope：

| Scope | Description |
|:------|:------------|
| `bookmark.write` | 添加和移除书签 |
| `tweet.read` | 读取帖子数据 |
| `users.read` | 读取用户数据 |
...
<CardGroup cols={2}>
  <Card title="书签查找" icon="bookmark" href="/zh/x-api/posts/bookmarks/quickstart/bookmarks-lookup">
    获取你已添加到书签的帖子
  </Card>

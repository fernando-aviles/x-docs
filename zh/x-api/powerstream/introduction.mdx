...
Powerstream 是我们速度最快的实时流式 API，用于访问公开的 X 数据。类似于旧版的 GNIP Powetrack API，它通过基于关键字、运算符和元数据的规则来过滤帖子。一旦与 Powerstream 端点建立持久的 HTTP 连接，就可以开始以接近实时的方式接收匹配的帖子。

目前，Powerstream 最多支持 1,000 条规则，并且每条规则最多可以包含 2048 个字符。
...
* **精确过滤**：使用带有运算符的布尔查询，精确筛选并找到您所需的数据。
* **传输方式**：通过 HTTP/1.1 分块传输编码返回 JSON 响应。
* **本地数据中心支持**：仅从本地数据中心获取帖子，通过避免复制延迟来减少网络延迟。

<Note>
  Powerstream API 是一项高级服务，仅在部分 Enterprise 方案下提供。

  如果您有意使用 Powerstream，或想进一步了解我们的 Enterprise 产品，请通过提交 [Enterprise Request Form](/zh/forms/enterprise-api-interest) 联系我们的销售团队。
...
本节展示如何使用 Python 搭配 `requests` 库快速上手 PowerStream 端点。通过 `pip install requests` 安装该库。所有示例都使用 OAuth 2.0 Bearer Token 认证。将 `YOUR_BEARER_TOKEN` 替换为你的实际令牌（请安全存储，例如通过 `os.getenv('BEARER_TOKEN')`）。

我们将通过代码片段依次介绍每个端点。假设在文件顶部包含以下导入：
...
**提示**：要删除所有规则，先发送 GET 请求获取规则，提取 ID，然后批量删除。
...
连接到流以获取实时帖子。使用 `stream=True` 进行逐行读取。实现重连逻辑以提高稳定性和健壮性。
...
为了优化延迟，Powerstream 提供了一个选项，只获取在建立连接所在本地数据中心中产生或创建的帖子。这样可以避免复制延迟，相比来自其他数据中心的帖子，实现更快的投递。要启用此功能，请在流端点后附加查询参数 `?localDcOnly=true`（例如，`/2/powerstream?localDcOnly=true`）。你所连接到的数据中心会在流的首个 data 负载中以及响应中的 HTTP 头中进行标明。
...
如果启用了 `localDcOnly` 参数，则在流首次建立连接时，响应头中会包含以下字段，用于指示正在使用的本地数据中心：
...
<Note>
  **提示：** 为降低延迟，请从不同地理位置建立连接（例如，一个在美国东海岸靠近亚特兰大的位置，另一个在美国西海岸靠近波特兰的位置），并为每个连接启用 `localDcOnly=true`。这样可以更快访问各自数据中心中的帖子。在你这边聚合这些流，将跨数据中心的 data 合并起来。
</Note>
...
要设置过滤规则，你可以使用关键字和运算符。请查看下方的可用运算符列表。
...
| `retweets_of:` | 匹配对特定用户帖子所做的转帖 | `retweets_of:xdevelopers` |
...
|----------|---------|---------|
| `has:` | 匹配包含特定实体的帖子（选项：mentions、geo、links、media、lang、symbols、images、videos） | `has:images`, `has:geo`, `has:mentions` |
| `is:` | 匹配特定类型或具有特定属性的帖子（选项：retweet、reply） | `is:retweet`, `is:reply` |
...
| 运算符 | 概要 | 示例 |
|----------|---------|---------|
| `bio:` | 匹配由个人简介中包含特定内容的用户发布的帖子（使用短语匹配） | N/A |
| `bio_name:` | 匹配由个人简介中包含特定名称的用户发布的帖子（使用短语匹配） | N/A |
...
* 速率限制：规则管理为每 24 小时 50 次请求；对流式数据本身没有限制（但有连接数限制）。
* 重新连接：在断开连接时使用指数退避重连策略。
* 监控：使用 `Connection: keep-alive` HTTP 头部。
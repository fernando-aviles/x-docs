...
Powerstream 是我们用于访问公共 X 数据的速度最快的实时流式 API。与旧版 GNIP Powetrack API 类似，它使用规则根据关键词、运算符和元数据来筛选帖子。一旦与 Powerstream 端点建立持久的 HTTP 连接，您就可以开始以接近实时的速度接收匹配的帖子。

目前，Powerstream 最多支持 1,000 条规则，每条规则最多可包含 2,048 个字符。

<div id="key-features">
  ## 关键特性：
</div>

* **实时数据传输**：几乎实时获取与您的规则匹配的数据显示。
* **精确过滤**：使用带有运算符的布尔查询，精确筛选您所需的数据。
* **传输方式**：通过 HTTP/1.1 分块传输编码（chunked transfer encoding）返回 JSON 响应。
* **本地数据中心支持**：仅从本地数据中心获取帖子，通过避免复制延迟来降低延迟。

<Note>
  Powerstream API 是一项高级服务，仅在特定的企业版方案中提供。

  如果您有兴趣使用 Powerstream 或了解更多企业级产品，请通过提交[企业级 API 需求表单](/zh/forms/enterprise-api-interest)联系销售团队。
...
本节演示如何使用 Python 搭配 `requests` 库快速上手 PowerStream 端点。通过 `pip install requests` 安装该库。所有示例都使用 OAuth 2.0 Bearer Token 进行认证。将 `YOUR_BEARER_TOKEN` 替换为你的实际令牌（请妥善保管，例如通过 `os.getenv('BEARER_TOKEN')` 读取）。

我们将通过代码示例介绍每个端点。假设在文件开头有如下导入：
...
```python
bearer_token = os.getenv('BEARER_TOKEN') or "YOUR_BEARER_TOKEN"  # 使用环境变量以确保安全
...
**提示**：要删除所有规则，先发送 GET 请求获取它们，提取 id，然后批量删除。
...
连接到流以获取实时帖子。使用 `stream=True` 进行逐行读取。实现重连逻辑以提高可靠性。
...
为了优化延迟，Powerstream 提供了一个选项，只获取在建立连接所在本地数据中心中产生或创建的帖子。这样可以避免复制延迟，相比来自其他数据中心的帖子，实现更快速的投递。要启用此功能，请在流式端点后追加查询参数 `?localDcOnly=true`（例如，`/2/powerstream?localDcOnly=true`）。所连接的数据中心会同时在流的初始 data 载荷中以及响应中的 HTTP 头中指明。
...
如果启用了 `localDcOnly` 参数，则在流首次建立连接时，响应头中会包含以下响应头，用于指示正在使用的本地数据中心：
...
<Note>
  **提示：** 为优化延迟，请在不同地理位置建立连接（例如，一个位于美国东海岸亚特兰大附近，另一个位于美国西海岸波特兰附近），并为每个连接启用 `localDcOnly=true`。这样可以更快访问来自各自数据中心的帖子。请在你这边聚合这些流，以合并跨数据中心的数据。
</Note>
...
要设置过滤规则，你可以使用关键字和运算符。请参阅下方的可用运算符列表。
...
| `to:` | 匹配发送给特定用户的帖子 | `to:jvaleski` |
| `retweets_of:` | 匹配对特定用户帖子所做的转发 | `retweets_of:xdevelopers` |
...
| Operator | Summary | Example |
|----------|---------|---------|
| `has:` | 匹配包含特定实体的帖子（选项包括：mentions、geo、links、media、lang、symbols、images、videos） | `has:images`, `has:geo`, `has:mentions` |
| `is:` | 匹配特定类型或具有特定属性的帖子（选项包括：retweet、reply） | `is:retweet`, `is:reply` |
...
| `point_radius:` | 匹配以某一点为圆心、指定半径范围内的帖子 | `point_radius:[-111.464973 46.371179 25mi]`, `point_radius:[-111.464973 46.371179 15km]` |
...
| 运算符 | 摘要 | 示例 |
|----------|---------|---------|
| `bio:` | 匹配个人简介包含特定内容的用户所发布的帖子（使用短语匹配） | N/A |
...
| Operator | Summary | Example |
|----------|---------|---------|
| `retweets_of_status_id:` | 匹配对特定帖子的转发 | `retweets_of_status_id:1234567890123456789` |
...
| 运算符 | 简要说明 | 示例 |
|----------|---------|---------|
| `OR` | 表达式之间的逻辑或 | `x OR facebook` |
| 空格/AND | 表达式之间的逻辑与 | `x facebook`（必须同时包含这两个词） |
...
Powestream API 的载荷格式与旧版 GNIP Powertrack API 相同。一个示例 JSON 响应如下：
...
* 请求频率限制：规则管理为每 24 小时 50 个请求；对流本身没有限制（但适用连接数限制）。
* 重连：在断开连接时使用指数退避重试策略。
* 监控：使用 `Connection: keep-alive` 头。
...
在使用 Batch compliance 端点时，开发者可以批量上传大量 X 数据，并了解为确保其数据集能够反映用户意图以及 X 上内容的当前状态所需采取的操作。当系统和网络连接稳定且可靠时，将大量数据上传到远程服务器是一个相对简单的操作。然而，情况并不总是如此。一些环境可能会设置连接超时限制，在指定时间后切断你的 App 与上传服务器之间的连接；你在上传过程中也可能遇到连接问题，例如尝试通过 Wi‑Fi 从笔记本电脑上传一个大文件时。在这些情况下，将文件拆分为较小部分分次上传会更合适，而不是依赖一次连续不断的单一连接。

X 的 batch compliance 端点依赖 Google Cloud Storage 来处理大文件。这种存储服务针对多种应用场景进行了优化；Cloud Storage 支持一种用于管理大文件的技术，称为可恢复上传（resumable uploads）。

如果上传过程中的任何阶段出现问题，Google Cloud Storage 能够从中断处恢复该操作。
...
首先，你需要创建一个合规作业，并指定你要上传的是 Post ID 还是 user ID（使用 `type` 参数）。此外，在请求体中加入 `resumable` 字段并将其设置为 `true`。请确保将下面的 `$APP&#95;ACCESS&#95;TOKEN` 替换为你的 App-only 访问令牌。
...
记下 `upload_url` 的值，在接下来的步骤中你会用到它。

<div id="step-two">
  #### 步骤二：
</div>

接下来，你需要启动可断点续传上传。为此，请向上一步返回的 `upload_url` 发起一次 **POST** 请求，并确保在请求头中包含以下字段：
...
如果此调用成功，将返回 201 状态码。然后在响应头中，复制 `Location` 头的值，它大致如下所示：

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

然后，你就可以按照[快速入门指南中的第二步及后续步骤](/zh/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints)，将你的帖子或用户 ID 上传到此位置。

由于其技术复杂度较高，断点续传上传功能最适合通过代码来配合使用。本文将使用 Node.js 搭配 [needle 请求库](https://www.npmjs.com/package/needle) 进行演示。

<div id="install-the-dependencies">
  ### 安装依赖
</div>

在继续之前，你需要先安装 Node.js 环境；你可以从其官方网站获取 Node.js。安装完成后，Node.js 会包含一个名为 npm 的工具；通过运行以下命令确认 Node.js 和 npm 都已安装，并确保执行时没有报错。

`$ npm -v
6.4.1`

如果看到类似的版本号，表示你的环境已准备就绪（注意，你的版本号可能不同）。我们将使用 npm 来安装用于上传的库。运行以下命令：
...
创建新任务时，将 `resumable` 参数设置为 `true`，即可获取一个支持断点续传的上传目标地址。在响应体中，你会收到一个 `upload_url` 值。
...
默认情况下，该库会在接收到一个上传位置（称为 bucket）以及你希望上传的文件名后，创建一个新的上传目标位置。由于批量合规端点会自行创建上传目标位置，因此我们需要告诉该库，我们已经有一个可以接收上传内容的位置。

我们需要将该值与包含待上传数据的文件名一起传递给上传库。创建一个文件，并将其命名为 ***twitter-upload.js***，然后添加以下代码：
...
将文件保存在最合适的位置。接下来，在命令行中调用该脚本，并传入两个参数：

1. 第一个参数是你希望上传的文件路径（其中包含帖子或用户 ID）。
2. 第二个参数是我们从合规端点响应中获取的上传 URL。

请确保用双引号将 URL 括起来；如果文件名中包含空格或其他字符，也同样要用双引号括起来：
...
你可以在任何时候通过按下 Ctrl + C 或关闭命令行窗口来暂停上传。稍后再次运行同一条命令时，你将能够从上次中断处继续上传。文件完全上传后，你会看到以下消息：

`Upload complete`

此时，你可以使用[合规状态端点](/zh/x-api/compliance/get-compliance-job)来检查合规作业的状态，并在作业完成后下载合规结果。

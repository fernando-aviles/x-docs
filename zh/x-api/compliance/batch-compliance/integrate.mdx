...
title: 集成指南
sidebarTitle: 集成指南
keywords: ["批量合规集成", "合规批量集成", "合规任务集成", "批量合规设置"]
---
...
在使用批量合规端点时，开发者可以批量上传大量 X 数据，并了解需要采取哪些操作，才能确保其数据集能够反映用户意图以及 X 上内容的当前状态。在系统和连接稳定可靠的情况下，将大量数据上传到远程服务器是一个相对简单的操作。然而，情况并非总是如此。有些环境可能会设置连接超时，在一段时间后切断你的应用与上传服务器之间的连接；你也可能遇到连接问题，例如尝试通过 Wi‑Fi 连接从笔记本电脑上传大文件时。在这些情况下，与其维持一个单一的持续连接，按次上传较小的文件片段会更为理想。

X 的批量合规端点依赖 Google Cloud Storage 来处理大型文件。这种存储类型针对多种应用场景进行了优化；Cloud Storage 支持一种用于处理大型文件的技术，称为可恢复上传（resumable uploads）。

如果上传过程中在任意环节出现问题，Google Cloud Storage 可以从中断的位置恢复该操作。
...
首先，你需要创建一个合规任务，并指定你将上传的是帖子 ID 还是用户 ID（使用 `type` 参数）。此外，在请求体中添加 `resumable` 字段，并将其设置为 `true`。请务必将下方的 `$APP&#95;ACCESS&#95;TOKEN` 替换为你的 App-only 访问令牌。
...
请记下 `upload_url` 的值，后续步骤中会用到它。

<div id="step-two">
  #### 步骤二：
</div>

接下来，你需要初始化断点续传上传。为此，请向上一步获取的 `upload_url` 发起一次 **POST** 请求，并确保在请求头中包含以下内容：
...
如果此调用成功，你将收到 201 响应状态码。然后在响应头中，复制 `Location` 头字段的值，它大致会是这样的：

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

然后，你可以按照[快速入门指南中从第二步开始的步骤](/zh/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints)，将你的帖子或用户 ID 上传到该位置。

由于其技术实现较为复杂，断点续传上传最适合配合代码使用。本指南将使用 Node.js 搭配 [needle 请求库](https://www.npmjs.com/package/needle)。

<div id="install-the-dependencies">
  ### 安装依赖项
</div>

在继续之前，你应当先安装好 Node.js 运行环境；可以从其官方网站获取 Node.js。安装完成后，Node.js 会包含一个名为 npm 的工具；通过运行以下命令确认 Node 和 npm 都已安装，并确保不会出现错误。

`$ npm -v
6.4.1`

显示类似这样的版本号表示你的环境已就绪（注意你的版本号可能不同）。我们将使用 npm 来安装上传库。运行以下命令：
...
在创建新作业时，将 `resumable` 参数设置为 `true`，以便获取一个支持断点续传的上传目的地。在响应负载中，你会收到一个 `upload&#95;url` 值。
...
默认情况下，该库会通过接收一个上传位置（称为 bucket）和你希望上传的文件名来创建一个新的上传目的地。由于批量合规性端点会创建它们自己的目的地，我们需要告诉该库，我们已经有一个可以接收上传内容的位置。

我们需要将此值连同包含要上传数据的文件名一起传递给上传库。创建一个文件，并将其命名为 ***twitter-upload.js***。添加以下代码：
...
async function uploadFile(file, url) {
  // rangeEnd 是文件中最后一个字节的索引,即文件中的字节数
...
将该文件保存在最合适的位置。接下来，在命令行中调用该脚本并传入两个参数：

1. 第一个参数是你要上传的文件的路径（其中包含帖子或用户 ID）。
2. 第二个参数是我们从合规性端点响应中获得的上传 URL。

请确保使用双引号将 URL 括起来；如果你的文件名中包含空格或其他字符，也同样使用双引号括起来：
...
`正在开始上传到： https://storage.googleapis.com/twttr-tweet-compliance/<redacted>
上传未完成，正在恢复
正在初始化上传`

你可以在任何时候通过按下 Ctrl + C 或关闭命令行窗口来暂停上传。稍后再次运行相同命令时，你可以从中断的位置恢复上传。文件完全上传后，你会看到以下消息：

`上传完成`

此时，你可以使用 [合规状态端点](/zh/x-api/compliance/get-compliance-job) 来检查合规任务的状态，并在任务完成后下载合规结果。

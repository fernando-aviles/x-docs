...
title: 集成指南
sidebarTitle: 集成指南
keywords: ["批量合规集成", "合规批量集成", "合规批量作业集成", "批量合规设置"]
---
...
在使用 Batch compliance 端点时，开发者可以批量上传大量 X data，并了解需要采取哪些操作，以确保其数据集反映用户意图以及 X 上内容的当前状态。当系统和网络连接稳定可靠时，将大量 data 上传到远程服务器是一个相对简单的操作。但情况并非总是如此。一些环境可能会设置连接超时时间限制，在设定的时间后切断你的应用与上传服务器之间的连接；你也可能会遇到连接问题，例如通过 Wi‑Fi 从笔记本电脑上传大文件时。在这种情况下，相比维持单一的持续连接，更理想的做法是一次上传该文件的较小片段。

X 的 Batch compliance 端点依赖 Google Cloud Storage 来处理大文件。这类存储针对多种应用场景进行了优化；Cloud Storage 支持一种用于管理大文件的技术，称为可恢复上传（resumable uploads）。

如果上传过程中在任何时间点出现问题，Google Cloud Storage 都可以从中断的位置恢复该操作。
...
首先，你需要创建一个合规作业，并通过 `type` 参数指定是上传帖子 id 还是用户 id。另外，在请求体中添加 `resumable` 字段并将其设置为 `true`。请确保将下面的 `$APP&#95;ACCESS&#95;TOKEN` 替换为你的 App-only 访问令牌。
...
请记下 `upload_url` 的值，后续步骤中将会用到它。

<div id="step-two">
  #### 步骤二：
</div>

接下来，你需要初始化可恢复上传会话。为此，请向上一步获取的 `upload_url` 发起一次 **POST** 请求，并确保包含以下请求头：
...
如果此调用成功，你会收到 201 状态码。然后，在响应头中复制 `Location` 头的值，它看起来大致如下所示：

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

然后，你可以按照[快速入门指南中的第二步及后续步骤](/zh/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints)，将你的帖子或用户 ID 上传到此位置。

由于其技术复杂度较高，断点续传上传最适合配合代码使用。本指南将使用 Node.js 搭配 [needle 请求库](https://www.npmjs.com/package/needle)。

<div id="install-the-dependencies">
  ### 安装依赖项
</div>

在继续之前，你需要先安装 Node.js 环境；你可以从其官网获取 Node.js。安装完成后，Node.js 会包含一个名为 npm 的工具。通过运行以下命令确认 Node 和 npm 都已安装，并确保不会出现错误。

`$ npm -v
6.4.1`

出现类似这样的版本号表示你的环境已经准备就绪（注意，你的版本号可能不同）。我们将使用 npm 来安装上传库。运行以下命令：
...
创建新任务时，将 `resumable` 参数设置为 `true`，这样就可以获得一个支持断点续传的上传目标。在响应载荷中，你会收到一个 `upload&#95;url` 值。
...
默认情况下，该库会通过接收一个上传位置（称为 bucket）以及你希望上传的文件名来创建一个新的上传目的地。由于批量合规端点会创建它们自己的目的地，因此我们需要告知该库，我们已经有一个可以接收上传内容的位置。

我们需要将该位置值与包含待上传 data 的文件名一起传递给上传库。创建一个文件，并将其命名为 ***twitter-upload.js***。添加以下代码：
...
将该文件保存在最合适的位置。然后，在命令行中调用该脚本，并传入两个参数：

1. 第一个参数是你希望上传的文件路径（其中包含帖子或 User 的 id）。
2. 第二个参数是我们从合规性端点的响应中获得的上传 URL。

请确保用双引号将该 URL 括起来；如果文件名中包含空格或其他字符，也以同样方式为文件名添加双引号：
...
您可以在任意时间通过按 Ctrl + C 或关闭命令行窗口来暂停上传。稍后再次执行相同命令时，您将能够从中断的位置继续上传。文件完全上传后，您将看到以下消息：

`Upload complete`

此时，您可以使用[合规状态端点](/zh/x-api/compliance/get-compliance-job)检查合规任务的状态，并在完成后下载合规结果。

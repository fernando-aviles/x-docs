...
本指南介绍了在将 Direct Messages 查询端点集成到你的应用程序时需要了解的关键概念。
...
<Warning>
  不支持 App-only 身份验证。所有私信（Direct Message）都是私密的。
</Warning>
...
|:------|:-------------|
| `dm.read` | 读取私信事件 |
| `tweet.read` | 与 dm.read 搭配使用时为必需 |
| `users.read` | 与 dm.read 搭配使用时为必需 |
...
<CardGroup cols={2}>
  <Card title="一对一" icon="message">
    始终恰好只有两名参与者。会话 ID 格式：`{smaller_user_id}-{larger_user_id}`
  </Card>
...
| 事件 | 描述 | 关键字段 |
|:------|:------------|:-----------|
| `MessageCreate` | 发送了一条消息 | `text`, `sender_id` |
...
| 字段 | 描述 | 事件 |
|:------|:------------|:-------|
| `dm_conversation_id` | 会话 ID | 所有 |
| `created_at` | 事件时间戳 | 所有 |
| `sender_id` | 发送方/邀请方 | MessageCreate, Join |
| `attachments` | 媒体附件 | MessageCreate |
| `referenced_tweets` | 分享的帖子 | MessageCreate |
...
| `participant_ids` | 每个参与者对应的 User 对象 |
...
会话和事件 ID 在 v1.1 和 v2 的端点之间是通用的。这意味着你可以：

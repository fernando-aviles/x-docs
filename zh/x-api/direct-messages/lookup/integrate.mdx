...
title: 集成指南
sidebarTitle: 集成指南
description: 集成 DM 查找端点的关键概念和最佳实践
keywords: ["私信集成", "DM 集成", "DM 查找集成"]
---

import { Button } from '/snippets/zh/button.mdx';

本指南介绍将私信（Direct Messages）查询端点集成到你的应用所需了解的关键概念。
...
| Scope | 必需条件 |
|:------|:-------------|
| `dm.read` | 读取私信事件（DM events） |
| `tweet.read` | 与 dm.read 一起使用时必需 |
| `users.read` | 与 dm.read 一起使用时必需 |
...
<CardGroup cols={2}>
  <Card title="一对一" icon="message">
    始终且仅有两名参与者。会话 ID 格式：`{smaller_user_id}-{larger_user_id}`
  </Card>
...
| 事件 | 描述 | 关键字段 |
|:------|:------------|:-----------|
| `MessageCreate` | 一条消息已发送 | `text`, `sender_id` |
...
| 字段 | 描述 | 事件 |
|:------|:------------|:-------|
| `dm_conversation_id` | 会话 ID | 全部 |
| `created_at` | 事件时间戳 | 全部 |
| `sender_id` | 发送者/邀请者 | MessageCreate, Join |
...
| 扩展项 | 返回内容 |
|:----------|:--------|
| `sender_id` | 发送方对应的 User 对象 |
| `participant_ids` | 参与者对应的 User 对象 |
| `attachments.media_keys` | Media 对象 |

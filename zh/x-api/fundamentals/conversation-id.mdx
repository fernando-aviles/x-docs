---
title: 会话 ID
sidebarTitle: 会话 ID
description: 用于跟踪和重建会话线程
keywords: ["会话 ID", "会话线程", "回复线程", "线程跟踪", "会话跟踪"]
---

X 上的每条回复都属于一个会话线程。`conversation_id` 字段可让你识别、跟踪并重建完整的会话树。

***

<div id="how-it-works">
  ## 工作原理
</div>

当有人发布帖子，其他人进行回复时，所有回复都会共享同一个 `conversation_id`——也就是发起该会话的原始帖子的 id。

```
Original post (ID: 1234567890)  ← conversation_id for all replies
├── Reply 1 (ID: 1234567891)    → conversation_id: 1234567890
│   └── Reply to Reply 1        → conversation_id: 1234567890
└── Reply 2 (ID: 1234567892)    → conversation_id: 1234567890
    └── Reply to Reply 2        → conversation_id: 1234567890
```

无论线程有多深，所有帖子都共享同一个 `conversation_id`。

***

<div id="requesting-conversation_id">
  ## 请求 conversation_id
</div>

在你的 `tweet.fields` 参数中添加 `conversation_id`：

```bash
curl "https://api.x.com/2/tweets/1234567891?tweet.fields=conversation_id,in_reply_to_user_id,referenced_tweets" \
  -H "Authorization: Bearer $TOKEN"
```

响应：

```json
{
  "data": {
    "id": "1234567891",
    "text": "@user Great point!",
    "conversation_id": "1234567890",
    "in_reply_to_user_id": "2244994945",
    "referenced_tweets": [{
      "type": "replied_to",
      "id": "1234567890"
    }]
  }
}
```

***

<div id="getting-a-full-conversation">
  ## 获取完整会话
</div>

使用 `conversation_id` 作为搜索运算符来检索该会话线程中的所有帖子：

```bash
curl "https://api.x.com/2/tweets/search/recent?\
query=conversation_id:1234567890&\
tweet.fields=author_id,created_at,in_reply_to_user_id&\
expansions=author_id" \
  -H "Authorization: Bearer $TOKEN"
```

这将返回原始帖子的所有回复，并按时间倒序排列。

***

<div id="use-cases">
  ## 使用场景
</div>

<Tabs>
  <Tab title="对话串重建">
    构建完整的会话树：

    ```python
    import requests

    conversation_id = "1234567890"
    url = f"https://api.x.com/2/tweets/search/recent"
    params = {
        "query": f"conversation_id:{conversation_id}",
        "tweet.fields": "author_id,in_reply_to_user_id,referenced_tweets,created_at",
        "max_results": 100
    }

    response = requests.get(url, headers=headers, params=params)
    replies = response.json()["data"]

    # 按 created_at 排序以获得时间顺序的结果
    replies.sort(key=lambda x: x["created_at"])
    ```
  </Tab>

  <Tab title="会话监控">
    实时流式获取特定会话的回复：

    ```bash
    # 为某个会话添加过滤流规则
    curl -X POST "https://api.x.com/2/tweets/search/stream/rules" \
      -H "Authorization: Bearer $TOKEN" \
      -d '{"add": [{"value": "conversation_id:1234567890"}]}'
    ```
  </Tab>

  <Tab title="会话分析">
    统计某个会话中的回复数量：

    ```bash
    curl "https://api.x.com/2/tweets/counts/recent?\
    query=conversation_id:1234567890" \
      -H "Authorization: Bearer $TOKEN"
    ```
  </Tab>
</Tabs>

***

<div id="related-fields">
  ## 相关字段
</div>

| 字段 | 描述 |
|:------|:------------|
| `conversation_id` | 发起该会话的原始帖子 ID |
| `in_reply_to_user_id` | 所回复帖子的作者用户 ID |
| `referenced_tweets` | 包含 `type: "replied_to"` 以及父帖子 ID 的数组 |

***

<div id="example-full-thread-retrieval">
  ## 示例：完整帖文串检索
</div>

```json
{
  "data": [
    {
      "id": "1234567893",
      "text": "@user2 @user1 I agree with you both!",
      "conversation_id": "1234567890",
      "author_id": "3333333333",
      "created_at": "2024-01-15T12:05:00.000Z",
      "in_reply_to_user_id": "2222222222",
      "referenced_tweets": [{"type": "replied_to", "id": "1234567892"}]
    },
    {
      "id": "1234567892",
      "text": "@user1 That's interesting!",
      "conversation_id": "1234567890",
      "author_id": "2222222222",
      "created_at": "2024-01-15T12:03:00.000Z",
      "in_reply_to_user_id": "1111111111",
      "referenced_tweets": [{"type": "replied_to", "id": "1234567890"}]
    },
    {
      "id": "1234567891",
      "text": "@user1 Great point!",
      "conversation_id": "1234567890",
      "author_id": "4444444444",
      "created_at": "2024-01-15T12:02:00.000Z",
      "in_reply_to_user_id": "1111111111",
      "referenced_tweets": [{"type": "replied_to", "id": "1234567890"}]
    }
  ],
  "meta": {
    "result_count": 3
  }
}
```

***

<div id="notes">
  ## 注意事项
</div>

* 原始帖子的 `conversation_id` 等于它自身的 `id`
* `conversation_id` 在所有返回帖子结果的 v2 端点中均可用
* 搭配 [filtered stream](/zh/x-api/posts/filtered-stream/introduction) 使用，以实时监控会话
* 与 [pagination](/zh/x-api/fundamentals/pagination) 结合使用，以处理较长的会话线程

***

<div id="next-steps">
  ## 后续步骤
</div>

<CardGroup cols={2}>
  <Card title="搜索帖子" icon="magnifying-glass" href="/zh/x-api/posts/search/introduction">
    按 conversation&#95;id 搜索。
  </Card>

  <Card title="过滤后的流" icon="signal-stream" href="/zh/x-api/posts/filtered-stream/introduction">
    实时监控会话。
  </Card>
</CardGroup>
...
keywords: ["chunked upload", "媒体上传", "分块媒体", "视频上传", "大文件上传", "分块上传指南"]
---

本指南将引导你使用分块上传流程来上传视频和大型媒体文件。

对于视频或大型媒体文件上传，你必须依次执行：

1. **INIT** — 初始化上传并获取一个 `media_id`
2. **APPEND** — 上传文件的每个分块
3. **FINALIZE** — 完成上传
4. **STATUS** —（如果需要）等待处理完成

<Note>
  完整的 Python 示例请参见[此示例代码](https://github.com/xdevplatform/large-video-upload-python)。
</Note>
...
上传文件的每个分块。例如，可以将一个 3 MB 的文件拆分成 3 个分块：
...
<Note>
  如果返回了 `processing_info`，请继续执行步骤 4 等待处理完成。否则，媒体即可使用。
</Note>
...
如果返回了 `processing_info`，轮询直至处理完成：

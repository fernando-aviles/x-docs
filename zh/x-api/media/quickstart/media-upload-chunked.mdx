...
title: 分块媒体上传
sidebarTitle: 快速开始
description: 使用分块上传视频和大型媒体文件
keywords: ["分块上传", "媒体上传", "分块媒体", "视频上传", "大型文件上传", "分块上传指南"]
---

本指南将演示如何使用分块上传流程来上传视频和大型媒体文件。
...
上传文件的每个分块。例如，将一个 3 MB 的文件切分为 3 个分块：
...
<CodeGroup dropdown>
  ...
  // 上传分块
  const chunkSize = 1024 * 1024; // 1 MB 分块
...
<Note>
  如果返回了 `processing_info`，请继续执行步骤 4 等待处理完成。如果没有返回，则说明媒体已可用。
</Note>
...
如果响应中返回了 `processing_info`，轮询直到处理完成：
...
<CodeGroup dropdown>
  ...
  console.log(`已发布：${response.data?.id}`);
  ```
</CodeGroup>

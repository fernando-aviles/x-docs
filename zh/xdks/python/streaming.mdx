...
X API 通过 [Filtered Stream Endpoint](https://docs.x.com/x-api/posts/filtered-stream/introduction) 等端点支持实时数据，在匹配的帖子出现时实时传递。这需要建立一个持久的 HTTP 连接。
...
规则用于定义筛选条件，以确定你希望获取的特定 data（例如关键词、用户等）。你可以通过阅读[本指南](https://docs.x.com/x-api/posts/filtered-stream/integrate/build-a-rule)来了解如何构建规则。
**添加规则**：
...
有关完整的规则语法，请参见 [X 流式规则文档](https://developer.x.com/en/docs/twitter-api/tweets/filtered-stream/integrate/build-a-rule)。
...
- **420 Enhance Your Calm**：触发速率限制；请等待后重试。
...
有关更多示例和 API 参考，请参阅内联 docstring（例如 `help(client.tweets.search_recent)`）或源码中自动生成的桩代码。欢迎通过 [GitHub 仓库](https://github.com/xdevplatform/xdk/tree/main/xdk/python) 提交反馈。
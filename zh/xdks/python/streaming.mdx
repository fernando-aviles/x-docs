...
X API 通过 [Filtered Stream Endpoint](https://docs.x.com/x-api/posts/filtered-stream/introduction) 等端点支持实时数据，在帖子产生时实时传递匹配的帖子。这需要建立一个持久的 HTTP 连接。
...
规则用于定义筛选条件，以确定你要检索的具体数据（例如关键字、用户等）。你可以参阅[本指南](https://docs.x.com/x-api/posts/filtered-stream/integrate/build-a-rule)了解更多关于如何构建规则的信息。
**添加规则**：
...
for page in client.stream.get_rules():
    ...
    break  # 移除 break 以获取所有页面
```

有关完整规则语法，请参阅 [X Streaming Rules 文档](https://developer.x.com/en/docs/twitter-api/tweets/filtered-stream/integrate/build-a-rule)。
...
- **420 Enhance Your Calm**：已达到速率限制；请等待后重试。
- **No Data**：使用 `get_rules()` 检查规则；确保存在与规则匹配的帖子。
如需使用 Python XDK 的详细代码示例，请查看我们的 [代码示例 GitHub 仓库](https://github.com/xdevplatform/samples/tree/main/python)。
更多示例和 API 参考，请参阅内联文档字符串（docstring）（例如 `help(client.tweets.search_recent)`）或源码中的生成存根文件。欢迎通过 [GitHub 仓库](https://github.com/xdevplatform/xdk/tree/main/xdk/python) 提交反馈。
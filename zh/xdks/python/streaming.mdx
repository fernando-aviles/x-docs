...
X API 通过诸如 [Filtered Stream Endpoint](https://docs.x.com/x-api/posts/filtered-stream/introduction) 之类的端点支持实时数据，在帖子产生的同时传递匹配的帖子。这需要建立一个持久的 HTTP 连接。
...
规则用于为你要查找的特定 data 定义过滤条件（例如关键词、用户等）。你可以通过[本指南](https://docs.x.com/x-api/posts/filtered-stream/integrate/build-a-rule)了解更多关于如何构建规则的信息。
**添加规则**：
...
要了解完整的规则语法，请参阅 [X Streaming Rules 文档](https://developer.x.com/en/docs/twitter-api/tweets/filtered-stream/integrate/build-a-rule)。
...
- **420 Enhance Your Calm**：已触发速率限制；请稍候后重试。
...
有关更多示例和 API 参考，请参阅内联文档字符串（例如 `help(client.tweets.search_recent)`）或源代码中的自动生成存根（stub）文件。可通过 [GitHub 仓库](https://github.com/xdevplatform/xdk/tree/main/xdk/python) 提交反馈。
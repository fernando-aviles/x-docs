...
该模块为安全的授权流程提供 OAuth1.0a 身份验证功能。包括请求令牌生成、授权 URL 生成、访问令牌交换以及 OAuth1 签名生成等功能。

<div id="class-xdkoauth1_authoauth1">
  ### `class xdk.oauth1_auth.OAuth1`
</div>

用于 X API 的 OAuth1 身份验证处理器。
...
获取请求 token，用于启动 OAuth 1.0 授权流程。
:returns: 包含 oauth_token 和 oauth_token_secret 的请求 token。

...
该模块提供 OAuth1.0a 认证功能，用于实现安全的授权流程。包括请求 token 生成、授权 URL 生成、访问 token 交换以及 OAuth1 签名生成。
...
获取请求 token，以开始 OAuth1 授权流程。
:returns: 返回包含 oauth_token 和 oauth_token_secret 的请求 token。

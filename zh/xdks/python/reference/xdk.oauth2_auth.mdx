...
该模块提供 OAuth2 PKCE（代码交换证明密钥）身份验证功能，用于安全的授权流程。支持代码验证器生成、令牌管理以及自动令牌刷新功能。
...
用于将授权码换取令牌（与 TypeScript API 一致）。
...
:returns: 当前的 code verifier，若未设置则为 None。
:rtype: Optional[str]

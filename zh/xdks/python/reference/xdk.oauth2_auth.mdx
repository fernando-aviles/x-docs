...
此模块提供 OAuth2 PKCE（证明密钥用于代码交换，Proof Key for Code Exchange）身份验证功能，用于实现安全的授权流程。包括 code verifier 生成、令牌管理以及自动令牌刷新等功能。
...
OAuth 2.0 授权的基础 URL（默认为 [https://x.com/i](https://x.com/i)）。
</ParamField>
...
通过授权响应 URL 获取令牌（旧版方法，内部使用 `exchange_code`）。
...
获取 OAuth 2.0 PKCE 授权流程所需的授权 URL。
...
:returns: 当前的 code challenge，如果尚未设置则返回 None。
:rtype: Optional[str]
...
:returns: 当前的 code verifier；如果尚未设置，则为 None。
:rtype: Optional[str]
...
检查令牌是否已过期。
:returns: 如果令牌已过期则为 True，否则为 False。
:rtype: bool

...
stream.on('data', (event) => {
  // event 是已解析的 JSON 行（data/includes/matching_rules）
  console.log('新数据:', event);
});

stream.on('error', (e) => console.error('流错误:', e));
...
for await (const event of stream) {
  // 每个 event 是一行已解析的 JSON（data/includes/matching_rules）
  console.log(event);
}
```
...
<Info>
  如需查看使用 JavaScript/TypeScript XDK 的详细代码示例，请访问我们的 [GitHub 代码示例仓库](https://github.com/xdevplatform/samples/tree/main/javascript)。
</Info>

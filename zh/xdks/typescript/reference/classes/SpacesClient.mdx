...
此 Client 提供用于与 X API 的 Spaces 端点交互的方法。它负责处理所有与 Spaces 相关操作的认证、请求格式化以及响应解析。
...
获取 Space 中的帖子\
通过 Space 的 id 检索在该 Space 中分享的帖子列表。
...
Search Spaces
搜索 Spaces，检索与指定搜索查询匹配的 Spaces 列表。
...
一个 Promise，最终解析为 API 响应；如果 `requestOptions.raw` 为 `true`，则返回原始 `Response`
...
一个 Promise，会解析为 API 响应；如果 `requestOptions.raw` 为 true，则返回原始 `Response` 对象

...
此客户端提供用于与 X API 的 Webhook 端点进行交互的方法。它负责处理所有与 Webhook 相关操作的认证、请求格式化以及响应解析。
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `webhookId` | `string` | 要与你的 FilteredStream 规则集关联的 webhook id。 |
...
删除流链接
删除从 FilteredStream 事件指向指定 webhook 的链接。
...
会解析为 API 响应的 Promise，或者当 requestOptions.raw 为 true 时返回原始 Response 对象
...
为 webhook 创建重放任务\
创建一个重放任务，用于检索最近 24 小时内所有已投递或尝试投递到该 webhook 的事件。
...
Promise 解析为 API 响应；如果 requestOptions.raw 为 true，则解析为原始响应
...
该 Promise 将解析为 API 响应；如果 `requestOptions.raw` 为 `true`，则解析为原始 Response

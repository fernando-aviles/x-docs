...
* 使用 `fetchNext()` 和 `fetchPrevious()` 进行手动分页控制
* 访问包含分页令牌和计数信息的元数据
* 错误处理和速率限制（rate limit）检测
* 同时支持向前和向后分页
...
console.log(followers.items.length); // 粉丝数量
console.log(followers.meta.nextToken); // 下一页令牌
...
此方法会获取下一页数据，并将这些条目追加到当前的分页器实例中。同时会更新分页状态和元数据。

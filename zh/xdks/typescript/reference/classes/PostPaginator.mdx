...
• **new PostPaginator**(`fetchPage`): [`PostPaginator`](/zh/xdks/typescript/reference/classes/PostPaginator)

创建一个新的 PostPaginator 分页器实例
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/zh/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`any`&gt;&gt; | 用于根据分页令牌获取单页数据的函数 |
...
此方法获取下一页数据，并将这些条目追加到当前的分页器实例中，同时更新分页状态和元数据。
...
此方法会创建一个新的 paginator 实例，该实例从下一页开始，

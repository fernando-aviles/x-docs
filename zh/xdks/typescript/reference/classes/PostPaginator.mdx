...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/zh/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`any`&gt;&gt; | 根据分页令牌拉取一页数据的函数 |
...
此方法会获取下一页的 data，并将这些条目追加到当前分页器实例中。它会更新分页状态和元数据。

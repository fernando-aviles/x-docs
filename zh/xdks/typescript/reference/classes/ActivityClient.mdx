...
该客户端提供与 X API 的 activity 端点交互的方法。它负责处理所有与活动相关操作的身份验证、请求格式化以及响应解析。
...
该 Promise 会解析为 API 响应；如果 requestOptions.raw 为 true，则解析为原始响应
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `subscriptionId` | `string` | 要更新的订阅的 ID。 |
...
Promise 会解析为 API 响应；如果 requestOptions.raw 为 true，则返回原始 Response
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `subscriptionId` | `string` | 要删除的订阅 id。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

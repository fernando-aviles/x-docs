---
title: "UsersClient"
sidebarTitle: "UsersClient"
---

用于用户相关操作的客户端

此客户端提供方法，用于与 X API 中的用户相关端点进行交互。它负责处理所有与用户相关操作的身份验证、请求构造以及响应解析。

<div id="constructors">
  ## 构造函数
</div>

<div id="constructor">
  ### constructor
</div>

• **new UsersClient**(`client`): [`UsersClient`](/zh/xdks/typescript/reference/classes/UsersClient)

创建一个新的 UsersClient 实例

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `client` | [`Client`](/zh/xdks/typescript/reference/classes/Client) | X API 的主客户端实例 |

<div id="returns">
  #### 返回值
</div>

[`UsersClient`](/zh/xdks/typescript/reference/classes/UsersClient)

#### 定义于

[users/client.ts:1234](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L1234)

<div id="methods">
  ## 方法
</div>

<div id="getbyusernames">
  ### getByUsernames
</div>

▸ **getByUsernames**(`usernames`, `options`): `Promise`&lt;`Response`&gt;

根据用户名获取用户
根据多个用户的用户名检索这些用户的详细信息。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `usernames` | `any`[] | 以逗号分隔的用户名列表。 |
| `options` | `GetByUsernamesOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个会解析为 API 响应的 Promise，或在 `requestOptions.raw` 为 `true` 时返回原始 Response

#### 定义于

[users/client.ts:1279](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L1279)

▸ **getByUsernames**(`usernames`, `options?`): `Promise`&lt;[`Get2UsersByResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersByResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `usernames` | `any`[] |
| `options?` | `GetByUsernamesOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersByResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersByResponse)&gt;

#### 定义于

[users/client.ts:1292](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L1292)

***

<div id="getrepostsofme">
  ### getRepostsOfMe
</div>

▸ **getRepostsOfMe**(`options`): `Promise`&lt;`Response`&gt;

获取转发我的帖子
检索转发已认证用户内容的帖子列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `options` | `GetRepostsOfMeOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则为原始 `Response` 对象

#### 定义于

[users/client.ts:1471](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L1471)

▸ **getRepostsOfMe**(`options?`): `Promise`&lt;[`Get2UsersRepostsOfMeResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersRepostsOfMeResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `options?` | `GetRepostsOfMeOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersRepostsOfMeResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersRepostsOfMeResponse)&gt;

#### 定义于

[users/client.ts:1480](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L1480)

***

<div id="unlikepost">
  ### unlikePost
</div>

▸ **unlikePost**(`id`, `tweetId`, `options`): `Promise`&lt;`Response`&gt;

取消点赞帖子
使已通过身份验证的用户根据其 ID 取消点赞指定的帖子。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 发起取消点赞该帖子请求的经过身份验证的源用户的 id。 |
| `tweetId` | `string` | 用户请求取消点赞的帖子的 id。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

Promise 解析为 API 响应；如果 requestOptions.raw 为 true，则为原始 Response 对象

#### 定义于

[users/client.ts:1741](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L1741)

▸ **unlikePost**(`id`, `tweetId`): `Promise`&lt;[`UsersLikesDeleteResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersLikesDeleteResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `tweetId` | `string` |

#### 返回

`Promise`&lt;[`UsersLikesDeleteResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersLikesDeleteResponse)&gt;

#### 定义于

[users/client.ts:1758](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L1758)

***

<div id="getbookmarks">
  ### getBookmarks
</div>

▸ **getBookmarks**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取书签列表
检索当前经过身份验证的用户已加书签的帖子列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要返回结果的已通过身份验证的源用户的 id。 |
| `options` | `GetBookmarksOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个解析为 API 响应的 Promise，如果 requestOptions.raw 为 true，则返回原始 Response

#### 定义于

[users/client.ts:1861](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L1861)

▸ **getBookmarks**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdBookmarksResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdBookmarksResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetBookmarksOptions` |

#### 返回

`Promise`&lt;[`Get2UsersIdBookmarksResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdBookmarksResponse)&gt;

#### 定义于

[users/client.ts:1874](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L1874)

***

<div id="createbookmark">
  ### createBookmark
</div>

▸ **createBookmark**(`id`, `body`, `options`): `Promise`&lt;`Response`&gt;

创建书签
将帖子添加到已认证用户的书签中。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要为其添加书签的已通过身份验证的源用户的 ID。 |
| `body` | [`BookmarkAddRequest`](/zh/xdks/typescript/reference/interfaces/Schemas.BookmarkAddRequest) | 请求正文 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个 Promise，其结果为 API 响应；如果 requestOptions.raw 为 true，则为原始响应 Response。

#### 定义于

[users/client.ts:2139](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L2139)

▸ **createBookmark**(`id`, `body`): `Promise`&lt;[`BookmarkMutationResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.BookmarkMutationResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `body` | [`BookmarkAddRequest`](/zh/xdks/typescript/reference/interfaces/Schemas.BookmarkAddRequest) |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`BookmarkMutationResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.BookmarkMutationResponse)&gt;

#### 定义于

[users/client.ts:2154](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L2154)

***

<div id="getfollowers">
  ### getFollowers
</div>

▸ **getFollowers**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取关注者
检索关注通过其 id 指定的特定用户的用户列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要查找的用户 ID。 |
| `options` | `GetFollowersOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 true，则返回原始 `Response`

#### 定义于

[users/client.ts:2245](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L2245)

▸ **getFollowers**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdFollowersResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdFollowersResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetFollowersOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersIdFollowersResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdFollowersResponse)&gt;

#### 定义于

[users/client.ts:2258](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L2258)

***

<div id="unpinlist">
  ### unpinList
</div>

▸ **unpinList**(`id`, `listId`, `options`): `Promise`&lt;`Response`&gt;

取消固定 List
使已通过身份验证的用户根据其 ID 取消固定指定的 List。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要为其返回结果的已认证源用户的 id。 |
| `listId` | `string` | 要取消固定的列表的 id。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则返回原始响应

#### 定义于

[users/client.ts:2474](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L2474)

▸ **unpinList**(`id`, `listId`): `Promise`&lt;[`ListUnpinResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.ListUnpinResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `listId` | `string` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`ListUnpinResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.ListUnpinResponse)&gt;

#### 定义于

[users/client.ts:2491](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L2491)

***

<div id="getbyid">
  ### getById
</div>

▸ **getById**(`id`, `options`): `Promise`&lt;`Response`&gt;

根据 id 获取用户
根据用户的 id 获取其详细信息。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要查询的用户的 id。 |
| `options` | `GetByIdOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 `true`，则返回原始响应（`Response`）

#### 定义于

[users/client.ts:2594](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L2594)

▸ **getById**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetByIdOptions` |

#### 返回

`Promise`&lt;[`Get2UsersIdResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdResponse)&gt;

#### 定义于

[users/client.ts:2607](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L2607)

***

<div id="getmentions">
  ### getMentions
</div>

▸ **getMentions**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取提及列表
根据用户的 id 检索提及该特定用户的帖子列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要查询的 User 的 id。 |
| `options` | `GetMentionsOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个会被解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 `true`，则解析为原始 `Response`

#### 定义于

[users/client.ts:2781](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L2781)

▸ **getMentions**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdMentionsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdMentionsResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetMentionsOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersIdMentionsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdMentionsResponse)&gt;

#### 定义于

[users/client.ts:2794](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L2794)

***

<div id="unrepostpost">
  ### unrepostPost
</div>

▸ **unrepostPost**(`id`, `sourceTweetId`, `options`): `Promise`&lt;`Response`&gt;

取消转发帖子
使认证用户通过其 ID 取消转发指定的帖子。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 说明 |
| :------ | :------ | :------ |
| `id` | `string` | 发起转发该帖子的已通过身份验证的源用户的 ID。 |
| `sourceTweetId` | `string` | 用户请求取消转发的帖子的 ID。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个解析为 API 响应的 Promise，或在 requestOptions.raw 为 true 时解析为原始 Response

#### 定义于

[users/client.ts:3149](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3149)

▸ **unrepostPost**(`id`, `sourceTweetId`): `Promise`&lt;[`UsersRetweetsDeleteResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersRetweetsDeleteResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `sourceTweetId` | `string` |

#### 返回

`Promise`&lt;[`UsersRetweetsDeleteResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersRetweetsDeleteResponse)&gt;

#### 定义于

[users/client.ts:3166](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3166)

***

<div id="deletebookmark">
  ### deleteBookmark
</div>

▸ **deleteBookmark**(`id`, `tweetId`, `options`): `Promise`&lt;`Response`&gt;

删除书签
根据其 ID，从已通过身份验证的用户的书签中移除一条帖子。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要移除其书签的已认证源用户的 ID。 |
| `tweetId` | `string` | 源用户要从书签中移除的帖子 ID。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则为原始 Response

#### 定义于

[users/client.ts:3273](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3273)

▸ **deleteBookmark**(`id`, `tweetId`): `Promise`&lt;[`BookmarkMutationResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.BookmarkMutationResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `tweetId` | `string` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`BookmarkMutationResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.BookmarkMutationResponse)&gt;

#### 定义于

[users/client.ts:3290](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3290)

***

<div id="unfollowlist">
  ### unfollowList
</div>

▸ **unfollowList**(`id`, `listId`, `options`): `Promise`&lt;`Response`&gt;

取消关注列表
使已授权用户根据列表 ID 取消关注指定的列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 将要取消关注该列表的已通过认证的源用户的 id。 |
| `listId` | `string` | 待取消关注的列表的 id。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则解析为原始响应

#### 定义于

[users/client.ts:3391](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3391)

▸ **unfollowList**(`id`, `listId`): `Promise`&lt;[`ListFollowedResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.ListFollowedResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `listId` | `string` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`ListFollowedResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.ListFollowedResponse)&gt;

#### 定义于

[users/client.ts:3408](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3408)

***

<div id="unmuteuser">
  ### unmuteUser
</div>

▸ **unmuteUser**(`sourceUserId`, `targetUserId`, `options`): `Promise`&lt;`Response`&gt;

取消静音用户
使已通过身份验证的用户根据目标用户的 ID 取消对该用户的静音。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `sourceUserId` | `string` | 发起请求以取消对目标用户静音的已通过身份验证的源用户的 ID。 |
| `targetUserId` | `string` | 源用户请求取消静音的目标用户的 ID。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个会解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 `true`，则返回原始 `Response`

#### 定义于

[users/client.ts:3515](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3515)

▸ **unmuteUser**(`sourceUserId`, `targetUserId`): `Promise`&lt;[`MuteUserMutationResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.MuteUserMutationResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `sourceUserId` | `string` |
| `targetUserId` | `string` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`MuteUserMutationResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.MuteUserMutationResponse)&gt;

#### 定义于

[users/client.ts:3532](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3532)

***

<div id="getme">
  ### getMe
</div>

▸ **getMe**(`options`): `Promise`&lt;`Response`&gt;

获取我的用户信息
获取已认证用户的详细信息。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `options` | `GetMeOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个 Promise，解析结果为 API 响应；如果 requestOptions.raw 为 true，则为原始 `Response`

#### 定义于

[users/client.ts:3631](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3631)

▸ **getMe**(`options?`): `Promise`&lt;[`Get2UsersMeResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersMeResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `options?` | `GetMeOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersMeResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersMeResponse)&gt;

#### 定义在

[users/client.ts:3640](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3640)

***

<div id="getmuting">
  ### getMuting
</div>

▸ **getMuting**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取静音列表
检索由已认证用户静音的用户列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要返回其结果的已通过身份验证的源用户的 id。 |
| `options` | `GetMutingOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个会解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则为原始响应

#### 定义于

[users/client.ts:3796](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3796)

▸ **getMuting**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdMutingResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdMutingResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetMutingOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersIdMutingResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdMutingResponse)&gt;

#### 定义于

[users/client.ts:3809](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L3809)

***

<div id="muteuser">
  ### muteUser
</div>

▸ **muteUser**(`id`, `options`): `Promise`&lt;`Response`&gt;

静音用户
使已认证用户将指定 ID 的用户静音。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 请求将目标 User 静音的经过身份验证的源 User 的 id。 |
| `options` | `MuteUserOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

Promise 将解析为 API 响应；如果 requestOptions.raw 为 true，则解析为原始 Response

#### 定义于

[users/client.ts:4015](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L4015)

▸ **muteUser**(`id`, `options?`): `Promise`&lt;[`MuteUserMutationResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.MuteUserMutationResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `MuteUserOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`MuteUserMutationResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.MuteUserMutationResponse)&gt;

#### 定义于

[users/client.ts:4028](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L4028)

***

<div id="search">
  ### search
</div>

▸ **search**(`query`, `options`): `Promise`&lt;`Response`&gt;

搜索用户
检索与查询匹配的用户列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `query` | `string` | 用于搜索用户的查询字符串。 |
| `options` | `SearchOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个解析为 API 响应的 Promise，或者在 requestOptions.raw 为 true 时返回原始 Response 对象

#### 定义于

[users/client.ts:4136](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L4136)

▸ **search**(`query`, `options?`): `Promise`&lt;[`Get2UsersSearchResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersSearchResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `query` | `string` |
| `options?` | `SearchOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersSearchResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersSearchResponse)&gt;

#### 定义于

[users/client.ts:4149](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L4149)

***

<div id="getblocking">
  ### getBlocking
</div>

▸ **getBlocking**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取屏蔽用户列表\
检索指定用户 id 已屏蔽的用户列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 将为其返回结果的已认证源用户的标识符。 |
| `options` | `GetBlockingOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个会解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则返回原始 Response

#### 定义于

[users/client.ts:4362](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L4362)

▸ **getBlocking**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdBlockingResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdBlockingResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetBlockingOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersIdBlockingResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdBlockingResponse)&gt;

#### 定义于

[users/client.ts:4375](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L4375)

***

<div id="getbyids">
  ### getByIds
</div>

▸ **getByIds**(`ids`, `options`): `Promise`&lt;`Response`&gt;

通过 id 批量获取用户
根据多个用户 id 检索其详细信息。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `ids` | `any`[] | 以逗号分隔的用户 id 列表。最多可指定 100 个 id。 |
| `options` | `GetByIdsOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则解析为原始响应（Response）

#### 定义于

[users/client.ts:4581](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L4581)

▸ **getByIds**(`ids`, `options?`): `Promise`&lt;[`Get2UsersResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `ids` | `any`[] |
| `options?` | `GetByIdsOptions` |

#### 返回

`Promise`&lt;[`Get2UsersResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersResponse)&gt;

#### 定义于

[users/client.ts:4594](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L4594)

***

<div id="getfollowing">
  ### getFollowing
</div>

▸ **getFollowing**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取关注列表
根据用户的 id，检索该用户关注的用户列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要查询的用户的 ID。 |
| `options` | `GetFollowingOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 true，则返回原始 `Response` 对象

#### 定义于

[users/client.ts:4777](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L4777)

▸ **getFollowing**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdFollowingResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdFollowingResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetFollowingOptions` |

#### 返回

`Promise`&lt;[`Get2UsersIdFollowingResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdFollowingResponse)&gt;

#### 定义于

[users/client.ts:4790](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L4790)

***

<div id="followuser">
  ### followUser
</div>

▸ **followUser**(`id`, `options`): `Promise`&lt;`Response`&gt;

关注用户
使经过身份验证的用户通过目标用户的 ID 关注该指定用户。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 发起关注目标用户请求的已通过认证的源用户的 id。 |
| `options` | `FollowUserOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个会解析为 API 响应的 Promise，或在 requestOptions.raw 为 true 时返回原始 Response

#### 定义于

[users/client.ts:5002](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L5002)

▸ **followUser**(`id`, `options?`): `Promise`&lt;[`UsersFollowingCreateResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersFollowingCreateResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `FollowUserOptions` |

#### 返回

`Promise`&lt;[`UsersFollowingCreateResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersFollowingCreateResponse)&gt;

#### 定义于

[users/client.ts:5015](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L5015)

***

<div id="likepost">
  ### likePost
</div>

▸ **likePost**(`id`, `options`): `Promise`&lt;`Response`&gt;

点赞帖子
使已认证用户对指定 ID 的帖子点喜欢。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 请求点赞该帖子的已认证源用户的 id。 |
| `options` | `LikePostOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个会解析为 API 响应的 Promise，或在 requestOptions.raw 为 true 时解析为原始 Response

#### 定义于

[users/client.ts:5123](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L5123)

▸ **likePost**(`id`, `options?`): `Promise`&lt;[`UsersLikesCreateResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersLikesCreateResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `LikePostOptions` |

#### 返回

`Promise`&lt;[`UsersLikesCreateResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersLikesCreateResponse)&gt;

#### 定义于

[users/client.ts:5136](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L5136)

***

<div id="getlikedposts">
  ### getLikedPosts
</div>

▸ **getLikedPosts**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取已点赞的帖子
根据特定用户的 ID，检索该用户点赞过的帖子列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要查询的用户 ID。 |
| `options` | `GetLikedPostsOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个将解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则解析为原始 Response

#### 定义于

[users/client.ts:5244](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L5244)

▸ **getLikedPosts**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdLikedTweetsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdLikedTweetsResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetLikedPostsOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersIdLikedTweetsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdLikedTweetsResponse)&gt;

#### 定义于

[users/client.ts:5257](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L5257)

***

<div id="getposts">
  ### getPosts
</div>

▸ **getPosts**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取帖子列表
根据用户的 ID 检索该用户发布的帖子列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要查找的用户 ID。 |
| `options` | `GetPostsOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则解析为原始 Response 对象

#### 定义于

[users/client.ts:5526](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L5526)

▸ **getPosts**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdTweetsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdTweetsResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetPostsOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersIdTweetsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdTweetsResponse)&gt;

#### 定义于

[users/client.ts:5539](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L5539)

***

<div id="blockdms">
  ### blockDms
</div>

▸ **blockDms**(`id`, `options`): `Promise`&lt;`Response`&gt;

屏蔽私信\
为经过认证的用户，根据指定用户的 id 屏蔽与该用户之间收发的私信。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 经过身份验证的用户请求对其阻止私信（DM）的目标 User 的 id。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个 `Promise`，其解析结果为 API 响应；如果 `requestOptions.raw` 为 `true`，则解析结果为原始 `Response`

#### 定义于

[users/client.ts:5909](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L5909)

▸ **blockDms**(`id`): `Promise`&lt;[`UsersDMBlockCreateResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersDMBlockCreateResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`UsersDMBlockCreateResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersDMBlockCreateResponse)&gt;

#### 定义于

[users/client.ts:5922](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L5922)

***

<div id="getlistmemberships">
  ### getListMemberships
</div>

▸ **getListMemberships**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取 List 成员关系
根据指定用户的 id，检索该用户作为成员加入的 List 列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要查询的用户 ID。 |
| `options` | `GetListMembershipsOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个 Promise，会解析为 API 响应；如果 requestOptions.raw 为 true，则解析为原始 Response

#### 定义于

[users/client.ts:6013](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6013)

▸ **getListMemberships**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdListMembershipsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdListMembershipsResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetListMembershipsOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersIdListMembershipsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdListMembershipsResponse)&gt;

#### 定义于

[users/client.ts:6026](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6026)

***

<div id="getpinnedlists">
  ### getPinnedLists
</div>

▸ **getPinnedLists**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取已置顶的列表
检索经过身份验证的用户置顶的列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要为其返回结果的已认证源用户的 ID。 |
| `options` | `GetPinnedListsOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则解析为原始响应

#### 定义于

[users/client.ts:6238](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6238)

▸ **getPinnedLists**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdPinnedListsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdPinnedListsResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetPinnedListsOptions` |

#### 返回

`Promise`&lt;[`Get2UsersIdPinnedListsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdPinnedListsResponse)&gt;

#### 定义于

[users/client.ts:6251](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6251)

***

<div id="pinlist">
  ### pinList
</div>

▸ **pinList**(`id`, `body`, `options`): `Promise`&lt;`Response`&gt;

固定列表\
使已认证用户根据其 id 固定指定的列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 将要固定该 List 的已认证源用户的 ID。 |
| `body` | [`ListPinnedRequest`](/zh/xdks/typescript/reference/interfaces/Schemas.ListPinnedRequest) | 请求体 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则为原始响应（Response）

#### 定义于

[users/client.ts:6421](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6421)

▸ **pinList**(`id`, `body`): `Promise`&lt;[`ListPinnedResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.ListPinnedResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `body` | [`ListPinnedRequest`](/zh/xdks/typescript/reference/interfaces/Schemas.ListPinnedRequest) |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`ListPinnedResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.ListPinnedResponse)&gt;

#### 定义于

[users/client.ts:6436](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6436)

***

<div id="getbyusername">
  ### getByUsername
</div>

▸ **getByUsername**(`username`, `options`): `Promise`&lt;`Response`&gt;

通过用户名获取用户信息\
检索指定用户名的用户详细信息。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `username` | `string` | 用户名。 |
| `options` | `GetByUsernameOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 `true`，则解析为原始 `Response` 对象

#### 定义于

[users/client.ts:6533](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6533)

▸ **getByUsername**(`username`, `options?`): `Promise`&lt;[`Get2UsersByUsernameUsernameResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersByUsernameUsernameResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `username` | `string` |
| `options?` | `GetByUsernameOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`Get2UsersByUsernameUsernameResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersByUsernameUsernameResponse)&gt;

#### 定义于

[users/client.ts:6546](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6546)

***

<div id="unfollowuser">
  ### unfollowUser
</div>

▸ **unfollowUser**(`sourceUserId`, `targetUserId`, `options`): `Promise`&lt;`Response`&gt;

取消关注用户
使已认证的用户通过其 id 取消关注特定用户。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `sourceUserId` | `string` | 发起取消关注请求的已认证源用户的 ID。 |
| `targetUserId` | `string` | 源用户请求取消关注的目标用户的 ID。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则解析为原始 Response

#### 定义于

[users/client.ts:6724](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6724)

▸ **unfollowUser**(`sourceUserId`, `targetUserId`): `Promise`&lt;[`UsersFollowingDeleteResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersFollowingDeleteResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `sourceUserId` | `string` |
| `targetUserId` | `string` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`UsersFollowingDeleteResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersFollowingDeleteResponse)&gt;

#### 定义于

[users/client.ts:6741](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6741)

***

<div id="gettimeline">
  ### getTimeline
</div>

▸ **getTimeline**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取时间线
检索经过身份验证的用户时间线中按时间倒序排列的帖子列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 已通过身份验证的源用户的 id，用于列出其按时间倒序排序的时间线帖子。 |
| `options` | `GetTimelineOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 `true`，则解析为原始 Response

#### 定义于

[users/client.ts:6844](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6844)

▸ **getTimeline**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdTimelinesReverseChronologicalResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdTimelinesReverseChronologicalResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetTimelineOptions` |

#### 返回

`Promise`&lt;[`Get2UsersIdTimelinesReverseChronologicalResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdTimelinesReverseChronologicalResponse)&gt;

#### 定义于

[users/client.ts:6857](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L6857)

***

<div id="getfollowedlists">
  ### getFollowedLists
</div>

▸ **getFollowedLists**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取已关注的列表\
根据特定用户的 id 检索其关注的列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要查找的用户 ID。 |
| `options` | `GetFollowedListsOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

#### 返回

`Promise`&lt;`Response`&gt;

一个解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 `true`，则解析为原始 `Response`

#### 定义于

[users/client.ts:7221](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7221)

▸ **getFollowedLists**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdFollowedListsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdFollowedListsResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetFollowedListsOptions` |

#### 返回

`Promise`&lt;[`Get2UsersIdFollowedListsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdFollowedListsResponse)&gt;

#### 定义于

[users/client.ts:7234](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7234)

***

<div id="followlist">
  ### followList
</div>

▸ **followList**(`id`, `options`): `Promise`&lt;`Response`&gt;

关注 List
使已通过身份验证的用户根据其 id 关注指定的 List。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 将要关注该列表的已认证源用户的 id。 |
| `options` | `FollowListOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个解析为 API 响应的 Promise，或在 `requestOptions.raw` 为 `true` 时返回原始 `Response`

#### 定义于

[users/client.ts:7446](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7446)

▸ **followList**(`id`, `options?`): `Promise`&lt;[`ListFollowedResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.ListFollowedResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `FollowListOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`ListFollowedResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.ListFollowedResponse)&gt;

#### 定义于

[users/client.ts:7459](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7459)

***

<div id="unblockdms">
  ### 解除私信屏蔽
</div>

▸ **unblockDms**(`id`, `options`): `Promise`&lt;`Response`&gt;

解除私信屏蔽\
为通过该 ID 指定的用户，解除已认证用户与其之间收发私信的屏蔽状态。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 已通过身份验证的用户请求解除其私信阻止的目标用户的 id。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个 Promise，该 Promise 解析为 API 响应；如果 requestOptions.raw 为 true，则解析为原始 Response 对象

<div id="defined-in">
  #### 定义位置
</div>

[users/client.ts:7567](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7567)

▸ **unblockDms**(`id`): `Promise`&lt;[`UsersDMUnBlockCreateResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersDMUnBlockCreateResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`UsersDMUnBlockCreateResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersDMUnBlockCreateResponse)&gt;

#### 定义于

[users/client.ts:7580](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7580)

***

<div id="repostpost">
  ### repostPost
</div>

▸ **repostPost**(`id`, `options`): `Promise`&lt;`Response`&gt;

转发帖子\
使已通过身份验证的用户通过其 ID 转发指定的帖子。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 请求转发该帖子的已通过身份验证的源用户的 id。 |
| `options` | `RepostPostOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

#### 返回

`Promise`&lt;`Response`&gt;

返回一个 Promise，该 Promise 解析为 API 响应；如果 `requestOptions.raw` 为 true，则解析为原始 `Response`

#### 定义于

[users/client.ts:7671](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7671)

▸ **repostPost**(`id`, `options?`): `Promise`&lt;[`UsersRetweetsCreateResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersRetweetsCreateResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `RepostPostOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`UsersRetweetsCreateResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.UsersRetweetsCreateResponse)&gt;

#### 定义于

[users/client.ts:7684](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7684)

***

<div id="getbookmarkfolders">
  ### getBookmarkFolders
</div>

▸ **getBookmarkFolders**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取书签文件夹
检索由经过身份验证的用户创建的书签文件夹列表。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要为其返回结果的已认证源用户的 ID。 |
| `options` | `GetBookmarkFoldersOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

一个会解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则解析为原始响应（Response）

#### 定义于

[users/client.ts:7792](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7792)

▸ **getBookmarkFolders**(`id`, `options?`): `Promise`&lt;[`BookmarkFoldersResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.BookmarkFoldersResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetBookmarkFoldersOptions` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`BookmarkFoldersResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.BookmarkFoldersResponse)&gt;

#### 定义于

[users/client.ts:7805](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7805)

***

<div id="getownedlists">
  ### getOwnedLists
</div>

▸ **getOwnedLists**(`id`, `options`): `Promise`&lt;`Response`&gt;

获取拥有的列表
根据指定用户的 id，检索其拥有的列表集合。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要查询的用户 id。 |
| `options` | `GetOwnedListsOptions` &amp; &#123; `requestOptions`: &#123; `raw`: `true`  &#125;  &#125; | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个会解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 true，则解析为原始 `Response`

#### 定义于

[users/client.ts:7944](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7944)

▸ **getOwnedLists**(`id`, `options?`): `Promise`&lt;[`Get2UsersIdOwnedListsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdOwnedListsResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `options?` | `GetOwnedListsOptions` |

#### 返回

`Promise`&lt;[`Get2UsersIdOwnedListsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.Get2UsersIdOwnedListsResponse)&gt;

#### 定义于

[users/client.ts:7957](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L7957)

***

<div id="getbookmarksbyfolderid">
  ### getBookmarksByFolderId
</div>

▸ **getBookmarksByFolderId**(`id`, `folderId`, `options`): `Promise`&lt;`Response`&gt;

通过文件夹 ID 获取书签\
为已授权用户检索指定书签文件夹中的帖子。

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要返回结果的已认证源用户的 id。 |
| `folderId` | `string` | 已认证用户要获取其中帖子列表的书签文件夹 id。 |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;`Response`&gt;

返回一个会解析为 API 响应的 Promise；如果 requestOptions.raw 为 true，则返回原始响应

#### 定义于

[users/client.ts:8173](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L8173)

▸ **getBookmarksByFolderId**(`id`, `folderId`): `Promise`&lt;[`BookmarkFolderPostsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.BookmarkFolderPostsResponse)&gt;

<div id="parameters">
  #### 参数
</div>

| 名称 | 类型 |
| :------ | :------ |
| `id` | `string` |
| `folderId` | `string` |

<div id="returns">
  #### 返回值
</div>

`Promise`&lt;[`BookmarkFolderPostsResponse`](/zh/xdks/typescript/reference/interfaces/Schemas.BookmarkFolderPostsResponse)&gt;

#### 定义于

[users/client.ts:8190](https://github.com/xdevplatform/xdk-typescript/blob/81aacb165e0802e188f608bdf462b60fc4e713a2/src/users/client.ts#L8190)
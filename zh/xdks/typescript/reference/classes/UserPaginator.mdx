...
| 名称 | 类型 | 说明 |
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/zh/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`any`&gt;&gt; | 使用分页令牌获取一页数据的函数 |
...
此方法会获取下一页数据，并将其中的条目追加到当前分页器实例中。该方法会更新分页状态和元数据。

...
该客户端提供与 X API 中帖子相关端点交互的方法。它负责处理身份验证、请求格式化以及所有帖子相关操作的响应解析。
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `tweetIds` | `any`[] | 28 小时指标对应的帖子 id 列表。 |
| `granularity` | `string` | 指标响应的粒度。 |
| `requestedMetrics` | `any`[] | 历史请求中要获取的指标。 |
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `ids` | `any`[] | 以逗号分隔的帖子 ID 列表。单个请求中最多可包含 100 个。 |
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `query` | `string` | 用于匹配帖子的单条查询/规则/过滤条件。请参阅 https://t.co/rulelength 以了解查询的最大长度限制。 |
...
一个解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 `true`，则返回原始响应（`Response`）
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `query` | `string` | 一个用于匹配帖子的查询/规则/过滤条件。请参阅 https://t.co/rulelength 了解查询长度上限。 |
...
返回一个 Promise，其解析结果为 API 响应；如果 `requestOptions.raw` 为 `true`，则解析结果为原始 `Response`。
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `ids` | `any`[] | 以逗号分隔的帖子 ID 列表。单个请求最多可包含 100 个。 |
...
返回一个解析为 API 响应的 Promise，或者在 requestOptions.raw 为 true 时返回原始响应
...
创建或编辑帖子
为经过身份验证的用户创建新帖子，或在提供 edit&#95;options 时编辑现有帖子。
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `query` | `string` | 用于匹配帖子的单条查询/规则/过滤条件。请参阅 https://t.co/rulelength 以确定查询的最大长度。 |
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `tweetIds` | `any`[] | 用于历史指标的 PostId 列表。 |
...
返回一个解析为 API 响应的 Promise，或者在 requestOptions.raw 为 true 时解析为原始 Response
...
解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 true，则为原始的 Response
...
删除帖子
如果指定帖子归经过身份验证的用户所有，则可根据其 id 将其删除。
...
一个会解析为 API 响应的 Promise，或者当 `requestOptions.raw` 为 true 时解析为原始 `Response`

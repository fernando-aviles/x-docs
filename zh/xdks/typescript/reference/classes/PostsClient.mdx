...
此 Client 提供与 X API 帖子端点交互的方法，负责处理所有与帖子相关操作的认证、请求格式化以及响应解析。
...
| :------ | :------ | :------ |
| `tweetIds` | `any`[] | 用于 28 小时指标的 PostIds 列表。 |
| `granularity` | `string` | 指标响应的粒度。 |
| `requestedMetrics` | `any`[] | 历史请求所需的指标。 |
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `ids` | `any`[] | 用逗号分隔的帖子 ID 列表。单个请求最多允许 100 个。 |
...
一个 Promise，解析后得到 API 响应；如果 `requestOptions.raw` 为 true，则返回原始 `Response` 对象
...
返回一个以 API 响应为结果的 Promise；如果 `requestOptions.raw` 为 true，则返回原始 `Response`
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `query` | `string` | 用于匹配帖子的单个查询/规则/过滤条件。请参阅 https://t.co/rulelength 了解最大查询长度。 |
...
一个最终会返回 API 响应的 Promise，或当 `requestOptions.raw` 为 true 时返回原始 Response
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `query` | `string` | 用于匹配帖子的一条查询、规则或过滤条件。有关查询的最大长度，请参阅 https://t.co/rulelength。 |
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `ids` | `any`[] | 以逗号分隔的帖子 id 列表。单个请求最多可包含 100 个 id。 |
...
一个 Promise，解析后得到 API 响应；如果 requestOptions.raw 为 true，则解析为原始 Response
...
创建或编辑帖子
为已通过身份验证的用户创建一条新帖子，或在提供 `edit_options` 时编辑现有帖子。
...
其解析结果为 API 响应的 Promise；如果 requestOptions.raw 为 true，则返回原始 Response 对象
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `query` | `string` | 用于匹配帖子的单条查询/规则/过滤条件。请参阅 https://t.co/rulelength 以确定最大查询长度。 |
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `tweetIds` | `any`[] | 用于历史指标的帖子 id 列表。 |
| `endTime` | `string` | YYYY-MM-DDTHH:mm:ssZ。表示时间范围结束时间的 UTC 时间戳。 |
| `startTime` | `string` | YYYY-MM-DDTHH:mm:ssZ。表示时间范围开始时间的 UTC 时间戳。 |
| `granularity` | `string` | 指标响应的粒度级别。 |
| `requestedMetrics` | `any`[] | 为历史请求指定的指标。 |
...
| 名称 | 类型 | 说明 |
| :------ | :------ | :------ |
| `query` | `string` | 用于匹配帖子的一条查询/规则/过滤条件。请参阅 https://t.co/rulelength 以确定查询的最大长度限制。 |
...
一个最终解析为 API 响应的 Promise，如果 `requestOptions.raw` 为 true，则解析为原始 `Response`
...
删除帖子
如果目标帖子归当前已认证用户所有，则根据其 id 将该帖子删除。
...
一个会解析为 API 响应的 Promise；如果 `requestOptions.raw` 为 `true`，则会解析为原始 `Response`

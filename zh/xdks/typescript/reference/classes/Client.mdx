...
这是与 X API 交互的主要入口点。它通过专用的 Client 模块提供对所有 API 端点的访问，并负责处理身份验证、请求配置和错误处理。
...
<ResponseField name="request" type="Promise<T>" required>
  ...
  此方法负责处理所有 API 请求的认证、请求格式化和错误处理，并会根据客户端配置自动添加合适的认证头。
</ResponseField>

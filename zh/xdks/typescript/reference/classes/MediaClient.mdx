...
此 Client 提供用于与 X API 媒体端点交互的方法，负责处理所有媒体相关操作的身份验证、请求格式化和响应解析。
...
根据该 media key 检索指定 Media 文件的详细信息。
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `mediaId` | `string` | 所请求媒体上传状态对应的媒体 id。 |
...
一个在解析后得到 API 响应的 Promise；如果 `requestOptions.raw` 为 true，则解析为原始 `Response`
...
| 名称 | type | 描述 |
| :------ | :------ | :------ |
| `mediaKeys` | `any`[] | 以逗号分隔的 Media Key 列表。单个请求最多允许 100 个 Media Key。 |
...
Promise 会解析为 API 响应；如果 requestOptions.raw 为 true，则解析为原始 Response
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要完成上传流程的目标媒体的 media id。 |
| `options` | `Object` | - |
...
一个 Promise，解析后得到 API 响应；如果 `requestOptions.raw` 为 `true`，则得到原始 `Response`

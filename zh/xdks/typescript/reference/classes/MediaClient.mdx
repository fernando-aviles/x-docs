...
该客户端提供与 X API 媒体端点交互的方法。它负责处理所有媒体相关操作的身份验证、请求格式化以及响应解析。
...
根据该 media key 检索特定 Media 文件的详细信息。
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `mediaId` | `string` | 要查询的媒体上传状态对应的媒体 id。 |
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `mediaKeys` | `any`[] | 以逗号分隔的 Media Key 列表。单个请求中最多可提供 100 个。 |
| `endTime` | `string` | 格式：YYYY-MM-DDTHH:mm:ssZ。表示时间范围结束时间的 UTC 时间戳。 |
| `startTime` | `string` | 格式：YYYY-MM-DDTHH:mm:ssZ。表示时间范围开始时间的 UTC 时间戳。 |
...
该 Promise 最终会解析为 API 响应；如果 requestOptions.raw 为 true，则返回原始 Response
...
一个 Promise，会解析为 API 响应；如果 requestOptions.raw 为 true，则解析为原始响应
...
一个会解析为 API 响应的 Promise；或者当 requestOptions.raw 为 true 时，解析为原始响应
...
| 名称 | 类型 | 描述 |
| :------ | :------ | :------ |
| `id` | `string` | 要完成上传的目标媒体的 media id。 |
| `options` | `Object` | - |
...
解析为 API 响应的 Promise，或者在 requestOptions.raw 为 true 时返回原始响应

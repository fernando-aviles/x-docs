...
• **new EventPaginator**(`fetchPage`): [`EventPaginator`](/zh/xdks/typescript/reference/classes/EventPaginator)

创建一个新的 EventPaginator 实例
...
此方法会获取下一页的数据，并将这些条目追加到当前分页器实例中。它会更新分页状态和元数据。
...
此方法会创建一个从下一页开始的新的 `Paginator` 实例，

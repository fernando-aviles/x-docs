...
TypeScript SDK 支持多种身份验证方法，适用于不同的用例。

<div id="bearer-token-app-only-auth">
  ## Bearer 令牌（仅限 App 的身份验证）
</div>
...
(async (): Promise<void> => {
  ...
  // 用户访问 authUrl 并授权应用
  // 授权完成后，用户会被重定向回来，并带有一个 code 参数
  // 从回调 URL 中提取该 code（例如，从查询参数中）
  const authCode: string = 'code-from-callback-url'; // 替换为实际 OAuth 回调中返回的 code
...
(async () => {
  ...
  // 授权完成后，用户会被重定向回来，并带有一个 code 参数
  // 从回调 URL 中提取该 code（例如，从查询参数中）
  const authCode = 'code-from-callback-url'; // 替换为实际 OAuth 回调中返回的 code
...
<Info>
  如需查看使用 JavaScript/TypeScript XDK 的更详细代码示例，请参阅我们的 [代码示例 GitHub 仓库](https://github.com/xdevplatform/samples/tree/main/javascript)。
</Info>

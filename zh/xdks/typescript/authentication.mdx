...
(async (): Promise<void> => {
  ...
  // 用户访问 authUrl 并授权应用
  // 授权后，用户会被重定向回你的应用，并带有一个 code 参数
  // 从回调 URL 中提取 code（例如，从查询参数中）
  const authCode: string = 'code-from-callback-url'; // 替换为 OAuth 回调实际返回的 code
...
(async () => {
  ...
  // 授权后，用户会被重定向回你的应用，并带有一个 code 参数
  // 从回调 URL 中提取 code（例如，从查询参数中）
  const authCode = 'code-from-callback-url'; // 替换为 OAuth 回调实际返回的 code
...
<Info>
  如需查看使用 JavaScript/TypeScript XDK 的详细代码示例，请参阅我们的 [代码示例 GitHub 仓库](https://github.com/xdevplatform/samples/tree/main/javascript)。
</Info>

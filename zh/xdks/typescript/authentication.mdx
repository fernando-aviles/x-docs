...
TypeScript SDK 支持多种身份验证方式，可用于不同的使用场景。

<div id="bearer-token-app-only-auth">
  ## Bearer Token（仅应用身份验证）
</div>
...
(async (): Promise<void> => {
  ...
  // 用户访问 authUrl 并授权应用
  // 授权后，用户会被重定向回来，并附带一个 code 参数
  // 从回调 URL 中提取该 code（例如，从查询参数中获取）
  const authCode: string = 'code-from-callback-url'; // 将此值替换为 OAuth 回调中实际返回的 code
...
(async () => {
  ...
  // 授权后，用户会被重定向回来，并附带一个 code 参数
  // 从回调 URL 中提取该 code（例如，从查询参数中获取）
  const authCode = 'code-from-callback-url'; // 将此值替换为 OAuth 回调中实际返回的 code
...
<Info>
  若要查看使用 JavaScript/TypeScript XDK 的详细示例代码，请访问我们的 [示例代码 GitHub 仓库](https://github.com/xdevplatform/samples/tree/main/javascript)。
</Info>

...
SDK 提供了通用的分页器工具，可用于任何返回分页响应的 endpoint。方法本身返回原始响应；你可以用分页器对它们进行封装。
...
for await (const user of followers) {
  ...
  console.log(typedUser.username);  // 具备完整类型信息的访问
}
```
...
if (!followers.done) {
  ...
  console.log(page2.users.length);  // 第二页中的项目数量
}
```
...
<Info>
如需查看使用 JavaScript/TypeScript XDK 的详细示例代码，请参阅我们的 [代码示例 GitHub 仓库](https://github.com/xdevplatform/samples/tree/main/javascript)。
</Info>
...
SDK 提供了通用的分页工具，可用于任何返回分页响应的端点。方法会返回原始响应，你可以再用分页器对其进行封装。
...
if (!followers.done) {
  ...
  console.log(page2.users.length);  // 第二页中的项
}
```
...
<Info>
如需查看基于 JavaScript/TypeScript XDK 的详细代码示例，请访问我们的[代码示例 GitHub 仓库](https://github.com/xdevplatform/samples/tree/main/javascript)。
</Info>
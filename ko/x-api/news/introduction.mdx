...
뉴스 조회 엔드포인트를 사용하면 개발자가 X에서 속보로 올라오는 뉴스와 헤드라인을 가져올 수 있습니다.
...
이 엔드포인트를 사용하려면 [Developer Console](https://developer.x.com/en/portal/dashboard)에서 [Bearer Token](/ko/fundamentals/authentication/oauth-2-0/application-only)을 발급받아야 합니다.

Bearer Token을 확보한 후에는 아래와 같이 뉴스 API를 호출할 수 있습니다:

...
뉴스 조회 endpoint를 사용하면 개발자는 X에서 발생하는 속보와 헤드라인을 가져올 수 있습니다.

이 endpoint는 OAuth1 및 OAuth2 PKCE에 대해 app-auth와 user-auth 방식의 인증을 모두 지원합니다.

<div id="getting-started">
  ## 시작하기
</div>

이 엔드포인트를 사용하려면 [bearer token](/ko/fundamentals/authentication/oauth-2-0/application-only)을(를) [개발자 콘솔](https://developer.x.com/en/portal/dashboard)에서 발급받아야 합니다.

Bearer Token을 발급받았다면 아래 예시와 같이 뉴스 API를 호출할 수 있습니다.

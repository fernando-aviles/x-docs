...
title: 청크 단위 미디어 업로드
sidebarTitle: 빠른 시작
description: 청크 업로드를 사용하여 동영상 및 대용량 미디어 파일을 업로드합니다.
keywords: ["청크 업로드", "미디어 업로드", "청크 미디어", "동영상 업로드", "대용량 파일 업로드", "청크 업로드 가이드"]
---

이 가이드는 청크 업로드 워크플로를 사용하여 동영상 및 대용량 미디어 파일을 업로드하는 방법을 설명합니다.
...
<Note>
  전체 Python 예제를 보려면 [이 샘플 코드](https://github.com/xdevplatform/large-video-upload-python)를 확인하세요.
</Note>
...
파일의 각 청크를 업로드합니다. 예를 들어, 3 MB 파일을 3개의 청크로 나눕니다:
...
<Info>
  ...
  * 느린 네트워크 환경에서도 신뢰성 향상
  * 업로드를 일시 중지하고 다시 재개 가능
  * 실패한 청크를 개별적으로 재시도 가능
</Info>
...
<Note>
  `processing_info`가 반환되면 처리 완료를 기다리기 위해 4단계로 진행합니다. 반환되지 않았다면 미디어는 바로 사용할 수 있습니다.
</Note>
...
`processing_info`가 포함되어 있는 경우, 처리가 완료될 때까지 상태를 주기적으로 조회합니다:
...
<CodeGroup dropdown>
  ...
  print(f"게시 완료: {response.data.id}")
  ```
  ...
  console.log(`게시 완료: ${response.data?.id}`);
  ```
</CodeGroup>

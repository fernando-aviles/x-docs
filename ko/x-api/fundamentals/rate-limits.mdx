...
title: X API 요청 한도
sidebarTitle: 요청 한도
description: X API v2 엔드포인트별 요청 한도
keywords: ["요청 한도", "API 요청 한도", "요청 제한", "요청 한도 관리(rate limiting)", "스로틀링(throttling)", "15분 시간 창"]
---

요청 한도는 각 엔드포인트에 대해 보낼 수 있는 요청 수를 제어합니다. 한도를 초과하면 시간 창이 다시 설정될 때까지 429 오류가 반환됩니다.
...
| 개념 | 설명 |
|:--------|:------------|
| **시간 창** | 일반적으로 15분 또는 24시간입니다 |
| **사용자별 제한** | OAuth 1.0a 또는 OAuth 2.0 사용자 토큰에 적용됩니다 |
| **앱별 제한** | Bearer Token(앱 전용)에 적용됩니다 |
| **엔드포인트별** | 각 엔드포인트마다 자체 제한이 있습니다 |
...
| `x-rate-limit-remaining` | 현재 시간 창에서 남은 요청 수          |
...
아래에서 각 엔드포인트별 레이트 리밋을 확인할 수 있습니다. 이 한도는 [개발자 콘솔](https://console.x.com)에서도 확인할 수 있습니다.

<Note>
  별도로 표기된 경우(예: &quot;/24hrs&quot; 또는 &quot;/sec&quot;)를 제외하면, 한도는 15분 단위입니다.
</Note>
...
| Method | Endpoint | 앱당 | 사용자당 | 비고 |
|:-------|:---------|:--------|:---------|:------|
| GET | `/2/tweets/search/all` | 초당 1회, 15분당 300회 | 초당 1회 | 기본 10개, 최대 500개 결과; 최대 쿼리 길이 1024자 |
...
| Method | Endpoint | Per App | Per User | 비고 |
|:-------|:---------|:--------|:---------|:------|
| GET | `/2/tweets/search/stream` | 50/15min | — | 연결 1개, 규칙 1000개, 규칙 길이 1024, 초당 포스트 250개 |
| GET | `/2/tweets/search/stream/rules` | 450/15min | — | 연결 1개, 규칙 1000개, 규칙 길이 1024 |
| POST | `/2/tweets/search/stream/rules` | 100/15min | — | 연결 1개, 규칙 1000개, 규칙 길이 1024 |
...
| 메서드 | 엔드포인트 | 앱별 | 사용자별 |
|:-------|:---------|:--------|:---------|
| GET | `/2/users/:id/tweets` | 10,000/15분 | 900/15분 |
| GET | `/2/users/:id/mentions` | 450/15분 | 300/15분 |
| GET | `/2/users/:id/timelines/reverse_chronological` | — | 180/15분 |
...
| Method | Endpoint | 앱당 | 사용자당 |
|:-------|:---------|:--------|:---------|
| POST | `/2/users/:id/likes` | — | 50/15분, 1,000/24시간 |
| DELETE | `/2/users/:id/likes/:tweet_id` | — | 50/15분, 1,000/24시간 |

<div id="retweets-lookup">
  #### 리트윗 조회
</div>

| Method | Endpoint | 앱 기준 | 사용자 기준 | 비고 |
|:-------|:---------|:--------|:---------|:------|
| GET | `/2/tweets/:id/retweeted_by` | 75/15분 | 75/15분 | — |
| GET | `/2/tweets/:id/quote_tweets` | 75/15분 | 75/15분 | — |
| GET | `/2/users/reposts_of_me` | — | 75/15분 | 최대 100개 결과 |
...
| 메서드 | 엔드포인트 | 앱당 | 사용자당 |
|:-------|:---------|:--------|:---------|
| POST | `/2/users/:id/bookmarks` | — | 50/15분 |
| DELETE | `/2/users/:id/bookmarks/:tweet_id` | — | 50/15분 |
...
| 메서드 | 엔드포인트 | 애플리케이션당 요청 한도 | 사용자당 요청 한도 |
|:-------|:---------|:--------|:---------|
...
| 메서드 | 엔드포인트 | 앱당 | 사용자당 |
|:-------|:---------|:--------|:---------|
| POST | `/2/media/upload` | 50,000/24시간 | 500/15분 |
| GET | `/2/media/upload` | 100,000/24시간 | 1,000/15분 |
| POST | `/2/media/upload/initialize` | 180,000/24시간 | 1,875/15분 |
| POST | `/2/media/upload/:id/append` | 180,000/24시간 | 1,875/15분 |
| POST | `/2/media/upload/:id/finalize` | 180,000/24시간 | 1,875/15분 |
| POST | `/2/media/metadata` | 50,000/24시간 | 500/15분 |
| POST | `/2/media/subtitles` | 10,000/24시간 | 100/15분 |
| DELETE | `/2/media/subtitles` | 10,000/24시간 | 100/15분 |
...
|:-------|:---------|:--------|:---------|:------|
| GET | `/2/activity/stream` | 450/15min | — | 동시 연결 2개; 초당 포스트 250개 |
...
1. 제한 창이 언제 초기화되는지 확인하기 위해 `x-rate-limit-reset` 값을 확인합니다.
2. 해당 시점까지 대기한 후 다시 시도합니다.
3. 필요하다면 지수적 백오프를 사용합니다.
...
<CardGroup cols={2}>
  ...
  <Card title="스트리밍 사용" icon="signal-stream">
    실시간 데이터의 경우 폴링 대신 필터드 스트림을 사용합니다.
  </Card>

  <Card title="헤더 모니터링" icon="gauge-high">
    남은 요청 수를 추적해 제한에 걸리는 것을 방지합니다.
  </Card>

  <Card title="요청 분산" icon="clock">
    요청을 시간 창 전반에 고르게 분산합니다.
  </Card>
</CardGroup>
...
Rate limits와 과금은 별개의 개념입니다:

| 개념 | 목적 |
|:--------|:--------|
| **Rate limits** | 시스템 안정성을 위해 요청 빈도를 제어 |
| **Usage billing** | 조회한 데이터에 대해 요금을 청구 (사용량 기반 과금) |

Rate limits 범위 내에 있더라도 사용량 요금이 발생할 수 있고, 추가 요금 없이도 Rate limits에 도달할 수 있습니다.
...
엔터프라이즈 고객에게는 맞춤형 요청 한도가 적용됩니다. 계정 담당자에게 문의하거나 [엔터프라이즈 액세스를 신청](/ko/enterprise/forms/enterprise-api-interest)하세요.

...
keywords: ["페이지네이션", "페이지네이션 토큰", "다음 토큰", "결과 페이지 순회", "커서 기반 페이지네이션"]
---

API 응답에 한 번에 반환할 수 있는 것보다 더 많은 결과가 포함되어 있을 때는, 페이지네이션을 사용하여 모든 데이터 페이지를 가져올 수 있습니다.
...
1. 최초 요청을 보낼 때 `max_results`를 설정합니다.
2. 응답의 `meta` 객체에 `next_token`이 있는지 확인합니다.
3. `next_token`이 있다면, 해당 토큰을 `pagination_token`으로 사용하여 다음 요청을 보냅니다.
4. `next_token`이 더 이상 반환되지 않을 때까지 이 과정을 반복합니다.
...
| `previous_token` | 응답의 `meta`에 포함됩니다. 이전 페이지로 돌아가는 데 사용합니다. |
...
| 매개변수 | 설명 | 기본값 |
|:----------|:------------|:--------|
| `max_results` | 페이지당 결과 수 | 엔드포인트별로 다름 |
| `pagination_token` | 이전 응답에서 받은 토큰 | 없음 |

각 엔드포인트의 API 레퍼런스에서 `max_results`에 대한 구체적인 제한을 확인하세요.
...
<CardGroup cols={2}>
  <Card title="max_results 사용" icon="arrow-up-1-9">
    API 호출을 최소화하려면 허용되는 최대 `max_results` 값을 요청하세요.
  </Card>

  <Card title="부분 페이지 처리" icon="square-check">
    마지막 페이지에는 `max_results`보다 적은 결과가 포함될 수 있습니다.
  </Card>

  <Card title="토큰 저장" icon="database">
    이후에 페이지네이션을 다시 시작해야 하는 경우를 대비해 `next_token`을 저장하세요.
  </Card>

  <Card title="페이지네이션으로 폴링하지 않기" icon="clock">
    새로운 데이터가 필요한 경우, 페이지네이션을 반복해서 수행하지 말고 `since_id`를 사용하세요.
  </Card>
</CardGroup>
...
결과는 **시간의 역순(최신순)** 으로 반환됩니다:

* 첫 페이지의 첫 번째 결과 = 가장 최근 항목
* 마지막 페이지의 마지막 결과 = 가장 오래된 항목

이 정렬 방식은 페이지 내 결과와 페이지 간 결과 모두에 적용됩니다.
...
* 페이지네이션 토큰은 불투명한 문자열입니다. 파싱하거나 수정하지 마십시오.
* 토큰은 시간이 지나면 만료될 수 있습니다.
* `max_results`보다 적은 결과를 받은 경우에도 더 많은 결과가 있을 수 있습니다. (`next_token`이 더 이상 반환되지 않을 때까지 계속 요청)
...
<CardGroup cols={2}>
  <Card title="Rate limits" icon="gauge-high" href="/ko/x-api/fundamentals/rate-limits">

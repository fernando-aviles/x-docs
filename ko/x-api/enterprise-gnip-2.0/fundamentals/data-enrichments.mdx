...
| Matching rules object | 수신된 게시물에 어떤 규칙이 매칭되었는지 나타냅니다. 이 객체는 응답 객체에서 규칙 태그와 규칙 ID를 반환합니다. |
...
* Full Archive Search API로 수행된 요청의 경우, 확장된 URL 인리치먼트 데이터는 최대 13개월 이내에 게시된 게시물에 대해서만 제공됩니다.

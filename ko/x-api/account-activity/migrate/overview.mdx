...
이 가이드는 레거시 Enterprise Account Activity API에서 v2 Account Activity API로 마이그레이션하는 데 도움이 되도록 작성되었습니다. 핵심 기능은 동일하지만, X API v2와의 일관성을 위해 엔드포인트 구조와 인증 방식이 변경되었습니다.
...
* **인증**: V2 엔드포인트에는 다음 중 하나가 필요합니다.  
  * 사용자별 작업(예: 사용자를 구독자로 등록)에 **OAuthUser**(3-legged OAuth)를 사용합니다.  
  * 앱 수준 작업(예: 구독 목록 조회/삭제)에 **OAuth2 App Only**(Bearer Token)를 사용합니다.  
  * V1.1에서는 다양한 인증 요구 사항을 사용했으며, 종종 OAuth 1.0a가 필요했습니다.  
* **엔드포인트 매핑**:
...
| PUT /1.1/account\_activity/webhooks/:webhook\_id.json | PUT /2/webhooks/:webhook\_id | CRC를 트리거합니다. [Docs](https://docs.x.com/x-api/webhooks/introduction). OAuth2AppOnly 필요. |
...
| DELETE /1.1/account\_activity/webhooks/:webhook\_id/subscriptions/all | 사용 중단 / 직접적인 대응 없음 | 개별 삭제를 사용하거나 DELETE /2/webhooks/:webhook\_id로 webhook을 제거하십시오. |

* **응답 형식**: AAAPI 문서에서 성공 및 오류 응답 구조를 검토하십시오.

마이그레이션할 때 새 기본 경로를 사용하도록 코드를 업데이트하고, 인증 플로를 조정하며, [V2 Webhooks API](https://docs.x.com/x-api/webhooks/introduction)를 사용해 webhook을 관리하십시오.
...
* **패키지 결정**:  
  * Account Activity API 액세스를 위해 Enterprise 패키지 또는 Self-serve 패키지를 선택할 때 다음을 고려하세요:  
    * 필요한 웹훅 개수  
    * 현재/예상 구독 수 또는 승인된 사용자 수  
    * X 클라이언트 애플리케이션 수  
    * 원하는 지원 수준(포럼 지원 vs. 관리형 엔터프라이즈 1:1 지원)  
    * 가격 세부 정보(Enterprise의 경우 영업팀에 문의)  
* **인증을 OAuth 2.0으로 업데이트**:  
  * X App에 "Read, Write, and Access Direct Messages" 권한이 있는지 확인하세요.  
  * V2 엔드포인트에 대해 OAuth 2.0을 사용하도록 업데이트하세요. 스코프가 변경된 경우 사용자 승인을 다시 받으세요.  
  * 개발자 콘솔을 사용해 다음을 관리하세요:  
    * OAuth 2.0 Access Token  
    * Client ID 및 Client Secret  
* **웹훅 확인 또는 재구성**:  
  * 기존 웹훅 URL을 사용하거나 새 URL을 설정하세요(예: https://your\_domain.com/webhook/twitter).  
  * [V2 Webhooks API documentation](https://docs.x.com/x-api/webhooks/introduction)을 참고하여 웹훅을 등록하고 관리하세요.  
* **샘플 앱:**  
  * [Simple webhook server](https://github.com/m-rosinsky/XWebhookTest/blob/main/app.py)  
    * CRC 검사에 응답하고 POST 이벤트를 수락하는 방법을 보여주는 단일 Python 스크립트입니다.  
  * [Account Activity API sample dashboard](https://github.com/xdevplatform/account-activity-dashboard-enterprise/tree/master)  
    * 웹훅과 구독을 관리하고, 앱 내에서 직접 실시간 이벤트를 수신할 수 있도록 [bun.sh](http://bun.sh)로 작성된 웹 앱입니다.
...
title: 빠른 시작
sidebarTitle: 빠른 시작
description: API를 사용해 커뮤니티 노트를 생성하고 검색하기
keywords: ["커뮤니티 노트 빠른 시작", "커뮤니티 노트 튜토리얼", "노트 생성 가이드"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 Community Notes API를 사용하여 노트를 남길 수 있는 포스트를 검색하고 노트를 제출하는 방법을 단계별로 안내합니다.
...
<Warning>
  현재 모든 요청에 대해 `test_mode`를 `true`로 설정해야 합니다. 테스트 노트는 공개적으로 표시되지 않습니다.
</Warning>
...
<Steps>
  <Step title="노트 작성 대상 포스트 검색" icon="magnifying-glass">
    <Tabs>
      <Tab title="cURL">
    ```bash
  ...
  <Step title="노트 작성 대상 포스트 검토" icon="eye">
    ```json
    {
      "data": [
        {
    ...
    커뮤니티 노트를 작성하기 위해 응답에 포함된 포스트 `id`를 사용하세요.
  </Step>
</Steps>
...
<Steps>
  <Step title="노트 준비" icon="pen">
    ...
    - `post_id` — 맥락을 추가하려는 대상 포스트
    - `text` — 노트 내용 (1–280자, 소스 URL을 포함해야 함)
    - `classification` — `misinformed_or_potentially_misleading` 또는 `not_misleading` 중 하나
    - `misleading_tags` — `classification`이 `misinformed_or_potentially_misleading`인 경우 필수
    - `trustworthy_sources` — 소스가 신뢰할 수 있는지 여부를 나타내는 Boolean 값
  </Step>
...
<AccordionGroup>
  <Accordion title="오해를 유발하는 태그">
    분류가 `misinformed_or_potentially_misleading`인 경우 하나 이상의 태그를 포함해야 합니다:
    ...
    | `misinterpreted_satire` | 풍자가 문맥에서 벗어나 오해되었음 |
    | `missing_important_context` | 중요한 맥락이 부족함 |
    | `outdated_information` | 더 이상 최신 정보가 아님 |
    | `other` | 기타 사유 |
  </Accordion>

  <Accordion title="오해를 유발하지 않음">
    분류가 `not_misleading`인 경우 오해를 유발하는 태그는 필수가 아닙니다.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="401 인증 실패(Unauthorized)">
    ```json
    {"title": "Unauthorized", "status": 401, "detail": "Unauthorized"}
    ```
    **해결 방법:** OAuth 자격 증명 정보가 올바른지 확인하세요.
  </Accordion>

  <Accordion title="403 권한 없음(Forbidden)">
    ```json
...
<CardGroup cols={2}>
  <Card title="커뮤니티 노트 가이드" icon="book" href="https://communitynotes.x.com/guide/en/api/overview">
    Community Notes 공식 문서
  </Card>

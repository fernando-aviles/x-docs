...
keywords: ["사용자 조회 빠른 시작", "사용자 가져오기 튜토리얼", "ID로 사용자 조회 가이드", "사용자 이름으로 사용자 조회 가이드"]
---
...
<CodeGroup dropdown>
  ...
  print(f"사용자 이름: {response.data.username}")
  print(f"팔로워 수: {response.data.public_metrics.followers_count}")
  ```
  ...
  console.log(`팔로워 수: ${response.data?.public_metrics?.followers_count}`);
  ```
</CodeGroup>
...
보호된 사용자의 data는 여전히 반환되지만, 해당 사용자를 팔로우하지 않으면 그 사용자의 포스트에는 접근할 수 없습니다.
...
<CardGroup cols={2}>
  ...
  <Card title="API 레퍼런스" icon="code" href="/ko/x-api/users/single-user-lookup">
    전체 엔드포인트 문서
  </Card>
</CardGroup>
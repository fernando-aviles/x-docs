...
keywords: ["인증된 사용자", "me 엔드포인트", "현재 사용자", "인증 사용자 조회"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 `/me` 엔드포인트를 사용해 현재 인증된 사용자의 프로필을 조회하는 방법을 단계별로 설명합니다.
...
User Access Token을 사용해 `/me` 엔드포인트로 요청을 보내세요:
...
<CodeGroup dropdown>
  ...
  print(f"Username: {response.data.username}")
  # 고정된 포스트는 response.includes.tweets 안에 있습니다
  ```
...
<Warning>
  `/me` 엔드포인트에서는 User Context 인증이 필요합니다. App-Only(Bearer Token) 인증은 지원되지 않습니다.
</Warning>

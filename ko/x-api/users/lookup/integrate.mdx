...
title: 통합 가이드
sidebarTitle: 통합 가이드
description: User lookup 통합을 위한 핵심 개념과 모범 사례
keywords: ["사용자 조회 통합", "사용자 조회 통합 가이드", "사용자 조회 구현"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 애플리케이션에 User lookup 엔드포인트를 통합하는 데 알아야 할 핵심 개념을 다룹니다.
...
모든 X API v2 엔드포인트는 인증이 필요합니다. 사용 사례에 가장 적합한 방식을 선택하세요:

| 방식 | 최적의 용도 | 비공개 메트릭 접근 가능 여부 |
|:-------|:---------|:---------------------------|
| [OAuth 2.0 App-Only](/ko/resources/fundamentals/authentication#oauth-2-0) | 서버 간 통신, 공개 데이터 | 아니요 |
| [OAuth 2.0 Authorization Code with PKCE](/ko/resources/fundamentals/authentication#oauth-2-0-authorization-code-flow-with-pkce-2) | 사용자 대상 앱 | 예 (권한이 부여된 사용자의 데이터) |
| [OAuth 1.0a User Context](/ko/resources/fundamentals/authentication) | 레거시 통합 | 예 (권한이 부여된 사용자의 데이터) |

<div id="app-only-authentication">
  ### 앱 전용 인증
</div>

공개 사용자 데이터의 경우 Bearer Token을 사용합니다:
...
인증된 사용자용 엔드포인트(`/2/users/me`)에 User Context 인증이 필요합니다:
...
<Accordion title="user.fields">
  ...
  | `created_at` | 계정 생성 시간 |
  | `description` | 사용자 소개 |
  | `entities` | 소개에 포함된 URL 파싱 정보 |
  | `location` | 사용자가 지정한 위치 |
  | `pinned_tweet_id` | 고정 포스트 ID |
  | `profile_image_url` | 프로필 이미지(URL) |
  | `protected` | 계정 보호 설정 여부 |
...
<Accordion title="tweet.fields (requires pinned_tweet_id expansion)">
  | 필드 | 설명 |
  |:------|:------------|
  | `created_at` | 포스트 생성 시간 |
  | `text` | 포스트 내용 |
  | `public_metrics` | 참여(engagement) 지표 |
...
<CodeGroup dropdown>
  ...
  print(response.data)
  print(response.includes)  # 확장된 고정 트윗이 포함됨
  ```
  ...
  console.log(response.data);
  console.log(response.includes); // 확장된 고정 트윗이 포함됨
  ```
</CodeGroup>
...
<Tip>
  일괄 요청은 최대 100명의 사용자로 제한됩니다. 더 큰 데이터 세트를 처리하려면 여러 요청을 사용하세요.
</Tip>
...
| 401 | 인증되지 않음 | 인증 자격 증명을 확인하세요 |
| 403 | 접근이 거부됨 | 앱 권한을 확인하세요 |
| 404 | 찾을 수 없음 | 사용자가 존재하지 않거나 계정이 정지되었습니다 |
| 429 | 요청 횟수 초과 | 잠시 기다린 후 다시 시도하세요 (요청 한도(rate limit)를 참조하세요) |
...
* 다중 사용자 조회에서는 결과에서 해당 사용자가 제외되고, 응답에는 `errors` 배열이 포함됩니다.
...
* 기본 정보(id, 이름, username)에는 접근할 수 있습니다
* 비공개 콘텐츠(고정 포스트)는 표시가 제한될 수 있습니다
* `protected: true`는 계정이 비공개 상태임을 나타냅니다
...
<CardGroup cols={2}>
  <Card title="요청 배치 처리" icon="layer-group">
  ...
  <Card title="사용자 데이터 캐시" icon="database">

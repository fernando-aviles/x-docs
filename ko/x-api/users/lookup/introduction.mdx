...
title: 사용자 조회
sidebarTitle: 소개
description: ID, 사용자 이름 또는 인증된 사용자를 기준으로 사용자 프로필을 조회합니다
keywords: ["사용자 조회", "사용자 가져오기", "ID별 사용자", "사용자 이름별 사용자", "사용자 프로필", "사용자 정보", "인증된 사용자", "사용자 상세 정보"]
---

import { Button } from '/snippets/ko/button.mdx';

User 조회 엔드포인트를 사용하면 하나 이상의 사용자의 프로필 정보를 조회할 수 있습니다. 사용자를 id나 사용자 이름으로 조회하거나, 현재 인증된 사용자의 프로필 정보를 가져올 수 있습니다.
...
<CardGroup cols={2}>
  ...
  <Card title="여러 사용자 조회" icon="users">
    요청당 최대 100명의 사용자를 조회합니다.
  </Card>

  <Card title="인증된 사용자 조회" icon="user-check">
    현재 사용자에 대한 자세한 정보를 가져옵니다.
  </Card>
</CardGroup>
...
| GET | [`/2/users`](/ko/x-api/users/get-users-by-ids) | ID로 여러 사용자 조회 (최대 100명) |
| GET | [`/2/users/by/username/:username`](/ko/x-api/users/get-user-by-username) | username으로 사용자 조회 |
| GET | [`/2/users/by`](/ko/x-api/users/get-users-by-usernames) | username으로 여러 사용자 조회 (최대 100명) |

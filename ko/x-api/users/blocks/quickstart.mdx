...
title: 퀵스타트
sidebarTitle: 퀵스타트
description: 사용자를 차단하고 차단 해제하며, 차단 목록을 조회합니다
keywords: ["차단 퀵스타트", "사용자 차단 퀵스타트", "차단 튜토리얼", "차단 가이드"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 사용자를 차단하고 차단을 해제하며, 차단한 사용자 목록을 조회하는 방법을 설명합니다.
...
<Steps>
  <Step title="내 사용자 ID 가져오기">
    차단 목록을 가져오려면 인증된 사용자 ID가 필요합니다. `/2/users/me` 엔드포인트에서 가져오거나 토큰에 포함된 ID를 사용할 수 있습니다.
  </Step>

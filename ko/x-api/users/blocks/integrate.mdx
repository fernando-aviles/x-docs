...
title: 통합 가이드
sidebarTitle: 통합 가이드
description: Blocks 엔드포인트를 통합하기 위한 핵심 개념과 모범 사례
keywords: ["차단 통합", "사용자 차단 통합", "차단 가이드"]
---
...
|:-------|:---------|:------------|
| GET | `/2/users/:id/blocking` | 차단된 계정 목록을 가져옵니다 |
...
<CardGroup cols={2}>
  <Card title="상대방이 할 수 없는 것" icon="xmark">
    * 사용자의 포스트를 볼 수 없습니다(로그아웃 상태인 경우 제외)
    * 사용자를 팔로우할 수 없습니다
    * 사용자에게 다이렉트 메시지(DM)를 보낼 수 없습니다
    * 사용자를 리스트에 추가할 수 없습니다
    * 사진에서 사용자를 태그할 수 없습니다
  </Card>

  <Card title="사용자가 할 수 없는 것" icon="xmark">
    * 상대방의 포스트를 볼 수 없습니다
    * 상대방을 팔로우할 수 없습니다
    * 상대방에게 다이렉트 메시지(DM)를 보낼 수 없습니다
  </Card>
</CardGroup>

<Note>
  어떤 계정이 사용자를 팔로우하고 있을 때 그 계정을 차단하면, 해당 계정은 자동으로 사용자를 언팔로우합니다.
</Note>
...
차단 목록이 많은 사용자의 경우, 결과는 페이지 단위로 제공됩니다:
...
| 403 | 접근 권한 없음 | 스코프 및 권한을 확인하세요 |
| 404 | 찾을 수 없음 | 해당 사용자가 존재하는지 확인하세요 |

---
title: 뮤트한 사용자 조회
sidebarTitle: 뮤트한 사용자 조회
description: 뮤트한 사용자 목록 가져오기
keywords: ["뮤트한 사용자 조회 빠른 시작", "뮤트 사용자 조회 빠른 시작", "뮤트한 사용자 조회 튜토리얼", "뮤트한 사용자 조회 가이드", "뮤트 빠른 시작"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 X API를 사용하여 자신이 뮤트한 사용자 목록을 가져오는 방법을 안내합니다.
...
<Steps>
  <Step title="사용자 ID 가져오기">
    인증된 사용자 계정의 ID가 필요합니다. [user lookup endpoint](/ko/x-api/users/lookup/introduction)를 사용하거나 액세스 토큰에서 확인할 수 있습니다(숫자 부분이 사용자 ID입니다).
  </Step>

  <Step title="뮤트한 사용자 조회하기">
    <CodeGroup dropdown>
      ```bash cURL
...
`expansions`를 사용해 고정된 포스트(pinned Posts)와 같은 관련 데이터를 가져올 수 있습니다:
...
<CodeGroup dropdown>
  ...
  # expansions를 사용해 음소거된 사용자 목록 가져오기
  for page in client.users.get_muting(
      "123456789",
...
SDK는 페이지네이션을 자동으로 처리합니다. cURL을 사용하는 경우 응답으로 반환된 `next_token`을 사용하세요:

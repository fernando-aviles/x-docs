---
title: 활동 스트림
sidebarTitle: 소개
description: 실시간 계정 활동 이벤트를 스트리밍합니다
keywords: ["활동 스트림", "실시간 활동", "계정 활동", "활동 API", "이벤트 스트림"]
---

import { Button } from '/snippets/ko/button.mdx';

Activity Stream 엔드포인트는 구독 중인 사용자의 계정 활동 이벤트를 실시간으로 제공합니다. 사용자가 포스트를 작성하거나, 좋아요를 누르거나, 팔로우하거나, DM을 받는 등의 이벤트를 수신할 수 있습니다.

<div id="overview">
  ## 개요
</div>

<CardGroup cols={2}>
  <Card title="실시간" icon="bolt">
    이벤트가 즉시 전달됩니다
  </Card>

  <Card title="종합적" icon="list">
    포스트, 좋아요, 팔로우, DM 등
  </Card>

  <Card title="구독 기반" icon="bell">
    사용자 활동을 구독하세요
  </Card>

  <Card title="웹훅 전송" icon="webhook">
    이벤트가 서버로 전달됩니다
  </Card>
</CardGroup>

***

<div id="event-types">
  ## 이벤트 유형
</div>

| Event | 설명 |
|:------|:------------|
| `tweet_create_events` | 사용자가 새 포스트를 작성함 |
| `favorite_events` | 사용자가 포스트에 좋아요를 표시함 |
| `follow_events` | 사용자가 다른 사용자를 팔로우하거나 팔로우됨 |
| `direct_message_events` | 사용자가 DM을 보내거나 받음 |
| `block_events` | 사용자가 다른 사용자를 차단하거나 차단 해제함 |
| `mute_events` | 사용자가 다른 사용자를 뮤트하거나 뮤트 해제함 |

***

<div id="endpoints">
  ## 엔드포인트
</div>

| Method | Endpoint | Description |
|:-------|:---------|:------------|
| GET | [`/2/activity/stream`](/ko/x-api/activity/activity-stream) | 액티비티 스트림 연결 |
| POST | [`/2/activity/subscriptions`](/ko/x-api/activity/create-x-activity-subscription) | 구독 생성 |
| GET | [`/2/activity/subscriptions`](/ko/x-api/activity/get-x-activity-subscriptions) | 구독 목록 조회 |
| PUT | [`/2/activity/subscriptions/:id`](/ko/x-api/activity/update-x-activity-subscription) | 구독 수정 |
| DELETE | [`/2/activity/subscriptions/:id`](/ko/x-api/activity/deletes-x-activity-subscription) | 구독 삭제 |

***

<div id="how-it-works">
  ## 동작 방식
</div>

1. **구독 생성** — 사용자 활동에 대한 구독을 생성합니다
2. **스트림에 연결** — 지속 연결을 설정합니다
3. **이벤트 수신** — 실시간 활동 이벤트를 수신합니다
4. **이벤트 처리** — 애플리케이션에서 이벤트를 처리합니다

***

<div id="example-connect-to-stream">
  ## 예제: 스트림에 연결하기
</div>

```python
import requests

def stream_activity(bearer_token):
    url = "https://api.x.com/2/activity/stream"
    headers = {"Authorization": f"Bearer {bearer_token}"}
    
    response = requests.get(url, headers=headers, stream=True)
    
    for line in response.iter_lines():
        if line:
            print(line.decode("utf-8"))
```

***

<div id="getting-started">
  ## 시작하기
</div>

<Note>
  **사전 준비 사항**

  * 승인된 [개발자 계정](https://developer.x.com/en/portal/petition/essential/basic-info)
  * Developer Console의 [Project 및 App](/ko/resources/fundamentals/developer-apps)
  * 앱의 [Bearer Token](/ko/resources/fundamentals/authentication)
</Note>

<CardGroup cols={2}>
  <Card title="퀵스타트" icon="rocket" href="/ko/x-api/activity/quickstart">
    첫 구독 설정
  </Card>

  <Card title="Account Activity API" icon="bell" href="/ko/x-api/account-activity/introduction">
    Webhook 기반 대체 수단
  </Card>

  <Card title="API 레퍼런스" icon="code" href="/ko/x-api/activity/activity-stream">
    전체 엔드포인트 문서
  </Card>
</CardGroup>
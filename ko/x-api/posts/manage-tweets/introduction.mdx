---
title: 포스트 관리
sidebarTitle: 소개
description: 인증된 사용자를 대신해 포스트를 생성하고 삭제합니다
keywords: ["트윗 관리", "트윗 생성", "트윗 게시", "트윗 삭제", "포스트 관리", "트윗 API"]
---

import { Button } from '/snippets/ko/button.mdx';

Manage Posts 엔드포인트를 사용하면 인증된 사용자를 대신해 포스트를 생성하고 삭제할 수 있습니다. 콘텐츠를 게시하고 스레드를 생성하거나 사용자 포스트를 관리하는 애플리케이션을 구축할 수 있습니다.

<div id="overview">
  ## 개요
</div>

<CardGroup cols={2}>
  <Card title="포스트 작성" icon="pen">
    새 포스트를 작성합니다
  </Card>

  <Card title="포스트 삭제" icon="trash">
    기존 포스트를 삭제합니다
  </Card>

  <Card title="답글" icon="reply">
    다른 포스트에 답글을 답니다
  </Card>

  <Card title="인용" icon="quote-right">
    다른 포스트를 인용합니다
  </Card>
</CardGroup>

***

<div id="endpoints">
  ## 엔드포인트
</div>

| Method | Endpoint | 설명 |
|:-------|:---------|:------------|
| POST | [`/2/tweets`](/ko/x-api/posts/create-post) | 새 포스트 생성 |
| DELETE | [`/2/tweets/:id`](/ko/x-api/posts/delete-post) | 포스트 삭제 |

***

<div id="creating-posts">
  ## 포스트 생성
</div>

<div id="basic-post">
  ### 기본 포스트
</div>

```bash
curl -X POST "https://api.x.com/2/tweets" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"text": "Hello from the API!"}'
```

<div id="reply-to-a-post">
  ### 포스트에 답글하기
</div>

```bash
curl -X POST "https://api.x.com/2/tweets" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "This is a reply!",
    "reply": {
      "in_reply_to_tweet_id": "1234567890"
    }
  }'
```

<div id="quote-a-post">
  ### 포스트 인용하기
</div>

```bash
curl -X POST "https://api.x.com/2/tweets" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "Check this out!",
    "quote_tweet_id": "1234567890"
  }'
```

<div id="post-with-media">
  ### 미디어가 포함된 포스트
</div>

```bash
curl -X POST "https://api.x.com/2/tweets" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "Photo of the day",
    "media": {
      "media_ids": ["1234567890123456789"]
    }
  }'
```

<Note>
  먼저 [Media Upload 엔드포인트](/ko/x-api/media/quickstart/media-upload-chunked)를 사용해 미디어를 업로드한 다음, 포스트에서 `media_id`를 참조하세요.
</Note>

<div id="post-with-poll">
  ### 투표가 있는 포스트
</div>

```bash
curl -X POST "https://api.x.com/2/tweets" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{
    "text": "What is your favorite color?",
    "poll": {
      "options": ["Red", "Blue", "Green", "Yellow"],
      "duration_minutes": 1440
    }
  }'
```

***

<div id="deleting-posts">
  ## 포스트 삭제하기
</div>

```bash
curl -X DELETE "https://api.x.com/2/tweets/1234567890" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN"
```

<Warning>
  인증된 사용자 본인이 작성한 포스트만 삭제할 수 있습니다.
</Warning>

***

<div id="getting-started">
  ## 시작하기
</div>

<Note>
  **사전 준비 사항**

  * 승인된 [개발자 계정](https://developer.x.com/en/portal/petition/essential/basic-info)
  * 개발자 콘솔의 [Project 및 앱](/ko/resources/fundamentals/developer-apps)
  * [OAuth 2.0 PKCE](/ko/resources/fundamentals/authentication#oauth-2-0-authorization-code-flow-with-pkce-2) 또는 [3-legged OAuth](/ko/resources/fundamentals/authentication#obtaining-access-tokens-using-3-legged-oauth-flow)를 통해 발급된 사용자 액세스 토큰
</Note>

<CardGroup cols={2}>
  <Card title="빠른 시작" icon="rocket" href="/ko/x-api/posts/manage-tweets/quickstart">
    첫 포스트를 만들어 보세요
  </Card>

  <Card title="통합 가이드" icon="book" href="/ko/x-api/posts/manage-tweets/integrate">
    핵심 개념과 모범 사례
  </Card>

  <Card title="미디어 업로드" icon="image" href="/ko/x-api/media/quickstart/media-upload-chunked">
    포스트에 사용할 미디어 업로드
  </Card>

  <Card title="API 레퍼런스" icon="code" href="/ko/x-api/posts/creation-of-a-post">
    모든 엔드포인트에 대한 문서
  </Card>
</CardGroup>
...
이러한 스트림은 모두 대량의 데이터를 전달하기 때문에 &quot;volume streams&quot;라고 부릅니다. 1% 스트림만으로도 매초 수십 개의 게시물을 전송할 수 있습니다. 이러한 스트림을 사용하면 트렌드를 식별·추적하고, 전반적인 감성을 모니터링하며, 전 세계적 이벤트를 모니터링하는 등 다양한 작업을 수행할 수 있습니다. 
...
이 volume stream 엔드포인트는 편집된 게시물을 지원합니다. 이 엔드포인트는 게시물의 편집 내역과 함께, 게시물 ID 배열을 포함하여 편집된 게시물을 전달합니다. 편집 내역이 없는 게시물의 경우 이 배열에는 단일 ID만 포함됩니다. 편집된 게시물의 경우 이 배열에는 여러 개의 ID가 포함되며, 편집 순서를 반영하여 오름차순으로 정렬되고, 배열의 마지막 위치에 가장 최신 버전이 있습니다. 게시물 편집이 어떻게 동작하는지 자세히 알아보려면 [Post edits fundamentals](/ko/x-api/fundamentals/edit-posts) 페이지를 참조하세요. 

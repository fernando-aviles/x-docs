...
<Steps>
  <Step title="쿼리 작성" icon="magnifying-glass">
    검색 쿼리에서는 연산자를 사용해 게시물을 검색합니다. 먼저 간단한 키워드부터 입력해 보세요:
    ...
    이는 영어로 작성된 게시물 중에서 &quot;python&quot;을 포함하되 리트윗은 제외합니다.
...
<AccordionGroup>
  ...
  <Accordion title="해시태그가 포함된 게시물">
    ```
    #Python -is:retweet
    ```
  </Accordion>

  <Accordion title="이미지가 포함된 게시물">
    ```
    "machine learning" has:images lang:en
    ```
  </Accordion>

  <Accordion title="사용자를 언급한 게시물">
    ```
    @elonmusk -is:retweet -is:reply
    ```
  </Accordion>

  <Accordion title="특정 도메인 링크가 포함된 게시물">
    ```

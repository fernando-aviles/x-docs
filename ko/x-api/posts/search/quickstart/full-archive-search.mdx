...
description: 2006년까지 거슬러 올라가는 전체 포스트 아카이브를 검색합니다
keywords: ["전체 아카이브 검색", "아카이브 검색 빠른 시작", "과거 데이터 검색", "전체 아카이브 튜토리얼", "아카이브 검색 가이드"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 2006년 3월까지 거슬러 올라가는 X 전체 아카이브에서 포스트를 찾기 위해, 첫 번째 전체 아카이브 검색 요청을 수행하는 방법을 단계별로 설명합니다.

<Warning>
  전체 아카이브 검색을 사용하려면 [Self-serve](/ko/x-api/getting-started/about-x-api) 또는 [Enterprise](/ko/x-api/getting-started/about-x-api) 액세스 권한이 필요합니다. 이 엔드포인트를 사용하려면 [액세스를 업그레이드](https://developer.x.com/en/portal/products)하세요.
</Warning>
...
<Note>
  ...
  * Developer Console의 &quot;Keys and tokens&quot;에서 확인할 수 있는 앱의 Bearer Token
</Note>
...
전체 보관 검색에서는 모든 쿼리 연산자를 사용할 수 있습니다. 최근 검색과 마찬가지 방식으로 쿼리를 작성하세요:
...
<Tip>
  전체 아카이브 검색에서는 최대 1,024자(엔터프라이즈의 경우 4,096자)까지 쿼리를 지원합니다.
</Tip>
...
기본적으로 결과에는 최근 30일 이내의 포스트가 반환됩니다. `start_time`과 `end_time`을 사용하여 특정 기간을 검색합니다:
...
<Note>
  2022년 9월에 편집 기능이 도입되기 전에 만든 포스트에는 `edit_history_tweet_ids`가 포함되어 있지 않습니다.
</Note>
...
SDK에서는 페이지네이션을 자동으로 처리합니다. cURL을 사용하는 경우, 응답에 포함된 `next_token`을 사용하세요:
...
| 접근 권한 | 모든 개발자 | 사용량 기반 결제, Enterprise |
| 요청당 최대 결과 수 | 100 | 500 |
| 쿼리 길이 | 512자 | 1,024자 |
| 요청 한도 | 15분당 450 | 15분당 300, 초당 1회 |
| 인증 | App-Only, User Context | App-Only만 |

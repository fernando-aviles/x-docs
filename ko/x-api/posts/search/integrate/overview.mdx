...
쿼리는 연산자를 사용해 게시물을 일치시킵니다. 불리언 논리로 연산자를 조합하세요:
...
| `referenced_tweets.id` | 인용 또는 답글 대상 게시물 |
| `geo.place_id` | 장소 객체 |
...
<CodeGroup dropdown>
  ...
  print(f"총 {len(all_posts)}개의 게시물을 찾았습니다")
  ```
...
게시물은 30분 이내에 최대 5번까지 수정할 수 있습니다. 검색 엔드포인트는 항상 최신 버전을 반환합니다.
...
* 30분 제한 시간이 지난 후에 가져온 게시물은 최종 버전으로 간주됩니다
* 실시간에 가까운 사용 사례에서는 최근에 게시된 게시물이 여전히 수정될 수 있습니다

...
title: 통합 가이드
sidebarTitle: 개요
description: 포스트 검색 통합을 위한 핵심 개념 및 모범 사례
keywords: ["검색 연동", "검색 개요", "검색 가이드", "검색 구현", "트윗 검색 연동"]
---

이 가이드는 애플리케이션에 포스트 검색 엔드포인트를 통합하는 데 필요한 핵심 개념을 다룹니다.
...
| 인증 방식 | 사용 사례 |
|:-------|:---------|
| [OAuth 2.0 App-Only](/ko/resources/fundamentals/authentication#oauth-2-0) | 공개 포스트 data |
...
전체 보관 검색은 [OAuth 2.0 App-Only](/ko/resources/fundamentals/authentication#oauth-2-0) 인증만 지원합니다.

<Warning>
  전체 보관 검색은 App-Only 인증만 지원하므로, 비공개 지표(`non_public_metrics`, `organic_metrics`, `promoted_metrics`)는 제공되지 않습니다.
</Warning>
...
쿼리는 연산자를 사용해 일치하는 포스트를 찾습니다. 연산자를 불리언 논리와 결합해 사용하세요:
...
| 유형 | 설명 | 예시 |
|:-----|:------------|:--------|
| **단독형** | 단독으로 사용할 수 있음 | `#python`, `from:user` |
| **결합 필수형** | 단독 연산자와 함께 사용해야 함 | `has:media`, `is:retweet` |
...
기본적으로 응답에는 `id`, `text`, `edit_history_tweet_ids`만 포함됩니다. 추가 데이터를 요청하려면 요청 매개변수를 사용하세요.
...
| `referenced_tweets.id` | 인용되었거나 답글이 달린 포스트 |
| `geo.place_id` | 장소 객체 |
...
검색 엔드포인트는 결과를 페이지 단위로 반환합니다. 추가 페이지를 가져오려면 응답에 포함된 `next_token` 값을 사용하세요.
...
포스트는 30분 이내에 최대 다섯 번까지 편집할 수 있습니다. 검색 엔드포인트는 항상 최신 버전을 반환합니다.
...
* 30분 편집 가능 시간이 지난 후에 가져온 포스트는 최종 버전을 나타냅니다.
* 거의 실시간에 가까운 사용 사례의 경우, 최근에 게시된 포스트가 여전히 편집 중일 수 있습니다.

<Card title="포스트 편집 기본 개념" icon="clock-rotate-left" href="/ko/x-api/fundamentals/edit-posts">
  포스트 편집에 대해 자세히 알아보세요
</Card>
...
<CardGroup cols={2}>
  <Card title="구체적으로 시작" icon="crosshairs">
    여러 연산자를 사용해 결과 범위를 좁히고 노이즈를 줄이세요.
  </Card>

  <Card title="반복적으로 테스트" icon="flask">
    처음에는 넓게 시작한 뒤 결과를 바탕으로 점진적으로 다듬으세요.
  </Card>
  ...
  <Card title="결과 캐시" icon="database">
    결과를 로컬에 저장해 반복 요청을 피하세요.
  </Card>
</CardGroup>

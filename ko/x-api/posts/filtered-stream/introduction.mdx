...
Filtered Stream 엔드포인트를 사용하면 필터 규칙과 일치하는 실시간 게시물을 받아볼 수 있습니다. 강력한 연산자를 사용해 규칙을 만든 다음, 지속적인 스트림에 연결하여 게시될 때마다 일치하는 게시물을 받아 보세요.
...
3. **게시물 수신** — 일치하는 게시물을 실시간으로 수신합니다
...
| `from:elonmusk` | @elonmusk가 작성한 게시물 |
| `"breaking news" has:images` | 해당 문구와 이미지를 모두 포함한 게시물 |
| `(@XDevelopers OR @X) -is:retweet` | 리트윗을 제외한 멘션 게시물 |
...
게시물을 수신하기 위해 지속적인 HTTP 연결을 설정합니다.
...
이 스트림은 편집된 게시물과 해당 편집 내역을 제공합니다. 각 편집마다 새로운 게시물 ID가 생성됩니다:
...
<Card title="게시물 편집 기본사항" icon="clock-rotate-left" href="/ko/x-api/fundamentals/edit-posts">
  게시물 편집에 대해 자세히 알아보기
</Card>
...
<CardGroup cols={2}>
  ...
  <Card title="반환된 게시물 매칭" icon="crosshairs" href="/ko/x-api/posts/filtered-stream/integrate/matching-returned-tweets">
    어떤 규칙이 일치했는지 식별하기
  </Card>
</CardGroup>
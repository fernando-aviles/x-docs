---
title: 최근 게시물 개수
sidebarTitle: 최근 게시물 개수
description: 지난 7일 동안의 게시물 수 가져오기
keywords: ["최근 Tweet 개수 빠른 시작", "최근 개수 빠른 시작", "Tweet 볼륨 빠른 시작", "개수 빠른 시작", "최근 검색 개수"]
---
...
<Steps>
  <Step title="쿼리 작성">
    최근 검색과 동일한 쿼리 구문을 사용합니다. 예를 들어 @XDevelopers 계정의 게시물 개수를 집계하려면 다음 쿼리를 사용할 수 있습니다:
  ...
  <Step title="요청 보내기">
    <CodeGroup dropdown>
      ...
      for bucket in response.data:
          print(f"{bucket.start}: {bucket.tweet_count} 게시물")

      print(f"Total: {response.meta.total_tweet_count} 게시물")
      ```
      ...
      response.data?.forEach((bucket) => {
        console.log(`${bucket.start}: ${bucket.tweet_count} 게시물`);
      });

      console.log(`Total: ${response.meta?.total_tweet_count} 게시물`);
      ```
    </CodeGroup>
  </Step>

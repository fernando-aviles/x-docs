...
description: 2006년까지의 과거 포스트 수를 가져옵니다
keywords: ["전체 아카이브 트윗 집계 빠른 시작", "아카이브 집계 빠른 시작", "과거 집계 빠른 시작", "전체 아카이브 집계"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 2006년 3월까지의 과거 포스트 개수를 조회하는 방법을 설명합니다.

<Warning>
  전체 아카이브 포스트 개수를 조회하려면 [Self-serve](/ko/x-api/getting-started/about-x-api) 또는 [Enterprise](/ko/x-api/getting-started/about-x-api) 액세스 권한이 필요합니다.
</Warning>
...
<Steps>
  <Step title="쿼리 작성">
    전체 아카이브 검색과 동일한 쿼리 구문을 사용합니다. 예를 들어 @XDevelopers 계정에서 작성한 포스트 수를 세려면 다음과 같이 입력합니다:
...
조회 기간이 길 경우 응답에 포함된 `next_token`을 사용하세요:
...
| `start_time` | 가장 오래된 타임스탬프 (ISO 8601 형식) | 30일 전 |
| `end_time` | 가장 최신 타임스탬프 (ISO 8601 형식) | 현재 |
...
<CardGroup cols={2}>
  ...
  <Card title="쿼리 빌드" icon="magnifying-glass" href="/ko/x-api/posts/counts/integrate/build-a-query">
    쿼리 문법 익히기
  </Card>

...
이 가이드는 역순(최신순) 홈 타임라인 엔드포인트에 첫 요청을 보내는 방법을 단계별로 설명합니다.
...
<Note>
  ...
  * 사용자 액세스 토큰(이 엔드포인트는 사용자 인증이 필요합니다)
</Note>
...
가져오려는 홈 타임라인의 계정에 대한 사용자 id가 필요합니다. username 조회 엔드포인트를 사용해 확인하세요:
...
사용자 ID와 User Access Token을 사용해 GET 요청을 보냅니다:
...
SDK는 페이지네이션을 자동으로 처리합니다. cURL을 사용하는 경우, 더 많은 결과를 가져오기 위해 응답에 포함된 `next_token`을 사용하세요:
...
<CardGroup cols={2}>
  ...
  <Card title="연동 가이드" icon="book" href="/ko/x-api/posts/timelines/integrate">
    핵심 개념과 모범 사례
  </Card>

...
<CodeGroup dropdown>
  ...
  // 페이지네이션을 사용해 사용자의 게시물 타임라인 가져오기
...
타임라인은 요청당 최대 100개의 게시물을 반환합니다. 더 많은 결과가 필요하면 페이지네이션을 사용하세요.
...
<CodeGroup dropdown>
  ...
  print(f"{len(all_posts)}개의 게시물을 찾았습니다")
  ```
...
<Note>
  이 한도를 초과하여 게시물을 요청하면 데이터가 없는 성공 응답이 반환됩니다.
</Note>
...
* 30분이 지난 게시물은 최종 버전으로 간주됩니다
* 준 실시간 사용 사례에서는 게시물이 수정될 수 있음을 고려해야 합니다
* 필요한 경우 Post 조회를 사용해 최종 상태를 확인하세요

<Card title="게시물 편집 기본 사항" icon="clock-rotate-left" href="/ko/x-api/fundamentals/edit-posts">
  게시물 편집에 대해 자세히 알아보기
</Card>
...
App-Only 또는 User Context 인증으로 접근하는 모든 게시물에 대해 제공됩니다:
...
* 최근 30일 이내의 게시물에만 사용할 수 있습니다
* 인증된 사용자가 작성한 게시물에 대해서만 반환됩니다
* 다른 사용자의 게시물에 대해서는 오류가 반환됩니다
...
<Accordion title="비공개 메트릭과 페이지네이션">
  30일이 지난 게시물에 대한 비공개 메트릭을 요청할 때 `result_count: 0`과 함께 `next_token`을 받을 수 있습니다. 이를 피하려면:

  * 요청 범위를 최근 30일 이내로 유지하세요
  * 최소 10 이상의 `max_results` 값을 사용하세요
</Accordion>

<Accordion title="프로모션되지 않은 게시물의 프로모션 메트릭">
  프로모션되지 않은 게시물에 대해 프로모션 메트릭을 요청하면 빈 응답이 반환됩니다. 이는 알려진 문제입니다.
</Accordion>

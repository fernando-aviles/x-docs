...
<Note>
  ...
  * 승인된 앱이 연결된 [개발자 계정](https://developer.x.com/en/portal/petition/essential/basic-info)
  * 앱의 Bearer Token (공개 데이터용) 또는 User Access Token (비공개 지표용)
</Note>

***

<div id="get-users-who-liked-a-post">
  ## 특정 포스트에 좋아요를 누른 사용자 목록 가져오기
</div>
...
<CodeGroup dropdown>
  ...
  # 페이지네이션을 사용해 포스트에 좋아요를 누른 사용자 목록 가져오기
...
<div id="get-a-users-liked-posts">
  ## 특정 사용자가 좋아한 포스트 가져오기
</div>

특정 사용자가 좋아한 포스트를 조회합니다:
...
<CodeGroup dropdown>
  ...
  # 페이지네이션을 사용해 특정 사용자가 좋아한 포스트를 가져옵니다.
...
expansions를 사용하여 고정된 포스트와 같은 관련 data를 가져옵니다:
...
<CodeGroup dropdown>
  ...
  # 고정된 포스트 확장을 포함하여 좋아요를 누른 사용자 가져오기
  for page in client.posts.get_liking_users(
      "1354143047324299264",

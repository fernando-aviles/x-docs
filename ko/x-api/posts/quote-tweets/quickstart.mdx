...
keywords: ["인용 포스트 빠른 시작", "인용 포스트 튜토리얼", "인용 포스트 가져오기 가이드"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 인용 포스트(다른 포스트를 인용한 포스트)를 조회하는 방법을 설명합니다.
...
<Steps>
  <Step title="포스트 ID 찾기" icon="message">
    인용한 포스트를 찾으려는 기준 포스트의 ID를 가져옵니다. 이 ID는 포스트의 URL에서 확인할 수 있습니다:
  ...
  <Step title="결과 페이지네이션하기" icon="arrow-right">
    SDK는 페이지네이션을 자동으로 처리합니다. cURL을 사용할 때는 더 많은 인용 포스트를 가져오기 위해 `next_token`을 사용하세요.

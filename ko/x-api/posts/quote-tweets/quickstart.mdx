...
<Steps>
  <Step title="게시물 ID 찾기" icon="message">
    인용 게시물을 조회하려는 대상 게시물의 ID를 가져옵니다. 이 ID는 게시물의 URL에서 확인할 수 있습니다.
  ...
  <Step title="인용 게시물 요청하기" icon="terminal">
    <CodeGroup dropdown>
      ```bash cURL
  ...
  <Step title="결과 페이지네이션 처리" icon="arrow-right">
    SDK는 페이지네이션을 자동으로 처리합니다. cURL을 사용할 때는 더 많은 인용 게시물을 가져오려면 `next_token`을 사용하세요.

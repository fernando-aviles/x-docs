...
Batch compliance 엔드포인트를 사용할 때, 개발자는 대량의 X 데이터를 일괄 업로드하고, 데이터셋이 사용자 의도와 X 상의 콘텐츠의 현재 상태를 제대로 반영하도록 어떤 조치가 필요한지 파악할 수 있습니다. 시스템과 연결 상태가 안정적이고 신뢰할 수 있을 때, 원격 서버로 대량의 데이터를 업로드하는 작업은 비교적 단순합니다. 그러나 항상 그런 것은 아닙니다. 일부 환경에서는 연결 시간 제한(타임아웃)이 설정되어 있어 일정 시간이 지나면 앱과 업로드 서버 간의 연결이 끊길 수 있습니다. 또한, 예를 들어 노트북으로 Wi-Fi 연결을 통해 큰 파일을 업로드하려 할 때처럼 연결 문제를 겪을 수도 있습니다. 이러한 상황에서는 하나의 단일 연속 연결을 사용하는 대신 파일을 더 작은 단위로 나누어 순차적으로 업로드하는 것이 바람직합니다.

X의 Batch compliance 엔드포인트는 대용량 파일을 처리하기 위해 Google Cloud Storage에 의존합니다. 이 유형의 스토리지는 다양한 애플리케이션에 맞게 최적화되어 있습니다. Cloud Storage는 재개 가능한 업로드(resumable uploads)라는 대용량 파일 관리 기술을 지원합니다.

업로드 과정의 어느 시점에서든 문제가 발생하는 경우, Google Cloud Storage는 중단된 지점부터 작업을 다시 이어서 수행할 수 있습니다.
...
먼저, 컴플라이언스 작업을 생성하고 `type` 매개변수를 사용해 포스트 ID를 업로드할지 사용자 ID를 업로드할지 지정해야 합니다. 추가로, 요청 본문에 `resumable`을 포함하고 값을 `true`로 설정하세요. 아래의 `$APP&#95;ACCESS&#95;TOKEN`을 실제 App only Access Token 값으로 반드시 교체해야 합니다.
...
API 호출이 성공적으로 완료되면 다음과 비슷한 응답을 받습니다.
...
`upload&#95;url`의 값을 확인해 메모해 두세요. 다음 단계에서 사용합니다.

<div id="step-two">
  #### 2단계:
</div>

다음으로, 재개 업로드(resumable upload)를 시작해야 합니다. 이를 위해 이전 단계에서 얻은 `upload_url`에 **POST** 요청을 보내고, 다음 헤더를 포함해야 합니다:
...
이 호출에 성공하면 201 응답 코드가 반환됩니다. 그런 다음 응답 헤더에서 `location` 헤더의 값을 복사합니다. 이 값은 대략 다음과 같은 형태입니다:

```
https://storage.googleapis.com/twttr-tweet-compliance/1430227686685757442/submission/1202726487847104512_1430227686685757442?X-Goog-Algorithm=GOOG4-RSA-SHA256&X-Goog-Credential=complianceapi-public-svc-acct%40twttr-compliance-public-prod.iam.gserviceaccount.com%2F20210824%2Fauto%2Fstorage%2Fgoog4_request&X-Goog-Date=20210824T175707Z&X-Goog-Expires=900&X-Goog-SignedHeaders=content-length%3Bcontent-type%3Bhost%3Bx-goog-resumable&X-Goog-Signature=&upload_id=ADPycds-_Ow7aqcpbG4XguXSVAgd_2fy-XiDA2qm-It9PCwBlZhF4e2bfOAQzEmRJ4T_l6jU6LfYdfrKa_KlFFBOyx3PjYzrxQ
```

그런 다음 [빠른 시작 가이드](/ko/x-api/compliance/batch-compliance#getting-started-with-the-batch-compliance-endpoints)의 2단계부터 나머지 단계를 따라 이 위치에 포스트 또는 사용자 ID를 업로드할 수 있습니다.

기술적으로 복잡하므로 레주머블 업로드는 코드와 함께 사용할 때 가장 적합합니다. 이 가이드에서는 Node.js와 [needle 요청 라이브러리](https://www.npmjs.com/package/needle)를 함께 사용합니다.

<div id="install-the-dependencies">
  ### 종속성 설치
</div>

계속 진행하기 전에 Node.js 환경이 설치되어 있어야 합니다. Node.js는 공식 웹사이트에서 다운로드할 수 있습니다. 설치가 완료되면 Node.js에는 npm이라는 유틸리티가 포함됩니다. 다음 명령을 실행해서 Node와 npm이 모두 설치되어 있고, 오류가 발생하지 않는지 확인하세요.

`$ npm -v
6.4.1`

이와 비슷한 버전 번호가 출력되면 환경이 준비된 것입니다(버전 번호는 다를 수 있습니다). 이제 npm을 사용해서 업로드 라이브러리를 설치하겠습니다. 다음 명령을 실행하세요.

`$ npm install -g needle`

이제 준비가 완료되었습니다. 추가로 설정할 사항은 없습니다.

<div id="request-a-resumable-destination">
  #### 재개 가능한 대상 요청하기
</div>

새 작업을 생성할 때 `resumable` 매개변수를 `true`로 설정하면, 재개 가능한 업로드를 지원하는 업로드 대상을 받을 수 있습니다. 응답 페이로드에서 `upload_url` 값을 받게 됩니다.
...
기본적으로 이 라이브러리는 업로드 위치(버킷이라고 함)와 업로드하려는 파일 이름을 받아 새 업로드 대상 위치를 만듭니다. 그러나 배치 컴플라이언스 엔드포인트는 대상 위치를 자체적으로 생성하므로, 라이브러리에 이미 업로드를 받을 준비가 된 위치가 있다는 것을 알려 주어야 합니다. 

업로드 라이브러리에 이 값과 함께 업로드할 data가 들어 있는 파일의 이름을 전달해야 합니다. 파일을 하나 만든 뒤 이름을 ***twitter-upload.js***&#xB85C; 지정하세요. 그런 다음 다음 코드를 추가합니다:
...
파일을 가장 적절한 위치에 저장합니다. 그런 다음 명령줄에서 스크립트를 실행하면서 두 개의 매개변수를 전달합니다:

1. 첫 번째 매개변수는 업로드하려는 파일의 위치입니다(포스트 또는 User ID가 포함된 파일).
2. 두 번째 매개변수는 컴플라이언스 엔드포인트 응답으로부터 받은 업로드 URL입니다.

URL은 큰따옴표로 감싸고, 파일 이름에 공백이나 다른 문자가 포함되어 있다면 파일 이름도 동일하게 큰따옴표로 감싸야 합니다:
...
Ctrl + C를 누르거나 명령줄 창을 닫아서 언제든지 업로드를 일시 중지할 수 있습니다. 나중에 동일한 명령을 다시 실행하면 중단한 지점부터 업로드를 재개할 수 있습니다. 파일 업로드가 완전히 완료되면 다음 메시지가 표시됩니다:

`Upload complete`

이제 [compliance status endpoint](/ko/x-api/compliance/get-compliance-job)를 사용하여 컴플라이언스 작업 상태를 확인할 수 있으며, 완료되면 컴플라이언스 결과를 다운로드할 수 있습니다.

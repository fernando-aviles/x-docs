---
title: 다이렉트 메시지 관리
sidebarTitle: 소개
description: 다이렉트 메시지 전송 및 삭제
keywords: ["DM 관리", "DM 보내기", "DM 삭제", "다이렉트 메시지 API", "DM 생성"]
---

import { Button } from '/snippets/ko/button.mdx';

Manage Direct Messages 엔드포인트를 통해 인증된 사용자를 대신해 다이렉트 메시지를 보내고 삭제할 수 있습니다.

<div id="overview">
  ## 개요
</div>

<CardGroup cols={2}>
  <Card title="메시지 보내기" icon="paper-plane">
    다른 사용자에게 DM 보내기
  </Card>

  <Card title="메시지 삭제" icon="trash">
    내 계정에서 DM 삭제하기
  </Card>

  <Card title="대화 생성" icon="comments">
    새 대화 시작하기
  </Card>

  <Card title="그룹 메시지" icon="users">
    그룹 대화로 보내기
  </Card>
</CardGroup>

***

<div id="endpoints">
  ## 엔드포인트
</div>

| Method | Endpoint | 설명 |
|:-------|:---------|:------------|
| POST | `/2/dm_conversations` | 새 대화 생성 |
| POST | `/2/dm_conversations/with/:participant_id/messages` | 일대일 대화에 메시지 전송 |
| POST | `/2/dm_conversations/:dm_conversation_id/messages` | 기존 대화에 메시지 전송 |
| DELETE | `/2/dm_events/:id` | DM 이벤트 삭제 |

***

<div id="example-send-a-message">
  ## 예제: 메시지 보내기
</div>

```bash
curl -X POST "https://api.x.com/2/dm_conversations/with/1234567890/messages" \
  -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"text": "Hello! How are you?"}'
```

<div id="example-response">
  ## 응답 예시
</div>

```json
{
  "data": {
    "dm_conversation_id": "1234567890-0987654321",
    "dm_event_id": "1122334455667788990"
  }
}
```

***

<div id="message-types">
  ## 메시지 유형
</div>

텍스트 메시지를 보내고 미디어를 첨부할 수 있습니다.

```json
{
  "text": "이 사진 확인해보세요!",
  "attachments": [{
    "media_id": "1234567890123456789"
  }]
}
```

***

<div id="getting-started">
  ## 시작하기
</div>

<Note>
  **사전 준비 사항**

  * 승인된 [개발자 계정](https://developer.x.com/en/portal/petition/essential/basic-info)
  * Developer Console의 [Project 및 App](/ko/resources/fundamentals/developer-apps)
  * [OAuth 2.0 PKCE](/ko/resources/fundamentals/authentication#oauth-2-0-authorization-code-flow-with-pkce-2)를 사용한 사용자 액세스 토큰
</Note>

<CardGroup cols={2}>
  <Card title="빠른 시작" icon="rocket" href="/ko/x-api/direct-messages/manage/quickstart">
    첫 번째 DM을 보내세요
  </Card>

  <Card title="통합 가이드" icon="book" href="/ko/x-api/direct-messages/manage/integrate">
    핵심 개념과 모범 사례
  </Card>

  <Card title="DM 조회" icon="messages" href="/ko/x-api/direct-messages/lookup/introduction">
    DM 이벤트를 조회하세요
  </Card>

  <Card title="API 레퍼런스" icon="code" href="/ko/x-api/direct-messages/send-new-message">
    전체 엔드포인트 문서
  </Card>
</CardGroup>
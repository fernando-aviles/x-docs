...
title: 빠른 시작
sidebarTitle: 빠른 시작
description: 다이렉트 메시지를 보내고 그룹 대화를 생성하기
keywords: ["다이렉트 메시지 관리 빠른 시작", "DM 보내기 빠른 시작", "DM 관리 빠른 시작", "그룹 DM 생성하기"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 다이렉트 메시지를 보내고 그룹 대화를 생성하는 방법을 안내합니다.
...
<Note>
  ...
  * User Access Token (`dm.write` 및 `dm.read` scope가 포함된 OAuth 2.0 PKCE)
</Note>
...
<Steps>
  <Step title="받는 사람의 사용자 ID 가져오기">
    메시지를 보내려는 사용자의 ID가 필요합니다. 이 값은 [사용자 조회 엔드포인트](/ko/x-api/users/lookup/introduction)에서 가져올 수 있습니다.
  </Step>
...
<Steps>
  <Step title="참여자 지정하기">
    그룹에 포함하고 싶은 모든 사용자의 ID(본인은 제외)를 수집합니다.
  </Step>

  <Step title="첫 메시지와 함께 그룹 생성하기">
    <CodeGroup dropdown>
      ```bash cURL
  ...
  <Step title="대화 ID 받기">
    ...
    나중에 더 많은 메시지를 추가할 수 있도록 `dm_conversation_id`를 저장하세요.
  </Step>
</Steps>
...
<Steps>
  <Step title="미디어 업로드">
    먼저 [Media Upload endpoint](/ko/x-api/media/quickstart/media-upload-chunked)를 사용하여 미디어를 업로드합니다.
  </Step>

  <Step title="미디어를 첨부해 전송">
    <CodeGroup dropdown>
      ```bash cURL
      curl -X POST "https://api.x.com/2/dm_conversations/with/9876543210/messages" \
        -H "Authorization: Bearer $USER_ACCESS_TOKEN" \
...
OAuth 2.0 PKCE를 사용할 때 액세스 토큰에는 다음 scope 권한이 포함되어 있어야 합니다:

| Scope | Description |
|:------|:------------|
| `dm.write` | 메시지 전송 및 삭제 |
| `dm.read` | 대화 읽기 (`dm.write`와 함께 필수) |

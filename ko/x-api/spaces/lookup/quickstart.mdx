...
title: 빠른 시작
sidebarTitle: 빠른 시작
description: ID 또는 생성자를 기준으로 Space 세부 정보를 조회합니다
keywords: ["Spaces 빠른 시작", "Space 조회 튜토리얼", "Spaces 가이드"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 Spaces 조회 엔드포인트를 사용해 Space 정보를 가져오는 방법을 단계별로 설명합니다.
...
| `participant_count` | 현재 참여 인원 수 |
| `scheduled_start` | 예정 시작 시간 |
| `started_at` | 실제 시작 시간 |
| `ended_at` | 종료 시간 |
| `is_ticketed` | 스페이스가 티켓을 사용하는지 여부 |
| `state` | 현재 상태 |

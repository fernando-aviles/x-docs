...
title: 개요
sidebarTitle: 개요
keywords: ["리스트 Tweet 마이그레이션", "리스트 타임라인 마이그레이션", "v1.1에서 v2로 리스트 Tweet 마이그레이션", "리스트 Tweet 마이그레이션 가이드", "리스트 Tweet 마이그레이션"]
---
...
v2 리스트 게시물 조회 엔드포인트는 기존 v1.1 표준 [GET lists/statuses](https://developer.x.com/en/docs/twitter-api/v1/accounts-and-users/create-manage-lists/api-reference/get-lists-statuses)를 대체합니다. 이 버전의 엔드포인트를 사용하는 코드, App 또는 도구가 있고 최신 X API v2 엔드포인트로 마이그레이션을 고려하고 있다면 이 가이드 모음이 도움이 될 것입니다.

...
keywords: ["리스트 트윗 퀵스타트", "리스트 타임라인 퀵스타트", "리스트 타임라인 포스트 가져오기 튜토리얼"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 리스트 타임라인에서 포스트를 가져오는 방법을 단계별로 안내합니다.
...
<Steps>
  ...
  <Step title="리스트 타임라인 요청" icon="terminal">
    <CodeGroup dropdown>
      ```bash cURL
  ...
  <Step title="결과 페이지네이션" icon="arrow-right">
    SDK는 페이지네이션을 자동으로 처리합니다. cURL을 사용하는 경우, 더 많은 포스트를 가져오려면 응답의 `next_token` 값을 사용하세요.

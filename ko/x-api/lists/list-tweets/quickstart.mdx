...
title: 빠른 시작
sidebarTitle: 빠른 시작
description: 리스트 타임라인에서 게시물 가져오기
keywords: ["리스트 트윗 빠른 시작", "리스트 타임라인 빠른 시작", "리스트 게시물 가져오기 튜토리얼"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 리스트 타임라인에서 게시물을 가져오는 방법을 설명합니다.
...
<Steps>
  ...
  <Step title="결과 페이지네이션하기" icon="arrow-right">
    SDK는 페이지네이션을 자동으로 처리합니다. cURL을 사용할 경우, 더 많은 게시물을 가져오려면 응답에 포함된 `next_token`을 사용합니다.
...
<Note>
  이 엔드포인트는 해당 리스트에서 가장 최근 게시물을 최대 800개까지 반환합니다.
</Note>

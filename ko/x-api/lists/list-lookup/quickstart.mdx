...
title: 빠른 시작
sidebarTitle: 빠른 시작
description: ID나 소유자를 기준으로 리스트 조회
keywords: ["리스트 조회 빠른 시작", "리스트 가져오기 튜토리얼", "리스트 조회 가이드"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 List 조회 엔드포인트를 사용하여 List 정보를 확인하는 방법을 단계별로 안내합니다.
...
<CodeGroup dropdown>
  ...
  # 페이지네이션을 사용하여 특정 사용자가 소유한 리스트 가져오기
...
<CardGroup cols={2}>
  <Card title="리스트 포스트 가져가기" icon="list" href="/ko/x-api/lists/list-tweets/quickstart">
    리스트에서 포스트를 가져오기
  </Card>

  <Card title="리스트 멤버 가져오기" icon="users" href="/ko/x-api/lists/list-members/quickstart/list-members-lookup">
    리스트 멤버를 가져오기
  </Card>

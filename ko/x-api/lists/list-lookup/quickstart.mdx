...
title: 빠른 시작
sidebarTitle: 빠른 시작
description: ID 또는 소유자를 기준으로 리스트를 조회합니다
keywords: ["리스트 조회 빠른 시작", "리스트 가져오기 튜토리얼", "리스트 조회 가이드"]
---

import { Button } from '/snippets/ko/button.mdx';

이 가이드는 List 조회 엔드포인트를 사용하여 List 정보를 조회하는 방법을 안내합니다.
...
<div id="get-lists-owned-by-a-user">
  ## 특정 사용자가 소유한 Lists 조회하기
</div>

특정 사용자가 소유한 모든 Lists를 조회합니다:
...
<CardGroup cols={2}>
  <Card title="리스트 포스트 가져오기" icon="list" href="/ko/x-api/lists/list-tweets/quickstart">
    리스트에서 포스트 가져오기
  </Card>

...
title: 고정된 리스트 개요
sidebarTitle: 개요
description: 고정된 리스트 엔드포인트 사용 시작하기
keywords: ["고정된 리스트 개요", "고정된 리스트 빠른 시작", "리스트 고정 개요", "리스트 고정 기능 개요"]
---

import { Button } from '/snippets/ko/button.mdx';

고정 목록 엔드포인트를 사용하면 사용자의 고정 목록을 조회하고, 어떤 목록을 고정할지 관리할 수 있습니다.
...
<Warning>
  조회 및 관리 작업에는 모두 사용자 컨텍스트 인증이 필요합니다. 앱 전용 인증(Bearer Token)은 지원되지 않습니다.
</Warning>

...
성공 응답은 200번대 HTTP 코드와 함께, 요청한 객체가 조회, 생성, 수정 또는 삭제된 결과와 서버가 해당 요청을 어떻게 해석했는지를 나타내는 표현을 포함하는 JSON 기반 페이로드로 표시됩니다.

성공적인 요청을 보낸 경우, 응답의 일부로 요청 내용을 그대로 반영하는 `request` 노드를 받게 됩니다.
...
JSON 응답의 `data` 필드에는 해당 리소스와 연결된 구체적인 객체가 포함됩니다. 응답에 하나 이상의 결과가 포함될 수 있는 경우 `data` 노드의 형식은 JSON 배열이 됩니다. 응답에서 하나의 결과만 가능한 경우에는 JSON 해시로 반환됩니다. 드물게, 일반적으로 컬렉션이 포함되는 응답에서 대신 해시맵이 포함된 응답을 볼 수 있습니다. 이 경우 단일 해시맵은 `type` 필드에 지정된 것과 동일한 유형의 객체라고 가정하면 됩니다.

<div id="error-response-structure">
  ## 오류 응답 구조
</div>

오류 응답은 200번대가 아닌 HTTP 코드와 함께 제공됩니다. 일반적으로 JSON 응답이 함께 전송되지만, 일부 오류는 다른 종류의 본문을 반환할 수 있습니다. 이와 같이 응답 구조를 파싱할 수 없을 때에는 HTTP 코드의 핵심 의미를 우선시해야 합니다. 예를 들어, 때때로 HTTP 404와 함께 HTML 응답이 반환될 수 있습니다. 이 경우, 해당 콘텐츠를 찾을 수 없다고 가정해도 안전합니다(HTTP 404는 “Not Found”를 의미합니다).

일반적인 오류 응답은 성공적인 응답과 유사한 구조를 따릅니다. 오류의 성격은 응답의 `errors` 노드에 전달됩니다. `errors/code` 노드는 코드에서 활용하여 문제 해결 결정을 내릴 수 있는 CAPS&#95;CASE 상수 오류 코드를 나타냅니다. `errors/message` 노드는 (대개) 사람이 읽을 수 있는 영어 오류 설명을 나타냅니다. 오류에 대한 보다 세밀한 세부 정보를 나타내기 위해 추가 필드가 포함될 수 있습니다.
...
위의 예시에서는 analytics 엔드포인트에 대한 요청에서 `start_time` 파라미터에 유효하지 않은 값이 사용되었습니다. 유효하지 않은 파라미터가 포함된 요청에 대한 `errors/code` 값은 `INVALID_PARAMETER`입니다.

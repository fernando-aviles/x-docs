...
keywords: ["DPA 캠페인", "동적 상품 광고", "DPA", "DPA 캠페인 생성", "상품 광고", "동적 광고"]
---

다음 예시는 App을 설치하고 구성했으며, twurl을 사용해 사용자 인증까지 완료했다고 가정합니다. twurl은 cURL과 유사한 명령줄 도구로, X OAuth 인증을 안정적으로 처리합니다. twurl은 Ads API(및 REST API) 기능을 빠르게 테스트하고 디버깅하는 데 유용한 도구입니다. 요청 및 응답의 전체 헤더를 확인하려면 `-t` 옵션을 사용해 호출을 추적하세요. 이는 cURL의 `-v` 옵션과 거의 동일합니다.

이 예시에서는 Dynamic Product Ads(DPA) 캠페인을 생성해 보겠습니다.
...
캠페인을 생성하고 해당 캠페인을 자금 수단(funding instrument)에 연결합니다.
캠페인의 시작 시간과 예산을 설정합니다.

* 필수 필드: 자금 수단(funding instrument), 캠페인 이름, 예산(총액/일 단위), 시작 시간, 광고 계정 ID
...
<div id="step-2-create-line-item-with-sales-objective">
  ## 2단계: Sales 목표로 라인 아이템 생성
</div>

캠페인에 연결된 라인 아이템을 생성합니다.

* 필수 필드: 캠페인 id (1단계에서 가져온 값), product type, placements, objective, goal, entity status, primary web event tag, catalog id, product set id, creative&#95;source, ads&#95;account&#95;id
* catalog&#95;id와 product&#95;set&#95;id는 Shopping Manager에서 확인할 수 있습니다.
* creative&#95;source는 항상 DYNAMIC&#95;PRODUCT&#95;CREATIVE로 설정해야 합니다.
* objective는 항상 WEBSITE&#95;CLICKS로 설정해야 합니다.
* goal은 항상 WEBSITE&#95;CONVERSIONS로 설정해야 합니다.
...
* 이 단계를 완료하면 캠페인이 X의 Ads Manager에 표시됩니다.
...
* 필수 매개변수: 광고 계정 ID와 URL:
* URL은 반드시 https://twitter.com 으로 시작해야 하며, 아래 예시처럼 추가 UTM 매개변수를 뒤에 붙일 수 있습니다:
  * `https://twitter.com?utm_source=%7B%7Bsite_source%7D%7D`
* 선택적으로, creative&#95;type 매개변수에 아래 값 중 하나를 지정하여 카드 형식을 선택할 수 있습니다: &quot;CAROUSEL&quot;, &quot;SINGLE&quot;, &quot;COLLECTION&quot;. 지정하지 않으면, 카드는 캐러셀 카드로 생성됩니다.
...
* 필수 매개변수: Ads 계정 ID, 사용자 ID, 카드 URI(3단계에서 가져온 값), 텍스트, 이름, &amp; dynamic product ad
* dynamic&#95;product&#95;ad 값은 항상 true로 설정해야 합니다.
...
* 필수 매개변수: 광고 계정 id, 라인 아이템 id(2단계에서 얻은 값) 및 Tweet id(4단계의 id&#95;str)
...
광고주가 UI에서 카드를 보고자 할 경우, UI에서도 확인할 수 있습니다.

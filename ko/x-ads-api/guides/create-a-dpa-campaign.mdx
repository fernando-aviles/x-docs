---
title: 'DPA 캠페인 생성 - 단계별 안내'
keywords: ["DPA 캠페인", "다이내믹 상품 광고", "DPA", "DPA 캠페인 생성", "상품 광고", "다이내믹 광고"]
---

다음 예시는 twurl을 사용하여 앱과 사용자를 설치하고 구성한 뒤 인증을 완료했다고 가정합니다. twurl은 X OAuth 인증을 원활하게 처리하는 cURL 스타일의 커맨드라인 도구입니다. twurl은 Ads API(및 REST API) 기능을 빠르게 테스트하고 디버깅하는 데 매우 유용한 도구입니다. 요청과 응답의 전체 헤더를 확인하려면 -t 옵션을 사용해 호출을 추적하십시오. 이는 cURL의 -v 옵션과 대략 동일합니다.

이 예시에서는 Dynamic Product Ads(DPA) 캠페인을 생성합니다.
...
이전에 실행한 명령에서 조회한 account id를 사용하여 GET accounts/:account&#95;id/funding&#95;instruments API를 호출합니다.
...
캠페인을 생성하고 이를 결제 수단과 연결합니다.
캠페인의 시작 시간과 예산을 설정합니다.

* 필수 항목: 결제 수단, 캠페인 이름, 예산(총액/일별), 시작 시간, 광고 계정 id
...
<div id="step-2-create-line-item-with-sales-objective">
  ## 2단계: Sales objective로 라인 아이템 생성
</div>

캠페인에 연결된 라인 아이템을 생성합니다.

* 필수 필드: campaign id (1단계에서 가져온 값), product type, placements, objective, goal, entity status, primary web event tag, catalog id, product set id, creative&#95;source, ads&#95;account&#95;id
* catalog&#95;id 및 product&#95;set&#95;id는 Shopping Manager에서 찾을 수 있습니다.
...
* 이 단계를 완료하면 캠페인을 X Ads Manager에서 확인할 수 있습니다.
...
* 필수 매개변수: 광고 계정 ID 및 URL
* URL은 반드시 https://twitter.com으로 시작해야 하며, 아래 예시와 같이 이후에 UTM 파라미터를 추가로 붙일 수 있습니다.
  * `https://twitter.com?utm_source=%7B%7Bsite_source%7D%7D`
* 선택적으로, `creative_type`에 다음 값 중 하나를 지정하여 카드 형식을 선택할 수 있습니다: &quot;CAROUSEL&quot;, &quot;SINGLE&quot;, &quot;COLLECTION&quot;. 지정하지 않으면 카드는 캐러셀 카드로 생성됩니다.
...
* 필수 매개변수: 광고 계정 ID, 사용자 ID, 카드 URI(3단계에서 생성), 텍스트, 이름, &amp; dynamic&#95;product&#95;ad
...
<div id="step-5-associate-post-with-line-item">
  ## Step 5: 포스트를 라인 아이템에 연결
</div>

포스트를 라인 아이템에 연결합니다.

* 필수 매개변수: Ads 계정 id, 라인 아이템 id(2단계에서 가져온 값), tweet id(4단계에서 가져온 id&#95;str)
...
광고주가 UI에서 카드를 보고자 할 경우에도 해당 카드는 UI에 표시됩니다.

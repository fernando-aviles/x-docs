...
다음 예제에서는 twurl을 사용해 앱과 사용자를 이미 설치, 구성하고 승인해 둔 상태라고 가정합니다. twurl은 X OAuth 인증을 매끄럽게 처리하는, cURL과 유사한 명령줄 도구입니다. twurl은 Ads API(및 REST API) 기능을 빠르게 테스트하고 디버깅하는 데 유용한 도구입니다. 요청과 응답의 전체 헤더를 확인하려면 호출을 추적하기 위해 -t 옵션을 사용하세요. 이는 cURL의 -v 옵션과 거의 동일합니다.
...
이전에 실행한 명령에서 가져온 account id를 사용해 GET accounts/:account&#95;id/funding&#95;instruments API 엔드포인트를 호출합니다.
...
캠페인을 생성한 다음 자금 수단에 연결합니다.
캠페인의 시작 시간과 예산을 지정합니다.

* 필수 필드: 자금 수단, 캠페인 이름, 예산(총액/일별), 시작 시간, Ads 계정 ID
...
* catalog&#95;id 및 product&#95;set&#95;id는 Shopping Manager에서 확인할 수 있습니다.
...
* 이 단계를 완료하면 X의 Ads Manager에서 캠페인을 볼 수 있게 됩니다.
...
* URL은 https://twitter.com으로 시작해야 합니다. 아래 예시와 같이 추가적인 UTM 파라미터는 뒤에 이어서 붙일 수 있습니다.
...
* 필수 매개변수: 광고 계정 ID, 사용자 ID, 카드 URI(3단계에서 가져온 것), 텍스트, 이름 및 동적 상품 광고
...
<div id="step-5-associate-post-with-line-item">
  ## 5단계: 포스트를 라인 아이템과 연결하기
</div>

포스트를 라인 아이템과 연결합니다.

* 필수 매개변수: Ads 계정 ID, 라인 아이템 ID(2단계에서 얻은 값), 트윗 ID(4단계의 id&#95;str)
...
광고주가 UI에서 카드를 보고자 할 경우, UI에서도 해당 카드를 확인할 수 있습니다.

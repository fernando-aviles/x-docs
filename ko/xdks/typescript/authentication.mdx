...
TypeScript SDK는 다양한 사용 사례에 맞게 여러 인증 방식을 지원합니다.

<div id="bearer-token-app-only-auth">
  ## Bearer Token (앱 전용 인증)
</div>

읽기 전용 작업 및 공개 data에 대한 액세스의 경우:
...
(async (): Promise<void> => {
  ...
  // 승인 후 사용자는 code 파라미터와 함께 다시 리다이렉트됩니다.
  // 콜백 URL에서 code를 추출합니다(예: 쿼리 파라미터에서).
  const authCode: string = 'code-from-callback-url'; // OAuth 콜백에서 받은 실제 code로 교체하세요.
...
(async () => {
  ...
  // 승인 후 사용자는 code 파라미터와 함께 다시 리다이렉트됩니다.
  // 콜백 URL에서 code를 추출합니다(예: 쿼리 파라미터에서).
  const authCode = 'code-from-callback-url'; // OAuth 콜백에서 받은 실제 code로 교체하세요.
...
<Info>
  JavaScript/TypeScript XDK를 사용하는 자세한 코드 예시는 [code samples GitHub repo](https://github.com/xdevplatform/samples/tree/main/javascript)에서 확인할 수 있습니다.
</Info>

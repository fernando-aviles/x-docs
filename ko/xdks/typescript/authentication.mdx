...
TypeScript SDK는 다양한 사용 사례에 맞는 여러 인증 방식을 지원합니다.

<div id="bearer-token-app-only-auth">
  ## Bearer Token (앱 전용 인증)
</div>

읽기 전용 작업 및 공개 데이터에 대한 액세스에 사용합니다:
...
(async (): Promise<void> => {
  ...
  // 사용자가 authUrl로 이동해 앱을 승인합니다.
  // 승인 후, 사용자는 code 매개변수와 함께 다시 리디렉션되어 돌아옵니다.
  // 콜백 URL(예: 쿼리 매개변수)에서 code를 추출합니다.
  const authCode: string = 'code-from-callback-url'; // OAuth 콜백에서 받은 실제 코드로 교체하세요.
...
(async () => {
  ...
  // 승인 후, 사용자는 code 매개변수와 함께 다시 리디렉션되어 돌아옵니다.
  // 콜백 URL(예: 쿼리 매개변수)에서 code를 추출합니다.
  const authCode = 'code-from-callback-url'; // OAuth 콜백에서 받은 실제 코드로 교체하세요.
...
<Info>
  JavaScript/TypeScript용 xdk를 사용하는 자세한 코드 예제는 [code samples GitHub repo](https://github.com/xdevplatform/samples/tree/main/javascript)를 참고하세요.
</Info>

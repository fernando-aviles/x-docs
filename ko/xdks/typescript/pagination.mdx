...
SDK는 페이지네이션 방식의 응답을 반환하는 모든 엔드포인트에서 사용할 수 있는 범용 paginator 유틸리티를 제공합니다. 메서드는 기본 응답을 반환하며, 이를 paginator로 래핑해 사용합니다.
...
for await (const user of followers) {
  ...
  console.log(typedUser.username);  // 완전히 타입이 지정된 형태로 접근
}
```
...
await followers.fetchNext();
if (!followers.done) {
  const page2: UserPaginator = await followers.next(); // 다음 페이지부터 시작하는 독립적인 페이지네이터 인스턴스
...
try {
  ...
  if (followers.rateLimited) {
    console.error('요청 속도 제한 상태입니다. 백오프가 필요합니다.');
    // 백오프 후 / 나중에 재시도
  } else {
...
<Info>
JavaScript/TypeScript XDK를 사용하는 자세한 코드 예제는 [코드 샘플 GitHub 리포지토리](https://github.com/xdevplatform/samples/tree/main/javascript)를 확인하세요.
</Info>
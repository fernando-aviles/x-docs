...
이 클라이언트는 X API의 사용자 엔드포인트와 상호작용하기 위한 메서드를 제공합니다. 인증, 요청 서식 지정, 응답 파싱을 처리하여 모든 사용자 관련 작업을 지원합니다.
...
`UsersClient` 클래스의 새 인스턴스를 생성합니다
...
username으로 사용자 조회
여러 사용자의 세부 정보를 username(사용자 이름)으로 조회합니다.
...
API 응답으로 완료되는 Promise이며, requestOptions.raw가 true인 경우 원시 `Response`입니다
...
| :------ | :------ | :------ |
| `id` | `string` | 포스트의 좋아요를 취소하도록 요청하는 인증된 소스 사용자의 ID입니다. |
| `tweetId` | `string` | 사용자가 좋아요 취소를 요청하는 포스트의 ID입니다. |
| `options` | `Object` | - |
...
API 응답으로 resolve되는 `Promise`이며, `requestOptions.raw`가 true인 경우 가공되지 않은 `Response`를 반환합니다.
...
| `listId` | `string` | 고정을 해제할 List의 ID입니다. |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |
...
API 응답으로 resolve되는 Promise이며, requestOptions.raw가 true인 경우 원시 응답(Response)입니다
...
ID로 User 조회
지정된 ID를 가진 특정 User의 상세 정보를 조회합니다.
...
포스트 리포스트 취소
인증된 사용자가 지정된 ID를 가진 특정 포스트의 리포스트를 취소합니다.
...
북마크 삭제
인증된 사용자의 북마크에서 해당 ID의 포스트를 제거합니다.

#### 매개변수

| Name | Type | Description |
| :------ | :------ | :------ |
| `id` | `string` | 북마크에서 제거할 인증된 소스 사용자의 ID입니다. |
...
API 응답으로 resolve되는 Promise이며, requestOptions.raw가 true이면 가공되지 않은(raw) Response입니다
...
리스트 언팔로우
인증된 사용자가 지정된 ID의 특정 리스트 팔로우를 취소합니다.

#### 매개변수

| Name | Type | Description |
| :------ | :------ | :------ |
| `id` | `string` | 언팔로우를 수행하는 인증된 소스 User의 id입니다. |
...
사용자 음소거하기\
인증된 사용자가 특정 사용자의 id로 해당 사용자를 음소거합니다.

#### 매개변수

| Name | Type | Description |
| :------ | :------ | :------ |
| `id` | `string` | 대상 사용자를 뮤트하도록 요청하는 인증된 소스 사용자의 id입니다. |
...
차단한 사용자 가져오기
지정된 사용자 ID가 차단한 사용자 목록을 가져옵니다.

#### 매개변수

| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `id` | `string` | 결과를 반환할 인증된 소스 사용자 ID입니다. |
...
ID로 사용자 가져오기
여러 사용자의 세부 정보를 ID로 조회합니다.

#### 매개변수

| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `ids` | `any`[] | 쉼표로 구분된 사용자 ID 목록입니다. 최대 100개의 ID까지 지정할 수 있습니다. |
...
팔로우 중인 사용자 가져오기
특정 사용자의 id를 기준으로, 해당 사용자가 팔로우 중인 사용자 목록을 조회합니다.
...
사용자 팔로우하기
인증된 사용자가 지정한 id를 가진 특정 사용자를 팔로우합니다.

#### 매개변수

| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `id` | `string` | 대상 사용자를 팔로우하도록 요청하는 인증된 소스 User의 ID입니다. |
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `id` | `string` | 포스트에 좋아요를 요청하는 인증된 소스 사용자의 ID입니다. |
...
API 응답으로 resolve되는 Promise이며, requestOptions.raw 가 true인 경우 원시 Response로 resolve됩니다.
...
API 응답으로 resolve되는 Promise 또는 requestOptions.raw가 true인 경우 raw Response
...
포스트 가져오기
특정 사용자의 ID를 기준으로 해당 사용자가 작성한 포스트 목록을 조회합니다.
...
DM 차단
인증된 사용자를 대신하여 특정 사용자의 id를 지정해, 해당 사용자와 주고받는 다이렉트 메시지를 차단합니다.

#### 매개변수

| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `id` | `string` | 인증된 사용자가 DM을 차단하도록 요청하는 대상 User의 ID입니다. |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |
...
리스트 멤버십 조회
특정 사용자의 id를 기준으로, 해당 사용자가 구성원으로 속해 있는 리스트 목록을 조회합니다.
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `id` | `string` | List를 고정할 인증된 소스 User의 id. |
...
지정된 username을 사용하여 특정 User의 상세 정보를 조회합니다.
...
타임라인 가져오기
인증된 사용자의 타임라인에 있는 포스트를 시간 역순(최신순)으로 정렬한 목록을 반환합니다.

#### 매개변수

| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `id` | `string` | 최신순 타임라인 포스트를 나열할 인증된 소스 User의 id입니다. |
...
팔로우한 리스트 가져오기
지정된 User의 id를 사용해 해당 User가 팔로우하는 리스트 목록을 조회합니다.
...
API 응답, 또는 requestOptions.raw가 true인 경우 원시 Response로 resolve되는 Promise입니다
...
리스트 팔로우
인증된 사용자가 지정된 id의 List를 팔로우하도록 합니다.

#### 매개변수

| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `id` | `string` | List를 팔로우할 인증된 소스 사용자의 id입니다. |
...
DM 차단 해제
인증된 사용자가 특정 사용자의 id를 사용해, 해당 사용자와 주고받는 다이렉트 메시지 차단을 해제합니다.

#### 매개변수

| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `id` | `string` | 인증된 사용자가 DM 차단 해제를 요청하려는 대상 User의 ID입니다. |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `id` | `string` | 포스트의 리포스트를 요청하는 인증된 원본 사용자의 ID입니다. |
...
API 응답으로 resolve되는 Promise이며, requestOptions.raw가 true인 경우에는 원시 응답을 반환합니다
...
API 응답으로 resolve되는 Promise이며, requestOptions.raw가 true이면 원시 응답(`Response`)을 반환합니다
...
폴더 ID로 북마크 가져오기
인증된 사용자의 특정 북마크 폴더에 포함된 포스트를 해당 폴더 ID로 조회합니다.
...
API 응답으로 resolve되는 Promise이며, `requestOptions.raw`가 true인 경우 원시 `Response`로 resolve됩니다

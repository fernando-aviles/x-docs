...
이 클래스는 X API와 상호 작용하기 위한 주요 진입점입니다. 전용 클라이언트 모듈을 통해 모든 API 엔드포인트에 대한 접근을 제공하며, 인증, 요청 구성 및 오류 처리를 담당합니다.
...
<ResponseField name="request" type="Promise<T>" required>
  X API에 대한 인증된 요청을 수행합니다

  이 메서드는 모든 API 요청에 대해 인증, 요청 포맷팅, 오류 처리를 수행합니다.
...
<ResponseField name="mapSecuritySchemeToAuthTypes" type="string[]" required>
  OpenAPI 보안 스킴 이름을 내부 인증 유형에 매핑합니다
</ResponseField>

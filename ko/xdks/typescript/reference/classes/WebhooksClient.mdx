...
이 클라이언트는 X API의 웹훅 엔드포인트와 상호작용하기 위한 메서드를 제공합니다. 모든 웹훅 관련 작업에 대해 인증, 요청 포맷팅, 응답 파싱을 처리합니다.
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `webhookId` | `string` | FilteredStream 규칙 세트에 연결할 웹후크 ID입니다. |
...
스트림 링크 삭제
지정된 webhook과 FilteredStream 이벤트 간의 연결을 삭제합니다.
...
API 응답으로 resolve되는 Promise이며, requestOptions.raw가 true인 경우에는 원시 응답(`Response`)입니다
...
스트림 링크 가져오기
필터링된 스트림 규칙 세트에 연결된 웹훅 링크 목록을 가져옵니다.
...
웹훅 재생 작업 생성
웹훅으로 전달되었거나 전달을 시도한 모든 이벤트에 대해, 최대 과거 24시간 동안의 이벤트를 가져오는 재생 작업을 생성합니다.
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `webhookId` | `string` | 확인할 웹후크의 ID입니다. |
| `options` | `Object` | - |
| `options.requestOptions` | `Object` | - |
| `options.requestOptions.raw` | `true` | - |
...
API 응답으로 resolve되는 `Promise`이며, requestOptions.raw가 true인 경우에는 가공되지 않은 원시 `Response`를 반환합니다
...
API 응답으로 resolve되는 Promise이며, `requestOptions.raw`가 `true`인 경우 원시 응답(`Response`)입니다

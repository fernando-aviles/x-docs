...
이 클라이언트는 X API의 리스트 엔드포인트와 상호 작용하기 위한 메서드를 제공합니다. 인증, 요청 형식 지정, 리스트 관련 모든 작업에 대한 응답 파싱을 처리합니다.
...
List 팔로워 목록 가져오기
특정 List의 id를 사용하여 해당 List를 팔로우하는 User 목록을 조회합니다.
...
API 응답으로 resolve되는 `Promise`이며, `requestOptions.raw`가 `true`인 경우 원시 응답(`Response`)입니다
...
API 응답(또는 requestOptions.raw가 true인 경우 원시 Response)으로 resolve되는 Promise
...
List 멤버 조회\
지정된 List의 id를 사용해 해당 List에 속한 사용자 목록을 조회합니다.
...
리스트 포스트 가져오기
지정된 id를 가진 특정 리스트에 포함된 포스트 목록을 가져옵니다.
...
API 응답으로 resolve되는 Promise이며, requestOptions.raw가 true인 경우에는 원시 응답(Response)을 반환합니다
...
리스트 멤버 제거
특정 리스트 ID와 사용자 ID를 기준으로 해당 리스트에서 해당 사용자를 제거합니다.
...
ID로 List 조회
지정된 ID를 사용해 특정 List의 세부 정보를 조회합니다.
...
API 응답으로 resolve되는 Promise 또는 requestOptions.raw가 true인 경우 원시 응답

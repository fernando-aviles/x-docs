...
새로운 `EventPaginator` 인스턴스를 생성합니다
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/ko/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`any`&gt;&gt; | 페이지네이션 토큰을 받아 data의 한 페이지를 가져오는 함수 |
...
현재 페이지네이터 인스턴스에 추가합니다. 페이지네이션 상태와 메타데이터를 업데이트합니다.

...
요청 한도(rate limit)에 도달했는지 확인합니다
...
이 메서드는 데이터의 다음 페이지를 가져와 해당 항목들을
현재 paginator 인스턴스에 추가합니다. 또한 페이지네이션 상태와 메타데이터를 업데이트합니다.
...
이 메서드는 현재 paginator 인스턴스의 상태에는 영향을 주지 않고,

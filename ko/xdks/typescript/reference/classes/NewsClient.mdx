...
이 클라이언트는 X API의 뉴스 엔드포인트와 상호작용하기 위한 메서드를 제공합니다. 인증, 요청 형식 지정, 그리고 모든 뉴스 관련 작업에 대한 응답 파싱을 처리합니다.
...
요청이 완료되면 API 응답으로 resolve되는 Promise이며, requestOptions.raw가 true인 경우 원시 응답(Response)을 반환합니다

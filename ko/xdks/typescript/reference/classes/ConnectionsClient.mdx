...
connections 관련 작업을 위한 클라이언트

이 클라이언트는 X API의 connections 엔드포인트와 상호 작용하기 위한 메서드를 제공합니다.
connections 관련 모든 작업에 대해 인증, 요청 서식 지정 및 응답 파싱을 처리합니다.
...
새로운 ConnectionsClient 인스턴스를 생성합니다
...
연결 이력 가져오기
인증된 애플리케이션에 대해 활성 및 과거 스트리밍 연결과 연결 해제 사유를 반환합니다.

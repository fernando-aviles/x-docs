...
이 클라이언트는 X API의 포스트 엔드포인트와 상호작용하기 위한 메서드를 제공합니다.
모든 포스트 관련 작업에 대해 인증, 요청 포맷팅, 응답 파싱을 처리합니다.
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `client` | [`Client`](/ko/xdks/typescript/reference/classes/Client) | 메인 X API 클라이언트 인스턴스입니다 |
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `tweetIds` | `any`[] | 28시간 메트릭용 포스트 id 목록입니다. |
| `granularity` | `string` | 메트릭 응답의 세분화 수준입니다. |
| `requestedMetrics` | `any`[] | 과거 데이터에 대해 요청할 메트릭입니다. |
...
포스트 분석 가져오기
지정한 포스트에 대해 정의된 기간의 분석 데이터를 가져옵니다.
...
| :------ | :------ | :------ |
| `ids` | `any`[] | 쉼표로 구분된 포스트 id 목록입니다. 단일 요청당 최대 100개까지 허용됩니다. |
| `endTime` | `string` | YYYY-MM-DDTHH:mm:ssZ 형식입니다. 시간 범위의 끝을 나타내는 UTC 타임스탬프입니다. |
| `startTime` | `string` | YYYY-MM-DDTHH:mm:ssZ 형식입니다. 시간 범위의 시작을 나타내는 UTC 타임스탬프입니다. |
| `granularity` | `string` | 검색 집계 결과에 대한 세분화 수준입니다. |
...
API 응답으로 resolve되는 Promise 또는 `requestOptions.raw`가 `true`인 경우 원시 응답
...
Reposts 가져오기
특정 포스트의 ID를 사용해, 해당 포스트를 리포스트한 포스트 목록을 조회합니다.
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `query` | `string` | 포스트를 매칭하는 데 사용하는 단일 쿼리/규칙/필터입니다. 최대 쿼리 길이를 확인하려면 https://t.co/rulelength를 참조하세요. |
...
모든 포스트 검색
전체 아카이브에서 검색 쿼리와 일치하는 포스트를 가져옵니다.
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `query` | `string` | 일치하는 포스트를 찾기 위한 하나의 쿼리/규칙/필터입니다. 최대 쿼리 길이를 확인하려면 https://t.co/rulelength를 참조하세요. |
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `ids` | `any`[] | 쉼표로 구분된 포스트 ID 목록입니다. 한 번의 요청당 최대 100개까지 허용됩니다. |
...
포스트 생성 또는 수정
인증된 사용자 계정에 새 포스트를 생성하거나, `edit_options`가 제공된 경우 기존 포스트를 수정합니다.
...
API 응답으로 resolve되는 `Promise`이거나, `requestOptions.raw`가 `true`인 경우 원시 응답(`Response`)
...
인용된 포스트 가져오기
특정 포스트의 ID를 지정하면, 해당 포스트를 인용한 포스트 목록을 가져옵니다.
...
최근 포스트 개수 가져오기
검색 쿼리와 일치하는 지난 7일간의 포스트 개수를 조회합니다.
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `query` | `string` | 포스트를 매칭하기 위한 하나의 쿼리/규칙/필터입니다. 최대 쿼리 길이를 확인하려면 https://t.co/rulelength를 참고하세요. |
...
과거 포스트 인사이트 가져오기
지정된 포스트에 대해 지정된 기간 내의 과거 참여 지표를 조회합니다.
...
| `granularity` | `string` | 메트릭 응답의 세분화 수준입니다. |
| `requestedMetrics` | `any`[] | 과거 조회에서 요청할 메트릭입니다. |
...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `query` | `string` | 포스트와 일치하는 항목을 찾기 위한 단일 쿼리/규칙/필터입니다. 최대 쿼리 길이를 확인하려면 https://t.co/rulelength를 참조하세요. |
...
API 응답으로 resolve되는 Promise이며, requestOptions.raw가 true이면 원시 Response 객체입니다
...
ID로 포스트 가져오기
지정된 ID를 사용해 특정 포스트의 세부 정보를 조회합니다.
...
포스트 삭제
인증된 사용자가 해당 포스트를 소유한 경우, 지정한 id의 포스트를 삭제합니다.
...
API 응답으로 resolve되는 Promise이며, requestOptions.raw가 true인 경우 가공되지 않은 Response입니다

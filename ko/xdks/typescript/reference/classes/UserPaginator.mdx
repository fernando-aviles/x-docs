...
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/ko/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`any`&gt;&gt; | 페이지네이션 토큰을 받아 데이터의 한 페이지를 가져오는 함수 |
...
추가합니다. 또한 페이지네이션 상태와 메타데이터를 업데이트합니다.
...
다음 페이지부터 시작하는 새 paginator 인스턴스를 생성합니다.

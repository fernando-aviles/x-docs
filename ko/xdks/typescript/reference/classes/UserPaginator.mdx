...
| 이름 | 유형 | 설명 |
| :------ | :------ | :------ |
| `fetchPage` | (`token?`: `string`) =&gt; `Promise`&lt;[`PaginatedResponse`](/ko/xdks/typescript/reference/interfaces/PaginatedResponse)&lt;`any`&gt;&gt; | 페이지네이션 토큰을 기준으로 데이터 한 페이지를 가져오는 함수 |
...
요청 한도(rate limit)가 초과되었는지 확인합니다
...
이 메서드는 데이터의 다음 페이지를 가져와 항목들을
현재 페이지네이터 인스턴스에 추가합니다. 또한 페이지네이션 상태와 메타데이터를 업데이트합니다.
...
이 메서드는 현재 `Paginator`의 상태에 영향을 주지 않고,

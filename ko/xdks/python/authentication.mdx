...
print(f"이 URL을 방문하여 인증하세요: {auth_url}")
...
# 5단계: 클라이언트 생성
# 옵션 1: bearer_token 사용 (OAuth2 액세스 토큰은 Bearer 토큰으로 작동)
...
```python
# 액세스 토큰이 만료되면 저장된 refresh_token을 사용하여 갱신
...
# 또는 전체 토큰 딕셔너리 전달: client = Client(token=tokens)
```
...
oauth_verifier = input("콜백에서 받은 OAuth 검증 코드를 입력하세요: ")
# 5단계: 액세스 토큰으로 교환

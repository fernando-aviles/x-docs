...
X API는 모든 엔드포인트에서 인증이 필요합니다. XDK는 다음 세 가지 인증 방식을 지원합니다:

1. Bearer Token (앱 전용)
2. OAuth 2.0 with PKCE
3. OAuth 1.0a (User Context)

- **Bearer Token**: 앱 인증을 지원하는 엔드포인트에서 읽기 전용 액세스에 사용합니다(예: 포스트 검색, 스트리밍 엔드포인트).
- **OAuth 2.0 PKCE**: 스코프 기반의, 사용자 승인을 거친 액세스를 위한 보안성이 높은 인증 방식입니다(예: 인증된 사용자의 포스트 `non_public` metrics 가져오기).
- **OAuth 1.0a**: 사용자별 작업을 위한 레거시 인증 방식입니다(예: 사용자를 대신해 포스트 작성, 리스트 관리).
[X Developer Console](https://developer.x.com/en/portal/dashboard)에서 자격 증명을 발급받으세요. 승인된 개발자 계정과 적절한 권한(예: Read + Write)을 가진 앱이 필요합니다.
...
이 예제는 Proof Key for Code Exchange (PKCE)를 사용한 OAuth 2.0 사용 방법을 보여줍니다. 이는 사용자별 액세스(예: 사용자를 대신해 포스트 작성, 사용자용 미디어 업로드 등)와 같이 사용자 단위 권한이 필요한 경우에 사용합니다.
**단계**:

1. Developer Console에서 앱을 등록하고 리디렉트 URI를 설정합니다(예: `http://localhost:8080/callback`).
2. Client ID를 가져옵니다(PKCE에는 Client secret이 필요하지 않습니다).
3. 인증 플로우를 시작하고, 사용자를 auth URL로 리다이렉트한 뒤 콜백을 처리합니다.
...
**토큰 갱신** (장기 세션의 경우 SDK에서 자동으로 갱신됨):
...
레거시 애플리케이션이거나 OAuth 1.0a 인증이 필요한 특정 사용 사례가 있는 경우:
**단계**:

1. Developer Console에서 API Key와 API Secret을 가져옵니다.
2. 이미 액세스 토큰이 있다면 그대로 사용합니다. 그렇지 않다면 OAuth 1.0a 플로우를 완료하여 토큰을 발급받습니다.
3. OAuth1 인스턴스를 생성하고 이를 클라이언트에 전달합니다.
   **예시** (기존 액세스 토큰이 있는 경우):
...
* 운영 환경에서는 비밀 정보를 절대 하드코딩하지 말고, 환경 변수나 시크릿 관리자(예: `os.getenv("X_BEARER_TOKEN")`)를 사용하세요.
* PKCE를 사용할 때는 운영 환경에서 리디렉션 URI에 HTTPS를 사용하는지 확인하세요.

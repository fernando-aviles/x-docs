...
게시물 분석 가져오기
지정된 기간 내 특정 게시물에 대한 분석 데이터를 가져옵니다.
...
전체 게시물 개수 가져오기
전체 아카이브에서 검색 쿼리에 일치하는 게시물 개수를 조회합니다.
...
YYYY-MM-DDTHH:mm:ssZ 형식입니다. 게시물이 제공될 가장 이른 UTC 타임스탬프(최근 7일 이내)입니다. 타임스탬프는 초 단위 정밀도를 가지며 포함 범위입니다(예: 12:00:01은 해당 분의 첫 번째 초를 포함합니다).
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ 형식입니다. 게시물이 제공될 가장 최근 UTC 타임스탬프입니다. 타임스탬프는 초 단위 정밀도를 가지며 제외 범위입니다(예: 12:00:01은 해당 분의 첫 번째 초를 제외합니다).
</ParamField>
...
28시간 게시물 인사이트 가져오기
지정된 게시물에 대한 지난 28시간 동안의 참여 지표를 가져옵니다.
...
과거 게시물 인사이트 가져오기
지정된 기간 동안 지정한 게시물의 과거 참여 지표를 조회합니다.
...
인용된 게시물 조회
특정 게시물의 ID를 받아, 해당 게시물을 인용한 게시물 목록을 가져옵니다.
...
리게시물한 사용자 조회
특정 게시물의 ID로 해당 게시물을 리게시물한 사용자 목록을 가져옵니다.
...
리게시물 가져오기
지정된 ID의 특정 게시물을 리게시물한 게시물 목록을 가져옵니다.
...
<ParamField path="path.query" type="str">
게시물을 매칭하기 위한 하나의 쿼리/규칙/필터입니다. 최대 쿼리 길이는 [https://t.co/rulelength](https://t.co/rulelength)을 참조하세요.
</ParamField>

<ParamField path="path.start_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ 형식입니다. 게시물을 제공하기 시작하는 가장 이른 UTC 타임스탬프입니다. 타임스탬프는 초 단위 정밀도를 가지며, 이 값은 포함됩니다(예: 12:00:01은 해당 분의 첫 번째 초를 포함).
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ 형식입니다. 게시물을 제공하는 가장 최신 UTC 타임스탬프입니다. 타임스탬프는 초 단위 정밀도를 가지며, 이 값은 포함되지 않습니다(예: 12:00:01은 해당 분의 첫 번째 초를 제외).
</ParamField>

<ParamField path="path.since_id" type="Any or None" default="None">
지정된 ID보다 큰(즉, 더 최근의) 게시물 ID를 가진 결과만 반환합니다.
</ParamField>

...
이 모듈은 자동 재연결, 지수적 백오프, 포괄적인 오류 처리를 포함한 스트리밍 연결 관리를 제공합니다. 클라이언트는 연결 관리를 신경 쓰지 않고 스트리밍 엔드포인트에서 데이터를 소비할 수 있으며, SDK가 모든 복구 작업을 자동으로 처리합니다.
...
자동 재연결 및 지수 백오프를 사용하여 엔드포인트에서 데이터를 스트리밍합니다.

...
이 모듈은 자동 재연결, 지수 백오프, 포괄적인 오류 처리를 포함한 스트리밍 연결 관리를 제공합니다. 클라이언트는 연결 관리에 신경 쓰지 않고 스트리밍 엔드포인트를 사용할 수 있으며, SDK가 모든 복구를 자동으로 처리합니다.

<div id="class-xdkstreamingstreamconfig">
  ### `class xdk.streaming.StreamConfig`
</div>

재시도 동작이 포함된 스트리밍 연결용 구성입니다.
...
분류가 지정된 스트리밍 오류가 발생했을 때 던져지는 예외입니다.
...
재시도 결정을 내릴 때 사용하는 스트리밍 오류 분류입니다.
...
재시도 및 콜백 설정이 포함된 StreamConfig입니다.
</ParamField>
...
이 모듈은 자동 재연결, 지수 백오프, 포괄적인 오류 처리 기능을 갖춘 스트리밍 연결 관리를 제공합니다. 클라이언트는 연결 관리에 신경 쓰지 않고 스트리밍 엔드포인트를 사용할 수 있으며, SDK가 모든 복구 작업을 자동으로 처리합니다.

<div id="class-xdkstreamingstreamconfig">
  ### `class xdk.streaming.StreamConfig`
</div>

재시도 동작을 지원하는 스트리밍 연결 구성입니다.
...
자동 재연결 및 지수 백오프를 사용해 엔드포인트에서 데이터를 스트리밍합니다.
이 함수는 다음을 포함한 모든 연결 관리 작업을 처리합니다:

- 연결 해제 시 자동 재연결
- 재시도 지연을 위한 지수 백오프 및 지터
...
HTTP 호출에 사용할 requests의 Session 객체입니다.
</ParamField>

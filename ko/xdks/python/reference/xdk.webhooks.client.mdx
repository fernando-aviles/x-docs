...
스트림 링크 생성
주어진 웹훅으로 FilteredStream 이벤트를 전달하기 위한 링크를 생성합니다.
...
웹훅 재생 작업 생성
웹훅에 전달되었거나 전달이 시도된 모든 이벤트에 대해, 최대 지난 24시간 동안의 이벤트를 가져오는 재생 작업을 생성합니다.
body: 요청 본문
:returns: 응답 data
...
스트림 링크 삭제
지정된 웹훅으로 전달되는 FilteredStream 이벤트에 대한 링크를 삭제합니다.
...
스트림 링크 가져오기
필터링된 스트림 규칙 세트와 연결된 webhook 링크 목록을 가져옵니다.
:returns: 응답 데이터

...
대화 ID로 DM 메시지 생성
지정된 ID를 가진 특정 대화에 새 다이렉트 메시지를 전송합니다.
...
참가자 ID로 DM 메시지 생성
특정 참가자의 ID를 사용해 새로운 다이렉트 메시지를 보냅니다.
...
DM 대화 생성
지정된 참여자들과 새 다이렉트 메시지 대화를 시작합니다.
body: 요청 본문
:returns: 응답 data
...
DM 이벤트 삭제
인증된 사용자가 소유한 특정 다이렉트 메시지 이벤트를 id를 사용해 삭제합니다.
...
DM 이벤트 가져오기
모든 대화에서 발생한 최근 다이렉트 메시지 이벤트 목록을 가져옵니다.

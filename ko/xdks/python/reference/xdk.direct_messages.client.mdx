...
대화 id로 DM 메시지 생성
지정된 대화 id를 사용하여 새 다이렉트 메시지를 전송합니다.
...
DM 대화 생성
지정된 참가자와의 새 다이렉트 메시지 대화를 시작합니다.
body: 요청 본문
:returns: 응답 데이터
...
DM 이벤트 조회
모든 대화에서 최근 다이렉트 메시지 이벤트 목록을 조회합니다.
...
ID로 DM 이벤트 조회
지정된 ID를 가진 특정 다이렉트 메시지(DM) 이벤트의 세부 정보를 조회합니다.
...
<ParamField path="path.expansions" type="List or None" default="None">
expansions 대상으로 할 필드의 쉼표로 구분된 목록입니다.
</ParamField>

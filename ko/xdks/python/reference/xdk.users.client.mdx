...
DM 차단
인증된 사용자가 특정 사용자의 id를 지정해, 해당 사용자와 주고받는 다이렉트 메시지를 차단합니다.

#### 매개변수

<ParamField path="path.id" type="Any">
인증된 사용자가 DM을 차단하도록 요청하는 대상 사용자의 id입니다.
</ParamField>
...
북마크 삭제
인증된 사용자의 북마크에서 해당 ID로 식별되는 포스트를 제거합니다.
...
사용자 팔로우
인증된 사용자가 특정 사용자의 id를 사용해 해당 사용자를 팔로우하게 합니다.
...
<ParamField path="path.pagination_token" type="Any or None" default="None">
다음 페이지의 결과를 가져오기 위해 사용하는 매개변수입니다.
</ParamField>
...
폴더 ID로 북마크 가져오기
인증된 사용자에 대해 특정 북마크 폴더의 ID를 사용해 해당 폴더에 있는 포스트를 조회합니다.
...
username으로 User 조회
여러 User에 대해 username을 기준으로 상세 정보를 조회합니다.

#### 매개변수

<ParamField path="path.usernames" type="List">
쉼표로 구분한 username 목록입니다.
</ParamField>
...
팔로우 중인 리스트 가져오기
특정 사용자가 팔로우 중인 리스트를 해당 사용자의 id로 조회합니다.
...
팔로워 조회
특정 User의 id로 해당 User를 팔로우하는 User 목록을 조회합니다.
...
List 멤버십 조회
특정 사용자의 id를 사용하여, 해당 사용자가 멤버로 포함된 List 목록을 조회합니다.
...
결과 집합에 포함될 최소 포스트 ID입니다. 이 파라미터와 start_time이 모두 지정된 경우, 이 파라미터가 우선 적용됩니다.
</ParamField>

<ParamField path="path.until_id" type="Any or None" default="None">
결과 집합에 포함될 최대 포스트 ID입니다. 이 파라미터와 end_time이 모두 지정된 경우, 이 파라미터가 우선 적용됩니다.
</ParamField>
...
YYYY-MM-DDTHH:mm:ssZ. 포스트가 반환되기 시작하는 가장 이른 UTC 타임스탬프입니다. since_id 파라미터도 지정된 경우, since_id가 우선 적용됩니다.
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. 포스트가 반환되는 가장 늦은 UTC 타임스탬프입니다. until_id 파라미터도 지정된 경우, until_id가 우선 적용됩니다.
</ParamField>
...
<ParamField path="path.pagination_token" type="Any or None" default="None">
이 매개변수는 특정 ‘페이지’의 결과를 가져오는 데 사용됩니다.
</ParamField>
...
<ParamField path="path.start_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. 포스트가 제공되기 시작하는 가장 이른 UTC 타임스탬프입니다. since_id 파라미터도 지정된 경우, since_id 파라미터가 우선 적용됩니다.
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. 포스트가 제공되는 범위 중 가장 늦은 UTC 타임스탬프입니다. until_id 파라미터도 지정된 경우, until_id 파라미터가 우선 적용됩니다.
</ParamField>
...
내 포스트의 리포스트 가져오기
인증된 사용자의 포스트를 리포스트한 포스트 목록을 가져옵니다.
...
타임라인 가져오기
인증된 사용자의 타임라인에 있는 포스트를 최신순(시간 역순)으로 정렬한 목록을 반환합니다.

#### 매개변수

<ParamField path="path.id" type="Any">
역순 시간순 타임라인 포스트 목록을 나열할 인증된 소스 사용자 ID입니다.
</ParamField>
...
<ParamField path="path.start_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. 포스트가 제공되기 시작하는 가장 이른 UTC 타임스탬프입니다. since_id 매개변수가 함께 지정된 경우, since_id가 우선합니다.
</ParamField>

<ParamField path="path.end_time" type="str or None" default="None">
YYYY-MM-DDTHH:mm:ssZ. 포스트가 제공되는 마지막 시점에 해당하는 가장 늦은 UTC 타임스탬프입니다. until_id 매개변수가 함께 지정된 경우, until_id가 우선합니다.
</ParamField>
...
인증된 사용자가 ID로 지정된 특정 포스트에 좋아요를 표시합니다.

#### 매개변수

<ParamField path="path.id" type="Any">
포스트에 좋아요를 요청하는 인증된 원본 사용자의 ID입니다.
</ParamField>
...
사용자 음소거
인증된 사용자가 지정된 id를 가진 특정 사용자를 음소거합니다.
...
<ParamField path="path.id" type="Any">
포스트를 리포스트하려는 인증된 소스 User의 id입니다.
</ParamField>
...
이 매개변수는 다음 ‘페이지’의 결과를 가져오는 데 사용됩니다. 이 매개변수에 사용하는 값은 API에서 제공하는 응답에서 직접 가져와야 하며, 수정해서는 안 됩니다.
</ParamField>
...
DM 차단 해제
인증된 사용자가 특정 사용자의 id를 지정하여, 해당 사용자와 주고받는 다이렉트 메시지에 대한 차단을 해제합니다.
...
<ParamField path="path.id" type="Any">
리스트 언팔로우를 수행할 인증된 소스 User의 ID입니다.
</ParamField>
...
사용자 언팔로우
인증된 사용자가 id로 지정된 특정 사용자에 대한 팔로우를 취소하도록 합니다.

#### 매개변수

<ParamField path="path.source_user_id" type="Any">
팔로우 해제를 요청하는 인증된 소스 사용자의 ID입니다.
</ParamField>
...
포스트 좋아요 취소
인증된 사용자가 ID로 지정된 특정 포스트의 좋아요를 취소합니다.

#### 매개변수

<ParamField path="path.id" type="Any">
포스트의 좋아요 취소를 요청하는 인증된 소스 사용자의 id입니다.
</ParamField>

<ParamField path="path.tweet_id" type="Any">
사용자가 좋아요 취소를 요청하는 포스트의 id입니다.
</ParamField>
...
포스트 리포스트 취소
인증된 사용자가 지정된 ID의 특정 포스트 리포스트를 취소합니다.

#### 매개변수

<ParamField path="path.id" type="Any">
Repost를 요청하는 인증된 소스 사용자의 ID입니다.
</ParamField>

<ParamField path="path.source_tweet_id" type="Any">
사용자가 리트윗 취소(unretweet)를 요청하는 포스트의 ID입니다.
</ParamField>

...
X API는 [Filtered Stream Endpoint](https://docs.x.com/x-api/posts/filtered-stream/introduction)와 같은 엔드포인트를 통해 실시간 데이터를 지원하며, 조건에 맞는 포스트를 실시간으로 전송합니다. 이를 위해서는 지속적인 HTTP 연결을 유지해야 합니다.

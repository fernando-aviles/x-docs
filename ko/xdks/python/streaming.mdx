...
X API는 [Filtered Stream Endpoint](https://docs.x.com/x-api/posts/filtered-stream/introduction)와 같은 엔드포인트를 통해 실시간 데이터를 제공하며, 조건과 일치하는 포스트를 생성되는 즉시 전달합니다. 이를 위해서는 지속적인 HTTP 연결을 유지해야 합니다.
...
규칙은 어떤 data를 대상으로 할지에 대한 필터를 정의합니다(예: 키워드, 사용자 등). 규칙을 작성하는 방법은 [이 가이드](https://docs.x.com/x-api/posts/filtered-stream/integrate/build-a-rule)에서 확인할 수 있습니다.
**규칙 추가**:
...
전체 규칙 구문은 [X Streaming Rules Docs](https://developer.x.com/en/docs/twitter-api/tweets/filtered-stream/integrate/build-a-rule)를 참고하세요.
...
- **403 Forbidden**: 인증 정보가 잘못되었거나 권한이 부족합니다.
- **420 Enhance Your Calm**: 요청 한도에 걸렸습니다. 잠시 기다렸다가 다시 시도하세요.
- **No Data**: `get_rules()`로 규칙을 확인하고, 일치하는 포스트가 존재하는지 확인하세요.
Python XDK를 사용한 자세한 코드 예시는 [code samples GitHub 리포지토리](https://github.com/xdevplatform/samples/tree/main/python)를 참고하세요.
더 많은 예제와 API 레퍼런스는 인라인 docstring(예: `help(client.tweets.search_recent)`)이나 소스에 있는 자동 생성된 스텁을 참고하세요. [GitHub 리포지토리](https://github.com/xdevplatform/xdk/tree/main/xdk/python)를 통해 피드백을 보내 주세요.